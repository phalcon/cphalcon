	...
	if (!eval_int) {
		...
		if (!zend_is_true(r3)) {
			...
			if (phalcon_file_exists(v11 TSRMLS_CC) == SUCCESS) {
				...
				PHALCON_INIT_VAR(r6);
				PHALCON_CONCAT_BOTH(r6,  "File for controller class ", v10, " doesn't exists");
				Z_ADDREF_P(r6);
				p2[0] = r6;
...
PHP_METHOD(Phalcon_Request, hasFiles){
	...
	if (zend_is_true(a0)) {
		PHALCON_ALLOC_ZVAL_MM(r1);
		PHALCON_CALL_FUNC_PARAMS_1(r1, "count", a0, 0x008);
		PHALCON_INIT_VAR(t0);
		ZVAL_LONG(t0, 0);
		PHALCON_INIT_VAR(r2);
...
PHP_METHOD(Phalcon_Model_Query, getConditions){
	...
	if (Z_TYPE_P(t2) != IS_ARRAY) {
		...
		if (Z_TYPE_P(t4) != IS_ARRAY) {
			...
			Z_ADDREF_P(v8);
			Z_ADDREF_P(v4);
			PHALCON_CALL_FUNC_PARAMS_2(r7, "in_array", v8, v4, 0x03C);
			Z_DELREF_P(v8);
			Z_DELREF_P(v4);
			PHALCON_INIT_VAR(r8);
		...
		ZVAL_STRING(c1, " AND ", 1);
		Z_ADDREF_P(v9);
		PHALCON_CALL_FUNC_PARAMS_2(r19, "implode", c1, v9, 0x03D);
		Z_DELREF_P(v9);
...
PHP_METHOD(Phalcon_Tag, displayTo){
	...
	PHALCON_ALLOC_ZVAL_MM(r0);
	Z_ADDREF_P(v1);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "is_scalar", v1, 0x03E);
	Z_DELREF_P(v1);
	if (!zend_is_true(r0)) {
		PHALCON_ALLOC_ZVAL_MM(i0);
...
PHP_METHOD(Phalcon_Tag, _getValueFromAction){
	...
	if (zend_is_true(t1)) {
		...
		if (eval_int) {
			PHALCON_ALLOC_ZVAL_MM(r0);
			PHALCON_CALL_FUNC(r0, "get_magic_quotes_gpc", 0x03F);
			if (!zend_is_true(r0)) {
				PHALCON_ALLOC_ZVAL_MM(r1);
				...
				phalcon_array_fetch(&r3, a0, v0, PHALCON_NOISY_FETCH TSRMLS_CC);
				Z_ADDREF_P(r3);
				PHALCON_CALL_FUNC_PARAMS_1(r2, "stripslashes", r3, 0x040);
				Z_DELREF_P(r3);
				PHALCON_RETURN_DZVAL(r2);
			}
		}
