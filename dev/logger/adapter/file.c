...
PHP_METHOD(Phalcon_Logger_Adapter_File, __construct){
	...
	PHALCON_CALL_FUNC_PARAMS_2(r1, "fopen", v0, v2, 0x026);
...
PHP_METHOD(Phalcon_Logger_Adapter_File, _applyFormat){
	...
	Z_ADDREF_P(t1);
	Z_ADDREF_P(v2);
	PHALCON_CALL_FUNC_PARAMS_2(r2, "date", t1, v2, 0x040);
	Z_DELREF_P(t1);
	Z_DELREF_P(v2);
	PG(display_errors) = silence;
	p1[1] = r2;
...
PHP_METHOD(Phalcon_Logger_Adapter_File, log){
	...
	PHALCON_ALLOC_ZVAL_MM(r0);
	Z_ADDREF_P(v0);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "is_scalar", v0, 0x041);
	Z_DELREF_P(v0);
	if (zend_is_true(r0)) {
		PHALCON_ALLOC_ZVAL_MM(r1);
		PHALCON_INIT_VAR(c0);
	...
	if (zend_is_true(t1)) {
		...
		Z_ADDREF_P(r4);
		PHALCON_CALL_FUNC_PARAMS_2_NORETURN("fputs", t3, r4, 0x042);
		Z_DELREF_P(t3);
		Z_DELREF_P(r4);
	}
	PHALCON_MM_RESTORE();
...
PHP_METHOD(Phalcon_Logger_Adapter_File, commit){
	...
	if (Z_TYPE_P(t1) != IS_ARRAY) {
		...
		Z_ADDREF_P(r4);
		PHALCON_CALL_FUNC_PARAMS_2_NORETURN("fputs", t2, r4, 0x042);
		Z_DELREF_P(t2);
		Z_DELREF_P(r4);
...
PHP_METHOD(Phalcon_Logger_Adapter_File, close){
	...
	phalcon_read_property(&t0, this_ptr, "_fileHandler", sizeof("_fileHandler")-1, PHALCON_NOISY_FETCH TSRMLS_CC);
	Z_ADDREF_P(t0);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "fclose", t0, 0x028);
	Z_DELREF_P(t0);
	PG(display_errors) = silence;
...
PHP_METHOD(Phalcon_Logger_Adapter_File, __wakeup){
	...
	PHALCON_INIT_VAR(c0);
	ZVAL_STRING(c0, "ab", 1);
	PHALCON_CALL_FUNC_PARAMS_2(r0, "fopen", t0, c0, 0x026);
	PG(display_errors) = silence;
