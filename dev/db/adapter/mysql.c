...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, __construct){
	...
	zval *descriptor = NULL, *persistent = NULL;
	zval *t0 = NULL;

	PHALCON_MM_GROW();
	
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|zz", &descriptor, &persistent) == FAILURE) {
		PHALCON_MM_RESTORE();
		RETURN_NULL();
	}
	...
	if (PHALCON_COMPARE_STRING(descriptor, "")) {
	...
	PHALCON_CALL_METHOD_PARAMS_2_NORETURN(this_ptr, "connect", descriptor, persistent, PHALCON_NO_CHECK);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, connect){
	...
	zval *descriptor = NULL, *persistent = NULL, *host = NULL, *username = NULL;
	zval *password = NULL, *dbstring = NULL, *client_flags = NULL, *dbname = NULL;
	zval *autocommit = NULL, *collation = NULL;
	...
	if (!zend_is_true(persistent)) {
		...
		ZVAL_BOOL(p0[3], 1);
		p0[4] = client_flags;
		PHALCON_CALL_FUNC_PARAMS(r3, "mysql_connect", 5, p0, 0x02A);
		...
		p1[2] = password;
		p1[3] = client_flags;
		PHALCON_CALL_FUNC_PARAMS(r4, "mysql_pconnect", 4, p1, 0x034);
		phalcon_update_property_zval(this_ptr, "_idConnection", strlen("_idConnection"), r4 TSRMLS_CC);
	}
	
	PHALCON_ALLOC_ZVAL_MM(t10);
	phalcon_read_property(&t10, this_ptr, "_idConnection", sizeof("_idConnection")-1, PHALCON_NOISY TSRMLS_CC);
	if (zend_is_true(t10)) {
		eval_int = phalcon_isset_property(descriptor, "name", strlen("name") TSRMLS_CC);
		PHALCON_INIT_VAR(t11);
		ZVAL_BOOL(t11, eval_int);
		if (zend_is_true(t11)) {
			PHALCON_ALLOC_ZVAL_MM(t12);
			phalcon_read_property(&t12, descriptor, "name", sizeof("name")-1, PHALCON_NOISY TSRMLS_CC);
			r5 = t12;
		} else {
		...
		ZVAL_STRING(t14, "", 1);
		PHALCON_INIT_VAR(r6);
		is_not_identical_function(r6, t14, dbname TSRMLS_CC);
		if (zend_is_true(r6)) {
			...
			PHALCON_CALL_FUNC_PARAMS_2(r7, "mysql_select_db", dbname, t15, 0x02B);
			if (!zend_is_true(r7)) {
				PHALCON_ALLOC_ZVAL_MM(i0);
				object_init_ex(i0, phalcon_db_exception_ce);
				PHALCON_ALLOC_ZVAL_MM(r8);
				PHALCON_CALL_METHOD(r8, this_ptr, "error", PHALCON_NO_CHECK);
				PHALCON_ALLOC_ZVAL_MM(r9);
				PHALCON_CALL_METHOD(r9, this_ptr, "noerror", PHALCON_NO_CHECK);
				PHALCON_CALL_METHOD_PARAMS_2_NORETURN(i0, "__construct", r8, r9, PHALCON_CHECK);
		...
		if (eval_int) {
			...
			PHALCON_CONCAT_LEFT(r10, "SET NAMES ", t18);
		...
		if (eval_int) {
			...
			PHALCON_CONCAT_LEFT(r11, "SET collation_connection=", collation);
			PHALCON_CALL_METHOD_PARAMS_1_NORETURN(this_ptr, "query", r11, PHALCON_NO_CHECK);
			PHALCON_ALLOC_ZVAL_MM(r12);
			PHALCON_CONCAT_LEFT(r12, "SET collation_database=", collation);
			PHALCON_CALL_METHOD_PARAMS_1_NORETURN(this_ptr, "query", r12, PHALCON_NO_CHECK);
		}
		...
		PHALCON_CALL_METHOD(r13, this_ptr, "error", PHALCON_NO_CHECK);
		PHALCON_ALLOC_ZVAL_MM(r14);
		PHALCON_CALL_METHOD(r14, this_ptr, "noerror", PHALCON_NO_CHECK);
		PHALCON_CALL_METHOD_PARAMS_2_NORETURN(i1, "__construct", r13, r14, PHALCON_CHECK);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, query){
	...
	PHALCON_ALLOC_ZVAL_MM(r0);
	PHALCON_CALL_FUNC_PARAMS_2(r0, "mysql_query", sql_statement, id_connection, 0x02C);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, close){
	...
	if (zend_is_true(t0)) {
		...
		phalcon_read_property(&t1, this_ptr, "_idConnection", sizeof("_idConnection")-1, PHALCON_NOISY TSRMLS_CC);
		Z_ADDREF_P(t1);
		PHALCON_CALL_FUNC_PARAMS_1(r0, "mysql_close", t1, 0x035);
		Z_DELREF_P(t1);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, fetchArray){
	...
	PHALCON_ALLOC_ZVAL_MM(r0);
	Z_ADDREF_P(result_query);
	PHALCON_ALLOC_ZVAL_MM(t2);
	phalcon_read_property(&t2, this_ptr, "_fetchMode", sizeof("_fetchMode")-1, PHALCON_NOISY TSRMLS_CC);
	Z_ADDREF_P(t2);
	PHALCON_CALL_FUNC_PARAMS_2(r0, "mysql_fetch_array", result_query, t2, 0x02D);
	Z_DELREF_P(result_query);
	Z_DELREF_P(t2);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, numRows){
	...
	PG(display_errors) = 0;
	PHALCON_ALLOC_ZVAL_MM(r0);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "mysql_num_rows", result_query, 0x036);
	PG(display_errors) = silence;
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, dataSeek){
	...
	PG(display_errors) = 0;
	PHALCON_ALLOC_ZVAL_MM(r0);
	PHALCON_CALL_FUNC_PARAMS_2(r0, "mysql_data_seek", result_query, number, 0x037);
	PG(display_errors) = silence;
	PHALCON_CPY_WRT(success, r0);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, affectedRows){
	...
	PHALCON_ALLOC_ZVAL_MM(t0);
	phalcon_read_property(&t0, this_ptr, "_idConnection", sizeof("_idConnection")-1, PHALCON_NOISY TSRMLS_CC);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "mysql_affected_rows", t0, 0x038);
	PG(display_errors) = silence;
	PHALCON_CPY_WRT(number_rows, r0);
	if (Z_TYPE_P(number_rows) != IS_BOOL || (Z_TYPE_P(number_rows) == IS_BOOL && Z_BVAL_P(number_rows))) {
		PHALCON_RETURN_CHECK_CTOR(number_rows);
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, error){
	...
	if (!zend_is_true(t0)) {
		PHALCON_ALLOC_ZVAL_MM(r0);
		PHALCON_CALL_FUNC(r0, "mysql_error", 0x039);
		PHALCON_CPY_WRT(error_message, r0);
	} else {
		PHALCON_ALLOC_ZVAL_MM(r1);
		PHALCON_ALLOC_ZVAL_MM(t1);
		phalcon_read_property(&t1, this_ptr, "_idConnection", sizeof("_idConnection")-1, PHALCON_NOISY TSRMLS_CC);
		PHALCON_CALL_FUNC_PARAMS_1(r1, "mysql_error", t1, 0x039);
		PHALCON_CPY_WRT(error_message, r1);
	}
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, noError){
	...
	phalcon_read_property(&t1, this_ptr, "_idConnection", sizeof("_idConnection")-1, PHALCON_NOISY TSRMLS_CC);
	Z_ADDREF_P(t1);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "mysql_errno", t1, 0x03A);
	Z_DELREF_P(t1);
	PHALCON_RETURN_DZVAL(r0);
}
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, lastInsertId){
	...
	if (!primary_key) {
		...
		ZVAL_STRING(primary_key, "", 1);
	}
	
	if (!sequence_name) {
		PHALCON_INIT_VAR(sequence_name);
		ZVAL_STRING(sequence_name, "", 1);
	}
	...
	PHALCON_ALLOC_ZVAL_MM(t1);
	phalcon_read_property(&t1, this_ptr, "_idConnection", sizeof("_idConnection")-1, PHALCON_NOISY TSRMLS_CC);
	PHALCON_CALL_FUNC_PARAMS_1(r0, "mysql_insert_id", t1, 0x03B);
	PHALCON_RETURN_DZVAL(r0);
}
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, createTable){
	...
	PHALCON_ALLOC_ZVAL_MM(r1);
	PHALCON_ALLOC_ZVAL_MM(r2);
	phalcon_array_fetch_string(&r2, definition, "columns", strlen("columns"), PHALCON_NOISY TSRMLS_CC);
	PHALCON_CALL_FUNC_PARAMS_1(r1, "count", r2, 0x007);
	if (!zend_is_true(r1)) {
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, describeIndexes){
	...
	if (phalcon_valid_foreach(describe TSRMLS_CC)) {
		...
		if (Z_REFCOUNT_P(t1) > 1) {
			phalcon_array_update(&indexes, key_name, t1, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_CTOR TSRMLS_CC);
		}
		Z_ADDREF_P(t1);
		if (Z_TYPE_P(t1) != IS_ARRAY) {
			convert_to_array(t1);
			phalcon_array_update(&indexes, key_name, t1, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_NO_CTOR TSRMLS_CC);
		}
...
PHP_METHOD(Phalcon_Db_Adapter_Mysql, describeReferences){
	...
	if (phalcon_valid_foreach(describe TSRMLS_CC)) {
		...
		if (Z_REFCOUNT_P(t1) > 1) {
			phalcon_array_update(&references, constraint_name, t1, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_CTOR TSRMLS_CC);
		}
		Z_ADDREF_P(t1);
		if (Z_TYPE_P(t1) != IS_ARRAY) {
			convert_to_array(t1);
			phalcon_array_update(&references, constraint_name, t1, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_NO_CTOR TSRMLS_CC);
		}
		if (Z_TYPE_P(t1) == IS_ARRAY) {
			PHALCON_INIT_VAR(t2);
		...
		if (Z_REFCOUNT_P(t2) > 1) {
			phalcon_array_update_string(&t1, "columns", strlen("columns"), t2, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_CTOR TSRMLS_CC);
		}
		Z_ADDREF_P(t2);
		if (Z_TYPE_P(t2) != IS_ARRAY) {
			convert_to_array(t2);
			phalcon_array_update_string(&t1, "columns", strlen("columns"), t2, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_NO_CTOR TSRMLS_CC);
		}
		...
		if (Z_REFCOUNT_P(t3) > 1) {
			phalcon_array_update(&references, constraint_name, t3, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_CTOR TSRMLS_CC);
		}
		Z_ADDREF_P(t3);
		if (Z_TYPE_P(t3) != IS_ARRAY) {
			convert_to_array(t3);
			phalcon_array_update(&references, constraint_name, t3, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_NO_CTOR TSRMLS_CC);
		}
		if (Z_TYPE_P(t3) == IS_ARRAY) {
			PHALCON_INIT_VAR(t4);
		...
		if (Z_REFCOUNT_P(t4) > 1) {
			phalcon_array_update_string(&t3, "referencedColumns", strlen("referencedColumns"), t4, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_CTOR TSRMLS_CC);
		}
		Z_ADDREF_P(t4);
		if (Z_TYPE_P(t4) != IS_ARRAY) {
			convert_to_array(t4);
			phalcon_array_update_string(&t3, "referencedColumns", strlen("referencedColumns"), t4, PHALCON_NO_SEPARATE_THX, PHALCON_NO_COPY, PHALCON_NO_CTOR TSRMLS_CC);
		}
