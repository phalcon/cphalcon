...
PHP_METHOD(Phalcon_Dispatcher, dispatch){
	...
	zval *a0 = NULL, *a1 = NULL;
	zval *p0[] = { NULL }, *p1[] = { NULL }, *p2[] = { NULL, NULL }, *p3[] = { NULL }, *p4[] = { NULL, NULL, NULL }, *p6[] = { NULL, NULL }, *p7[] = { NULL, NULL }, *p8[] = { NULL }, *p9[] = { NULL };
	int eval_int;
	...
	Z_ADDREF_P(r3);
	v8 = r3;
	eval_int = phalcon_array_isset(v7, v8 TSRMLS_CC);
	if (!eval_int) {
		if (!r4) {
		...
		concat_function(r4, v2, v8 TSRMLS_CC);
		if (!r5) {
		...
		if (phalcon_file_exists(v9 TSRMLS_CC) == SUCCESS) {
			...
			phalcon_concat_both(r6,  "File for controller class ", v8, " doesn't exists" TSRMLS_CC);
			Z_ADDREF_P(r6);
			p1[0] = r6;
		...
		if (!zend_is_true(r7)) {
			...
			phalcon_concat_both(r8,  "Class ", v8, " was not found on controller file" TSRMLS_CC);
			Z_ADDREF_P(r8);
			p3[0] = r8;
		...
		ce0 = zend_fetch_class(Z_STRVAL_P(v8), Z_STRLEN_P(v8), ZEND_FETCH_CLASS_DEFAULT TSRMLS_CC);
		if (!i2) {
		...
		if (phalcon_method_exists(v4, t6 TSRMLS_CC) == SUCCESS) {
			PHALCON_CALL_METHOD_NORETURN(v4, "initialize", PHALCON_CALL_DEFAULT);
		}
		if (!t7) {
		...
		zval_copy_ctor(t7);
		{
			...
			phalcon_array_update(t7, v8, copy TSRMLS_CC);
		}
		{
			zval *copy;
			ALLOC_ZVAL(copy);
		...
		phalcon_array_fetch(r9, v7, v8, PHALCON_NOISY_FETCH TSRMLS_CC);
		if (v4) {
			Z_DELREF_P(v4);
			if (!Z_REFCOUNT_P(v4)) {
				FREE_ZVAL(v4);
	...
	phalcon_concat_right(r10, v11, "Action" TSRMLS_CC);
	if (v12) {
		Z_DELREF_P(v12);
		if (!Z_REFCOUNT_P(v12)) {
			FREE_ZVAL(v12);
	...
	Z_ADDREF_P(r10);
	v12 = r10;
	if (phalcon_method_exists(v4, v12 TSRMLS_CC) == SUCCESS) {
		if (!r11) {
		...
		if (phalcon_method_exists(v4, t11 TSRMLS_CC) == SUCCESS) {
			if (!r12) {
			...
			phalcon_concat_left(r14, "Action '", v11 TSRMLS_CC);
			if (!r13) {
			...
			phalcon_concat_vboth(r13, r14, "' was not found on controller '", v6 TSRMLS_CC);
			if (!r15) {
	...
	ZVAL_LONG(t12, 256);
	PHALCON_SMALLER_FUNCTION(r16, t12, v5);
	if (zend_is_true(r16)) {
		if (!i4) {
...
PHP_METHOD(Phalcon_Dispatcher, forward){
	...
	if (eval_int) {
		...
		eval_int = phalcon_array_isset_string(v1, "controller", strlen("controller")+1 TSRMLS_CC);
		if (eval_int) {
			PHALCON_ALLOC_ZVAL(r3);
	...
	if (eval_int) {
		...
		eval_int = phalcon_array_isset_string(v1, "action", strlen("action")+1 TSRMLS_CC);
		if (eval_int) {
			PHALCON_ALLOC_ZVAL(r9);
