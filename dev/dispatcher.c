...
PHP_METHOD(Phalcon_Dispatcher, dispatch){
	...
	PHALCON_INIT_VAR(r1);
	PHALCON_CALL_STATIC_PARAMS_1(r1, "phalcon_utils", "camelize", controller_name);
	PHALCON_INIT_VAR(r2);
	...
	if (!eval_int) {
		...
		PHALCON_INIT_VAR(c0);
		ZVAL_BOOL(c0, 0);
		PHALCON_CALL_FUNC_PARAMS_2(r3, "class_exists", controller_class, c0, 0x00E);
		...
		if (!zend_is_true(r3)) {
			...
			PHALCON_INIT_VAR(c1);
			ZVAL_BOOL(c1, 0);
			PHALCON_CALL_FUNC_PARAMS_2(r7, "class_exists", controller_class, c1, 0x00E);
			Z_DELREF_P(controller_class);
			if (!zend_is_true(r7)) {
				PHALCON_INIT_VAR(i1);
	...
	if (phalcon_method_exists(controller, action_method TSRMLS_CC) == SUCCESS) {
		...
		Z_ADDREF_P(a0);
		Z_ADDREF_P(params);
		PHALCON_CALL_FUNC_PARAMS_2(r12, "call_user_func_array", a0, params, 0x00F);
		Z_DELREF_P(a0);
		Z_DELREF_P(params);
		...
		if (phalcon_method_exists_ex(controller, "notfoundaction", strlen("notfoundaction") TSRMLS_CC) == SUCCESS) {
			...
			Z_ADDREF_P(a1);
			Z_ADDREF_P(params);
			PHALCON_CALL_FUNC_PARAMS_2(r13, "call_user_func_array", a1, params, 0x00F);
			Z_DELREF_P(a1);
			Z_DELREF_P(params);
