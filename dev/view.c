...
PHP_METHOD(Phalcon_View, render){
	...
	if (Z_TYPE_P(v7) == IS_ARRAY) { 
		...
		phalcon_concat_right(r6, r5, "/" TSRMLS_CC);
		if (v6) {
			Z_DELREF_P(v6);
			if (!Z_REFCOUNT_P(v6)) {
				FREE_ZVAL(v6);
		...
		FOREACH_V(v7, ac1, fes8, fee8, ah1, hp1, v8)
			...
			concat_function(r7, v6, v8 TSRMLS_CC);
			if (!r8) {
			...
			phalcon_concat_right(r8, r7, ".phtml" TSRMLS_CC);
			if (v9) {
				Z_DELREF_P(v9);
				if (!Z_REFCOUNT_P(v9)) {
					FREE_ZVAL(v9);
			...
			if (phalcon_file_exists(v9 TSRMLS_CC) == SUCCESS) {
				...
				PHALCON_CALL_FUNC(r9, "ob_get_contents");
				{
					zval *copy;
					ALLOC_ZVAL(copy);
				...
				phalcon_concat_both(r10,  "Template '", v8, "' was not found on templates directory" TSRMLS_CC);
				Z_ADDREF_P(r10);
				p5[0] = r10;
	...
	concat_function(r12, v2, v3 TSRMLS_CC);
	PHALCON_ALLOC_ZVAL(r11);
	phalcon_concat_vboth(r11, r12, "/", v0 TSRMLS_CC);
	PHALCON_ALLOC_ZVAL(r13);
	phalcon_concat_right(r13, r11, ".phtml" TSRMLS_CC);
	if (v6) {
		Z_DELREF_P(v6);
		if (!Z_REFCOUNT_P(v6)) {
			FREE_ZVAL(v6);
	...
	if (Z_TYPE_P(v10) == IS_ARRAY) { 
		...
		concat_function(r15, v2, v3 TSRMLS_CC);
		PHALCON_ALLOC_ZVAL(r16);
		phalcon_concat_right(r16, r15, "/" TSRMLS_CC);
		if (v6) {
			Z_DELREF_P(v6);
			if (!Z_REFCOUNT_P(v6)) {
				FREE_ZVAL(v6);
		...
		FOREACH_V(v10, ac2, fes9, fee9, ah2, hp2, v11)
			...
			concat_function(r17, v6, v11 TSRMLS_CC);
			if (!r18) {
			...
			phalcon_concat_right(r18, r17, ".phtml" TSRMLS_CC);
			if (v9) {
				Z_DELREF_P(v9);
				if (!Z_REFCOUNT_P(v9)) {
					FREE_ZVAL(v9);
			...
			if (phalcon_file_exists(v9 TSRMLS_CC) == SUCCESS) {
				...
				PHALCON_CALL_FUNC(r19, "ob_get_contents");
				{
					zval *copy;
					ALLOC_ZVAL(copy);
					ZVAL_ZVAL(copy, r19, 1, 0);
					Z_SET_REFCOUNT_P(copy, 0);
					phalcon_update_property_zval(this_ptr, "_content", strlen("_content"), copy TSRMLS_CC);
				}
			} else {
				...
				phalcon_concat_both(r20,  "Template '", v8, "' was not found on templates directory" TSRMLS_CC);
				Z_ADDREF_P(r20);
				p10[0] = r20;
	...
	phalcon_concat_right(r21, v2, "index.phtml" TSRMLS_CC);
	if (v6) {
		Z_DELREF_P(v6);
		if (!Z_REFCOUNT_P(v6)) {
			FREE_ZVAL(v6);
