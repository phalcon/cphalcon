...
class File extends Validator
{
	...
	public function validate(<Validation> validation, string! field) -> boolean
	{
		...
		if _SERVER["REQUEST_METHOD"] == "POST" && empty _POST && empty _FILES && _SERVER["CONTENT_LENGTH"] > 0 || isset value["error"] && value["error"] === UPLOAD_ERR_INI_SIZE {
			...
			validation->appendMessage(message, replacePairs, field, "FileIniSize");
			return false;
		}
		...
		if !isset value["error"] || !isset value["tmp_name"] || value["error"] !== UPLOAD_ERR_OK || !is_uploaded_file(value["tmp_name"]) {
			...
			validation->appendMessage(message, replacePairs, field, "FileEmpty");
			return false;
		}
		...
		if !isset value["name"] || !isset value["type"] || !isset value["size"] {
			...
			validation->appendMessage(message, replacePairs, field, "FileValid");
			return false;
		}
		...
		if this->isSetOption("maxSize") {
			...
			if floatval(value["size"]) > floatval(bytes) {
				...
				validation->appendMessage(message, replacePairs, field, "FileSize");
				return false;
			}
		}
		...
		if this->isSetOption("allowedTypes") {
			...
			if !in_array(mime, types) {
				...
				validation->appendMessage(message, replacePairs, field, "FileType");
				return false;
			}
		}
		...
		if this->isSetOption("minResolution") || this->isSetOption("maxResolution") {
			...
			if width < minWidth || height < minHeight {
				...
				validation->appendMessage(message, replacePairs, field, "FileMinResolution");
				return false;
			}
			...
			if this->isSetOption("maxResolution") {
				...
				if width > maxWidth || height > maxHeight {
					...
					validation->appendMessage(message, replacePairs, field, "FileMaxResolution");
					return false;
				}
			}
		}
