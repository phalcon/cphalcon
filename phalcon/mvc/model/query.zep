...
class Query implements QueryInterface, InjectionAwareInterface
{
    ...
    public function __construct(string phql = null, <DiInterface> container = null, array options = [])
    {
        ...
        if fetch enableImplicitJoins, options["enable_implicit_joins"] {
            ...
            let this->enableImplicitJoins = globals_get("orm.enable_implicit_joins");
        }
    }
    ...
    public function execute(var bindParams = null, var bindTypes = null)
    {
        ...
        switch type {
            ...
            case PHQL_T_SELECT:
                let result = this->executeSelect(intermediate, mergedParams, mergedTypes);
                break;

            case PHQL_T_INSERT:
                let result = this->executeInsert(intermediate, mergedParams, mergedTypes);
                break;

            case PHQL_T_UPDATE:
                let result = this->executeUpdate(intermediate, mergedParams, mergedTypes);
                break;

            case PHQL_T_DELETE:
                let result = this->executeDelete(intermediate, mergedParams, mergedTypes);
                break;
    ...
    public function getSql() -> array
    {
        ...
        if this->type == PHQL_T_SELECT {
            return this->executeSelect(intermediate, this->bindParams, this->bindTypes, true);
        }
    ...
    public function parse() -> array
    {
        ...
        if typeof ast == "array" {
            ...
            if fetch uniqueId, ast["id"] {
                if fetch irPhql, self::irPhqlCache[uniqueId] {
        ...
        if typeof uniqueId == "int" {
            let self::irPhqlCache[uniqueId] = irPhql;
        }
     ...
     * @param array bindTypes
     */
    protected final function executeDelete(array intermediate, var bindParams, var bindTypes) -> <StatusInterface>
    {
         ...
         */
        let records = this->getRelatedRecords(model, intermediate, bindParams, bindTypes);
     ...
     * @param array bindTypes
     */
    protected final function executeInsert(array intermediate, var bindParams, var bindTypes) -> <StatusInterface>
    {
        ...
        let metaData = this->metaData, attributes = metaData->getAttributes(model);
     ...
     */
    protected final function executeSelect(array intermediate, var bindParams, var bindTypes, bool simulate = false) -> <ResultsetInterface> | array
    {
     ...
     * @param array bindTypes
     */
    protected final function executeUpdate(array intermediate, var bindParams, var bindTypes) -> <StatusInterface>
    {
         ...
         */
        let records = this->getRelatedRecords(model, intermediate, selectBindParams, selectBindTypes);
     ...
     */
    protected final function getCallArgument(array! argument) -> array
    {
     ...
     * Resolves an expression in a single call argument
     */
    protected final function getCaseExpression(array! expr) -> array
    {
     ...
     */
    protected final function getExpression(array expr, bool quoting = true) -> string
    {
        ...
        if fetch exprType, expr["type"] {
            ...
            switch exprType {
                ...
                case PHQL_T_SELECT:
                    let exprReturn = ["type": "select", "value": this->prepareSelect(expr, true)];
                    break;
     ...
     * Resolves an expression in a single call argument
     */
    protected final function getFunctionCall(array! expr) -> array
    {
        ...
        if fetch arguments, expr["arguments"] {
            ...
            if isset arguments[0] {
                ...
                let functionArgs = [this->getCallArgument(arguments)];
            }
     ...
     */
    protected final function getGroupClause(array! group) -> array
    {
        ...
        if isset group[0] {
            ...
            let groupParts = [this->getExpression(group)];
        }
        return groupParts;
    }
     ...
     */
    protected final function getJoin(<ManagerInterface> manager, array join) -> array
    {
     ...
     */
    protected final function getJoins(array select) -> array
    {
                ...
                if !fetch preCondition, joinPreCondition[joinAlias] {
                    ...
                    if typeof relation == "object" {
                        ...
                        if !relation->isThrough() {
                            let sqlJoin = this->getSingleJoin(joinType, joinSource, modelAlias, joinAlias, relation);
                        } else {
                            let sqlJoin = this->getMultiJoin(joinType, joinSource, modelAlias, joinAlias, relation);
                        }
     ...
     */
    protected final function getJoinType(array join) -> string
    {
        var type;
        ...
        throw new Exception("Unknown join type " . type . ", when preparing: " . this->phql);
    }

    /**
     * Returns a processed limit clause for a SELECT statement
     */
    protected final function getLimitClause(array! limitClause) -> array
    {
        var number, offset;
        array limit = [];
     ...
     */
    protected final function getMultiJoin(string! joinType, joinSource, string modelAlias, string joinAlias, <RelationInterface> relation) -> array
    {
     ...
     */
    protected final function getOrderClause(order) -> array
    {
        ...
        for orderItem in orderColumns {
            ...
            let orderPartExpr = this->getExpression(orderItem["column"]);
     ...
     */
    protected final function getQualified(array! expr) -> array
    {
        ...
        if fetch columnDomain, expr["domain"] {
            ...
            if !fetch source, sqlAliases[columnDomain] {
                throw new Exception("Unknown model or alias '" . columnDomain . "' (11), when preparing: " . this->phql);
            }
            ...
            for model in this->modelsInstances {
                ...
                if metaData->hasAttribute(model, columnName) {
                    let number++;
                    if number > 1 {
                        throw new Exception("The column '" . columnName . "' is ambiguous, when preparing: " . this->phql);
                    }
                    let hasModel = model;
                }
            }
     ...
     * @param array bindTypes
     */
    protected final function getRelatedRecords(<ModelInterface> model, array intermediate, var bindParams, var bindTypes) -> <ResultsetInterface>
    {
     ...
     */
    protected final function getSelectColumn(array! column) -> array
    {
        ...
        if columnType == PHQL_T_DOMAINALL {
            ...
            if !fetch source, sqlAliases[columnDomain] {
                throw new Exception("Unknown model or alias '" . columnDomain . "' (2), when preparing: " . this->phql);
            }
     ...
     * @param string joinSource
     */
    protected final function getSingleJoin(string! joinType, joinSource, string modelAlias, string joinAlias, <RelationInterface> relation) -> array
    {
     ...
     * @return string
     */
    protected final function getTable(<ManagerInterface> manager, array qualifiedName)
    {
     ...
     */
    protected final function prepareDelete() -> array
    {
     ...
     */
    protected final function prepareInsert() -> array
    {
     ...
     */
    protected final function prepareSelect(var ast = null, var merge = null) -> array
    {
        int position;
        ...
        for selectedModel in selectedModels {
            ...
            if fetch alias, selectedModel["alias"] {
                ...
                if isset sqlAliases[alias] {
                    throw new Exception("Alias '" . alias . "' is used more than once, when preparing: " . this->phql);
                }
     ...
     */
    protected final function prepareUpdate() -> array
    {
