...
class Oracle extends \Phalcon\Db\Dialect
{
	...
	{
		let limit = (int)trim(offset, "'") + (int)trim(limit, "'");

		let sql = "SELECT * FROM (SELECT Z1.*, ROWNUM PHALCON_RN FROM (" . sql . ") Z1 WHERE ROWNUM <= " . limit . ")";

		if (offset != 0) {
			let sql .= " WHERE PHALCON_RN >= " . offset;
		}
	 ...
	 * @see http://docs.oracle.com/cd/B28359_01/server.111/b28286/statements_8004.htm#SQLRF01504
	 */
	public function createView(string! viewName, definition, string! schemaName) -> string
	{
		...
		if schemaName {
			let view = this->escape(Text::upper(schemaName)) . "." . this->escape(Text::upper(viewName));
		} else {
	...
	{
		if alias != "" {
			return qualified . " " . this->escape(alias);
		} else {
			return qualified;
		}
	}
	...
	protected function prepareTable(string! table, string schema = null, string alias = null) -> string
	{
		 ...
		 */
		if alias != "" {
			let table = table . " " . this->escape(alias);
		}
