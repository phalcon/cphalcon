...
class ViewEnginesVoltTest extends PHPUnit_Framework_TestCase
{
	...
	{
		@unlink(PATH_VIEWS . 'layouts/test10.volt.php');

		$volt = new \Phalcon\Mvc\View\Engine\Volt\Compiler();

		//Simple file
		$volt->compile(
            PATH_VIEWS . 'layouts/test10.volt',
            PATH_VIEWS . 'layouts/test10.volt.php');

		$compilation = file_get_contents(PATH_VIEWS . 'layouts/test10.volt.php');
...
<?php } ?>');
		...
		$volt->compile(
            PATH_VIEWS . 'test10/children.volt',
            PATH_VIEWS . 'test10/children.volt.php');

		$compilation = file_get_contents(PATH_VIEWS . 'test10/children.volt.php');
	...
	public function testVoltCompilerFileOptions()
	{
		...
		@unlink(PATH_VIEWS . 'test10/index.volt.php');
		@unlink(PATH_CACHE . 'unit-tests.views.test10.index.volt.compiled');

		$view->start();
		$volt->render(
            PATH_VIEWS . 'test10/index.volt',
            array('song' => 'Lights'),
            true
        );
		$view->finish();

		$this->assertTrue(file_exists(PATH_CACHE . 'unit-tests.views.test10.index.volt.compiled'));
		$this->assertEquals(file_get_contents(PATH_CACHE . 'unit-tests.views.test10.index.volt.compiled'), 'Hello <?php echo $song; ?>!');
	...
	public function testVoltEngine()
	{
		...
		@unlink(PATH_VIEWS . 'layouts/test10.volt.php');
		@unlink(PATH_VIEWS . 'test10/index.volt.php');
		@unlink(PATH_VIEWS . 'test10/other.volt.php');
		...
		file_put_contents(PATH_VIEWS . 'test10/other.volt', '{{song}} {{song}}');
		...
		//Change the view
		file_put_contents(PATH_VIEWS . 'test10/other.volt', 'Two songs: {{song}} {{song}}');
