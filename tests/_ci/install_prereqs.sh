...
if [ ${TRAVIS_PHP_VERSION} == "5.4" ]; then
    ...
    ( mkdir -p /tmp/apcu && cd /tmp/apcu && git clone --depth=1 https://github.com/krakjoe/apcu /tmp/apcu && phpize && ./configure && make -j 4 && sudo make install && phpenv config-add "$DIR/apcu.ini" && pecl install apcu_bc-beta) &
else
