...
PHP_METHOD(Phalcon_Validation_Validator_Numericality, validate){
	...
	if (PHALCON_IS_FALSE(valid)) {
		...
		if (!zend_is_true(message_str)) {
			...
			RETURN_MM_ON_FAILURE(phalcon_validation_getdefaultmessage_helper(Z_OBJCE_P(validator), &message_str, validator, "Numericality" TSRMLS_CC));
		}
		...
		message = phalcon_validation_message_construct_helper(prepared, attribute, "Numericality", code TSRMLS_CC);
		Z_DELREF_P(message);
