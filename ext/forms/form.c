...
PHP_METHOD(Phalcon_Forms_Form, isValid){
	...
	zval *element_messages = NULL, *current_data = NULL;
	HashTable *ah0, *ah1;
	 ...
	 */
	if (Z_TYPE_P(data) != IS_ARRAY) {
		PHALCON_OBS_NVAR(current_data);
		phalcon_read_property_this(&current_data, this_ptr, SL("_data"), PH_NOISY_CC);
	} else {
		PHALCON_CPY_WRT(current_data, data);
	}
	...
	if (phalcon_method_exists_ex(this_ptr, SS("beforevalidation") TSRMLS_CC) == SUCCESS) {
		...
		phalcon_call_method_p2(status, this_ptr, "beforevalidation", current_data, entity);
	...
	while (zend_hash_get_current_data_ex(ah0, (void**) &hd, &hp0) == SUCCESS) {
		...
		if (Z_TYPE_P(validators) == IS_ARRAY) {
			if (phalcon_fast_count_ev(validators TSRMLS_CC)) {
				...
				phalcon_call_method_p2(element_messages, validation, "validate", current_data, entity);
