...
PHP_METHOD(Phalcon_DI, setRaw){
	...
	if (Z_TYPE_P(raw_definition) != IS_OBJECT || !instanceof_function_ex(Z_OBJCE_P(raw_definition), phalcon_di_serviceinterface_ce, 1 TSRMLS_CC)) {
		PHALCON_THROW_EXCEPTION_STRW(phalcon_di_exception_ce, "The service definition must implement Phalcon\\DI\\ServiceInterface interface");
		return;
	}
