...
void zephir_concat_ssssssssssssssssssssssssssssssvssvss(zval *result, const char *op1, uint32_t op1_len, const char *op2, uint32_t op2_len, const char *op3, uint32_t op3_len, const char *op4, uint32_t op4_len, const char *op5, uint32_t op5_len, const char *op6, uint32_t op6_len, const char *op7, uint32_t op7_len, const char *op8, uint32_t op8_len, const char *op9, uint32_t op9_len, const char *op10, uint32_t op10_len, const char *op11, uint32_t op11_len, const char *op12, uint32_t op12_len, const char *op13, uint32_t op13_len, const char *op14, uint32_t op14_len, const char *op15, uint32_t op15_len, const char *op16, uint32_t op16_len, const char *op17, uint32_t op17_len, const char *op18, uint32_t op18_len, const char *op19, uint32_t op19_len, const char *op20, uint32_t op20_len, const char *op21, uint32_t op21_len, const char *op22, uint32_t op22_len, const char *op23, uint32_t op23_len, const char *op24, uint32_t op24_len, const char *op25, uint32_t op25_len, const char *op26, uint32_t op26_len, const char *op27, uint32_t op27_len, const char *op28, uint32_t op28_len, const char *op29, uint32_t op29_len, const char *op30, uint32_t op30_len, zval *op31, const char *op32, uint32_t op32_len, const char *op33, uint32_t op33_len, zval *op34, const char *op35, uint32_t op35_len, const char *op36, uint32_t op36_len, int self_var){
	...
	length += op9_len;
	if (UNEXPECTED(op10_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op10_len;
	if (UNEXPECTED(op11_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op11_len;
	if (UNEXPECTED(op12_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op12_len;
	if (UNEXPECTED(op13_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op13_len;
	if (UNEXPECTED(op14_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op14_len;
	if (UNEXPECTED(op15_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op15_len;
	if (UNEXPECTED(op16_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op16_len;
	if (UNEXPECTED(op17_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op17_len;
	if (UNEXPECTED(op18_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op18_len;
	if (UNEXPECTED(op19_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op19_len;
	if (UNEXPECTED(op20_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op20_len;
	if (UNEXPECTED(op21_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op21_len;
	if (UNEXPECTED(op22_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op22_len;
	if (UNEXPECTED(op23_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op23_len;
	if (UNEXPECTED(op24_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op24_len;
	if (UNEXPECTED(op25_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op25_len;
	if (UNEXPECTED(op26_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op26_len;
	if (UNEXPECTED(op27_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op27_len;
	if (UNEXPECTED(op28_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op28_len;
	if (UNEXPECTED(op29_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op29_len;
	if (UNEXPECTED(op30_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op30_len;
	if (UNEXPECTED(Z_STRLEN_P(op31) > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += Z_STRLEN_P(op31);
	if (UNEXPECTED(op32_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op32_len;
	if (UNEXPECTED(op33_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op33_len;
	if (UNEXPECTED(Z_STRLEN_P(op34) > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += Z_STRLEN_P(op34);
	if (UNEXPECTED(op35_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op35_len;
	if (UNEXPECTED(op36_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op36_len;
	if (self_var) {
...
void zephir_concat_ssssssssvssvss(zval *result, const char *op1, uint32_t op1_len, const char *op2, uint32_t op2_len, const char *op3, uint32_t op3_len, const char *op4, uint32_t op4_len, const char *op5, uint32_t op5_len, const char *op6, uint32_t op6_len, const char *op7, uint32_t op7_len, const char *op8, uint32_t op8_len, zval *op9, const char *op10, uint32_t op10_len, const char *op11, uint32_t op11_len, zval *op12, const char *op13, uint32_t op13_len, const char *op14, uint32_t op14_len, int self_var){
	...
	length += op8_len;
	if (UNEXPECTED(Z_STRLEN_P(op9) > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += Z_STRLEN_P(op9);
	if (UNEXPECTED(op10_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op10_len;
	...
	length += op11_len;
	if (UNEXPECTED(Z_STRLEN_P(op12) > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += Z_STRLEN_P(op12);
	if (UNEXPECTED(op13_len > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += op13_len;
...
void zephir_concat_ssvsv(zval *result, const char *op1, uint32_t op1_len, const char *op2, uint32_t op2_len, zval *op3, const char *op4, uint32_t op4_len, zval *op5, int self_var){
...
void zephir_concat_svsss(zval *result, const char *op1, uint32_t op1_len, zval *op2, const char *op3, uint32_t op3_len, const char *op4, uint32_t op4_len, const char *op5, uint32_t op5_len, int self_var){
...
void zephir_concat_svvvvvvvvvv(zval *result, const char *op1, uint32_t op1_len, zval *op2, zval *op3, zval *op4, zval *op5, zval *op6, zval *op7, zval *op8, zval *op9, zval *op10, zval *op11, int self_var){
	...
	length += Z_STRLEN_P(op10);
	if (UNEXPECTED(Z_STRLEN_P(op11) > ZSTR_MAX_LEN - length)) {
		goto zephir_concat_overflow;
	}
	length += Z_STRLEN_P(op11);
	if (self_var) {
...
void zephir_concat_vvsvv(zval *result, zval *op1, zval *op2, const char *op3, uint32_t op3_len, zval *op4, zval *op5, int self_var){
	...
	memcpy(Z_STRVAL_P(result) + offset + Z_STRLEN_P(op1) + Z_STRLEN_P(op2), op3, op3_len);
	memcpy(Z_STRVAL_P(result) + offset + Z_STRLEN_P(op1) + Z_STRLEN_P(op2) + op3_len, Z_STRVAL_P(op4), Z_STRLEN_P(op4));
	memcpy(Z_STRVAL_P(result) + offset + Z_STRLEN_P(op1) + Z_STRLEN_P(op2) + op3_len + Z_STRLEN_P(op4), Z_STRVAL_P(op5), Z_STRLEN_P(op5));
	Z_STRVAL_P(result)[length] = 0;
	zend_string_forget_hash_val(Z_STR_P(result));
	goto zephir_concat_cleanup;
