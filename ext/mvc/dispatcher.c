...
PHP_METHOD(Phalcon_Mvc_Dispatcher, getControllerName){
	...
	is_exact = phalcon_fetch_nproperty_this(getThis(), SL("_isExactControllerName"), PH_NOISY TSRMLS_CC);

	if (!zend_is_true(is_exact)) {
		RETURN_MEMBER(this_ptr, "_handlerName");
	}

	name = phalcon_fetch_nproperty_this(getThis(), SL("_handlerName"), zend_inline_hash_func(SS("_handlerName")) TSRMLS_CC);
