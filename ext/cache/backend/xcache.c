...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, save){
	...
	zval *prepared_content, *ttl = NULL, *is_buffering, *success;
	...
	/**
	 * xcache_set() could fail because of Out of Memory condition.
	 * I don't think it is appropriate to throw an exception here
	 * (like Phalcon\Cache\Backend\Memcache::save() does): first,
	 * to be consistent with Phalcon\Cache\Backend\Apc::save(),
	 * second, because xCache is usually given much less RAM than memcached
	 */
	if (zend_is_true(success)) {
		/**
		 * xcache_list() is available only to the administrator (unless
		 * XCache was patched). We have to update the list of the stored keys.
		 */
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, queryKeys){
	...
	zval *prefix = NULL, *options, *special_key, *empty_arr;
	...
	PHALCON_CONCAT_SV(prefix, "_PHCX", prefix);
	...
	/**
	 * Get the key from XCache (we cannot use xcache_list() as it is available
