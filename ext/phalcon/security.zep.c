	...
	do {
		if (hash == 1 || hash == 2) {
			...
			if (Z_TYPE_P(saltBytes) != IS_STRING) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_security_exception_ce, "Unable to get random bytes for the salt", "phalcon/security.zep", 223);
				return;
			}
			ZEPHIR_RETURN_CALL_FUNCTION("crypt", &_6, 415, password, saltBytes);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (hash == 3 || hash == 8 || hash == 9) {
			...
			zephir_check_call_status();
			if (Z_TYPE_P(saltBytes) != IS_STRING) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_security_exception_ce, "Unable to get random bytes for the salt", "phalcon/security.zep", 240);
				return;
			}
			ZEPHIR_INIT_VAR(_8$$15);
			ZEPHIR_CONCAT_SVSVS(_8$$15, "$", variant, "$", saltBytes, "$");
			ZEPHIR_RETURN_CALL_FUNCTION("crypt", &_6, 415, password, _8$$15);
			zephir_check_call_status();
			RETURN_MM();
		}
		...
		zephir_check_call_status();
		if (Z_TYPE_P(saltBytes) != IS_STRING) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_security_exception_ce, "Unable to get random bytes for the salt", "phalcon/security.zep", 263);
			return;
		}
		if (workFactor < 4) {
			workFactor = 4;
		...
		ZEPHIR_RETURN_CALL_FUNCTION("crypt", &_6, 415, password, _13$$17);
		zephir_check_call_status();
		RETURN_MM();
	} while(0);
...
PHP_METHOD(Phalcon_Security, checkHash) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "crypt", NULL, 415, password, passwordHash);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Security, getTokenKey) {
	...
	if (Z_TYPE_P(_0) == IS_NULL) {
		...
		if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_security_exception_ce, "A dependency injection container is required to access the 'session' service", "phalcon/security.zep", 328);
			return;
		}
...
PHP_METHOD(Phalcon_Security, getToken) {
	...
	if (Z_TYPE_P(_0) == IS_NULL) {
		...
		if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_security_exception_ce, "A dependency injection container is required to access the 'session' service", "phalcon/security.zep", 352);
			return;
		}
...
PHP_METHOD(Phalcon_Security, checkToken) {
	...
	if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_security_exception_ce, "A dependency injection container is required to access the 'session' service", "phalcon/security.zep", 372);
		return;
	}
	ZEPHIR_INIT_VAR(_2);
...
PHP_METHOD(Phalcon_Security, computeHmac) {
	...
	ZEPHIR_CALL_FUNCTION(&hmac, "hash_hmac", NULL, 416, algo, data, key, &_0);
	zephir_check_call_status();
