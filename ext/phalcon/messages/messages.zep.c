...
PHP_METHOD(Phalcon_Messages_Messages, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 947, &messages);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Messages_Messages, appendMessages)
{
	...
	if (_0) {
		...
		zephir_throw_exception_debug(&_2$$3, "phalcon/Messages/Messages.zep", 73);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_memory_observe(&currentMessages);
	zephir_read_property_cached(&currentMessages, this_ptr, _zephir_prop_0, 947, PH_NOISY_CC);
	if (Z_TYPE_P(messages) == IS_ARRAY) {
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 947, &finalMessages);
	} else {
...
PHP_METHOD(Phalcon_Messages_Messages, filter)
{
	...
	array_init(&filtered);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 947, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&messages, &_0);
	if (Z_TYPE_P(&messages) == IS_ARRAY) {
		zephir_is_iterable(&messages, 0, "phalcon/Messages/Messages.zep", 129);
		if (Z_TYPE_P(&messages) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&messages), _1$$3)
			{
				...
				if ((zephir_method_exists_ex(&message, ZEND_STRL("getfield")) == SUCCESS)) {
					...
					if (ZEPHIR_IS_EQUAL(&fieldName_zv, &_2$$5)) {
						zephir_array_append(&filtered, &message, PH_SEPARATE, "phalcon/Messages/Messages.zep", 125);
					}
				}
...
PHP_METHOD(Phalcon_Messages_Messages, jsonSerialize)
{
	...
	array_init(&records);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 947, PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_0, 0, "phalcon/Messages/Messages.zep", 152);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _1)
		{
			...
			zephir_check_call_status();
			zephir_array_append(&records, &_2$$3, PH_SEPARATE, "phalcon/Messages/Messages.zep", 149);
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_check_call_status();
				zephir_array_append(&records, &_6$$4, PH_SEPARATE, "phalcon/Messages/Messages.zep", 149);
		}
	}
	ZEPHIR_INIT_NVAR(&message);
	RETURN_CTOR(&records);
...
PHP_METHOD(Phalcon_Messages_Messages, current)
{
	...
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_1, 948, PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Messages_Messages, offsetUnset)
{
	...
	if (zephir_array_isset_value(&_0, offset)) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 947, PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_2$$3, 1);
...
PHP_METHOD(Phalcon_Messages_Messages, valid)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 947, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 948, PH_NOISY_CC | PH_READONLY);
