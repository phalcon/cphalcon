...
ZEPHIR_INIT_CLASS(Phalcon_Acl_Role)
{
	 ...
	 * @var string | null
	 */
	zend_declare_property_null(phalcon_acl_role_ce, SL("description"), ZEND_ACC_PROTECTED);
	/**
	 * Element name
	 *
	 * @var string
	 */
	zend_declare_property_null(phalcon_acl_role_ce, SL("name"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Acl_Role, __construct)
{
	...
	if (UNEXPECTED(ZEPHIR_IS_STRING_IDENTICAL(&name_zv, "*"))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_acl_exceptions_forbiddenwildcard_ce, "role", "phalcon/Acl/Role.zep", 29);
		return;
	}
