...
PHP_METHOD(Phalcon_Acl_Adapter_Storage, load)
{
	...
	if (Z_TYPE_P(&data) == IS_OBJECT) {
		ZEPHIR_CALL_METHOD(&_2$$3, this_ptr, "normalizetoarray", NULL, 289, &data);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&_17) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_17), _19, _20, _18)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_21$$8, "__construct", &_22, 216, &name, &description);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_25$$9, "__construct", &_22, 216, &name, &description);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(&_26) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_26), _28, _29, _27)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_30$$10, "__construct", &_31, 208, &name, &description);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_34$$11, "__construct", &_31, 208, &name, &description);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Storage, normalizeToArray)
{
	...
	if (Z_TYPE_P(value) == IS_OBJECT) {
		ZEPHIR_CALL_FUNCTION(&_0$$3, "get_object_vars", NULL, 290, value);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(value), _2, _3, _1)
		{
			...
			ZVAL_COPY(&item, _1);
			ZEPHIR_CALL_METHOD(&_4$$5, this_ptr, "normalizetoarray", &_5, 289, &item);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_8$$6, this_ptr, "normalizetoarray", &_5, 289, &item);
				zephir_check_call_status();
