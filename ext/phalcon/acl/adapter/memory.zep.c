...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, isAllowed) {
	...
	if (zephir_is_callable(&funcAccess TSRMLS_CC)) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&reflectionParameters), _12$$17)
		{
			...
			if (Z_TYPE_P(&reflectionClass) != IS_NULL) {
				...
				if (_19$$20) {
					...
					zephir_array_fetch(&_20$$20, &parameters, &parameterToCheck, PH_NOISY, "phalcon/Acl/Adapter/Memory.zep", 737 TSRMLS_CC);
					_19$$20 = Z_TYPE_P(&_20$$20) == IS_OBJECT;
				}
				_21$$20 = _19$$20;
				if (_21$$20) {
					zephir_array_fetch(&_23$$20, &parameters, &parameterToCheck, PH_NOISY | PH_READONLY, "phalcon/Acl/Adapter/Memory.zep", 737 TSRMLS_CC);
				...
				if (UNEXPECTED(_21$$20)) {
					...
					zephir_array_fetch(&_26$$23, &parameters, &parameterToCheck, PH_NOISY | PH_READONLY, "phalcon/Acl/Adapter/Memory.zep", 739 TSRMLS_CC);
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_24$$23, "phalcon/Acl/Adapter/Memory.zep", 740 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
			if (zephir_array_isset(&parameters, &parameterToCheck)) {
				zephir_array_fetch(&_30$$24, &parameters, &parameterToCheck, PH_NOISY | PH_READONLY, "phalcon/Acl/Adapter/Memory.zep", 749 TSRMLS_CC);
				zephir_array_append(&parametersForFunction, &_30$$24, PH_SEPARATE, "phalcon/Acl/Adapter/Memory.zep", 749);
			}
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&reflectionParameter);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_40$$17, "phalcon/Acl/Adapter/Memory.zep", 790 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (Z_TYPE_P(access) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(access), _7$$5)
		{
			...
			if (UNEXPECTED(!(zephir_array_isset(&accessList, &accessKey)))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_8$$7, "phalcon/Acl/Adapter/Memory.zep", 850 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} ZEND_HASH_FOREACH_END();
		...
		if (!ZEPHIR_IS_STRING(access, "*")) {
			...
			if (UNEXPECTED(!(zephir_array_isset(&accessList, &accessKey)))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_11$$12, "phalcon/Acl/Adapter/Memory.zep", 869 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
	...
	if (zephir_array_isset(&_0, &roleName)) {
		...
		while (1) {
			...
			if (zephir_array_isset(&_9$$8, &checkRoleToInherit)) {
				...
				zephir_array_fetch(&_11$$13, &_10$$13, &checkRoleToInherit, PH_NOISY | PH_READONLY, "phalcon/Acl/Adapter/Memory.zep", 977 TSRMLS_CC);
				zephir_is_iterable(&_11$$13, 0, "phalcon/Acl/Adapter/Memory.zep", 980);
