...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addComponent)
{
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &componentObject, "__construct", NULL, 162, componentValue);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_METHOD(&componentName, &componentObject, "getname", NULL, 163);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addComponentAccess)
{
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkexists", NULL, 164, &_0, &componentName, &_1);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addInherit)
{
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkexists", NULL, 164, &_0, &roleName, &_1, &_2);
	zephir_check_call_status();
			...
			if (zephir_array_isset(&_14$$6, &roleInheritName)) {
				...
				if (Z_TYPE_P(&_16$$12) == IS_ARRAY) {
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_16$$12), _17$$12)
					{
						...
						ZEPHIR_CALL_FUNCTION(NULL, "array_push", &_19, 165, &checkRoleToInherits, &usedRoleToInherit);
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addRole)
{
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &roleObject, "__construct", NULL, 166, role);
		zephir_check_call_status();
	} else {
	...
	ZEPHIR_CALL_METHOD(&roleName, &roleObject, "getname", NULL, 167);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, allow)
{
	...
	if (Z_TYPE_P(&rolesArray) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&rolesArray), _2)
		{
			...
			ZVAL_COPY(&role, _2);
			ZVAL_LONG(&_4$$4, 1);
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", &_5, 168, &role, &componentName, access, &_4$$4, func);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				ZVAL_LONG(&_6$$5, 1);
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", &_5, 168, &role, &componentName, access, &_6$$5, func);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, isAllowed)
{
	...
	ZEPHIR_CALL_METHOD(&accessKey, this_ptr, "canaccess", NULL, 169, roleName, componentName, &access);
	zephir_check_call_status();
	...
	if (zephir_is_callable(&funcAccess)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflectionFunction, "__construct", NULL, 170, &funcAccess);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&reflectionParameters, &reflectionFunction, "getparameters", NULL, 171);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&numberOfRequiredParameters, &reflectionFunction, "getnumberofrequiredparameters", NULL, 172);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, allowOrDeny)
{
	...
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "Role");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkexists", NULL, 164, &_0, &roleName, &_1);
	zephir_check_call_status();
	zephir_read_property(&_2, this_ptr, ZEND_STRL("componentsNames"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_NVAR(&_1);
	ZVAL_STRING(&_1, "Component");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkexists", NULL, 164, &_2, &componentName, &_1);
	zephir_check_call_status();
	...
	if (zephir_array_isset(&_0, &roleName)) {
		...
		if (Z_TYPE_P(&_2$$6) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_2$$6), _3$$6)
			{
				...
				ZEPHIR_MAKE_REF(&checkRoleToInherits);
				ZEPHIR_CALL_FUNCTION(NULL, "array_push", &_5, 165, &checkRoleToInherits, &usedRoleToInherit);
				ZEPHIR_UNREF(&checkRoleToInherits);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
