...
PHP_METHOD(Phalcon_Assets_Resource, __construct) {
	...
	if (!local_param || Z_TYPE_P(local_param) == IS_NULL) {
		local = 1;
	} else {
		local = zephir_get_boolval(local_param);
	}
	if (!filter_param || Z_TYPE_P(filter_param) == IS_NULL) {
		filter = 1;
	} else {
...
PHP_METHOD(Phalcon_Assets_Resource, getContent) {
	...
	zval *basePath_param = NULL, *sourcePath = NULL, *completePath, *content, *_0, *_1 = NULL, *_2, *_3 = NULL;
	zval *basePath = NULL;

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &basePath_param);

	if (!basePath_param || Z_TYPE_P(basePath_param) == IS_NULL) {
	...
	if (zephir_is_true(_0)) {
		...
		if (!(zephir_is_true(_1))) {
			...
			ZEPHIR_INIT_VAR(_3);
			ZEPHIR_CONCAT_SVS(_3, "Resource's content for '", completePath, "' cannot be read");
...
PHP_METHOD(Phalcon_Assets_Resource, getRealTargetPath) {
	...
	zval *basePath_param = NULL, *targetPath = NULL, *completePath, *_0, *_1;
	zval *basePath = NULL;
