...
PHP_METHOD(Phalcon_Assets_Manager, addCss) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 127, &path, local, filter, attributes);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "css");
...
PHP_METHOD(Phalcon_Assets_Manager, addJs) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 130, &path, local, filter, attributes);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "js");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "addassetbytype", NULL, 0, &_1, &_0);
...
PHP_METHOD(Phalcon_Assets_Manager, output) {
	...
	zephir_is_iterable(&assets, 0, "phalcon/Assets/Manager.zep", 691);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&assets), _7)
	{
		...
		if (zephir_fast_count_int(&filters TSRMLS_CC)) {
			if (zephir_is_true(&local)) {
				...
				if (UNEXPECTED(!zephir_is_true(&sourcePath))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_8$$15, "phalcon/Assets/Manager.zep", 481 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			} else {
			...
			if (UNEXPECTED(!zephir_is_true(&targetPath))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_10$$17, "phalcon/Assets/Manager.zep", 508 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			if (zephir_is_true(&local)) {
				if (UNEXPECTED(ZEPHIR_IS_EQUAL(&targetPath, &sourcePath))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_12$$19, "phalcon/Assets/Manager.zep", 519 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			...
			if (Z_TYPE_P(&attributes) == IS_ARRAY) {
				...
				zephir_array_append(&parameters, &attributes, PH_SEPARATE, "phalcon/Assets/Manager.zep", 554);
			} else {
				zephir_array_append(&parameters, &prefixedPath, PH_SEPARATE, "phalcon/Assets/Manager.zep", 556);
			}
			zephir_array_append(&parameters, &local, PH_SEPARATE, "phalcon/Assets/Manager.zep", 558);
			...
			if (ZEPHIR_IS_TRUE(&mustFilter)) {
				...
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&filters), _15$$31)
				{
					...
					if (UNEXPECTED(Z_TYPE_P(&filter) != IS_OBJECT)) {
						ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_assets_exception_ce, "Filter is invalid", "phalcon/Assets/Manager.zep", 599);
						return;
					}
		...
		if (!(zephir_is_true(&join))) {
			...
			if (Z_TYPE_P(&attributes) == IS_ARRAY) {
				zephir_array_update_long(&attributes, 0, &prefixedPath, PH_COPY | PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
				zephir_array_append(&parameters, &attributes, PH_SEPARATE, "phalcon/Assets/Manager.zep", 669);
			} else {
				zephir_array_append(&parameters, &prefixedPath, PH_SEPARATE, "phalcon/Assets/Manager.zep", 671);
			}
			zephir_array_append(&parameters, &local, PH_SEPARATE, "phalcon/Assets/Manager.zep", 673);
			ZEPHIR_INIT_NVAR(&html);
			ZEPHIR_CALL_USER_FUNC_ARRAY(&html, callback, &parameters);
			zephir_check_call_status();
			if (ZEPHIR_IS_TRUE(&useImplicitOutput)) {
	...
	if (zephir_fast_count_int(&filters TSRMLS_CC)) {
		if (ZEPHIR_IS_TRUE(&join)) {
			...
			if (Z_TYPE_P(&attributes) == IS_ARRAY) {
				zephir_array_update_long(&attributes, 0, &prefixedPath, PH_COPY | PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
				zephir_array_append(&parameters, &attributes, PH_SEPARATE, "phalcon/Assets/Manager.zep", 728);
			} else {
				zephir_array_append(&parameters, &prefixedPath, PH_SEPARATE, "phalcon/Assets/Manager.zep", 730);
			}
			zephir_array_append(&parameters, &local, PH_SEPARATE, "phalcon/Assets/Manager.zep", 732);
			ZEPHIR_INIT_NVAR(&html);
			ZEPHIR_CALL_USER_FUNC_ARRAY(&html, callback, &parameters);
			zephir_check_call_status();
			if (ZEPHIR_IS_TRUE(&useImplicitOutput)) {
...
PHP_METHOD(Phalcon_Assets_Manager, outputInline) {
	...
	if (zephir_fast_count_int(&codes TSRMLS_CC)) {
		zephir_is_iterable(&codes, 0, "phalcon/Assets/Manager.zep", 824);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&codes), _0$$3)
		{
			...
			zephir_check_call_status();
			zephir_is_iterable(&filters, 0, "phalcon/Assets/Manager.zep", 817);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&filters), _1$$4)
			{
				ZEPHIR_INIT_NVAR(&filter);
				ZVAL_COPY(&filter, _1$$4);
				if (UNEXPECTED(Z_TYPE_P(&filter) != IS_OBJECT)) {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_assets_exception_ce, "Filter is invalid", "phalcon/Assets/Manager.zep", 807);
					return;
				}
