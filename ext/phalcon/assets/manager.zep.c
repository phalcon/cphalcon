	...
	if (Z_TYPE_P(&assets) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&assets), _3)
		{
			...
			if (1 != ZEPHIR_IS_EMPTY(&filters)) {
				...
				zephir_check_call_status();
				zephir_read_property_cached(&_18$$12, this_ptr, _zephir_prop_1, 417, PH_NOISY_CC | PH_READONLY);
			...
			if (!zephir_is_true(&join)) {
				...
				zephir_check_call_status();
				zephir_read_property_cached(&_27$$19, this_ptr, _zephir_prop_1, 417, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (1 != ZEPHIR_IS_EMPTY(&filters)) {
					...
					zephir_check_call_status();
					zephir_read_property_cached(&_39$$26, this_ptr, _zephir_prop_1, 417, PH_NOISY_CC | PH_READONLY);
				...
				if (!zephir_is_true(&join)) {
					...
					zephir_check_call_status();
					zephir_read_property_cached(&_45$$33, this_ptr, _zephir_prop_1, 417, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Assets_Manager, processParameters)
{
	...
	zephir_read_property_cached(&_19, this_ptr, _zephir_prop_1, 414, PH_NOISY_CC | PH_READONLY);
