...
PHP_METHOD(Phalcon_Assets_Manager, collectionAssetsByType) {
	...
	array_init(&filtered);
	zephir_is_iterable(&assets, 0, "phalcon/Assets/Manager.zep", 239);
	if (Z_TYPE_P(&assets) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&assets), _0)
		{
			...
			if (ZEPHIR_IS_EQUAL(&_2$$3, &type)) {
				zephir_array_append(&filtered, &asset, PH_SEPARATE, "phalcon/Assets/Manager.zep", 235);
			}
...
PHP_METHOD(Phalcon_Assets_Manager, get) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&collection, &_0, &id, 0)))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_assets_exception_ce, "The collection does not exist in the manager", "phalcon/Assets/Manager.zep", 269);
		return;
	}
	RETURN_CCTOR(&collection);
	...
	if (zephir_fast_count_int(&filters)) {
		...
		if (zephir_is_true(&join)) {
			if (UNEXPECTED(!zephir_is_true(&completeTargetPath))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_2$$10, "phalcon/Assets/Manager.zep", 432);
				ZEPHIR_MM_RESTORE();
				return;
			}
			ZEPHIR_CALL_FUNCTION(&_4$$9, "is_dir", NULL, 126, &completeTargetPath);
			zephir_check_call_status();
			if (UNEXPECTED(zephir_is_true(&_4$$9))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_5$$11, "phalcon/Assets/Manager.zep", 438);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
	}
	zephir_is_iterable(&assets, 0, "phalcon/Assets/Manager.zep", 696);
	if (Z_TYPE_P(&assets) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&assets), _7)
		{
			...
			if (zephir_fast_count_int(&filters)) {
				if (zephir_is_true(&local)) {
					...
					if (UNEXPECTED(!zephir_is_true(&sourcePath))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_9$$15, "phalcon/Assets/Manager.zep", 476);
						ZEPHIR_MM_RESTORE();
						return;
					}
				} else {
				...
				if (UNEXPECTED(!zephir_is_true(&targetPath))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_11$$17, "phalcon/Assets/Manager.zep", 502);
					ZEPHIR_MM_RESTORE();
					return;
				}
				if (zephir_is_true(&local)) {
					if (UNEXPECTED(ZEPHIR_IS_EQUAL(&targetPath, &sourcePath))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_13$$19, "phalcon/Assets/Manager.zep", 512);
						ZEPHIR_MM_RESTORE();
						return;
					}
					...
					if (Z_TYPE_P(&filters) == IS_ARRAY) {
						ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&filters), _28$$32)
						{
							...
							if (UNEXPECTED(Z_TYPE_P(&filter) != IS_OBJECT)) {
								ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_assets_exception_ce, "Filter is invalid", "phalcon/Assets/Manager.zep", 597);
								return;
							}
			...
			if (!(zephir_is_true(&join))) {
				...
				zephir_array_append(&parameters, &local, PH_SEPARATE, "phalcon/Assets/Manager.zep", 678);
				ZEPHIR_INIT_NVAR(&html);
				ZEPHIR_CALL_USER_FUNC_ARRAY(&html, callback, &parameters);
				zephir_check_call_status();
				if (ZEPHIR_IS_TRUE(&useImplicitOutput)) {
				...
				if (zephir_fast_count_int(&filters)) {
					if (zephir_is_true(&local)) {
						...
						if (UNEXPECTED(!zephir_is_true(&sourcePath))) {
							...
							zephir_check_call_status();
							zephir_throw_exception_debug(&_40$$55, "phalcon/Assets/Manager.zep", 476);
							ZEPHIR_MM_RESTORE();
							return;
						}
					} else {
					...
					if (UNEXPECTED(!zephir_is_true(&targetPath))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_42$$57, "phalcon/Assets/Manager.zep", 502);
						ZEPHIR_MM_RESTORE();
						return;
					}
					if (zephir_is_true(&local)) {
						if (UNEXPECTED(ZEPHIR_IS_EQUAL(&targetPath, &sourcePath))) {
							...
							zephir_check_call_status();
							zephir_throw_exception_debug(&_44$$59, "phalcon/Assets/Manager.zep", 512);
							ZEPHIR_MM_RESTORE();
							return;
						}
						if ((zephir_file_exists(&targetPath) == SUCCESS)) {
...
PHP_METHOD(Phalcon_Assets_Manager, outputInline) {
	...
	if (zephir_fast_count_int(&codes)) {
		zephir_is_iterable(&codes, 0, "phalcon/Assets/Manager.zep", 832);
		if (Z_TYPE_P(&codes) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&codes), _0$$3)
			{
				...
				zephir_check_call_status();
				zephir_is_iterable(&filters, 0, "phalcon/Assets/Manager.zep", 823);
				if (Z_TYPE_P(&filters) == IS_ARRAY) {
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&filters), _2$$4)
					{
						...
						if (UNEXPECTED(Z_TYPE_P(&filter) != IS_OBJECT)) {
							ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_assets_exception_ce, "Filter is invalid", "phalcon/Assets/Manager.zep", 813);
							return;
						}
