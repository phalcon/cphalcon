...
PHP_METHOD(Phalcon_Forms_Element_Check, setUncheckedValue)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 769, value);
	if (1) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 770, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 770, &__$false);
	}
	RETURN_THISW();
}
