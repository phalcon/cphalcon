...
PHP_METHOD(Phalcon_Forms_Element_Check, setUncheckedValue)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 753, value);
	if (1) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 754, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 754, &__$false);
	}
	RETURN_THISW();
}
