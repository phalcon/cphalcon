...
PHP_METHOD(Phalcon_Forms_Loader_YamlLoader, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 791, &source_zv);
}
...
PHP_METHOD(Phalcon_Forms_Loader_YamlLoader, load)
{
	...
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 791, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&source, &_3);
	ZEPHIR_CALL_FUNCTION(&_4, "is_file", NULL, 454, &source);
	zephir_check_call_status();
	...
	if (_5) {
		ZEPHIR_CALL_FUNCTION(&definitions, "yaml_parse_file", NULL, 457, &source);
		zephir_check_call_status();
	} else {
