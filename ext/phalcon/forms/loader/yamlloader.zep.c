...
PHP_METHOD(Phalcon_Forms_Loader_YamlLoader, load)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "yaml");
	ZEPHIR_CALL_FUNCTION(&_1, "extension_loaded", NULL, 381, &_0);
	...
	ZEPHIR_CALL_FUNCTION(&_4, "is_file", NULL, 378, &source);
	zephir_check_call_status();
	...
	if (_5) {
		ZEPHIR_CALL_FUNCTION(&definitions, "yaml_parse_file", NULL, 380, &source);
		zephir_check_call_status();
	} else {
