...
PHP_METHOD(Phalcon_Forms_Form, isValid)
{
	...
	if (_7) {
		...
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 282);
		zephir_check_call_status();
	}
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_8), _9)
		{
			...
			if (Z_TYPE_P(&validators) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _11$$10)
				{
					...
					ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_13, 283, &name, &validator);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
			...
			if (Z_TYPE_P(&filters) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_14, 284, &name, &filters);
				zephir_check_call_status();
			}
		} ZEND_HASH_FOREACH_END();
	} else {
	...
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(&messages, &validation, "validate", NULL, 285, data, entity);
	zephir_check_call_status();
