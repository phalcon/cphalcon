...
PHP_METHOD(Phalcon_Forms_Form, __construct)
{
	...
	if (UNEXPECTED(_0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_forms_exception_ce, "The base entity is not valid", "phalcon/Forms/Form.zep", 87);
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, add)
{
	...
	if (_0) {
		...
		zephir_is_iterable(&_2$$4, 0, "phalcon/Forms/Form.zep", 162);
...
PHP_METHOD(Phalcon_Forms_Form, bind)
{
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data_param = NULL, *entity = NULL, entity_sub, *whitelist = NULL, whitelist_sub, __$null, filter, key, value, element, filters, container, filteredValue, _0, *_1, _2, _5$$4, _8$$8, _9$$8, _10$$10, _11$$10, _12$$12, _14$$16, _15$$16, _16$$18, _17$$18;
	zval data, assignData, filteredData;
	zval *this_ptr = getThis();
	...
	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 2, &data_param, &entity, &whitelist);
	...
	if (UNEXPECTED(ZEPHIR_IS_EMPTY(&_0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_forms_exception_ce, "There are no elements in the form", "phalcon/Forms/Form.zep", 181);
		return;
	}
	...
	zephir_is_iterable(&data, 0, "phalcon/Forms/Form.zep", 244);
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _3, _4, _1)
		{
			...
			if (Z_TYPE_P(entity) != IS_NULL) {
				...
				zephir_camelize(&_10$$10, &key, NULL  );
				ZEPHIR_INIT_NVAR(&_11$$10);
				ZEPHIR_CONCAT_SV(&_11$$10, "set", &_10$$10);
				zephir_get_strval(&method, &_11$$10);
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_OBS_NVAR(&element);
				zephir_read_property(&_12$$12, this_ptr, ZEND_STRL("elements"), PH_NOISY_CC | PH_READONLY);
				if (!(zephir_array_isset_fetch(&element, &_12$$12, &key, 0))) {
					continue;
				}
				_13$$12 = Z_TYPE_P(whitelist) == IS_ARRAY;
				if (_13$$12) {
					_13$$12 = !(zephir_fast_in_array(&key, whitelist));
				}
				if (_13$$12) {
					continue;
				}
				...
				if (zephir_is_true(&filters)) {
					if (Z_TYPE_P(&filter) != IS_OBJECT) {
						...
						ZVAL_STRING(&_15$$16, "filter");
						ZEPHIR_CALL_METHOD(&_14$$16, &container, "getshared", NULL, 0, &_15$$16);
						zephir_check_call_status();
						ZEPHIR_CPY_WRT(&filter, &_14$$16);
					}
					ZEPHIR_CALL_METHOD(&filteredValue, &filter, "sanitize", NULL, 0, &value, &filters);
					zephir_check_call_status();
				} else {
				...
				if (Z_TYPE_P(entity) != IS_NULL) {
					...
					zephir_camelize(&_16$$18, &key, NULL  );
					ZEPHIR_INIT_NVAR(&_17$$18);
					ZEPHIR_CONCAT_SV(&_17$$18, "set", &_16$$18);
					zephir_get_strval(&method, &_17$$18);
					if ((zephir_method_exists(entity, &method)  == SUCCESS)) {
						ZEPHIR_CALL_METHOD_ZVAL(NULL, entity, &method, NULL, 0, &filteredValue);
						zephir_check_call_status();
						continue;
...
PHP_METHOD(Phalcon_Forms_Form, clear)
{
	...
	if (Z_TYPE_P(fields) == IS_NULL) {
		...
		array_init(&data);
		zephir_is_iterable(&elements, 0, "phalcon/Forms/Form.zep", 276);
		...
		zephir_is_iterable(fields, 0, "phalcon/Forms/Form.zep", 293);
		if (Z_TYPE_P(fields) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Forms_Form, get)
{
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&element, &_0, &name, 0)))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 332);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, getAttributes)
{
	...
	if (UNEXPECTED(Z_TYPE_P(&_0) == IS_NULL)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 25);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Form, getLabel)
{
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&element, &_0, &name, 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 8, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 386);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, isValid)
{
	...
	zend_bool validationStatus = 0, _2, _6;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zephir_fcall_cache_entry *_12 = NULL, *_13 = NULL, *_20 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data = NULL, data_sub, *entity = NULL, entity_sub, __$null, messages, element, validators, name, filters, validator, validation, elementMessage, _0, _3, _7, *_8, _9, _16, _1$$4, _4$$5, _5$$8, *_10$$11, _11$$11, *_14$$16, _15$$16, _18$$22, _19$$22;
	zval *this_ptr = getThis();
	...
	_2 = Z_TYPE_P(entity) != IS_OBJECT;
	if (_2) {
		ZEPHIR_OBS_VAR(&_3);
		zephir_read_property(&_3, this_ptr, ZEND_STRL("entity"), PH_NOISY_CC);
		_2 = Z_TYPE_P(&_3) == IS_OBJECT;
	}
	if (_2) {
		zephir_read_property(&_4$$5, this_ptr, ZEND_STRL("entity"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CPY_WRT(entity, &_4$$5);
	}
	if (Z_TYPE_P(entity) == IS_OBJECT) {
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "bind", NULL, 0, data, entity);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "bind", NULL, 0, data);
		zephir_check_call_status();
	}
	if ((zephir_method_exists_ex(this_ptr, ZEND_STRL("beforevalidation")) == SUCCESS)) {
		ZEPHIR_CALL_METHOD(&_5$$8, this_ptr, "beforevalidation", NULL, 0, data, entity);
		zephir_check_call_status();
	...
	zephir_check_call_status();
	_6 = Z_TYPE_P(&validation) != IS_OBJECT;
	if (!(_6)) {
		_6 = !((zephir_instance_of_ev(&validation, phalcon_validation_validationinterface_ce)));
	}
	if (_6) {
		ZEPHIR_INIT_NVAR(&validation);
		object_init_ex(&validation, phalcon_validation_ce);
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 281);
		zephir_check_call_status();
	}
	zephir_read_property(&_7, this_ptr, ZEND_STRL("elements"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_7, 0, "phalcon/Forms/Form.zep", 684);
	if (Z_TYPE_P(&_7) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_7), _8)
		{
			...
			ZEPHIR_CALL_METHOD(&name, &element, "getname", NULL, 0);
			zephir_check_call_status();
			zephir_is_iterable(&validators, 0, "phalcon/Forms/Form.zep", 671);
			if (Z_TYPE_P(&validators) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _10$$11)
				{
					ZEPHIR_INIT_NVAR(&validator);
					ZVAL_COPY(&validator, _10$$11);
					ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_12, 282, &name, &validator);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
			...
			if (Z_TYPE_P(&filters) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_13, 283, &name, &filters);
				zephir_check_call_status();
			}
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			ZEPHIR_CALL_METHOD(&element, &_7, "current", NULL, 0);
			zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&validators, &element, "getvalidators", NULL, 0);
				zephir_check_call_status();
				if (zephir_fast_count_int(&validators) == 0) {
	...
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(&messages, &validation, "validate", NULL, 284, data, entity);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Form, label)
{
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&element, &_0, &name, 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 8, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 734);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, render)
{
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&element, &_0, &name, 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 8, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 758);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
zend_object *zephir_init_properties_Phalcon_Forms_Form(zend_class_entry *class_type)
{
	...
	
	{
		...
		zephir_read_property_ex(&_2, this_ptr, ZEND_STRL("elementsIndexed"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_2) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_3$$4);
			array_init(&_3$$4);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("elementsIndexed"), &_3$$4);
		}
		...
		zephir_read_property_ex(&_6, this_ptr, ZEND_STRL("filteredData"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_6) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_7$$6);
			array_init(&_7$$6);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("filteredData"), &_7$$6);
		}
		zephir_read_property_ex(&_8, this_ptr, ZEND_STRL("data"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_8) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_9$$7);
			array_init(&_9$$7);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("data"), &_9$$7);
		}
		ZEPHIR_MM_RESTORE();
