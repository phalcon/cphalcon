...
PHP_METHOD(Phalcon_Forms_Form, bind)
{
	...
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _19, _20, _18)
		{
			...
			zephir_read_property_cached(&_21$$15, this_ptr, _zephir_prop_0, 788, PH_NOISY_CC | PH_READONLY);
			if (!(zephir_array_isset_fetch(&element, &_21$$15, &key, 0))) {
				ZEPHIR_INIT_NVAR(&element);
				ZVAL_NULL(&element);
				zephir_read_property_cached(&_22$$16, this_ptr, _zephir_prop_0, 788, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_OBS_NVAR(&element);
				zephir_read_property_cached(&_43$$30, this_ptr, _zephir_prop_0, 788, PH_NOISY_CC | PH_READONLY);
				if (!(zephir_array_isset_fetch(&element, &_43$$30, &key, 0))) {
					ZEPHIR_INIT_NVAR(&element);
					ZVAL_NULL(&element);
					zephir_read_property_cached(&_44$$31, this_ptr, _zephir_prop_0, 788, PH_NOISY_CC | PH_READONLY);
