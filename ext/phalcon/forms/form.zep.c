...
ZEPHIR_INIT_CLASS(Phalcon_Forms_Form) {
	...
	zend_declare_property_null(phalcon_forms_form_ce, SL("action"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Forms_Form, add) {
	...
	if (_0) {
		...
		zephir_is_iterable(&_2$$4, 0, "phalcon/Forms/Form.zep", 123);
...
PHP_METHOD(Phalcon_Forms_Form, bind) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data_param = NULL, *entity, entity_sub, *whitelist = NULL, whitelist_sub, __$null, filter, key, value, element, filters, container, filteredValue, method, _0, *_1, _2, _5$$4, _9$$4, _7$$9, _8$$9, _10$$12, _13$$12, _11$$17, _12$$17;
	zval data;
	zval *this_ptr = getThis();
	...
	if (ZEPHIR_IS_EMPTY(&_0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_forms_exception_ce, "There are no elements in the form", "phalcon/Forms/Form.zep", 140);
		return;
	}
	ZEPHIR_INIT_VAR(&filter);
	ZVAL_NULL(&filter);
	zephir_is_iterable(&data, 0, "phalcon/Forms/Form.zep", 198);
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _3, _4, _1)
		{
			...
			if (zephir_is_true(&filters)) {
				if (Z_TYPE_P(&filter) != IS_OBJECT) {
					ZEPHIR_CALL_METHOD(&container, this_ptr, "getdi", &_6, 0);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_8$$9);
					ZVAL_STRING(&_8$$9, "filter");
					ZEPHIR_CALL_METHOD(&_7$$9, &container, "getshared", NULL, 0, &_8$$9);
					zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_OBS_NVAR(&element);
				zephir_read_property(&_10$$12, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
				...
				if (zephir_is_true(&filters)) {
					if (Z_TYPE_P(&filter) != IS_OBJECT) {
						...
						ZEPHIR_CALL_METHOD(&_11$$17, &container, "getshared", NULL, 0, &_12$$17);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Form, clear) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *fields = NULL, fields_sub, __$null, elements, element, data, field, _0, *_1$$5, _2$$5, *_3$$13, _4$$13, _6$$14, _7$$14, _8$$15, _9$$15, *_10$$17, _11$$17, _12$$18, _13$$19, _14$$19, _15$$20, _16$$21, _17$$21, _18$$23, _19$$23;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(fields) == IS_NULL) {
		...
		if (Z_TYPE_P(fields) == IS_ARRAY) {
			zephir_is_iterable(fields, 0, "phalcon/Forms/Form.zep", 222);
			if (Z_TYPE_P(fields) == IS_ARRAY) {
				...
				zephir_check_call_status();
				while (1) {
					ZEPHIR_CALL_METHOD(&_2$$5, fields, "valid", NULL, 0);
					zephir_check_call_status();
	...
	if (Z_TYPE_P(&elements) == IS_ARRAY) {
		if (Z_TYPE_P(fields) == IS_NULL) {
			zephir_is_iterable(&elements, 0, "phalcon/Forms/Form.zep", 245);
			if (Z_TYPE_P(&elements) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&elements), _3$$13)
				{
					ZEPHIR_INIT_NVAR(&element);
					ZVAL_COPY(&element, _3$$13);
					ZEPHIR_CALL_METHOD(&_6$$14, &element, "getname", NULL, 0);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_7$$14, &element, "getdefault", NULL, 0);
					zephir_check_call_status();
					ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_5, 0, &_6$$14, &_7$$14);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
				...
				zephir_check_call_status();
				while (1) {
					ZEPHIR_CALL_METHOD(&_4$$13, &elements, "valid", NULL, 0);
					zephir_check_call_status();
					if (!zend_is_true(&_4$$13)) {
						break;
					}
					ZEPHIR_CALL_METHOD(&element, &elements, "current", NULL, 0);
					zephir_check_call_status();
						ZEPHIR_CALL_METHOD(&_8$$15, &element, "getname", NULL, 0);
						zephir_check_call_status();
						ZEPHIR_CALL_METHOD(&_9$$15, &element, "getdefault", NULL, 0);
						zephir_check_call_status();
						ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_5, 0, &_8$$15, &_9$$15);
						zephir_check_call_status();
			...
			if (Z_TYPE_P(fields) == IS_ARRAY) {
				zephir_is_iterable(&elements, 0, "phalcon/Forms/Form.zep", 255);
				if (Z_TYPE_P(&elements) == IS_ARRAY) {
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&elements), _10$$17)
					{
						ZEPHIR_INIT_NVAR(&element);
						ZVAL_COPY(&element, _10$$17);
						ZEPHIR_CALL_METHOD(&_12$$18, &element, "getname", NULL, 0);
						zephir_check_call_status();
						if (zephir_fast_in_array(&_12$$18, fields TSRMLS_CC)) {
							ZEPHIR_CALL_METHOD(&_13$$19, &element, "getname", NULL, 0);
							zephir_check_call_status();
							ZEPHIR_CALL_METHOD(&_14$$19, &element, "getdefault", NULL, 0);
							zephir_check_call_status();
							ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_5, 0, &_13$$19, &_14$$19);
							zephir_check_call_status();
						}
					} ZEND_HASH_FOREACH_END();
				} else {
					...
					zephir_check_call_status();
					while (1) {
						ZEPHIR_CALL_METHOD(&_11$$17, &elements, "valid", NULL, 0);
						zephir_check_call_status();
						...
						ZEPHIR_CALL_METHOD(&element, &elements, "current", NULL, 0);
						zephir_check_call_status();
							ZEPHIR_CALL_METHOD(&_15$$20, &element, "getname", NULL, 0);
							zephir_check_call_status();
							if (zephir_fast_in_array(&_15$$20, fields TSRMLS_CC)) {
								ZEPHIR_CALL_METHOD(&_16$$21, &element, "getname", NULL, 0);
								zephir_check_call_status();
								ZEPHIR_CALL_METHOD(&_17$$21, &element, "getdefault", NULL, 0);
								zephir_check_call_status();
								ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_5, 0, &_16$$21, &_17$$21);
								zephir_check_call_status();
							}
						ZEPHIR_CALL_METHOD(NULL, &elements, "next", NULL, 0);
						zephir_check_call_status();
				...
				if (zephir_array_isset_fetch(&element, &elements, fields, 0 TSRMLS_CC)) {
					ZEPHIR_CALL_METHOD(&_18$$23, &element, "getname", NULL, 0);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_19$$23, &element, "getdefault", NULL, 0);
					zephir_check_call_status();
					ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_5, 0, &_18$$23, &_19$$23);
					zephir_check_call_status();
				}
			}
		}
...
PHP_METHOD(Phalcon_Forms_Form, get) {
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(&_1, "phalcon/Forms/Form.zep", 303 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Phalcon_Forms_Form, getLabel) {
	...
	if (!(zephir_array_isset_fetch(&element, &_0, &name, 0 TSRMLS_CC))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 342 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, getMessages) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *byItemName_param = NULL, messages, messagesByItem, elementMessage, fieldName, _0, _2$$4, _4$$6, _5$$5;
	zend_bool byItemName, _1;
	zval *this_ptr = getThis();
	...
	if (_1) {
		if (UNEXPECTED(byItemName)) {
			...
			zephir_check_call_status();
			while (1) {
				ZEPHIR_CALL_METHOD(&_2$$4, &messages, "valid", NULL, 0);
				zephir_check_call_status();
				if (!(zephir_is_true(&_2$$4))) {
					break;
				}
				ZEPHIR_CALL_METHOD(&elementMessage, &messages, "current", &_3, 0);
				zephir_check_call_status();
				...
				if (!(zephir_array_isset(&messagesByItem, &fieldName))) {
					...
					array_init(&_4$$6);
					zephir_array_update_zval(&messagesByItem, &fieldName, &_4$$6, PH_COPY | PH_SEPARATE);
				}
				ZEPHIR_INIT_NVAR(&_5$$5);
				object_init_ex(&_5$$5, phalcon_messages_messages_ce);
				ZEPHIR_INIT_NVAR(&_6$$5);
				zephir_create_array(&_6$$5, 1, 0 TSRMLS_CC);
				zephir_array_fast_append(&_6$$5, &elementMessage);
				ZEPHIR_CALL_METHOD(NULL, &_5$$5, "__construct", &_7, 3, &_6$$5);
				zephir_check_call_status();
				zephir_array_update_multi(&messagesByItem, &_5$$5 TSRMLS_CC, SL("za"), 2, &fieldName);
				ZEPHIR_CALL_METHOD(NULL, &messages, "next", &_8, 0);
				zephir_check_call_status();
			}
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", &_7, 3);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Forms_Form, getValue) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *name_param = NULL, __$true, entity, method, value, data, internal, forbidden, element, _0, _2, _3, _1$$4;
	zval name;
	zval *this_ptr = getThis();
	...
	ZEPHIR_CALL_CE_STATIC(&_3, phalcon_tag_ce, "hasvalue", &_4, 0, &name);
	zephir_check_call_status();
	if (zephir_is_true(&_3)) {
		ZEPHIR_RETURN_CALL_CE_STATIC(phalcon_tag_ce, "getvalue", &_5, 0, &name);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_OBS_VAR(&element);
...
PHP_METHOD(Phalcon_Forms_Form, isValid) {
	...
	zend_object_iterator *_18$$21;
	zend_bool validationStatus = 0, _5, _9$$11, _14$$16;
	zephir_fcall_cache_entry *_12 = NULL, *_13 = NULL, *_21 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data = NULL, data_sub, *entity = NULL, entity_sub, __$null, messages, element, validators, name, filters, validator, validation, elementMessage, _0, _6, *_7, _8, _17, _1$$4, _2$$6, _3$$7, _4$$8, *_10$$11, _11$$11, *_15$$16, _16$$16, _19$$22, _20$$22;
	zval *this_ptr = getThis();
	...
	if (_5) {
		...
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 190);
		zephir_check_call_status();
	}
	zephir_read_property(&_6, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_6, 0, "phalcon/Forms/Form.zep", 648);
	if (Z_TYPE_P(&_6) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_6), _7)
		{
			...
			ZEPHIR_CALL_METHOD(&name, &element, "getname", NULL, 0);
			zephir_check_call_status();
			zephir_is_iterable(&validators, 0, "phalcon/Forms/Form.zep", 635);
			if (Z_TYPE_P(&validators) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _10$$11)
				{
					ZEPHIR_INIT_NVAR(&validator);
					ZVAL_COPY(&validator, _10$$11);
					ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_12, 191, &name, &validator);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
				...
				zephir_check_call_status();
				while (1) {
					ZEPHIR_CALL_METHOD(&_11$$11, &validators, "valid", NULL, 0);
					zephir_check_call_status();
			...
			if (Z_TYPE_P(&filters) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_13, 192, &name, &filters);
				zephir_check_call_status();
			}
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&validators, &element, "getvalidators", NULL, 0);
				zephir_check_call_status();
				_14$$16 = Z_TYPE_P(&validators) != IS_ARRAY;
				if (!(_14$$16)) {
					_14$$16 = zephir_fast_count_int(&validators TSRMLS_CC) == 0;
				}
				if (_14$$16) {
					continue;
				}
				...
				zephir_is_iterable(&validators, 0, "phalcon/Forms/Form.zep", 635);
				if (Z_TYPE_P(&validators) == IS_ARRAY) {
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _15$$16)
					{
						ZEPHIR_INIT_NVAR(&validator);
						ZVAL_COPY(&validator, _15$$16);
						ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_12, 191, &name, &validator);
						zephir_check_call_status();
					} ZEND_HASH_FOREACH_END();
				} else {
					...
					zephir_check_call_status();
					while (1) {
						ZEPHIR_CALL_METHOD(&_16$$16, &validators, "valid", NULL, 0);
						zephir_check_call_status();
						if (!zend_is_true(&_16$$16)) {
							break;
						}
						ZEPHIR_CALL_METHOD(&validator, &validators, "current", NULL, 0);
						zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(&messages, &validation, "validate", NULL, 193, data, entity);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_17, &messages, "count", NULL, 0);
	zephir_check_call_status();
	if (zephir_is_true(&_17)) {
		_18$$21 = zephir_get_iterator(&messages TSRMLS_CC);
		_18$$21->funcs->rewind(_18$$21 TSRMLS_CC);
		for (;_18$$21->funcs->valid(_18$$21 TSRMLS_CC) == SUCCESS && !EG(exception); _18$$21->funcs->move_forward(_18$$21 TSRMLS_CC)) {
			{
				ZEPHIR_ITERATOR_COPY(&elementMessage, _18$$21);
			}
			ZEPHIR_CALL_METHOD(&_20$$22, &elementMessage, "getfield", NULL, 0);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_19$$22, this_ptr, "get", &_21, 0, &_20$$22);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, &_19$$22, "appendmessage", NULL, 0, &elementMessage);
			zephir_check_call_status();
		}
		zend_iterator_dtor(_18$$21);
		ZEPHIR_CALL_METHOD(NULL, &messages, "rewind", NULL, 0);
		zephir_check_call_status();
		validationStatus = 0;
	}
...
PHP_METHOD(Phalcon_Forms_Form, label) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 1, &_2);
	zephir_check_call_status();
	zephir_throw_exception_debug(&_1, "phalcon/Forms/Form.zep", 699 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Phalcon_Forms_Form, render) {
	...
	if (!(zephir_array_isset_fetch(&element, &_0, &name, 0 TSRMLS_CC))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 1, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 720 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, rewind) {
	...
	ZEPHIR_OBS_VAR(&_1);
	zephir_read_property(&_1, this_ptr, SL("elements"), PH_NOISY_CC);
	if (Z_TYPE_P(&_1) == IS_ARRAY) {
		zephir_read_property(&_2$$3, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_values", NULL, 113, &_2$$3);
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, SL("elementsIndexed"), &_3$$3);
	} else {
		ZEPHIR_INIT_VAR(&_4$$4);
		array_init(&_4$$4);
		zephir_update_property_zval(this_ptr, SL("elementsIndexed"), &_4$$4);
	}
	ZEPHIR_MM_RESTORE();
