...
PHP_METHOD(Phalcon_Forms_Form, bind) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data_param = NULL, *entity, entity_sub, *whitelist = NULL, whitelist_sub, __$null, filter, key, value, element, filters, container, filteredValue, method, _0, *_1, _4$$4, _8$$4, _6$$9, _7$$9;
	zval data;
	zval *this_ptr = getThis();
...
PHP_METHOD(Phalcon_Forms_Form, clear) {
	...
	if (Z_TYPE_P(&elements) == IS_ARRAY) {
		if (Z_TYPE_P(fields) == IS_NULL) {
			zephir_is_iterable(&elements, 0, "phalcon/Forms/Form.zep", 242);
			...
			if (Z_TYPE_P(fields) == IS_ARRAY) {
				zephir_is_iterable(&elements, 0, "phalcon/Forms/Form.zep", 249);
...
PHP_METHOD(Phalcon_Forms_Form, get) {
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(&_1, "phalcon/Forms/Form.zep", 292 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Phalcon_Forms_Form, getLabel) {
	...
	if (!(zephir_array_isset_fetch(&element, &_0, &name, 0 TSRMLS_CC))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 329 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Form, isValid) {
	...
	zephir_fcall_cache_entry *_10 = NULL, *_11 = NULL, *_16 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data = NULL, data_sub, *entity = NULL, entity_sub, __$null, messages, element, validators, name, filters, validator, validation, elementMessage, _0, _6, *_7, _12, _1$$4, _2$$6, _3$$7, _4$$8, *_9$$11, _14$$16, _15$$16;
	zval *this_ptr = getThis();
	...
	if (_5) {
		...
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 191);
		zephir_check_call_status();
	}
	zephir_read_property(&_6, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_6, 0, "phalcon/Forms/Form.zep", 630);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_6), _7)
	{
		...
		ZEPHIR_CALL_METHOD(&name, &element, "getname", NULL, 0);
		zephir_check_call_status();
		zephir_is_iterable(&validators, 0, "phalcon/Forms/Form.zep", 617);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _9$$11)
		{
			ZEPHIR_INIT_NVAR(&validator);
			ZVAL_COPY(&validator, _9$$11);
			ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_10, 192, &name, &validator);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&validator);
		ZEPHIR_CALL_METHOD(&filters, &element, "getfilters", NULL, 0);
		...
		if (Z_TYPE_P(&filters) == IS_ARRAY) {
			ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_11, 193, &name, &filters);
			zephir_check_call_status();
		}
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(&messages, &validation, "validate", NULL, 194, data, entity);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Form, label) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 1, &_2);
	zephir_check_call_status();
	zephir_throw_exception_debug(&_1, "phalcon/Forms/Form.zep", 679 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Phalcon_Forms_Form, render) {
	...
	if (!(zephir_array_isset_fetch(&element, &_0, &name, 0 TSRMLS_CC))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 1, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Forms/Form.zep", 698 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
