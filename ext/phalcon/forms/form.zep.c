...
PHP_METHOD(Phalcon_Forms_Form, isValid) {
	...
	if (_5) {
		...
		ZEPHIR_CALL_METHOD(NULL, validation, "__construct", NULL, 216);
		zephir_check_call_status();
	}
	...
	for (
	  ...
	  ; zend_hash_move_forward_ex(_8, &_7)
		...
		if (Z_TYPE_P(filters) == IS_ARRAY) {
			ZEPHIR_CALL_METHOD(NULL, validation, "setfilters", &_15, 218, name, filters);
			zephir_check_call_status();
		}
	}
	ZEPHIR_CALL_METHOD(&messages, validation, "validate", NULL, 219, data, entity);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Form, rewind) {
	...
	if (Z_TYPE_P(_1) == IS_ARRAY) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_values", NULL, 220, _2$$3);
		zephir_check_call_status();
