...
PHP_METHOD(Phalcon_Forms_Form, isValid)
{
	...
	if (_6) {
		...
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 280);
		zephir_check_call_status();
	}
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_7), _8)
		{
			...
			if (Z_TYPE_P(&validators) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _10$$11)
				{
					...
					ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_12, 281, &name, &validator);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
			...
			if (Z_TYPE_P(&filters) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_13, 282, &name, &filters);
				zephir_check_call_status();
			}
		} ZEND_HASH_FOREACH_END();
	} else {
	...
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(&messages, &validation, "validate", NULL, 283, data, entity);
	zephir_check_call_status();
