...
PHP_METHOD(Phalcon_Forms_Form, isValid) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_5), _6)
	{
		...
		if (Z_TYPE_P(&validation) == IS_OBJECT) {
			...
			ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", &_10, 220, &preparedValidators);
			zephir_check_call_status();
		}
		ZEPHIR_CALL_METHOD(&filters, &element, "getfilters", NULL, 0);
		zephir_check_call_status();
		if (Z_TYPE_P(&filters) == IS_ARRAY) {
			ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_11, 221, &name, &filters);
			zephir_check_call_status();
		}
		ZEPHIR_CALL_METHOD(&elementMessages, &validation, "validate", &_12, 222, data, entity);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Form, getMessages) {
	...
	if (Z_TYPE_P(&messages) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&messages), _1$$5)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &group, "appendmessages", &_2, 223, &elementMessages);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&elementMessages);
	}
...
PHP_METHOD(Phalcon_Forms_Form, clear) {
	...
	if (Z_TYPE_P(fields) == IS_NULL) {
		...
		if (Z_TYPE_P(fields) == IS_ARRAY) {
			zephir_is_iterable(fields, 0, "phalcon/forms/form.zep", 677);
	...
	if (Z_TYPE_P(&elements) == IS_ARRAY) {
		zephir_is_iterable(&elements, 0, "phalcon/forms/form.zep", 697);
