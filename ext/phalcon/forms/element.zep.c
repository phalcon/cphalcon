...
PHP_METHOD(Phalcon_Forms_Element, label) {
	...
	ZEPHIR_CALL_CE_STATIC(&code, phalcon_tag_ce, "renderattributes", &_0, 172, _1, attributes);
...
PHP_METHOD(Phalcon_Forms_Element, getValue) {
	...
	if (Z_TYPE_P(form) == IS_OBJECT) {
		...
		if (_0) {
			ZEPHIR_CALL_CE_STATIC(&_1, phalcon_tag_ce, "hasvalue", &_2, 173, name);
			zephir_check_call_status();
			_0 = zephir_is_true(_1);
		}
		if (_0) {
			ZEPHIR_CALL_CE_STATIC(&value, phalcon_tag_ce, "getvalue", &_3, 174, name);
			zephir_check_call_status();
		}
	}
	if (Z_TYPE_P(value) == IS_NULL) {
...
PHP_METHOD(Phalcon_Forms_Element, getMessages) {
	...
	ZEPHIR_CALL_METHOD(NULL, messages, "__construct", NULL, 175);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Element, appendMessage) {
	...
	if (Z_TYPE_P(messages) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0, "__construct", NULL, 175);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Element, clear) {
	...
	ZEPHIR_INIT_VAR(_2);
	ZVAL_NULL(_2);
	ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_0, 176, _1, _2);
	zephir_check_call_status();
	RETURN_THIS();
