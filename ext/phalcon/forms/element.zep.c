...
PHP_METHOD(Phalcon_Forms_Element, __construct) {
	...
	if (ZEPHIR_IS_EMPTY(&name)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Form element name is required", "phalcon/forms/element.zep", 63);
		return;
	}
...
PHP_METHOD(Phalcon_Forms_Element, label) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *attributes_param = NULL, internalAttributes, label, name, code, _1, _3$$7, _4$$8;
	zval attributes;
	zval *this_ptr = getThis();
