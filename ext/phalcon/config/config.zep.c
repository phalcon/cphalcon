...
PHP_METHOD(Phalcon_Config_Config, merge)
{
	...
	if (Z_TYPE_P(toMerge) == IS_ARRAY) {
		...
		if (_0) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_1$$5, "__construct", NULL, 37);
			zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&result, this_ptr, "internalmerge", NULL, 38, &source, &target);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Config, path)
{
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&key, "array_shift", &_2, 39, &keys);
		ZEPHIR_UNREF(&keys);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Config, setData)
{
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$4, "__construct", NULL, 40, value, &_3$$4, &_4$$4, &_5$$4);
		zephir_check_call_status();
