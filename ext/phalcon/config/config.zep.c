...
ZEPHIR_INIT_CLASS(Phalcon_Config_Config) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Phalcon\\Config, Config, phalcon, config_config, phalcon_collection_collection_ce, phalcon_config_config_method_entry, 0);

	/**
	 * @var string
	 */
	zend_declare_property_null(phalcon_config_config_ce, SL("pathDelimiter"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Config_Config, merge) {
	...
	if (_0) {
		_0 = zephir_instance_of_ev(toMerge, phalcon_config_config_ce TSRMLS_CC);
	}
	if (Z_TYPE_P(toMerge) == IS_ARRAY) {
		ZEPHIR_INIT_VAR(&config);
		object_init_ex(&config, phalcon_config_config_ce);
		ZEPHIR_CALL_METHOD(NULL, &config, "__construct", NULL, 11, toMerge);
		zephir_check_call_status();
	} else if (_0) {
		ZEPHIR_CPY_WRT(&config, toMerge);
	} else {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "Invalid data type for merge.", "phalcon/Config/Config.zep", 95);
		return;
	}
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&result, this_ptr, "internalmerge", NULL, 12, &source, &target);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Config, path) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&key, "array_shift", &_1, 13, &keys);
		ZEPHIR_UNREF(&keys);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Config, toArray) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&data, 0, "phalcon/Config/Config.zep", 195);
...
PHP_METHOD(Phalcon_Config_Config, internalMerge) {
	...
	zephir_is_iterable(&target, 0, "phalcon/Config/Config.zep", 220);
	if (Z_TYPE_P(&target) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&target), _2, _3, _0)
		{
			...
			if (_5$$3) {
				...
				zephir_array_fetch(&_6$$3, &source, &key, PH_NOISY, "phalcon/Config/Config.zep", 211 TSRMLS_CC);
				_5$$3 = Z_TYPE_P(&_6$$3) == IS_ARRAY;
			}
			if (_5$$3) {
				zephir_array_fetch(&_8$$4, &source, &key, PH_NOISY | PH_READONLY, "phalcon/Config/Config.zep", 212 TSRMLS_CC);
				ZEPHIR_CALL_METHOD(&_7$$4, this_ptr, "internalmerge", &_9, 12, &_8$$4, &value);
				zephir_check_call_status();
				zephir_array_update_zval(&source, &key, &_7$$4, PH_COPY | PH_SEPARATE);
			} else if (Z_TYPE_P(&key) == IS_LONG) {
				zephir_array_append(&source, &value, PH_SEPARATE, "phalcon/Config/Config.zep", 214);
			} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$7) {
					...
					zephir_array_fetch(&_12$$7, &source, &key, PH_NOISY, "phalcon/Config/Config.zep", 211 TSRMLS_CC);
					_11$$7 = Z_TYPE_P(&_12$$7) == IS_ARRAY;
				}
				if (_11$$7) {
					zephir_array_fetch(&_14$$8, &source, &key, PH_NOISY | PH_READONLY, "phalcon/Config/Config.zep", 212 TSRMLS_CC);
					ZEPHIR_CALL_METHOD(&_13$$8, this_ptr, "internalmerge", &_9, 12, &_14$$8, &value);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Config, setData) {
	...
	if (zephir_is_true(&_1)) {
		ZEPHIR_CALL_FUNCTION(&key, "mb_strtolower", NULL, 14, element);
		zephir_check_call_status();
	} else {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		object_init_ex(&data, phalcon_config_config_ce);
		ZEPHIR_CALL_METHOD(NULL, &data, "__construct", NULL, 11, value);
		zephir_check_call_status();
	} else {
