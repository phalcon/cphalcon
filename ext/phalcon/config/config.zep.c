...
PHP_METHOD(Phalcon_Config_Config, merge)
{
	...
	if (Z_TYPE_P(toMerge) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&result, this_ptr, "internalmerge", NULL, 26, &source, toMerge);
		zephir_check_call_status();
	...
	if (_0) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&result, this_ptr, "internalmerge", NULL, 26, &source, &_1$$4);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "init", NULL, 0, &result);
		zephir_check_call_status();
		RETURN_THIS();
...
PHP_METHOD(Phalcon_Config_Config, path)
{
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&key, "array_shift", &_2, 27, &keys);
		ZEPHIR_UNREF(&keys);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&target) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&target), _2, _3, _0)
		{
			...
			if (_5$$3) {
				...
				ZEPHIR_CALL_METHOD(&_7$$4, this_ptr, "internalmerge", &_9, 26, &_8$$4, &value);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$5) {
					...
					ZEPHIR_CALL_METHOD(&_13$$6, this_ptr, "internalmerge", &_9, 26, &_14$$6, &value);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Config, setData)
{
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 28, value);
		zephir_check_call_status();
