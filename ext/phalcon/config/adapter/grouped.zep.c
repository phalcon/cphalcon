...
PHP_METHOD(Phalcon_Config_Adapter_Grouped, __construct)
{
	...
	if (Z_TYPE_P(&configFactory) == IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &configFactory, "__construct", NULL, 366);
		zephir_check_call_status();
	}
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arrayConfig), _1)
		{
			...
			if (_2$$4) {
				...
				if (ZEPHIR_IS_IDENTICAL(&_4$$6, &defaultAdapter_zv)) {
					ZEPHIR_CALL_METHOD(&_5$$7, &configFactory, "load", &_6, 367, &configName);
					zephir_check_call_status();
			...
			if (ZEPHIR_IS_IDENTICAL(&_9$$4, &_8$$4)) {
				if (!(zephir_array_isset_value_string(&configInstance, SL("config")))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_10$$10, "__construct", &_11, 368);
					zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(&_14$$11, &configFactory, "load", &_6, 367, &configInstance);
				zephir_check_call_status();
			...
			zephir_check_call_status();
				...
				if (_17$$12) {
					...
					if (ZEPHIR_IS_IDENTICAL(&_18$$14, &defaultAdapter_zv)) {
						ZEPHIR_CALL_METHOD(&_19$$15, &configFactory, "load", &_6, 367, &configName);
						zephir_check_call_status();
				...
				if (ZEPHIR_IS_IDENTICAL(&_22$$12, &_21$$12)) {
					if (!(zephir_array_isset_value_string(&configInstance, SL("config")))) {
						...
						ZEPHIR_CALL_METHOD(NULL, &_23$$18, "__construct", &_11, 368);
						zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_25$$19, &configFactory, "load", &_6, 367, &configInstance);
					zephir_check_call_status();
