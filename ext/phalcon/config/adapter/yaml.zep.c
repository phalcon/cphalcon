...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct)
{
	...
	if (UNEXPECTED(!zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 450);
		zephir_check_call_status();
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&yamlConfig))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_4$$7, "__construct", NULL, 445, &_5$$7);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpYamlParseFile)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("yaml_parse_file", NULL, 452, &filename_zv, &_0, &ndocs, &callbacks);
	ZEPHIR_UNREF(&ndocs);
	zephir_check_call_status();
	RETURN_MM();
}
