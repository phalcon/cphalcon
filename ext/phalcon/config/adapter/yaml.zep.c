...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct) {
	...
	ZEPHIR_OBS_VAR_ONCE(&callbacks);
	ZVAL_COPY(&callbacks, callbacks_param);
	}
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_config_adapter_yaml_ce, this_ptr, "__construct", &_8, 16, &yamlConfig);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
