...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct)
{
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&yamlConfig))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_4$$6, "__construct", NULL, 30, &_6$$6);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpYamlParseFile)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("yaml_parse_file", NULL, 210, filename, pos, ndocs, callbacks);
	ZEPHIR_UNREF(ndocs);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpExtensionLoaded)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("extension_loaded", NULL, 211, &name);
	zephir_check_call_status();
	RETURN_MM();
}
