...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct)
{
	...
	if (UNEXPECTED(!zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 361);
		zephir_check_call_status();
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&yamlConfig))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_5$$7, "__construct", NULL, 357, &_6$$7);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpYamlParseFile)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("yaml_parse_file", NULL, 362, filename, pos, ndocs, callbacks);
	ZEPHIR_UNREF(ndocs);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpExtensionLoaded)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("extension_loaded", NULL, 363, &name_zv);
	zephir_check_call_status();
	RETURN_MM();
}
