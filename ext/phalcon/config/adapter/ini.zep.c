...
PHP_METHOD(Phalcon_Config_Adapter_Ini, __construct) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&iniConfig)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$4, "phalcon/Config/Adapter/Ini.zep", 74 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&config);
	array_init(&config);
	zephir_is_iterable(&iniConfig, 0, "phalcon/Config/Adapter/Ini.zep", 95);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&iniConfig), _4, _5, _3)
	{
		...
		if (Z_TYPE_P(&directives) == IS_ARRAY) {
			...
			array_init(&sections);
			zephir_is_iterable(&directives, 0, "phalcon/Config/Adapter/Ini.zep", 87);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&directives), _7$$6, _8$$6, _6$$6)
			{
				...
				zephir_check_call_status();
				zephir_array_append(&sections, &_9$$7, PH_SEPARATE, "phalcon/Config/Adapter/Ini.zep", 85);
...
PHP_METHOD(Phalcon_Config_Adapter_Ini, cast) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *ini, ini_sub, key, val, *_0$$3, _3$$4, _7$$5, _10$$5, _11$$9, _12$$9, _13$$9, _14$$9;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(ini) == IS_ARRAY) {
		zephir_is_iterable(ini, 1, "phalcon/Config/Adapter/Ini.zep", 110);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(ini), _1$$3, _2$$3, _0$$3)
		{
			...
			ZVAL_COPY(&val, _0$$3);
			ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "cast", &_4, 116, &val);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(ini) == IS_STRING) {
		...
		if (zephir_is_numeric(ini)) {
			...
			ZVAL_STRING(&_14$$9, "/[.]+/");
			zephir_preg_match(&_13$$9, &_14$$9, ini, &_11$$9, 0, 0 , 0  TSRMLS_CC);
