...
PHP_METHOD(Phalcon_Config_Adapter_Ini, parseIniString)
{
	...
	ZEPHIR_CALL_METHOD(&result, this_ptr, "parseinistring", NULL, 437, &path, &castValue);
	zephir_check_call_status();
	zephir_create_array(return_value, 1, 0);
...
PHP_METHOD(Phalcon_Config_Adapter_Ini, phpIniGet)
{
	...
	ZEPHIR_CALL_FUNCTION(&value, "ini_get", NULL, 438, &input_zv);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Ini, phpParseIniFile)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("parse_ini_file", NULL, 439, &filename_zv, &_0, &_1);
	zephir_check_call_status();
	RETURN_MM();
}
