...
PHP_METHOD(Phalcon_Config_Factory, loadClass) {
	...
	zend_bool _0;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *namespace_param = NULL, *config = NULL, config_sub, adapter, className, mode, callbacks, filePath, _1$$3, _2$$6, _3$$6, _4$$6, _11$$6, _5$$7, _6$$7, _7$$9, _9$$11;
	zval namespace;
	zval *this_ptr = getThis();
	...
	if (zephir_array_isset_string_fetch(&adapter, config, SL("adapter"), 0)) {
		...
		if (!(zephir_is_true(&_4$$6))) {
			ZEPHIR_CALL_FUNCTION(&_5$$7, "lcfirst", NULL, 63, &adapter);
			zephir_check_call_status();
		...
		if (ZEPHIR_IS_STRING(&className, "Phalcon\\Config\\Adapter\\Ini")) {
			...
			if (zephir_array_isset_string_fetch(&mode, config, SL("mode"), 0)) {
				...
				_8$$9 = zephir_fetch_class_str_ex(Z_STRVAL_P(&_7$$9), Z_STRLEN_P(&_7$$9), ZEND_FETCH_CLASS_AUTO);
			...
			if (zephir_array_isset_string_fetch(&callbacks, config, SL("callbacks"), 0)) {
				...
				_10$$11 = zephir_fetch_class_str_ex(Z_STRVAL_P(&_9$$11), Z_STRLEN_P(&_9$$11), ZEND_FETCH_CLASS_AUTO);
		...
		_12$$6 = zephir_fetch_class_str_ex(Z_STRVAL_P(&_11$$6), Z_STRLEN_P(&_11$$6), ZEND_FETCH_CLASS_AUTO);
