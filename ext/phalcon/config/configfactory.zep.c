...
PHP_METHOD(Phalcon_Config_ConfigFactory, parseConfig)
{
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		if (1 == ZEPHIR_IS_EMPTY(&extension)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 403);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(config) != IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_5$$6, "__construct", NULL, 404);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_5$$6, "phalcon/Config/ConfigFactory.zep", 202);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkconfigarray", NULL, 405, config);
	zephir_check_call_status();
