...
PHP_METHOD(Phalcon_Config_ConfigFactory, load)
{
	...
	ZVAL_LONG(&_1, 4);
	ZEPHIR_CALL_FUNCTION(&_2, "pathinfo", NULL, 182, &filePath, &_1);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_ConfigFactory, parseConfig)
{
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		ZVAL_LONG(&_0$$3, 4);
		ZEPHIR_CALL_FUNCTION(&extension, "pathinfo", NULL, 182, config, &_0$$3);
		zephir_check_call_status();
		if (1 == ZEPHIR_IS_EMPTY(&extension)) {
			ZEPHIR_INIT_VAR(&_1$$4);
			object_init_ex(&_1$$4, phalcon_config_exceptions_missingfileextension_ce);
			ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 375);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(config) != IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_5$$6, "__construct", NULL, 376);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_5$$6, "phalcon/Config/ConfigFactory.zep", 202);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkconfigarray", NULL, 377, config);
	zephir_check_call_status();
