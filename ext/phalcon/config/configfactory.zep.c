...
PHP_METHOD(Phalcon_Config_ConfigFactory, load) {
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		ZVAL_LONG(&_0$$3, 4);
		ZEPHIR_CALL_FUNCTION(&extension, "pathinfo", NULL, 102, config, &_0$$3);
		zephir_check_call_status();
		if (UNEXPECTED(ZEPHIR_IS_EMPTY(&extension))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "You need to provide the extension in the file path", "phalcon/Config/ConfigFactory.zep", 62);
			return;
		}
	...
	if (_2) {
		_2 = zephir_instance_of_ev(config, phalcon_config_ce);
	}
	if (_2) {
	...
	if (UNEXPECTED(Z_TYPE_P(config) != IS_ARRAY)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "Config must be array or Phalcon\\Config object", "phalcon/Config/ConfigFactory.zep", 78);
		return;
	}
	if (UNEXPECTED(!(zephir_array_isset_string(config, SL("filePath"))))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "You must provide 'filePath' option in factory config parameter.", "phalcon/Config/ConfigFactory.zep", 84);
		return;
	}
	if (UNEXPECTED(!(zephir_array_isset_string(config, SL("adapter"))))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "You must provide 'adapter' option in factory config parameter.", "phalcon/Config/ConfigFactory.zep", 90);
		return;
	}
	zephir_array_fetch_string(&_4, config, SL("adapter"), PH_NOISY | PH_READONLY, "phalcon/Config/ConfigFactory.zep", 93);
	ZEPHIR_INIT_VAR(&adapter);
	zephir_fast_strtolower(&adapter, &_4);
	ZEPHIR_OBS_VAR(&first);
	zephir_array_fetch_string(&first, config, SL("filePath"), PH_NOISY, "phalcon/Config/ConfigFactory.zep", 94);
	...
	if (!(zephir_is_true(&_6))) {
		ZEPHIR_CALL_FUNCTION(&_7$$9, "lcfirst", NULL, 89, &adapter);
		zephir_check_call_status();
	...
	if (ZEPHIR_IS_IDENTICAL(&_9, &adapter)) {
		...
		ZVAL_LONG(&_13$$10, 1);
		ZEPHIR_CALL_CE_STATIC(&second, phalcon_helper_arr_ce, "get", &_11, 14, config, &_12$$10, &_13$$10);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_CE_STATIC(&second, phalcon_helper_arr_ce, "get", &_11, 14, config, &_15$$11, &_14$$11);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Config_ConfigFactory, newInstance) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *name_param = NULL, *fileName_param = NULL, *params = NULL, params_sub, __$null, definition, options, _0, _6, _7, _1$$3, _2$$3, _4$$3, _5$$3;
	zval name, fileName;
	zval *this_ptr = getThis();
	...
	if (!(zephir_array_isset(&_0, &name))) {
		...
		zephir_array_fetch(&definition, &_1$$3, &name, PH_NOISY, "phalcon/Config/ConfigFactory.zep", 120);
		...
		ZEPHIR_LAST_CALL_STATUS = zephir_create_instance_params(&_5$$3, &definition, &options);
		zephir_check_call_status();
		zephir_update_property_array(this_ptr, SL("services"), &name, &_5$$3);
	}
	zephir_read_property(&_6, this_ptr, SL("services"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch(&_7, &_6, &name, PH_NOISY | PH_READONLY, "phalcon/Config/ConfigFactory.zep", 131);
	RETURN_CTOR(&_7);
