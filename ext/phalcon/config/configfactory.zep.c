...
PHP_METHOD(Phalcon_Config_ConfigFactory, load) {
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		ZVAL_LONG(&_0$$3, 4);
		ZEPHIR_CALL_FUNCTION(&extension, "pathinfo", NULL, 105, config, &_0$$3);
		zephir_check_call_status();
	...
	ZVAL_NULL(&second);
	ZVAL_LONG(&_5, 4);
	ZEPHIR_CALL_FUNCTION(&_6, "pathinfo", NULL, 105, &first, &_5);
	zephir_check_call_status();
	if (ZEPHIR_IS_EMPTY(&_6)) {
		ZEPHIR_CALL_FUNCTION(&_7$$9, "lcfirst", NULL, 92, &adapter);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_ConfigFactory, newInstance) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *name_param = NULL, *fileName_param = NULL, *params = NULL, params_sub, __$null, definition, options, _0, _1, _3;
	zval name, fileName;
	zval *this_ptr = getThis();
