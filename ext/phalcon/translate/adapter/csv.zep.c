...
PHP_METHOD(Phalcon_Translate_Adapter_Csv, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_translate_adapter_csv_ce, this_ptr, "__construct", &_0, 448, options);
...
PHP_METHOD(Phalcon_Translate_Adapter_Csv, _load) {
	...
	ZEPHIR_SINIT_VAR(_0);
	ZVAL_STRING(&_0, "rb", 0);
	ZEPHIR_CALL_FUNCTION(&fileHandler, "fopen", NULL, 291, file, &_0);
	zephir_check_call_status();
	...
	while (1) {
		ZEPHIR_CALL_FUNCTION(&data, "fgetcsv", &_3, 450, fileHandler, length, delimiter, enclosure);
		zephir_check_call_status();
