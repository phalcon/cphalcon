...
PHP_METHOD(Phalcon_Session_Manager, get)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(&value, phalcon_helper_arr_ce, "get", &_1, 10, &_SESSION, &uniqueKey, defaultValue);
	zephir_check_call_status();
	if (remove) {
...
PHP_METHOD(Phalcon_Session_Manager, setId)
{
	...
	if (UNEXPECTED(ZEPHIR_IS_TRUE_IDENTICAL(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 277, &_2$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Manager, start)
{
	...
	ZEPHIR_CALL_FUNCTION(&_1, "headers_sent", NULL, 367);
	zephir_check_call_status();
