...
PHP_METHOD(Phalcon_Session_Bag, remove) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *property_param = NULL, data, _0$$3, _1$$3;
	zval property;
	ZEPHIR_INIT_THIS();
	...
	if (zephir_array_isset(&data, &property)) {
		...
		zephir_read_property(&_0$$3, this_ptr, SL("_session"), PH_NOISY_CC | PH_READONLY);
		zephir_read_property(&_1$$3, this_ptr, SL("_name"), PH_NOISY_CC | PH_READONLY);
