...
PHP_METHOD(Phalcon_Session_Adapter_Files, __construct) {
	...
	if (!(zephir_array_isset_string_fetch(&path, &options, SL("save_path"), 0))) {
		...
		ZEPHIR_CALL_FUNCTION(&path, "ini_get", NULL, 466, &_2$$3);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_FUNCTION(&_3, "is_writable", NULL, 259, &path);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Files, destroy) {
	...
	if (1 == (zephir_file_exists(&name TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_CALL_FUNCTION(NULL, "unlink", NULL, 467, &name);
		zephir_check_call_status();
	}
	RETURN_MM_BOOL(1);
...
PHP_METHOD(Phalcon_Session_Adapter_Files, gc) {
	...
	ZEPHIR_CALL_FUNCTION(&_2, "glob", NULL, 468, &pattern);
	zephir_check_call_status();
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_2), _3)
	{
		...
		if (_4$$3) {
			ZEPHIR_CALL_FUNCTION(NULL, "unlink", &_7, 467, &file);
			zephir_check_call_status();
		}
