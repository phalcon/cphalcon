...
PHP_METHOD(Phalcon_Session_Adapter_Stream, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1321, &options);
	ZEPHIR_INIT_NVAR(&_1);
	...
	if (UNEXPECTED(1 == ZEPHIR_IS_EMPTY(&path))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_4$$3, "phalcon/Session/Adapter/Stream.zep", 94);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (UNEXPECTED(!ZEPHIR_IS_TRUE_IDENTICAL(&_5))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_6$$4, "phalcon/Session/Adapter/Stream.zep", 98);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_METHOD(&_7, this_ptr, "todirseparator", NULL, 0, &path);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1322, &_7);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy)
{
	...
	if (_3) {
		ZEPHIR_CALL_FUNCTION(&_4, "is_file", NULL, 462, &file);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, gc)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 1322, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 1320, PH_NOISY_CC | PH_READONLY);
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&glob)) {
		...
		if (zephir_array_isset_value_string(&last, SL("message"))) {
			zephir_array_fetch_string(&_3$$4, &last, SL("message"), PH_NOISY | PH_READONLY, "phalcon/Session/Adapter/Stream.zep", 136);
	...
	if (!(ZEPHIR_IS_EMPTY(&glob))) {
		zephir_is_iterable(&glob, 0, "phalcon/Session/Adapter/Stream.zep", 151);
		if (Z_TYPE_P(&glob) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&glob), _5$$6)
			{
				...
				if (_8$$7) {
					ZEPHIR_CALL_FUNCTION(&_9$$7, "is_file", &_10, 462, &file);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_17$$9) {
						ZEPHIR_CALL_FUNCTION(&_18$$9, "is_file", &_10, 462, &file);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, read)
{
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_2)) {
		...
		zephir_check_call_status();
		ZVAL_LONG(&_4$$3, 1);
		ZEPHIR_CALL_FUNCTION(&_5$$3, "flock", NULL, 299, &pointer, &_4$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, getPrefixedName)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 1320, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, phpIniGet)
{
	...
	ZEPHIR_CALL_FUNCTION(&value, "ini_get", NULL, 460, &input_zv);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, phpParseIniFile)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("parse_ini_file", NULL, 461, &filename_zv, &_0, &_1);
	zephir_check_call_status();
	RETURN_MM();
}
