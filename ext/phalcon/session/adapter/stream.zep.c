...
PHP_METHOD(Phalcon_Session_Adapter_Stream, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1267, &options);
	ZEPHIR_INIT_NVAR(&_1);
	...
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1268, &_7);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy)
{
	...
	if (_3) {
		ZEPHIR_CALL_FUNCTION(&_4, "is_file", NULL, 405, &file);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, gc)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 1268, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 1266, PH_NOISY_CC | PH_READONLY);
	...
	if (!(ZEPHIR_IS_EMPTY(&glob))) {
		...
		if (Z_TYPE_P(&glob) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&glob), _5$$6)
			{
				...
				if (_8$$7) {
					ZEPHIR_CALL_FUNCTION(&_9$$7, "is_file", &_10, 405, &file);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_17$$9) {
						ZEPHIR_CALL_FUNCTION(&_18$$9, "is_file", &_10, 405, &file);
						zephir_check_call_status();
