...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy)
{
	...
	if (_2) {
		ZEPHIR_CALL_FUNCTION(&_3, "is_file", NULL, 372, &file);
		zephir_check_call_status();
		_2 = zephir_is_true(&_3);
	}
	if (_2) {
		ZEPHIR_CALL_FUNCTION(NULL, "unlink", NULL, 211, &file);
		zephir_check_call_status();
	}
	RETURN_MM_BOOL(1);
}
	...
	if (!(ZEPHIR_IS_EMPTY(&glob))) {
		...
		if (Z_TYPE_P(&glob) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&glob), _5$$6)
			{
				...
				if (_6$$7) {
					ZEPHIR_CALL_FUNCTION(&_7$$7, "is_file", &_8, 372, &file);
					zephir_check_call_status();
				...
				if (_9$$7) {
					ZEPHIR_CALL_FUNCTION(NULL, "unlink", &_11, 211, &file);
					zephir_check_call_status();
				}
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_14$$9) {
						ZEPHIR_CALL_FUNCTION(&_15$$9, "is_file", &_8, 372, &file);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, read)
{
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_2)) {
		...
		zephir_check_call_status();
		ZVAL_LONG(&_4$$3, 1);
		ZEPHIR_CALL_FUNCTION(&_5$$3, "flock", NULL, 247, &pointer, &_4$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, phpFilePutContents)
{
	...
	ZVAL_LONG(&_0, flags);
	ZEPHIR_RETURN_CALL_FUNCTION("file_put_contents", NULL, 252, &filename_zv, data, &_0, context);
	zephir_check_call_status();
	RETURN_MM();
}
