...
PHP_METHOD(Phalcon_Session_Adapter_Stream, __construct) {
	...
	if (!(zephir_array_isset_string_fetch(&path, &options, SL("savePath"), 0))) {
		...
		ZEPHIR_CALL_FUNCTION(&path, "ini_get", NULL, 488, &_2$$3);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy) {
	...
	if (_2) {
		ZEPHIR_CALL_FUNCTION(&_3, "is_file", NULL, 489, &file);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, gc) {
	...
	ZEPHIR_CALL_FUNCTION(&_3, "glob", NULL, 490, &pattern);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&_3) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3), _4)
		{
			...
			if (_6$$3) {
				ZEPHIR_CALL_FUNCTION(&_7$$3, "is_file", &_8, 489, &file);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_12$$5) {
					ZEPHIR_CALL_FUNCTION(&_13$$5, "is_file", &_8, 489, &file);
					zephir_check_call_status();
