...
PHP_METHOD(Phalcon_Session_Adapter_Stream, __construct)
{
	...
	ZEPHIR_CALL_METHOD(&_6, this_ptr, "getdirseparator", NULL, 510, &path);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy)
{
	...
	if (_2) {
		ZEPHIR_CALL_FUNCTION(&_3, "is_file", NULL, 511, &file);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, gc)
{
	...
	if (Z_TYPE_P(&_3) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3), _4)
		{
			...
			if (_6$$3) {
				ZEPHIR_CALL_FUNCTION(&_7$$3, "is_file", &_8, 511, &file);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_12$$5) {
					ZEPHIR_CALL_FUNCTION(&_13$$5, "is_file", &_8, 511, &file);
					zephir_check_call_status();
