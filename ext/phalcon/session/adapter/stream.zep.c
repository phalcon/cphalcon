...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy)
{
	...
	if (_3) {
		ZEPHIR_CALL_FUNCTION(&_4, "is_file", NULL, 396, &file);
		zephir_check_call_status();
	...
	if (!(ZEPHIR_IS_EMPTY(&glob))) {
		...
		if (Z_TYPE_P(&glob) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&glob), _5$$6)
			{
				...
				if (_8$$7) {
					ZEPHIR_CALL_FUNCTION(&_9$$7, "is_file", &_10, 396, &file);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_17$$9) {
						ZEPHIR_CALL_FUNCTION(&_18$$9, "is_file", &_10, 396, &file);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, read)
{
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_2)) {
		...
		zephir_check_call_status();
		ZVAL_LONG(&_4$$3, 1);
		ZEPHIR_CALL_FUNCTION(&_5$$3, "flock", NULL, 279, &pointer, &_4$$3);
		zephir_check_call_status();
