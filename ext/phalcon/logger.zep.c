...
PHP_METHOD(Phalcon_Logger, excludeAdapters) {
	...
	zephir_is_iterable(&adapters, 0, "/home/nikos/Work/niden/cphalcon/phalcon/Logger.zep", 191);
...
PHP_METHOD(Phalcon_Logger, getAdapter) {
	...
	if (!(zephir_array_isset_fetch(&adapter, &adapters, &name, 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "Adapter does not exist for this logger", "/home/nikos/Work/niden/cphalcon/phalcon/Logger.zep", 208);
		return;
	}
...
PHP_METHOD(Phalcon_Logger, log) {
	...
	ZEPHIR_CALL_METHOD(&intLevel, this_ptr, "getlevelnumber", NULL, 411, level);
	zephir_check_call_status();
	zephir_get_strval(&_0, message);
...
PHP_METHOD(Phalcon_Logger, removeAdapter) {
	...
	if (1 != zephir_array_isset(&adapters, &name)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "Adapter does not exist for this logger", "/home/nikos/Work/niden/cphalcon/phalcon/Logger.zep", 283);
		return;
	}
...
PHP_METHOD(Phalcon_Logger, addMessage) {
	...
	if (ZEPHIR_GE_LONG(&_0, level)) {
		...
		if (zephir_fast_count_int(&registered) == 0) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "No adapters specified", "/home/nikos/Work/niden/cphalcon/phalcon/Logger.zep", 352);
			return;
		}
		ZEPHIR_CALL_METHOD(&levels, this_ptr, "getlevels", NULL, 0);
		zephir_check_call_status();
		...
		if (!(zephir_array_isset_long_fetch(&levelName, &levels, level, 0))) {
			...
			zephir_array_fetch_long(&levelName, &levels, 8, PH_NOISY, "/home/nikos/Work/niden/cphalcon/phalcon/Logger.zep", 358);
		}
		...
		ZEPHIR_CALL_METHOD(NULL, &item, "__construct", NULL, 412, &message, &levelName, &_1$$3, &_2$$3, &context);
		zephir_check_call_status();
		zephir_is_iterable(&registered, 0, "/home/nikos/Work/niden/cphalcon/phalcon/Logger.zep", 379);
