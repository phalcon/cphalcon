...
PHP_METHOD(Phalcon_Logger, __construct)
{
	...
	if (ZEPHIR_IS_EMPTY(timezone)) {
		ZEPHIR_CALL_FUNCTION(&localTimeZone, "date_default_timezone_get", NULL, 412);
...
PHP_METHOD(Phalcon_Logger, addMessage)
{
	...
	if (ZEPHIR_GE_LONG(&_0, level)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &item, "__construct", NULL, 414, &message, &levelName, &_4$$3, &_2$$3, &context);
		zephir_check_call_status();
