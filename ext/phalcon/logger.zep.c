...
ZEPHIR_INIT_CLASS(Phalcon_Logger) {
	...
	zend_class_implements(phalcon_logger_ce, 1, zephir_get_internal_ce(SL("psrext\\log\\loggerinterface")));
	return SUCCESS;
...
PHP_METHOD(Phalcon_Logger, log) {
	...
	ZEPHIR_CALL_METHOD(&intLevel, this_ptr, "getlevelnumber", NULL, 409, level);
	zephir_check_call_status();
	zephir_get_strval(&_0, message);
...
PHP_METHOD(Phalcon_Logger, addMessage) {
	...
	if (ZEPHIR_GE_LONG(&_0, level)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &item, "__construct", NULL, 410, &message, &levelName, &_1$$3, &_2$$3, &context);
		zephir_check_call_status();
