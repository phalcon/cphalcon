...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, validate) {
	...
	if (!(zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 361, &_4$$3, field, &_6$$3, &_5$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, getColumnNameReal) {
	...
	if (_7) {
		...
		zephir_array_fetch(&_10$$4, &_9$$4, &field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Uniqueness.zep", 140 TSRMLS_CC);
		RETURN_CTOR(&_10$$4);
	}
	RETURN_CTOR(&field);
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniqueness) {
	...
	if (Z_TYPE_P(field) != IS_ARRAY) {
		...
		array_init(field);
		zephir_array_append(field, &singleField, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 154);
	}
	...
	zephir_check_call_status();
	zephir_is_iterable(field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 164);
	...
	if (Z_TYPE_P(&convert) != IS_NULL) {
		...
		if (!(Z_TYPE_P(&values) == IS_ARRAY)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Value conversion must return an array", "phalcon/Validation/Validator/Uniqueness.zep", 168);
			return;
		}
	}
	ZEPHIR_INIT_NVAR(&_0);
	...
	if (_5) {
		...
		if (ZEPHIR_IS_EMPTY(&record)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Model of record must be set to property \"model\"", "phalcon/Validation/Validator/Uniqueness.zep", 178);
			return;
		}
	}
	...
	if (isModel) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "The uniqueness validator works only with Phalcon\\Mvc\\Model or Phalcon\\Mvc\\Collection", "phalcon/Validation/Validator/Uniqueness.zep", 190);
		return;
	}
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniquenessCollection) {
	...
	zephir_is_iterable(&field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 252);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _1)
	{
		ZEPHIR_INIT_NVAR(&singleField);
		ZVAL_COPY(&singleField, _1);
		...
		zephir_array_fetch(&value, &values, &singleField, PH_NOISY, "phalcon/Validation/Validator/Uniqueness.zep", 211 TSRMLS_CC);
		...
		if (zephir_is_true(&except)) {
			...
			if (_4$$6) {
				if (zephir_array_isset(&except, &singleField)) {
					...
					zephir_array_fetch(&fieldExcept, &except, &singleField, PH_NOISY, "phalcon/Validation/Validator/Uniqueness.zep", 224 TSRMLS_CC);
				}
			}
			...
			if (Z_TYPE_P(&fieldExcept) != IS_NULL) {
				if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
					zephir_is_iterable(&fieldExcept, 0, "phalcon/Validation/Validator/Uniqueness.zep", 233);
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _6$$10)
					{
						ZEPHIR_INIT_NVAR(&singleExcept);
						ZVAL_COPY(&singleExcept, _6$$10);
						zephir_array_append(&notInValues, &singleExcept, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 231);
					} ZEND_HASH_FOREACH_END();
					ZEPHIR_INIT_NVAR(&singleExcept);
				...
				zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 243);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _7$$13)
				{
					ZEPHIR_INIT_NVAR(&singleExcept);
					ZVAL_COPY(&singleExcept, _7$$13);
					zephir_array_append(&notInValues, &singleExcept, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 241);
				} ZEND_HASH_FOREACH_END();
				ZEPHIR_INIT_NVAR(&singleExcept);
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniquenessModel) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 350);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _1)
	{
		ZEPHIR_INIT_NVAR(&singleField);
		ZVAL_COPY(&singleField, _1);
		...
		if (zephir_is_true(&except)) {
			...
			if (_8$$6) {
				...
				ZEPHIR_CALL_FUNCTION(&_12$$6, "range", &_13, 485, &_10$$6, &_11$$6);
				zephir_check_call_status();
				_8$$6 = !ZEPHIR_IS_IDENTICAL(&_9$$6, &_12$$6);
			}
			if (_8$$6) {
				zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 313);
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&except), _15$$7, _16$$7, _14$$7)
				{
					...
					zephir_check_call_status();
					if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
						zephir_is_iterable(&fieldExcept, 0, "phalcon/Validation/Validator/Uniqueness.zep", 306);
						ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _19$$9)
						{
							...
							zephir_array_append(&notInValues, &_21$$10, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 302);
						...
						zephir_array_append(&exceptConditions, &_23$$9, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 306);
					} else {
						ZEPHIR_SINIT_NVAR(_24$$11);
						ZVAL_LONG(&_24$$11, index);
						ZEPHIR_INIT_LNVAR(_25$$11);
						ZEPHIR_CONCAT_VSV(&_25$$11, &attribute, " <> ?", &_24$$11);
						zephir_array_append(&exceptConditions, &_25$$11, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 308);
				...
				zephir_array_fetch_long(&_27$$12, &field, 0, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Uniqueness.zep", 314 TSRMLS_CC);
				...
				if (Z_TYPE_P(&except) == IS_ARRAY) {
					zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 321);
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _29$$13)
					{
						...
						zephir_array_append(&notInValues, &_31$$14, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 317);
						zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
						index++;
					} ZEND_HASH_FOREACH_END();
					ZEPHIR_INIT_NVAR(&singleExcept);
					...
					zephir_array_append(&exceptConditions, &_33$$13, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 321);
				} else {
				...
				zephir_is_iterable(&field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 343);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _36$$16)
				{
					...
					zephir_check_call_status();
					if (Z_TYPE_P(&except) == IS_ARRAY) {
						zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 336);
						ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _39$$18)
						{
							...
							zephir_array_append(&notInValues, &_41$$19, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 332);
							zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
							index++;
						} ZEND_HASH_FOREACH_END();
						ZEPHIR_INIT_NVAR(&singleExcept);
						...
						zephir_array_append(&exceptConditions, &_43$$18, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 336);
					} else {
	...
	if (ZEPHIR_IS_LONG(&_46, 0)) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_49$$21, 0, "phalcon/Validation/Validator/Uniqueness.zep", 358);
	...
	ZEPHIR_INIT_NVAR(&_0);
	zephir_array_fetch_string(&_58, &params, SL("conditions"), PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Uniqueness.zep", 364 TSRMLS_CC);
