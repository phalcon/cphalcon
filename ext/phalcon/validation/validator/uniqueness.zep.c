...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, validate) {
	...
	if (!(zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 358, &_4$$3, field, &_6$$3, &_5$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniqueness) {
	...
	if (_7) {
		...
		if (ZEPHIR_IS_EMPTY(&record)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Model of record must be set to property \"model\"", "phalcon/Validation/Validator/Uniqueness.zep", 191);
			return;
		}
	}
	...
	if (isModel) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "The uniqueness validator works only with Phalcon\\Mvc\\Model or Phalcon\\Mvc\\Collection", "phalcon/Validation/Validator/Uniqueness.zep", 205);
		return;
	}
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniquenessCollection) {
	...
	zephir_is_iterable(&field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 268);
	if (Z_TYPE_P(&field) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _1)
		{
			ZEPHIR_INIT_NVAR(&singleField);
			...
			zephir_array_fetch(&value, &values, &singleField, PH_NOISY, "phalcon/Validation/Validator/Uniqueness.zep", 227 TSRMLS_CC);
			...
			if (zephir_is_true(&except)) {
				...
				if (_5$$6) {
					if (zephir_array_isset(&except, &singleField)) {
						...
						zephir_array_fetch(&fieldExcept, &except, &singleField, PH_NOISY, "phalcon/Validation/Validator/Uniqueness.zep", 240 TSRMLS_CC);
					}
				}
				...
				if (Z_TYPE_P(&fieldExcept) != IS_NULL) {
					if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
						...
						if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
							ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _7$$10)
							{
								...
								zephir_array_append(&notInValues, &singleExcept, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 247);
							} ZEND_HASH_FOREACH_END();
						} else {
					...
					zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 259);
					if (Z_TYPE_P(&except) == IS_ARRAY) {
						ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _9$$14)
						{
							ZEPHIR_INIT_NVAR(&singleExcept);
							ZVAL_COPY(&singleExcept, _9$$14);
							zephir_array_append(&notInValues, &singleExcept, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 257);
						} ZEND_HASH_FOREACH_END();
					} else {
					...
					zephir_create_array(&arrayValue$$14, 1, 0 TSRMLS_CC);
					zephir_array_update_string(&arrayValue$$14, SL("$nin"), &notInValues, PH_COPY | PH_SEPARATE);
					zephir_array_update_multi(&params, &arrayValue$$14 TSRMLS_CC, SL("sz"), 3, SL("conditions"), &singleField);
				} else if (zephir_fast_count_int(&field TSRMLS_CC) == 1) {
					ZEPHIR_INIT_NVAR(&arrayValue$$17);
					zephir_create_array(&arrayValue$$17, 1, 0 TSRMLS_CC);
					zephir_array_update_string(&arrayValue$$17, SL("$ne"), &except, PH_COPY | PH_SEPARATE);
					zephir_array_update_multi(&params, &arrayValue$$17 TSRMLS_CC, SL("sz"), 3, SL("conditions"), &singleField);
				}
			}
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (zephir_is_true(&except)) {
					...
					if (_12$$21) {
						_12$$21 = zephir_fast_count_int(&field TSRMLS_CC) > 1;
					}
					if (_12$$21) {
						if (zephir_array_isset(&except, &singleField)) {
							ZEPHIR_OBS_NVAR(&fieldExcept);
					...
					if (_13$$21) {
						_13$$21 = zephir_fast_count_int(&field TSRMLS_CC) == 1;
					}
					if (Z_TYPE_P(&fieldExcept) != IS_NULL) {
						if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
							zephir_is_iterable(&fieldExcept, 0, "phalcon/Validation/Validator/Uniqueness.zep", 249);
							...
							zephir_create_array(&arrayValue$$25, 1, 0 TSRMLS_CC);
							...
							zephir_create_array(&arrayValue$$28, 1, 0 TSRMLS_CC);
						...
						zephir_create_array(&arrayValue$$29, 1, 0 TSRMLS_CC);
						zephir_array_update_string(&arrayValue$$29, SL("$nin"), &notInValues, PH_COPY | PH_SEPARATE);
						zephir_array_update_multi(&params, &arrayValue$$29 TSRMLS_CC, SL("sz"), 3, SL("conditions"), &singleField);
					} else if (zephir_fast_count_int(&field TSRMLS_CC) == 1) {
						ZEPHIR_INIT_NVAR(&arrayValue$$32);
						zephir_create_array(&arrayValue$$32, 1, 0 TSRMLS_CC);
						zephir_array_update_string(&arrayValue$$32, SL("$ne"), &except, PH_COPY | PH_SEPARATE);
						zephir_array_update_multi(&params, &arrayValue$$32 TSRMLS_CC, SL("sz"), 3, SL("conditions"), &singleField);
					}
				}
			ZEPHIR_CALL_METHOD(NULL, &field, "next", NULL, 0);
			zephir_check_call_status();
	...
	if (ZEPHIR_IS_LONG(&_18, 0)) {
		...
		zephir_create_array(&arrayValue$$33, 1, 0 TSRMLS_CC);
		ZEPHIR_CALL_METHOD(&_19$$33, record, "getid", NULL, 0);
		zephir_check_call_status();
		zephir_array_update_string(&arrayValue$$33, SL("$ne"), &_19$$33, PH_COPY | PH_SEPARATE);
		zephir_array_update_multi(&params, &arrayValue$$33 TSRMLS_CC, SL("ss"), 4, SL("conditions"), SL("_id"));
	}
	if (!(ZEPHIR_IS_EMPTY(&exceptConditions))) {
		ZEPHIR_INIT_VAR(&_20$$34);
		zephir_create_array(&_20$$34, 1, 0 TSRMLS_CC);
		zephir_array_fast_append(&_20$$34, &exceptConditions);
		zephir_array_update_multi(&params, &_20$$34 TSRMLS_CC, SL("ss"), 4, SL("conditions"), SL("$or"));
	}
	RETURN_CCTOR(&params);
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniquenessModel) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 379);
	if (Z_TYPE_P(&field) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _1)
		{
			ZEPHIR_INIT_NVAR(&singleField);
			...
			array_init(&notInValues);
			ZEPHIR_OBS_NVAR(&value);
			zephir_array_fetch(&value, &values, &singleField, PH_NOISY, "phalcon/Validation/Validator/Uniqueness.zep", 300 TSRMLS_CC);
			ZEPHIR_INIT_NVAR(&_3$$3);
			...
			if (zephir_is_true(&except)) {
				...
				if (_9$$6) {
					...
					ZEPHIR_CALL_FUNCTION(&_13$$6, "range", &_14, 482, &_11$$6, &_12$$6);
					zephir_check_call_status();
					_9$$6 = !ZEPHIR_IS_IDENTICAL(&_10$$6, &_13$$6);
				}
				if (_9$$6) {
					zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 334);
					if (Z_TYPE_P(&except) == IS_ARRAY) {
						ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&except), _17$$7, _18$$7, _15$$7)
						{
							...
							zephir_check_call_status();
							if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
								zephir_is_iterable(&fieldExcept, 0, "phalcon/Validation/Validator/Uniqueness.zep", 327);
								if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
									ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _21$$9)
									{
										...
										zephir_array_append(&notInValues, &_24$$10, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
									...
									while (1) {
										...
										zephir_check_call_status();
											...
											zephir_array_append(&notInValues, &_26$$11, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
											zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
											index++;
										ZEPHIR_CALL_METHOD(NULL, &fieldExcept, "next", NULL, 0);
										zephir_check_call_status();
								...
								zephir_array_append(&exceptConditions, &_28$$9, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 327);
							} else {
								ZEPHIR_SINIT_NVAR(_29$$12);
								ZVAL_LONG(&_29$$12, index);
								ZEPHIR_INIT_LNVAR(_30$$12);
								ZEPHIR_CONCAT_VSV(&_30$$12, &attribute, " <> ?", &_29$$12);
								zephir_array_append(&exceptConditions, &_30$$12, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 329);
								...
								if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
									...
									if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
										ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _33$$14)
										{
											...
											zephir_array_append(&notInValues, &_36$$15, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
											zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
											index++;
										} ZEND_HASH_FOREACH_END();
									} else {
										...
										while (1) {
											...
											zephir_check_call_status();
												...
												zephir_array_append(&notInValues, &_38$$16, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
												zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
												index++;
											ZEPHIR_CALL_METHOD(NULL, &fieldExcept, "next", NULL, 0);
											zephir_check_call_status();
									...
									zephir_array_append(&exceptConditions, &_40$$14, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 327);
								} else {
									ZEPHIR_SINIT_NVAR(_41$$17);
									ZVAL_LONG(&_41$$17, index);
									ZEPHIR_INIT_LNVAR(_42$$17);
									ZEPHIR_CONCAT_VSV(&_42$$17, &attribute, " <> ?", &_41$$17);
									zephir_array_append(&exceptConditions, &_42$$17, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 329);
									zephir_array_update_multi(&params, &fieldExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
									index++;
								}
							ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
					...
					zephir_array_fetch_long(&_44$$18, &field, 0, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Uniqueness.zep", 337 TSRMLS_CC);
					...
					zephir_check_call_status();
					if (Z_TYPE_P(&except) == IS_ARRAY) {
						zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 346);
						if (Z_TYPE_P(&except) == IS_ARRAY) {
							ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _46$$19)
							{
								...
								zephir_array_append(&notInValues, &_49$$20, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 342);
								zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
								index++;
							} ZEND_HASH_FOREACH_END();
						} else {
							...
							while (1) {
								...
								zephir_check_call_status();
									...
									zephir_array_append(&notInValues, &_51$$21, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 342);
									zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
									index++;
								ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
								zephir_check_call_status();
						...
						zephir_array_append(&exceptConditions, &_53$$19, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 346);
					} else {
					...
					zephir_is_iterable(&field, 0, "phalcon/Validation/Validator/Uniqueness.zep", 372);
					if (Z_TYPE_P(&field) == IS_ARRAY) {
						ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&field), _56$$23)
						{
							...
							zephir_check_call_status();
							if (Z_TYPE_P(&except) == IS_ARRAY) {
								zephir_is_iterable(&except, 0, "phalcon/Validation/Validator/Uniqueness.zep", 365);
								if (Z_TYPE_P(&except) == IS_ARRAY) {
									ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _60$$25)
									{
										...
										zephir_array_append(&notInValues, &_63$$26, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
										zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
										index++;
									} ZEND_HASH_FOREACH_END();
								} else {
									...
									while (1) {
										...
										zephir_check_call_status();
											...
											zephir_array_append(&notInValues, &_65$$27, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
											zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
											index++;
										ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
										zephir_check_call_status();
								...
								zephir_array_append(&exceptConditions, &_67$$25, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 365);
							} else {
								...
								if (Z_TYPE_P(&except) == IS_ARRAY) {
									...
									if (Z_TYPE_P(&except) == IS_ARRAY) {
										ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _72$$30)
										{
											...
											zephir_array_append(&notInValues, &_75$$31, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
											zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
											index++;
										} ZEND_HASH_FOREACH_END();
									} else {
										...
										while (1) {
											...
											zephir_check_call_status();
												...
												zephir_array_append(&notInValues, &_77$$32, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
												zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
												index++;
											ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
											zephir_check_call_status();
									...
									zephir_array_append(&exceptConditions, &_79$$30, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 365);
								} else {
			...
			zephir_check_call_status();
				...
				if (zephir_is_true(&except)) {
					...
					if (_87$$37) {
						...
						ZEPHIR_CALL_FUNCTION(&_91$$37, "range", &_14, 482, &_89$$37, &_90$$37);
						zephir_check_call_status();
					...
					if (_87$$37) {
								...
								if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
									...
									if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
										ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _98$$40)
										{
											...
											zephir_array_append(&notInValues, &_101$$41, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
											zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
											index++;
										} ZEND_HASH_FOREACH_END();
									} else {
										...
										while (1) {
											...
											zephir_check_call_status();
												...
												zephir_array_append(&notInValues, &_103$$42, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
												zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
												index++;
											ZEPHIR_CALL_METHOD(NULL, &fieldExcept, "next", NULL, 0);
											zephir_check_call_status();
									...
									zephir_array_append(&exceptConditions, &_105$$40, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 327);
								} else {
									ZEPHIR_SINIT_NVAR(_106$$43);
									ZVAL_LONG(&_106$$43, index);
									ZEPHIR_INIT_LNVAR(_107$$43);
									ZEPHIR_CONCAT_VSV(&_107$$43, &attribute, " <> ?", &_106$$43);
									zephir_array_append(&exceptConditions, &_107$$43, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 329);
									zephir_array_update_multi(&params, &fieldExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
									index++;
								}
							} ZEND_HASH_FOREACH_END();
									...
									if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
										...
										if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
											ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _110$$45)
											{
												...
												zephir_array_append(&notInValues, &_113$$46, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
												zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
												index++;
											} ZEND_HASH_FOREACH_END();
										} else {
											...
											while (1) {
												...
												zephir_check_call_status();
													...
													zephir_array_append(&notInValues, &_115$$47, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 323);
													zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
													index++;
												ZEPHIR_CALL_METHOD(NULL, &fieldExcept, "next", NULL, 0);
												zephir_check_call_status();
										...
										zephir_array_append(&exceptConditions, &_117$$45, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 327);
									} else {
										ZEPHIR_SINIT_NVAR(_118$$48);
										ZVAL_LONG(&_118$$48, index);
										ZEPHIR_INIT_LNVAR(_119$$48);
										ZEPHIR_CONCAT_VSV(&_119$$48, &attribute, " <> ?", &_118$$48);
										zephir_array_append(&exceptConditions, &_119$$48, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 329);
										zephir_array_update_multi(&params, &fieldExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
										index++;
									}
								ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
						...
						zephir_array_fetch_long(&_121$$49, &field, 0, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Uniqueness.zep", 337 TSRMLS_CC);
						...
						if (Z_TYPE_P(&except) == IS_ARRAY) {
							...
							if (Z_TYPE_P(&except) == IS_ARRAY) {
								ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _123$$50)
								{
									...
									zephir_array_append(&notInValues, &_126$$51, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 342);
									zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
									index++;
								} ZEND_HASH_FOREACH_END();
							} else {
								...
								while (1) {
									...
									zephir_check_call_status();
										...
										zephir_array_append(&notInValues, &_128$$52, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 342);
										zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
										index++;
									ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
									zephir_check_call_status();
							...
							zephir_array_append(&exceptConditions, &_130$$50, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 346);
						} else {
								...
								if (Z_TYPE_P(&except) == IS_ARRAY) {
									...
									if (Z_TYPE_P(&except) == IS_ARRAY) {
										ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _137$$56)
										{
											...
											zephir_array_append(&notInValues, &_140$$57, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
											zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
											index++;
										} ZEND_HASH_FOREACH_END();
									} else {
										...
										while (1) {
											...
											zephir_check_call_status();
												...
												zephir_array_append(&notInValues, &_142$$58, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
												zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
												index++;
											ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
											zephir_check_call_status();
									...
									zephir_array_append(&exceptConditions, &_144$$56, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 365);
								} else {
									...
									if (Z_TYPE_P(&except) == IS_ARRAY) {
										...
										if (Z_TYPE_P(&except) == IS_ARRAY) {
											ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _149$$61)
											{
												...
												zephir_array_append(&notInValues, &_152$$62, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
												zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
												index++;
											} ZEND_HASH_FOREACH_END();
										} else {
											...
											while (1) {
												...
												zephir_check_call_status();
													...
													zephir_array_append(&notInValues, &_154$$63, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 361);
													zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
													index++;
												ZEPHIR_CALL_METHOD(NULL, &except, "next", NULL, 0);
												zephir_check_call_status();
										...
										zephir_array_append(&exceptConditions, &_156$$61, PH_SEPARATE, "phalcon/Validation/Validator/Uniqueness.zep", 365);
									} else {
	...
	if (ZEPHIR_IS_LONG(&_159, 0)) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_162$$65, 0, "phalcon/Validation/Validator/Uniqueness.zep", 391);
	...
	ZEPHIR_INIT_NVAR(&_0);
	zephir_array_fetch_string(&_177, &params, SL("conditions"), PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Uniqueness.zep", 397 TSRMLS_CC);
