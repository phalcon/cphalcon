...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	ZEPHIR_CALL_METHOD(&valid, this_ptr, "verifybyluhnalgorithm", NULL, 495, &value);
	zephir_check_call_status();
	if (!(zephir_is_true(&valid))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 337, &_1$$3, field, &_2$$3, &code);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	ZVAL_STRING(&hash, "");
	ZEPHIR_CALL_FUNCTION(&_3, "array_reverse", NULL, 219, &digits);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 496, &_8);
	zephir_check_call_status();
