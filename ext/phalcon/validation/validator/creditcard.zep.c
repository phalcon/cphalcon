...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	if (!(zephir_is_true(&valid))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 404, &_1$$3, field, &_2$$3, &code);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	zval digits, _1;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *number_param = NULL, digit, position, hash, result, _0, _2, _3, *_4, _8, _7$$3;
	zval number;
	zval *this_ptr = getThis();
	...
	ZEPHIR_CALL_FUNCTION(&_3, "array_reverse", NULL, 256, &digits);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 0, &_8);
	zephir_check_call_status();
