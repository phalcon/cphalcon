...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	ZEPHIR_CALL_METHOD(&valid, this_ptr, "verifybyluhnalgorithm", NULL, 474, value);
	zephir_check_call_status();
	if (!(zephir_is_true(valid))) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0$$3, "__construct", NULL, 470, _1$$3, field, _2$$3, code);
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	ZVAL_STRING(hash, "", 1);
	ZEPHIR_CALL_FUNCTION(&_3, "array_reverse", NULL, 447, digits);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 475, _8);
	zephir_check_call_status();
