...
PHP_METHOD(Phalcon_Validation_Validator_Ip, validate) {
	...
	if (Z_TYPE_P(&label) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &label, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Ip.zep", 87 TSRMLS_CC);
	...
	if (Z_TYPE_P(&message) == IS_ARRAY) {
		zephir_array_fetch(&_2$$5, &message, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Ip.zep", 95 TSRMLS_CC);
	...
	if (Z_TYPE_P(&version) == IS_ARRAY) {
		zephir_array_fetch(&_5$$7, &version, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Ip.zep", 103 TSRMLS_CC);
	...
	if (Z_TYPE_P(&allowPrivate) == IS_ARRAY) {
		zephir_array_fetch(&_7$$8, &allowPrivate, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Ip.zep", 108 TSRMLS_CC);
	...
	if (Z_TYPE_P(&allowReserved) == IS_ARRAY) {
		zephir_array_fetch(&_9$$9, &allowReserved, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Ip.zep", 113 TSRMLS_CC);
	...
	if (Z_TYPE_P(&allowEmpty) == IS_ARRAY) {
		...
		if (zephir_array_isset(&allowEmpty, field)) {
			...
			zephir_array_fetch(&_10$$10, &allowEmpty, field, PH_NOISY, "phalcon/Validation/Validator/Ip.zep", 118 TSRMLS_CC);
		} else {
	...
	ZVAL_LONG(&_4, 275);
	ZEPHIR_CALL_FUNCTION(&_15, "filter_var", NULL, 184, &value, &_4, &options);
	zephir_check_call_status();
	if (!(zephir_is_true(&_15))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_16$$12, "__construct", NULL, 358, &_17$$12, field, &_18$$12);
		zephir_check_call_status();
