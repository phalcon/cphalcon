...
PHP_METHOD(Phalcon_Validation_Validator_File, validate) {
	...
	if (zephir_is_true(&_42)) {
		...
		if ((zephir_function_exists_ex(SL("finfo_open") TSRMLS_CC) == SUCCESS)) {
			ZVAL_LONG(&_44$$13, 16);
			ZEPHIR_CALL_FUNCTION(&tmp, "finfo_open", NULL, 337, &_44$$13);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 338, &tmp, &_45$$13);
			zephir_check_call_status();
	...
	if (_50) {
		...
		ZEPHIR_CALL_FUNCTION(&tmp, "getimagesize", NULL, 345, &_52$$16);
		zephir_check_call_status();
