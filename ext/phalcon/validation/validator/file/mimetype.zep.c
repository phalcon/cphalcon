...
PHP_METHOD(Phalcon_Validation_Validator_File_MimeType, validate) {
	...
	if ((zephir_function_exists_ex(SL("finfo_open") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_2$$6, 16);
		ZEPHIR_CALL_FUNCTION(&tmp, "finfo_open", NULL, 358, &_2$$6);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 359, &tmp, &_3$$6);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 360, &tmp);
		zephir_check_call_status();
	} else {
