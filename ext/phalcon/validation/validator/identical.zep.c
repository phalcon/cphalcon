...
PHP_METHOD(Phalcon_Validation_Validator_Identical, validate) {
	...
	if (zephir_is_true(&_0)) {
		...
		if (Z_TYPE_P(&accepted) == IS_ARRAY) {
			zephir_array_fetch(&_3$$4, &accepted, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Identical.zep", 74 TSRMLS_CC);
		...
		if (zephir_is_true(&_4$$5)) {
			...
			if (Z_TYPE_P(&valueOption) == IS_ARRAY) {
				zephir_array_fetch(&_7$$7, &valueOption, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/Identical.zep", 83 TSRMLS_CC);
	...
	if (!(valid)) {
		...
		ZEPHIR_CALL_FUNCTION(&_9$$8, "strtr", NULL, 63, &message, &replacePairs);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_10$$8);
		ZVAL_STRING(&_10$$8, "Identical");
		ZEPHIR_CALL_METHOD(NULL, &_8$$8, "__construct", NULL, 401, &_9$$8, field, &_10$$8, &code);
		zephir_check_call_status();
