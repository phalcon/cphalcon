...
PHP_METHOD(Phalcon_Validation_Validator_StringLength_Max, validate) {
	...
	if (Z_TYPE_P(&maximum) == IS_ARRAY) {
		zephir_array_fetch(&_1$$5, &maximum, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/StringLength/Max.zep", 87);
	...
	if (Z_TYPE_P(&included) == IS_ARRAY) {
		ZEPHIR_OBS_VAR(&_2$$6);
		zephir_array_fetch(&_2$$6, &included, field, PH_NOISY, "phalcon/Validation/Validator/StringLength/Max.zep", 93);
