...
PHP_METHOD(Phalcon_Validation_Validator_StringLength, validate) {
	...
	if (_1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "A minimum or maximum must be set", "phalcon/Validation/Validator/StringLength.zep", 85);
		return;
	}
	...
	if (zephir_is_true(&isSetMax)) {
		...
		if (Z_TYPE_P(&maximum) == IS_ARRAY) {
			zephir_array_fetch(&_3$$7, &maximum, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/StringLength.zep", 106 TSRMLS_CC);
		...
		if (ZEPHIR_GT(&length, &maximum)) {
			...
			ZEPHIR_CALL_FUNCTION(&_6$$8, "strtr", NULL, 63, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_5$$8);
			ZVAL_STRING(&_5$$8, "TooLong");
			ZEPHIR_CALL_METHOD(NULL, &_4$$8, "__construct", NULL, 361, &_6$$8, field, &_5$$8, &code);
			zephir_check_call_status();
	...
	if (zephir_is_true(&isSetMin)) {
		...
		if (Z_TYPE_P(&minimum) == IS_ARRAY) {
			zephir_array_fetch(&_8$$10, &minimum, field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator/StringLength.zep", 132 TSRMLS_CC);
		...
		if (ZEPHIR_LT(&length, &minimum)) {
			...
			ZEPHIR_CALL_FUNCTION(&_11$$11, "strtr", NULL, 63, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_10$$11);
			ZVAL_STRING(&_10$$11, "TooShort");
			ZEPHIR_CALL_METHOD(NULL, &_9$$11, "__construct", NULL, 361, &_11$$11, field, &_10$$11, &code);
			zephir_check_call_status();
