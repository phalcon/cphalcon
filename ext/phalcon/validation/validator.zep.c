...
ZEPHIR_INIT_CLASS(Phalcon_Validation_Validator) {
	...
	zend_declare_property_null(phalcon_validation_validator_ce, SL("options"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Validation_Validator, prepareLabel) {
	...
	if (Z_TYPE_P(&label) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &label, &field, PH_NOISY | PH_READONLY, "phalcon/Validation/Validator.zep", 106 TSRMLS_CC);
