...
PHP_METHOD(Phalcon_Config, toArray) {
	...
	zend_string *_4;
	zend_ulong _3;
	zval key, value, arrayConfig, _0, *_1, _2, _6$$4, _8$$6;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _3, _4, _1)
		{
			...
			if (_5$$3) {
				_5$$3 = (zephir_method_exists_ex(&value, SL("toarray") TSRMLS_CC) == SUCCESS);
			}
			if (_5$$3) {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_7$$5) {
					_7$$5 = (zephir_method_exists_ex(&value, SL("toarray") TSRMLS_CC) == SUCCESS);
				}
				if (_7$$5) {
...
PHP_METHOD(Phalcon_Config, internalMerge) {
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _3, _4, _1)
		{
			...
			ZEPHIR_INIT_NVAR(&value);
			ZVAL_COPY(&value, _1);
			ZEPHIR_CALL_FUNCTION(&property, "strval", &_5, 8, &key);
			zephir_check_call_status();
			...
			if (zephir_fetch_property_zval(&localObject, instance, &property, PH_SILENT_CC)) {
				...
				if (_6$$5) {
					...
					if (_7$$6) {
						ZEPHIR_CALL_METHOD(NULL, this_ptr, "internalmerge", &_8, 9, &value, &localObject);
						zephir_check_call_status();
						continue;
					}
				}
			...
			if (zephir_is_numeric(&key)) {
				ZEPHIR_CALL_FUNCTION(&_9$$8, "strval", &_5, 8, &key);
				zephir_check_call_status();
				...
				while (1) {
					...
					ZEPHIR_CALL_FUNCTION(&key, "strval", &_5, 8, &number);
					zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (zephir_is_numeric(&key)) {
					ZEPHIR_CALL_FUNCTION(&_13$$14, "strval", &_5, 8, &key);
					zephir_check_call_status();
