...
PHP_METHOD(Phalcon_Config, merge) {
	...
	if (Z_TYPE_P(toMerge) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &config, "__construct", NULL, 20, toMerge);
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&result, this_ptr, "internalmerge", NULL, 21, &source, &target);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config, path) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&key, "array_shift", &_1, 22, &keys);
		ZEPHIR_UNREF(&keys);
		zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$7) {
					...
					ZEPHIR_CALL_METHOD(&_13$$8, this_ptr, "internalmerge", &_9, 21, &_14$$8, &value);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config, setData) {
	...
	if (zephir_is_true(&_1)) {
		ZEPHIR_CALL_FUNCTION(&key, "mb_strtolower", NULL, 23, element);
		zephir_check_call_status();
	} else {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &data, "__construct", NULL, 20, value);
		zephir_check_call_status();
	} else {
