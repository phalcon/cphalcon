...
PHP_METHOD(Phalcon_Support_Debug_Dump, __construct)
{
	...
	zephir_check_call_status();
	if (detailed) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1344, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1344, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Support_Debug_Dump, output)
{
	...
	if (Z_TYPE_P(variable) == IS_OBJECT) {
		...
		if (zephir_instance_of_ev(variable, phalcon_di_diinterface_ce)) {
			...
			zephir_read_property_cached(&_47$$9, this_ptr, _zephir_prop_0, 1344, PH_NOISY_CC | PH_READONLY);
		...
		ZEPHIR_INIT_NVAR(&_39$$9);
		zephir_get_class(&_39$$9, variable, 0);
		zephir_read_property_cached(&_107$$9, this_ptr, _zephir_prop_1, 1347, PH_NOISY_CC | PH_READONLY);
