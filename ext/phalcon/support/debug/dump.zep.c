...
PHP_METHOD(Phalcon_Support_Debug_Dump, output)
{
	...
	if (Z_TYPE_P(variable) == IS_OBJECT) {
		...
		if (zephir_instance_of_ev(variable, phalcon_di_diinterface_ce)) {
			...
			if (_43$$9) {
				...
				ZEPHIR_CALL_METHOD(&props$$15, &reflect$$15, "getproperties", NULL, 266, &_69$$15);
				zephir_check_call_status();
	...
	if (zephir_is_numeric(variable)) {
		...
		ZEPHIR_CALL_FUNCTION(&_157$$28, "mb_strlen", NULL, 421, variable);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(variable) == IS_STRING) {
		...
		ZEPHIR_CALL_FUNCTION(&_161$$29, "mb_strlen", NULL, 421, variable);
		zephir_check_call_status();
