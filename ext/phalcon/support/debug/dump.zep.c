...
PHP_METHOD(Phalcon_Support_Debug_Dump, __construct)
{
	...
	zephir_check_call_status();
	if (detailed) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1329, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1329, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Support_Debug_Dump, output)
{
	...
	if (Z_TYPE_P(variable) == IS_OBJECT) {
		...
		if (zephir_instance_of_ev(variable, phalcon_di_diinterface_ce)) {
			...
			zephir_read_property_cached(&_47$$9, this_ptr, _zephir_prop_0, 1329, PH_NOISY_CC | PH_READONLY);
			_48$$9 = !zephir_is_true(&_47$$9);
			if (!(_48$$9)) {
				_48$$9 = zephir_is_instance_of(variable, SL("stdClass"));
			}
			if (_48$$9) {
				ZEPHIR_CALL_FUNCTION(&_52$$12, "get_object_vars", NULL, 341, variable);
				zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &reflect, "__construct", NULL, 240, variable);
				zephir_check_call_status();
				ZVAL_LONG(&_76$$15, ((1 | 2) | 4));
				ZEPHIR_CALL_METHOD(&props, &reflect, "getproperties", NULL, 353, &_76$$15);
				zephir_check_call_status();
		...
		ZEPHIR_INIT_NVAR(&_39$$9);
		zephir_get_class(&_39$$9, variable, 0);
		zephir_read_property_cached(&_107$$9, this_ptr, _zephir_prop_1, 1332, PH_NOISY_CC | PH_READONLY);
