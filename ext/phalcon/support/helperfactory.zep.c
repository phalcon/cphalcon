...
PHP_METHOD(Phalcon_Support_HelperFactory, newInstance)
{
	...
	zephir_memory_observe(&name_zv);
	ZVAL_STR_COPY(&name_zv, name);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 1376, PH_NOISY_CC | PH_READONLY);
	...
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 1376, PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch(&_4, &_3, &name_zv, PH_NOISY | PH_READONLY, "phalcon/Support/HelperFactory.zep", 177);
	RETURN_CTOR(&_4);
}
