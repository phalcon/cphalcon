...
ZEPHIR_INIT_CLASS(Phalcon_Support_Debug) {
	 ...
	 * @var bool
	 */
	zend_declare_property_null(phalcon_support_debug_ce, SL("isActive"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);
...
PHP_METHOD(Phalcon_Support_Debug, halt) {
	...
	ZEPHIR_INIT_VAR(&_0);
	if (!_1) {
	_1 = zephir_fetch_class_str_ex(SL("Phalcon\\Support\\Exception"), ZEND_FETCH_CLASS_AUTO);
	}
	object_init_ex(&_0, _1);
	if (zephir_has_constructor(&_0)) {
		ZEPHIR_INIT_VAR(&_2);
		ZVAL_STRING(&_2, "Halted request");
		ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 0, &_2);
		zephir_check_call_status();
	}
	zephir_throw_exception_debug(&_0, "phalcon/Support/Debug.zep", 150);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Phalcon_Support_Debug, onUncaughtLowSeverity) {
	...
	if (UNEXPECTED(zephir_is_true(&_1))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$3, "phalcon/Support/Debug.zep", 246);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Support_Debug, renderHtml) {
	...
	if (zephir_is_true(&showBackTrace)) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_8$$3, 0, "phalcon/Support/Debug.zep", 327);
		...
		zephir_check_call_status();
		zephir_is_iterable(&_REQUEST, 0, "phalcon/Support/Debug.zep", 347);
		...
		zephir_check_call_status();
		zephir_is_iterable(&_SERVER, 0, "phalcon/Support/Debug.zep", 363);
		...
		zephir_check_call_status();
		zephir_is_iterable(&_49$$3, 0, "phalcon/Support/Debug.zep", 376);
		...
		if (Z_TYPE_P(&dataVars) == IS_ARRAY) {
			...
			zephir_is_iterable(&dataVars, 0, "phalcon/Support/Debug.zep", 398);
			if (Z_TYPE_P(&dataVars) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&dataVars), _61$$21, _62$$21, _59$$21)
				{
					...
					zephir_array_fetch_long(&_64$$22, &dataVar, 0, PH_NOISY | PH_READONLY, "phalcon/Support/Debug.zep", 395);
				...
				while (1) {
					...
					zephir_check_call_status();
						zephir_array_fetch_long(&_67$$23, &dataVar, 0, PH_NOISY | PH_READONLY, "phalcon/Support/Debug.zep", 395);
...
PHP_METHOD(Phalcon_Support_Debug, setBlacklist) {
	...
	array_init(&result);
	zephir_is_iterable(&area, 0, "phalcon/Support/Debug.zep", 428);
	...
	array_init(&subArray);
	zephir_is_iterable(&area, 0, "phalcon/Support/Debug.zep", 437);
	if (Z_TYPE_P(&area) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Support_Debug, getArrayDump) {
	...
	array_init(&dump);
	zephir_is_iterable(&argument, 0, "phalcon/Support/Debug.zep", 537);
	if (Z_TYPE_P(&argument) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&argument), _3, _4, _1)
		{
			...
			zephir_array_append(&dump, &_10$$5, PH_SEPARATE, "phalcon/Support/Debug.zep", 534);
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_append(&dump, &_14$$12, PH_SEPARATE, "phalcon/Support/Debug.zep", 534);
...
PHP_METHOD(Phalcon_Support_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, &trace, SL("class"), 0)) {
		...
		if (zephir_is_true(&_4$$3)) {
			...
			zephir_array_fetch_long(&_6$$4, &parts, 0, PH_NOISY | PH_READONLY, "phalcon/Support/Debug.zep", 642);
			zephir_array_fetch_long(&_7$$4, &parts, 1, PH_NOISY | PH_READONLY, "phalcon/Support/Debug.zep", 642);
		...
		zephir_array_fetch_string(&_13$$3, &trace, SL("type"), PH_NOISY | PH_READONLY, "phalcon/Support/Debug.zep", 675);
		zephir_concat_self(&html, &_13$$3);
	}
	ZEPHIR_OBS_VAR(&functionName);
	zephir_array_fetch_string(&functionName, &trace, SL("function"), PH_NOISY, "phalcon/Support/Debug.zep", 681);
	...
	if (zephir_array_isset_string_fetch(&traceArgs, &trace, SL("args"), 0)) {
		...
		zephir_is_iterable(&traceArgs, 0, "phalcon/Support/Debug.zep", 733);
		if (Z_TYPE_P(&traceArgs) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&traceArgs), _18$$14)
			{
				...
				zephir_array_append(&arguments, &_22$$15, PH_SEPARATE, "phalcon/Support/Debug.zep", 727);
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_array_append(&arguments, &_24$$16, PH_SEPARATE, "phalcon/Support/Debug.zep", 727);
	...
	if (zephir_array_isset_string_fetch(&filez, &trace, SL("file"), 0)) {
		...
		zephir_array_fetch_string(&_27$$17, &trace, SL("line"), PH_NOISY, "phalcon/Support/Debug.zep", 741);
		...
		if (zephir_is_true(&showFiles)) {
			...
			while (1) {
				...
				zephir_array_fetch(&currentLine, &lines, &linePosition, PH_NOISY, "phalcon/Support/Debug.zep", 814);
				if (zephir_is_true(&showFileFragment)) {
