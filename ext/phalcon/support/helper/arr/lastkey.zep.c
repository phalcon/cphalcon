...
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Support\\Helper\\Arr, LastKey, phalcon, support_helper_arr_lastkey, phalcon_support_helper_arr_lastkey_method_entry, 0);
...
PHP_METHOD(Phalcon_Support_Helper_Arr_LastKey, __invoke)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("array_key_last", NULL, 20, &filtered);
	zephir_check_call_status();
	RETURN_MM();
}
 ...
 * @return array
 */
PHP_METHOD(Phalcon_Support_Helper_Arr_LastKey, toFilter)
{
	zend_bool _0;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	...
	ZEPHIR_RETURN_CALL_FUNCTION("array_filter", NULL, 303, &collection, method);
	zephir_check_call_status();
	RETURN_MM();
}
