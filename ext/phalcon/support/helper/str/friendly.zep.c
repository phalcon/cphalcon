...
PHP_METHOD(Phalcon_Support_Helper_Str_Friendly, checkReplace)
{
	...
	if (_0) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_support_helper_str_exceptions_invalidreplaceformat_ce, "Parameter replace must be an array or a string", "phalcon/Support/Helper/Str/Friendly.zep", 71);
		return;
	}
...
PHP_METHOD(Phalcon_Support_Helper_Str_Friendly, getMatrix)
{
	...
	add_assoc_stringl_ex(&matrix, SL("\n"), SL(" "));
	zephir_is_iterable(&replace, 0, "phalcon/Support/Helper/Str/Friendly.zep", 118);
