...
PHP_METHOD(Phalcon_ADR_Responder_StatusResponder, __construct)
{
	...
	if (Z_TYPE_P(mapper) == IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 297);
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 333, &_0$$3);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 333, mapper);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Responder_StatusResponder, __invoke)
{
	...
	if (Z_TYPE_P(&status) != IS_NULL) {
		zephir_read_property_cached(&_0$$3, this_ptr, _zephir_prop_0, 333, PH_NOISY_CC | PH_READONLY);
