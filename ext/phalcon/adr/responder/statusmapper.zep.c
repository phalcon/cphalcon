...
ZEPHIR_INIT_CLASS(Phalcon_ADR_Responder_StatusMapper)
{
	 ...
	 */
	{
		zval _zc0;
		ZVAL_UNDEF(&_zc0);
		zephir_declare_typed_property(phalcon_adr_responder_statusmapper_ce, SL("map"), &_zc0, ZEND_ACC_PROTECTED, MAY_BE_ARRAY, NULL, 0);
	}
...
PHP_METHOD(Phalcon_ADR_Responder_StatusMapper, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 360, &_1);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Responder_StatusMapper, toHttpCode)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 360, PH_NOISY_CC | PH_READONLY);
	if (zephir_array_isset_value(&_1, &status_zv)) {
		zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 360, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_OBS_NVAR(&_0);
