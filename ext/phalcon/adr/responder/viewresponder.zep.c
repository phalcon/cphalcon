...
PHP_METHOD(Phalcon_ADR_Responder_ViewResponder, __construct)
{
	...
	ZEND_PARSE_PARAMETERS_START(2, 3)
		Z_PARAM_OBJECT_OF_CLASS(renderer, zephir_get_internal_ce(SL("phalcon\\contracts\\view\\renderer")))
		Z_PARAM_OBJECT_OF_CLASS(statusMapper, zephir_get_internal_ce(SL("phalcon\\adr\\responder\\statusmapper")))
		Z_PARAM_OPTIONAL
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 368, renderer);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 369, statusMapper);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 370, &template_zv);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Responder_ViewResponder, __invoke)
{
	...
	zephir_check_call_status();
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_2, 369, PH_NOISY_CC | PH_READONLY);
