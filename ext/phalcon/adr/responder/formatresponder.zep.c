...
ZEPHIR_INIT_CLASS(Phalcon_ADR_Responder_FormatResponder)
{
	 ...
	 * @var array
	 */
	zend_declare_property_null(phalcon_adr_responder_formatresponder_ce, SL("formatters"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_ADR_Responder_FormatResponder, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 353, &formatters);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Responder_FormatResponder, __invoke)
{
	...
	zval *request, request_sub, *response, response_sub, *payload, payload_sub, accept, formatter, chosen, _0, _1, _2, _4, *_5, _7, _11, _12, _13, _6$$4, _9$$6, _10$$8;
	zval *this_ptr = getThis();
	...
	ZVAL_NULL(&chosen);
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_0, 353, PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_4, 0, "phalcon/ADR/Responder/FormatResponder.zep", 58);
	...
	if (Z_TYPE_P(&chosen) == IS_NULL) {
		zephir_read_property_cached(&_10$$8, this_ptr, _zephir_prop_0, 353, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_OBS_NVAR(&chosen);
		zephir_array_fetch_long(&chosen, &_10$$8, 0, PH_NOISY, "phalcon/ADR/Responder/FormatResponder.zep", 59);
	}
