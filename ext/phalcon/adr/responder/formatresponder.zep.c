...
PHP_METHOD(Phalcon_ADR_Responder_FormatResponder, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 344, &formatters);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Responder_FormatResponder, __invoke)
{
	...
	if (Z_TYPE_P(&chosen) == IS_NULL) {
		zephir_read_property_cached(&_10$$8, this_ptr, _zephir_prop_0, 344, PH_NOISY_CC | PH_READONLY);
