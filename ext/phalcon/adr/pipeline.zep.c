...
PHP_METHOD(Phalcon_ADR_Pipeline, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 341, &middleware);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 342, terminal);
	ZVAL_UNDEF(&_0);
	ZVAL_LONG(&_0, index);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 343, &_0);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Pipeline, __invoke)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 341, PH_NOISY_CC | PH_READONLY);
	if (ZEPHIR_GE_LONG(&_0, zephir_fast_count_int(&_1))) {
		zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_2, 342, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_RETURN_CALL_METHOD(&_2$$3, "__invoke", NULL, 0, request);
		zephir_check_call_status();
		RETURN_MM();
	}
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_1, 341, PH_NOISY_CC | PH_READONLY);
	zephir_memory_observe(&mw);
	zephir_memory_observe(&_4);
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_0, 343, PH_NOISY_CC);
	...
	zephir_read_property_cached(&_6, this_ptr, _zephir_prop_2, 342, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_7, this_ptr, _zephir_prop_0, 343, PH_NOISY_CC | PH_READONLY);
	ZVAL_LONG(&_8, (zephir_get_numberval(&_7) + 1));
	ZEPHIR_CALL_METHOD(NULL, &next, "__construct", NULL, 297, &_5, &_6, &_8);
	zephir_check_call_status();
