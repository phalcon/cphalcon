...
PHP_METHOD(Phalcon_ADR_Pipeline, __construct)
{
	...
	ZEND_PARSE_PARAMETERS_START(2, 3)
		...
		Z_PARAM_OBJECT_OF_CLASS(terminal, zephir_get_internal_ce(SL("phalcon\\contracts\\adr\\handler")))
		Z_PARAM_OPTIONAL
		Z_PARAM_LONG(index)
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 359, &middleware);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 360, terminal);
	ZVAL_UNDEF(&_0);
	ZVAL_LONG(&_0, index);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 361, &_0);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Pipeline, __invoke)
{
	...
	if (ZEPHIR_GE_LONG(&_0, zephir_fast_count_int(&_1))) {
		zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_2, 360, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_RETURN_CALL_METHOD(&_2$$3, "__invoke", NULL, 0, request);
		zephir_check_call_status();
		RETURN_MM();
	}
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_1, 359, PH_NOISY_CC | PH_READONLY);
