...
PHP_METHOD(Phalcon_ADR_ErrorResponder, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 342, chain);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 343, logger);
	if (debug) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 344, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 344, &__$false);
	}
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "defaultmap", NULL, 317);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	zephir_add_function(&_1, &exceptionMap, &_0);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 345, &_1);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_ErrorResponder, handle)
{
	...
	ZEPHIR_CALL_METHOD(&ref, this_ptr, "correlationid", NULL, 318, request);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&status, this_ptr, "resolvestatus", NULL, 319, exception);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_5);
	...
	ZEPHIR_CALL_METHOD(&_8, &_5, "withstatus", NULL, 320, &status);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_9, this_ptr, "details", NULL, 321, exception, &ref, &status);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&payload, &_8, "withresult", NULL, 0, &_9);
	zephir_check_call_status();
	zephir_read_property_cached(&_10, this_ptr, _zephir_prop_1, 342, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_ADR_ErrorResponder, correlationId)
{
	...
	if (ZEPHIR_IS_EMPTY(&id)) {
		ZVAL_LONG(&_1$$3, 8);
		ZEPHIR_CALL_FUNCTION(&_2$$3, "random_bytes", NULL, 322, &_1$$3);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&id, "bin2hex", NULL, 323, &_2$$3);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_ADR_ErrorResponder, details)
{
	...
	zephir_create_array(return_value, 2, 0);
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "reason", NULL, 324, &status);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_ADR_ErrorResponder, resolveStatus)
{
	...
	if (zephir_array_isset_value(&_0, &className)) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 345, PH_NOISY_CC | PH_READONLY);
		zephir_array_fetch(&_2$$3, &_1$$3, &className, PH_NOISY | PH_READONLY, "phalcon/ADR/ErrorResponder.zep", 157);
		RETURN_CTOR(&_2$$3);
	}
	ZEPHIR_CALL_FUNCTION(&_3, "class_parents", NULL, 325, exception);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_4, "array_values", NULL, 27, &_3);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_5, "class_implements", NULL, 326, exception);
	zephir_check_call_status();
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&ancestors), _7)
	{
		...
		zephir_read_property_cached(&_8$$4, this_ptr, _zephir_prop_0, 345, PH_NOISY_CC | PH_READONLY);
		if (zephir_array_isset_value(&_8$$4, &ancestor)) {
			zephir_read_property_cached(&_9$$5, this_ptr, _zephir_prop_0, 345, PH_NOISY_CC | PH_READONLY);
