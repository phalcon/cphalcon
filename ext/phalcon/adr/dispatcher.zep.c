...
ZEPHIR_INIT_CLASS(Phalcon_ADR_Dispatcher)
{
	 ...
	 * @var IocContainer
	 */
	zend_declare_property_null(phalcon_adr_dispatcher_ce, SL("container"), ZEND_ACC_PROTECTED);
	/**
	 * @var Manager
	 */
	zend_declare_property_null(phalcon_adr_dispatcher_ce, SL("events"), ZEND_ACC_PROTECTED);
	/**
	 * @var array
	 */
	zend_declare_property_null(phalcon_adr_dispatcher_ce, SL("globalMiddleware"), ZEND_ACC_PROTECTED);
	/**
	 * @var array|null
	 */
	zend_declare_property_null(phalcon_adr_dispatcher_ce, SL("resolvedGlobal"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_ADR_Dispatcher, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 328, container);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 329, events);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 330, &globalMiddleware);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_ADR_Dispatcher, dispatch)
{
	...
	zval actionClass_zv, *request, request_sub, *routeMiddleware_param = NULL, action, middleware, terminal, pipeline, response, _0, _3, _4, _5, _6, _7, _8, _1$$3;
	...
	if (!(zephir_instance_of_ev(&action, phalcon_contracts_adr_action_ce))) {
		...
		ZEPHIR_CONCAT_SVS(&_2$$3, "Class '", &actionClass_zv, "' is not an ADR Action.");
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 8, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/ADR/Dispatcher.zep", 69);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "resolveglobal", NULL, 294);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "resolveall", NULL, 295, &routeMiddleware);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&middleware);
	zephir_fast_array_merge(&middleware, &_3, &_4);
	ZEPHIR_INIT_VAR(&terminal);
	object_init_ex(&terminal, phalcon_adr_eventfulhandler_ce);
	zephir_read_property_cached(&_5, this_ptr, _zephir_prop_1, 329, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(NULL, &terminal, "__construct", NULL, 296, &action, &_5);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&pipeline);
	object_init_ex(&pipeline, phalcon_adr_pipeline_ce);
	ZEPHIR_CALL_METHOD(NULL, &pipeline, "__construct", NULL, 297, &middleware, &terminal);
	zephir_check_call_status();
	zephir_read_property_cached(&_6, this_ptr, _zephir_prop_1, 329, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_7);
	ZVAL_STRING(&_7, "pipeline:beforeDispatch");
	ZEPHIR_CALL_METHOD(NULL, &_6, "fire", NULL, 0, &_7, this_ptr, request);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&response, &pipeline, "__invoke", NULL, 298, request);
	zephir_check_call_status();
	zephir_read_property_cached(&_8, this_ptr, _zephir_prop_1, 329, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_NVAR(&_7);
	ZVAL_STRING(&_7, "pipeline:afterDispatch");
	ZEPHIR_CALL_METHOD(NULL, &_8, "fire", NULL, 0, &_7, this_ptr, &response);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_ADR_Dispatcher, resolveAll)
{
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *classes_param = NULL, result, className, *_0, _3, _1$$3, _2$$3, _5$$4, _6$$4;
	zval classes;
	zval *this_ptr = getThis();
	...
	array_init(&result);
	zephir_is_iterable(&classes, 0, "phalcon/ADR/Dispatcher.zep", 94);
	if (Z_TYPE_P(&classes) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&classes), _0)
		{
			ZEPHIR_INIT_NVAR(&className);
			ZVAL_COPY(&className, _0);
			zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 328, PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_METHOD(&_2$$3, &_1$$3, "getservice", NULL, 0, &className);
			zephir_check_call_status();
			zephir_array_append(&result, &_2$$3, PH_SEPARATE, "phalcon/ADR/Dispatcher.zep", 91);
		...
		while (1) {
			...
			zephir_check_call_status();
				zephir_read_property_cached(&_5$$4, this_ptr, _zephir_prop_0, 328, PH_NOISY_CC | PH_READONLY);
				ZEPHIR_CALL_METHOD(&_6$$4, &_5$$4, "getservice", NULL, 0, &className);
				zephir_check_call_status();
				zephir_array_append(&result, &_6$$4, PH_SEPARATE, "phalcon/ADR/Dispatcher.zep", 91);
		}
	}
	ZEPHIR_INIT_NVAR(&className);
	RETURN_CCTOR(&result);
...
PHP_METHOD(Phalcon_ADR_Dispatcher, resolveGlobal)
{
	...
	if (Z_TYPE_P(&_0) == IS_NULL) {
		zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_1, 330, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_METHOD(&_1$$3, this_ptr, "resolveall", NULL, 295, &_2$$3);
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 331, &_1$$3);
	}
