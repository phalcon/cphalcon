...
PHP_METHOD(Phalcon_ADR_EventfulHandler, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 332, action);
...
PHP_METHOD(Phalcon_ADR_EventfulHandler, __invoke)
{
	...
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_1, 332, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&response, &_3, "__invoke", NULL, 0, request);
	zephir_check_call_status();
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_0, 333, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_5, this_ptr, _zephir_prop_1, 332, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_NVAR(&_2);
