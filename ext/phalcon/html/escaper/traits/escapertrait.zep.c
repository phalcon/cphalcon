...
PHP_METHOD(Phalcon_Html_Escaper_Traits_EscaperTrait, setDoubleEncode)
{
	...
	if (doubleEncode) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 810, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 810, &__$false);
	}
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Html_Escaper_Traits_EscaperTrait, setEncoding)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 811, &encoding_zv);
	RETURN_THISW();
}
