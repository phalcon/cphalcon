...
PHP_METHOD(Phalcon_Html_Escaper_AbstractEscaper, setDoubleEncode)
{
	...
	if (doubleEncode) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 57, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 57, &__$false);
	}
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Html_Escaper_AbstractEscaper, setEncoding)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 58, &encoding_zv);
	RETURN_THISW();
}
