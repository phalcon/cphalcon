...
PHP_METHOD(Phalcon_Html_Link_AbstractLink, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 219, &_0);
	ZEPHIR_INIT_VAR(&_1);
	object_init_ex(&_1, phalcon_support_collection_ce);
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 41);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 220, &_1);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 221, &href_zv);
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "hrefistemplated", NULL, 0, &href_zv);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 222, &_2);
	if (1 != ZEPHIR_IS_EMPTY(&rel_zv)) {
		zephir_read_property_cached(&_3$$3, this_ptr, _zephir_prop_1, 220, PH_NOISY_CC | PH_READONLY);
		ZVAL_BOOL(&_4$$3, 1);
