...
PHP_METHOD(Phalcon_Html_Helper_Ol, __construct)
{
	...
	if (forceRaw) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 267, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 267, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Ol, add)
{
	...
	if (!(_4)) {
		ZEPHIR_OBS_NVAR(&_3);
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_1, 267, PH_NOISY_CC);
