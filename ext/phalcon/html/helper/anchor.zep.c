...
PHP_METHOD(Phalcon_Html_Helper_Anchor, __construct)
{
	...
	if (forceRaw) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 760, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 760, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Anchor, __invoke)
{
	...
	if (!(_2)) {
		...
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 760, PH_NOISY_CC);
