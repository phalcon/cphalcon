...
PHP_METHOD(Phalcon_Html_Helper_Preload, __construct)
{
	...
	if (UNEXPECTED(!_zephir_prop_0)) {
		_zephir_prop_0 = zend_string_init("response", 8, 1);
	}
	...
	ZEND_PARSE_PARAMETERS_START(1, 2)
		...
		Z_PARAM_OPTIONAL
		Z_PARAM_OBJECT_OF_CLASS_OR_NULL(response, zephir_get_internal_ce(SL("phalcon\\http\\responseinterface")))
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 854, response);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Preload, __invoke)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 854, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_1) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &link, "__construct", NULL, 0, &_3$$3, &href_zv, &_2$$3);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&_4$$3, &_3$$3, "serialize", NULL, 0, &_5$$3);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&header);
		ZEPHIR_CONCAT_SV(&header, "Link: ", &_4$$3);
		zephir_read_property_cached(&_6$$3, this_ptr, _zephir_prop_0, 854, PH_NOISY_CC | PH_READONLY);
