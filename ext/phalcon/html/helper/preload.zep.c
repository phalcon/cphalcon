...
PHP_METHOD(Phalcon_Html_Helper_Preload, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 831, response);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Preload, __invoke)
{
	...
	if (Z_TYPE_P(&_1) != IS_NULL) {
		...
		zephir_read_property_cached(&_6$$3, this_ptr, _zephir_prop_0, 831, PH_NOISY_CC | PH_READONLY);
