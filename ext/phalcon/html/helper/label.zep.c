...
PHP_METHOD(Phalcon_Html_Helper_Label, __construct)
{
	...
	if (forceRaw) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 792, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 792, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Label, __invoke)
{
	...
	if (!(_0)) {
		...
		zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 792, PH_NOISY_CC);
