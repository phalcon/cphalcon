...
PHP_METHOD(Phalcon_Html_Helper_Element, __construct)
{
	...
	if (forceRaw) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 774, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 774, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Element, __invoke)
{
	...
	if (!(_0)) {
		...
		zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 774, PH_NOISY_CC);
