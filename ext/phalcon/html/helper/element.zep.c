...
PHP_METHOD(Phalcon_Html_Helper_Element, __construct)
{
	...
	if (forceRaw) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 773, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 773, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Html_Helper_Element, __invoke)
{
	...
	zend_bool raw, _0;
	zval attributes;
	zval tag_zv, text_zv, *attributes_param = NULL, *raw_param = NULL, _1, _2;
	...
	if (!(_0)) {
		...
		zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 773, PH_NOISY_CC);
