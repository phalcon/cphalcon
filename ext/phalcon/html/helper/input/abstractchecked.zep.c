...
PHP_METHOD(Phalcon_Html_Helper_Input_AbstractChecked, label)
{
	...
	if (!(zephir_array_isset_string_fetch(&text, &attributes, SL("text"), 0))) {
		...
		zephir_read_property_cached(&_0$$4, this_ptr, _zephir_prop_0, 212, PH_NOISY_CC | PH_READONLY);
	...
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_1, 213, PH_NOISY_CC | PH_READONLY);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 211, &_6);
	RETURN_THIS();
}
	...
	if (Z_TYPE_P(&checked) != IS_NULL) {
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(&checked)) {
			...
			if (_1$$4) {
				...
				zephir_read_property_cached(&_3$$7, this_ptr, _zephir_prop_1, 214, PH_NOISY_CC | PH_READONLY);
