...
PHP_METHOD(Phalcon_Html_Helper_Input_CheckboxGroup, isChecked)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 819, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_0) == IS_NULL) {
		RETURN_MM_BOOL(0);
	}
	zephir_memory_observe(&_1);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 819, PH_NOISY_CC);
	if (Z_TYPE_P(&_1) == IS_ARRAY) {
		zephir_memory_observe(&selected);
		zephir_read_property_cached(&selected, this_ptr, _zephir_prop_0, 819, PH_NOISY_CC);
	} else {
		ZEPHIR_INIT_VAR(&_2$$5);
		zephir_create_array(&_2$$5, 1, 0);
		zephir_memory_observe(&_3$$5);
		zephir_read_property_cached(&_3$$5, this_ptr, _zephir_prop_0, 819, PH_NOISY_CC);
