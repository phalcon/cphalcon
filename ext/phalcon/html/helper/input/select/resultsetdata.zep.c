...
PHP_METHOD(Phalcon_Html_Helper_Input_Select_ResultsetData, resolve)
{
	...
	if (Z_TYPE_P(&_2) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_2), _3)
		{
			...
			zephir_read_property_cached(&_8$$3, this_ptr, _zephir_prop_2, 847, PH_NOISY_CC);
			if (!(ZEPHIR_IS_EMPTY(&_8$$3))) {
				ZEPHIR_INIT_NVAR(&optionAttrs);
				array_init(&optionAttrs);
				zephir_read_property_cached(&_9$$5, this_ptr, _zephir_prop_2, 847, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_read_property_cached(&_24$$15, this_ptr, _zephir_prop_2, 847, PH_NOISY_CC);
				if (!(ZEPHIR_IS_EMPTY(&_24$$15))) {
					ZEPHIR_INIT_NVAR(&optionAttrs);
					array_init(&optionAttrs);
					zephir_read_property_cached(&_25$$17, this_ptr, _zephir_prop_2, 847, PH_NOISY_CC | PH_READONLY);
