...
PHP_METHOD(Phalcon_Html_Helper_Input_Checkbox, __toString) {
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "processchecked", NULL, 111);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&unchecked, this_ptr, "processunchecked", NULL, 112);
	zephir_check_call_status();
	ZEPHIR_CALL_PARENT(&element, phalcon_html_helper_input_checkbox_ce, getThis(), "__tostring", &_0, 0);
	zephir_check_call_status();
	zephir_read_property(&_1, this_ptr, SL("label"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Html_Helper_Input_Checkbox, processUnchecked) {
	...
	if (!(ZEPHIR_IS_EMPTY(&unchecked))) {
		...
		zephir_read_property(&_5$$3, this_ptr, SL("attributes"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_OBS_VAR(&_6$$3);
