...
PHP_METHOD(Phalcon_Html_Helper_Input_AbstractInput, __invoke)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 64, PH_NOISY_CC);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 65, &_0);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 65, &_4);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Html_Helper_Input_AbstractInput, __toString)
{
	...
	ZEPHIR_INIT_NVAR(&_5);
	array_init(&_5);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 65, &_5);
	RETURN_CCTOR(&output);
}
