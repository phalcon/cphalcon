...
PHP_METHOD(Phalcon_Cache_CacheFactory, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 487, factory);
}
...
PHP_METHOD(Phalcon_Cache_CacheFactory, load)
{
	...
	zephir_array_fetch_string(&name, config, SL("adapter"), PH_NOISY, "phalcon/Cache/CacheFactory.zep", 71);
...
PHP_METHOD(Phalcon_Cache_CacheFactory, newInstance)
{
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 454, &adapter);
	zephir_check_call_status();
	RETURN_MM();
}
