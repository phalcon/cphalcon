...
PHP_METHOD(Phalcon_Cache_Backend_File, get) {
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS) == 1) {
		...
		if (!(zephir_is_true(lifetime))) {
			...
			ZEPHIR_CALL_METHOD(&_5$$6, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_6$$6);
			zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(NULL, "clearstatcache", NULL, 103, &__$true, &cacheFile);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_10$$5);
		ZVAL_STRING(&_10$$5, "created");
		ZEPHIR_CALL_METHOD(&_9$$5, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_10$$5);
		zephir_check_call_status();
		...
		if (!((zephir_get_numberval(&_10$$5) - ttl) > createdTime)) {
			...
			ZEPHIR_CALL_METHOD(&_13$$14, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_14$$14);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, queryKeys) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_4, "__construct", NULL, 105, &cacheDir);
	zephir_check_call_status();
	...
	if (zephir_is_true(&lastKey)) {
		...
		if (cacheFileExists) {
			...
			if (!(lifetime)) {
				...
				ZEPHIR_CALL_METHOD(&_4$$7, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_5$$7);
				zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_9$$6, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_10$$6);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, increment) {
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS)) {
		...
		ZEPHIR_CALL_METHOD(&_5$$3, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_6$$3);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&_9$$3, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_6$$3);
		zephir_check_call_status();
		...
		if (!((zephir_get_numberval(&_6$$3) - ttl) > modifiedTime)) {
			...
			ZEPHIR_CALL_METHOD(&_12$$10, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_13$$10);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, decrement) {
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS)) {
		...
		if (!(lifetime)) {
			...
			ZEPHIR_CALL_METHOD(&_5$$4, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_6$$4);
			zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&_10$$3, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_11$$3);
		zephir_check_call_status();
		...
		if (!((zephir_get_numberval(&_11$$3) - ttl) > modifiedTime)) {
			...
			ZEPHIR_CALL_METHOD(&_14$$12, this_ptr, "isvalidarray", NULL, 102, &cachedContent, &_15$$12);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, flush) {
	...
	for (;_1->funcs->valid(_1 TSRMLS_CC) == SUCCESS && !EG(exception); _1->funcs->move_forward(_1 TSRMLS_CC)) {
		...
		if (EXPECTED(ZEPHIR_IS_TRUE(&_3$$4))) {
			...
			if (_4$$5) {
				ZEPHIR_CALL_FUNCTION(&_5$$6, "unlink", &_6, 104, &cacheFile);
				zephir_check_call_status();
				if (!(zephir_is_true(&_5$$6))) {
					RETURN_MM_BOOL(0);
				}
