...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, get) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *keyName, keyName_sub, *lifetime = NULL, lifetime_sub, __$null, frontend, prefixedKey, cachedContent, _0;
	ZEPHIR_INIT_THIS();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, save) {
	...
	zval *keyName = NULL, keyName_sub, *content = NULL, content_sub, *lifetime = NULL, lifetime_sub, *stopBuffer_param = NULL, __$true, __$false, __$null, lastKey, frontend, cachedContent, preparedContent, tmp, tt1, success, isBuffering, options, keys, specialKey, _0$$4, _2$$18;
	ZEPHIR_INIT_THIS();
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Cache must be started first", "phalcon/cache/backend/xcache.zep", 122);
		return;
	}
	ZEPHIR_OBS_VAR(&frontend);
	zephir_read_property(&frontend, this_ptr, SL("_frontend"), PH_NOISY_CC);
	...
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Failed storing the data in xcache", "phalcon/cache/backend/xcache.zep", 157);
		return;
	}
	...
	if (zephir_is_true(&success)) {
		...
		zephir_read_property(&_2$$18, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
		if (!(zephir_array_isset_string_fetch(&specialKey, &_2$$18, SL("statsKey"), 0))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Unexpected inconsistency in options", "phalcon/cache/backend/xcache.zep", 176);
			return;
		}
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, delete) {
	...
	if (!(zephir_array_isset_string_fetch(&specialKey, &_1, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Unexpected inconsistency in options", "phalcon/cache/backend/xcache.zep", 210);
		return;
	}
	if (!ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_CALL_FUNCTION(&keys, "xcache_get", NULL, 90, &specialKey);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, queryKeys) {
	...
	zend_ulong _3$$7;
	int ZEPHIR_LAST_CALL_STATUS;
	zval *prefix = NULL, prefix_sub, __$null, options, prefixed, specialKey, keys, retval, key, realKey, _0, _1$$7, *_2$$7, _5$$9;
	ZEPHIR_INIT_THIS();
	...
	if (!(zephir_array_isset_string_fetch(&specialKey, &_0, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Unexpected inconsistency in options", "phalcon/cache/backend/xcache.zep", 244);
		return;
	}
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCM')!", "phalcon/cache/backend/xcache.zep", 248);
		return;
	}
	...
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		zephir_is_iterable(&keys, 0, "phalcon/cache/backend/xcache.zep", 265);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _3$$7, _4$$7, _2$$7)
		{
			ZEPHIR_INIT_NVAR(&key);
			if (_4$$7 != NULL) { 
				ZVAL_STR_COPY(&key, _4$$7);
			} else {
				ZVAL_LONG(&key, _3$$7);
			}
			ZEPHIR_INIT_NVAR(&_1$$7);
			ZVAL_COPY(&_1$$7, _2$$7);
			if (zephir_start_with(&key, &prefixed, NULL)) {
				ZVAL_LONG(&_5$$9, 5);
				ZEPHIR_INIT_NVAR(&realKey);
				zephir_substr(&realKey, &key, 5 , 0, ZEPHIR_SUBSTR_NO_LENGTH);
				zephir_array_append(&retval, &realKey, PH_SEPARATE, "phalcon/cache/backend/xcache.zep", 262);
			}
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		ZEPHIR_RETURN_CALL_FUNCTION("xcache_isset", NULL, 118, &lastKey);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, increment) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Cache must be started first", "phalcon/cache/backend/xcache.zep", 311);
		return;
	}
	if ((zephir_function_exists_ex(SL("xcache_inc") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_1$$6, value);
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_inc", NULL, 119, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, decrement) {
	...
	if ((zephir_function_exists_ex(SL("xcache_dec") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_1$$6, value);
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_dec", NULL, 120, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&origVal, "xcache_get", NULL, 90, &lastKey);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, flush) {
	...
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _3$$5, _4$$5, _2$$5)
		{
			...
			ZEPHIR_CALL_FUNCTION(NULL, "xcache_unset", &_5, 121, &key);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&_1$$5);
		ZEPHIR_INIT_NVAR(&key);
