...
PHP_METHOD(Phalcon_Cache_Backend_Redis, get) {
	...
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "getstorekey", NULL, 96, &keyName);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, save) {
	...
	if (Z_TYPE_P(keyName) == IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(&prefixedKey, this_ptr, "getprefixedkey", NULL, 97, keyName);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "getstorekey", NULL, 96, keyName);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, exists) {
	...
	if (zephir_is_true(keyName)) {
		ZEPHIR_CALL_METHOD(&_0$$3, this_ptr, "getstorekey", NULL, 96, keyName);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, increment) {
	...
	if (zephir_is_true(keyName)) {
		ZEPHIR_CALL_METHOD(&_2$$4, this_ptr, "getstorekey", NULL, 96, keyName);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, getStoreKey) {
	...
	zephir_read_property(&_0, this_ptr, SL("statsKey"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "getprefixedkey", NULL, 97, &keyName);
	zephir_check_call_status();
