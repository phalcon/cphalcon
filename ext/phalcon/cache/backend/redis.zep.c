...
PHP_METHOD(Phalcon_Cache_Backend_Redis, connect) {
	...
	if (!(zephir_is_true(&success))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$6, "__construct", NULL, 3, &_4$$6);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, get) {
	...
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "getstorekey", NULL, 102, &keyName);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, save) {
	...
	if (Z_TYPE_P(keyName) == IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(&prefixedKey, this_ptr, "getprefixedkey", NULL, 103, keyName);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "getstorekey", NULL, 102, keyName);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, queryKeys) {
	...
	zend_ulong _4;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *prefix_param = NULL, redis, keys, key, idx, _0, _2, *_3, _1$$3;
	zval prefix;
	zval *this_ptr = getThis();
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _4, _5, _3)
	{
		...
		if (_5 != NULL) { 
			ZVAL_STR_COPY(&idx, _5);
		} else {
			ZVAL_LONG(&idx, _4);
		}
		ZEPHIR_INIT_NVAR(&key);
		ZVAL_COPY(&key, _3);
		_6$$6 = !(ZEPHIR_IS_EMPTY(&prefix));
		if (_6$$6) {
			_6$$6 = !(zephir_start_with(&key, &prefix, NULL));
		}
		if (_6$$6) {
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, exists) {
	...
	if (zephir_is_true(keyName)) {
		ZEPHIR_CALL_METHOD(&_0$$3, this_ptr, "getstorekey", NULL, 102, keyName);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, increment) {
	...
	if (zephir_is_true(keyName)) {
		ZEPHIR_CALL_METHOD(&_2$$4, this_ptr, "getstorekey", NULL, 102, keyName);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, flush) {
	...
	zval redis, keys, key, lastKey, _0, _1, _2$$4, *_3$$5, _4$$6, _5$$6;
	...
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&keys), _3$$5)
		{
			ZEPHIR_INIT_NVAR(&key);
			ZVAL_COPY(&key, _3$$5);
			zephir_read_property(&_4$$6, this_ptr, SL("statsKey"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_INIT_NVAR(&lastKey);
			ZEPHIR_CONCAT_VV(&lastKey, &_4$$6, &key);
			zephir_read_property(&_5$$6, this_ptr, SL("statsKey"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_METHOD(NULL, &redis, "srem", &_6, 0, &_5$$6, &key);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, &redis, "delete", &_7, 0, &lastKey);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&key);
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, getStoreKey) {
	...
	zephir_read_property(&_0, this_ptr, SL("statsKey"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "getprefixedkey", NULL, 103, &keyName);
	zephir_check_call_status();
