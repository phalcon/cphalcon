...
PHP_METHOD(Phalcon_Cache_Backend_Apc, save) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "apc_store", NULL, 80, lastKey, preparedContent, ttl);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, increment) {
	...
	if ((zephir_function_exists_ex(SS("apc_inc") TSRMLS_CC) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&result, "apc_inc", NULL, 100, prefixedKey, _1$$3);
		zephir_check_call_status();
		RETURN_CCTOR(result);
	} else {
		ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 79, prefixedKey);
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, decrement) {
	...
	if ((zephir_function_exists_ex(SS("apc_dec") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_INIT_VAR(_1$$3);
		ZVAL_LONG(_1$$3, value);
		ZEPHIR_RETURN_CALL_FUNCTION("apc_dec", NULL, 101, lastKey, _1$$3);
		zephir_check_call_status();
		RETURN_MM();
	} else {
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, delete) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("apc_delete", NULL, 102, _1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, exists) {
	...
	if (zephir_is_true(lastKey)) {
		ZEPHIR_CALL_FUNCTION(&_1$$5, "apc_exists", NULL, 103, lastKey);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, flush) {
	...
	for (;_0->funcs->valid(_0 TSRMLS_CC) == SUCCESS && !EG(exception); _0->funcs->move_forward(_0 TSRMLS_CC)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "apc_delete", &_5, 102, _4$$3);
		zephir_check_call_status();
	}
