...
PHP_METHOD(Phalcon_Cache_Backend, __construct) {
	...
	if (!(zephir_is_instance_of(frontend, SL("Phalcon\\Cache\\FrontendInterface") TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_STRW(spl_ce_InvalidArgumentException, "Parameter 'frontend' must be an instance of 'Phalcon\\Cache\\FrontendInterface'");
		return;
	}
...
PHP_METHOD(Phalcon_Cache_Backend, start) {
	...
	zend_bool fresh;
	zval *keyName, *lifetime = NULL, *existingCache, *_0;
	...
	if ((Z_TYPE_P(existingCache) == IS_NULL)) {
		...
		zephir_call_method_noret(_0, "start");
	} else {
		fresh = 0;
	}
...
PHP_METHOD(Phalcon_Cache_Backend, stop) {
	...
	if ((stopBuffer == 1)) {
		_0 = zephir_fetch_nproperty_this(this_ptr, SL("_frontend"), PH_NOISY_CC);
		zephir_call_method_noret(_0, "stop");
	}
