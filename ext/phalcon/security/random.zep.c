...
PHP_METHOD(Phalcon_Security_Random, bytes) {
	...
	if ((zephir_function_exists_ex(SL("random_bytes") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_0$$4, len);
		ZEPHIR_RETURN_CALL_FUNCTION("random_bytes", NULL, 435, &_0$$4);
	...
	if ((zephir_file_exists(&_3 TSRMLS_CC) == SUCCESS)) {
		...
		ZVAL_STRING(&_5$$7, "rb");
		ZEPHIR_CALL_FUNCTION(&handle, "fopen", NULL, 220, &_4$$7, &_5$$7);
		zephir_check_call_status();
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&handle)) {
			ZVAL_LONG(&_6$$8, 0);
			ZEPHIR_CALL_FUNCTION(NULL, "stream_set_read_buffer", NULL, 436, &handle, &_6$$8);
			zephir_check_call_status();
			ZVAL_LONG(&_6$$8, len);
			ZEPHIR_CALL_FUNCTION(&ret, "fread", NULL, 437, &handle, &_6$$8);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Security_Random, hex) {
	...
	ZEPHIR_INIT_VAR(&_2);
	ZVAL_STRING(&_2, "H*");
	ZEPHIR_CALL_FUNCTION(&_3, "unpack", NULL, 438, &_2, &_0);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(&_3);
...
PHP_METHOD(Phalcon_Security_Random, number) {
	...
	if ((zephir_function_exists_ex(SL("random_int") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_0$$4, 0);
		ZVAL_LONG(&_1$$4, len);
		ZEPHIR_RETURN_CALL_FUNCTION("random_int", NULL, 439, &_0$$4, &_1$$4);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	ZVAL_LONG(&_4, len);
	ZEPHIR_CALL_FUNCTION(&hex, "dechex", NULL, 440, &_4);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_VAR(&_6);
	ZVAL_STRING(&_6, "H*");
	ZEPHIR_CALL_FUNCTION(&_7, "pack", NULL, 441, &_6, &hex);
	zephir_check_call_status();
	...
	do {
		...
		ZVAL_LONG(&_19$$7, 0);
		ZVAL_LONG(&_20$$7, 1);
		ZEPHIR_CALL_FUNCTION(&_21$$7, "substr_replace", &_22, 442, &rnd, &_16$$7, &_19$$7, &_20$$7);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(&rnd, &_21$$7);
	} while (ZEPHIR_LT(&bin, &rnd));
	ZEPHIR_INIT_NVAR(&_6);
	ZVAL_STRING(&_6, "H*");
	ZEPHIR_CALL_FUNCTION(&ret, "unpack", NULL, 438, &_6, &rnd);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(&ret);
	ZEPHIR_CALL_FUNCTION(&_23, "array_shift", NULL, 11, &ret);
	ZEPHIR_UNREF(&ret);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("hexdec", NULL, 443, &_23);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Security_Random, base) {
	...
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "C*");
	ZEPHIR_CALL_FUNCTION(&bytes, "unpack", NULL, 438, &_1, &_0);
	zephir_check_call_status();
