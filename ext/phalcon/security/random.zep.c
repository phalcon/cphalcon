...
PHP_METHOD(Phalcon_Security_Random, bytes) {
	...
	ZVAL_LONG(&_0, len);
	ZEPHIR_RETURN_CALL_FUNCTION("random_bytes", NULL, 486, &_0);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Security_Random, number) {
	...
	ZVAL_LONG(&_0, 0);
	ZVAL_LONG(&_1, len);
	ZEPHIR_RETURN_CALL_FUNCTION("random_int", NULL, 488, &_0, &_1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Security_Random, uuid) {
	...
	ZEPHIR_MAKE_REF(&ary);
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 481, &ary, &_7);
	ZEPHIR_UNREF(&ary);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_7);
