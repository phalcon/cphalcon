...
PHP_METHOD(Phalcon_Validation, __construct) {
	...
	if (zephir_fast_count_int(validators TSRMLS_CC)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "array_filter", &_2, 426, validators, _0$$3);
		...
		ZEPHIR_CALL_FUNCTION(&_4$$3, "array_filter", &_2, 426, validators, _3$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, appendMessage) {
	...
	ZEPHIR_CALL_METHOD(NULL, messages, "appendmessage", NULL, 427, message);
	zephir_check_call_status();
	zephir_update_property_this(this_ptr, SL("_messages"), messages TSRMLS_CC);
	RETURN_THIS();
			...
			if (Z_TYPE_P(entity) == IS_OBJECT) {
				...
				if ((zephir_method_exists(entity, method TSRMLS_CC)  == SUCCESS)) {
					...
					if ((zephir_method_exists_ex(entity, SS("writeattribute") TSRMLS_CC) == SUCCESS)) {
						...
						ZEPHIR_CALL_FUNCTION(&_4$$28, "property_exists", NULL, 288, entity, field);
						zephir_check_call_status();
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		for (
		  ...
		  ; zend_hash_move_forward_ex(_1$$3, &_0$$3)
			...
			ZEPHIR_GET_HVALUE(singleField, _2$$3);
			ZEPHIR_CALL_METHOD(&result, this_ptr, "prechecking", &_3, 428, singleField, validator);
			zephir_check_call_status();
