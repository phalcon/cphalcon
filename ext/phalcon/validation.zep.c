...
PHP_METHOD(Phalcon_Validation, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "array_filter", NULL, 244, &validators, &_0);
	...
	ZEPHIR_CALL_FUNCTION(&_3, "array_filter", NULL, 244, &validators, &_2);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, rules) {
	...
	if (Z_TYPE_P(&validators) == IS_ARRAY) {
		...
		while (1) {
			...
			zephir_check_call_status();
				if (zephir_instance_of_ev(&validator, phalcon_validation_validatorinterface_ce)) {
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "add", &_2, 239, field, &validator);
					zephir_check_call_status();
				}
