...
PHP_METHOD(Phalcon_Cli_Console, handle)
{
	...
	if (Z_TYPE_P(&_0) == IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 348);
	...
	if (zephir_is_true(&moduleName)) {
		...
		if (UNEXPECTED(_16$$9)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_17$$12, "__construct", NULL, 349, &moduleName, &_18$$12);
			zephir_check_call_status();
		...
		if (Z_TYPE_P(&module) == IS_ARRAY) {
			...
			if (zephir_array_isset_string_fetch(&path, &module, SL("path"), 0)) {
				if (UNEXPECTED(!((zephir_file_exists(&path) == SUCCESS)))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_19$$16, "__construct", NULL, 350, &path);
					zephir_check_call_status();
			...
			if (UNEXPECTED(!((zephir_is_instance_of(&module, SL("Closure")))))) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_24$$19, "__construct", NULL, 349, &moduleName, &_25$$19);
				zephir_check_call_status();
