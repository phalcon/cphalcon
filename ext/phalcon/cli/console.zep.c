...
PHP_METHOD(Phalcon_Cli_Console, setArgument) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arguments), _1)
	{
		...
		if (Z_TYPE_P(&arg) == IS_STRING) {
			...
			ZVAL_STRING(&_2$$5, "--");
			ZVAL_LONG(&_3$$5, 2);
			ZEPHIR_CALL_FUNCTION(&_4$$5, "strncmp", &_5, 135, &arg, &_2$$5, &_3$$5);
			zephir_check_call_status();
			if (ZEPHIR_IS_LONG(&_4$$5, 0)) {
				...
				ZVAL_STRING(&_17$$9, "-");
				ZVAL_LONG(&_18$$9, 1);
				ZEPHIR_CALL_FUNCTION(&_19$$9, "strncmp", &_5, 135, &arg, &_17$$9, &_18$$9);
				zephir_check_call_status();
	...
	if (str) {
		...
		if (zephir_fast_count_int(&args TSRMLS_CC)) {
			...
			ZEPHIR_CALL_FUNCTION(&_25$$15, "array_shift", NULL, 10, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
		...
		if (zephir_fast_count_int(&args TSRMLS_CC)) {
			ZEPHIR_MAKE_REF(&args);
			ZEPHIR_CALL_FUNCTION(&_26$$16, "array_shift", NULL, 10, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
