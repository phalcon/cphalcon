...
PHP_METHOD(Phalcon_Cli_Console, handle) {
	...
	if (Z_TYPE_P(&container) != IS_OBJECT) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Cli/Console.zep", 41 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (zephir_is_true(&moduleName)) {
		...
		if (!(zephir_array_isset(&modules, &moduleName))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_14$$12, "phalcon/Cli/Console.zep", 81 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		ZEPHIR_OBS_VAR(&module);
		zephir_array_fetch(&module, &modules, &moduleName, PH_NOISY, "phalcon/Cli/Console.zep", 84 TSRMLS_CC);
		if (Z_TYPE_P(&module) != IS_ARRAY) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cli_console_exception_ce, "Invalid module definition path", "phalcon/Cli/Console.zep", 86);
			return;
		}
		...
		if (zephir_array_isset_string_fetch(&path, &module, SL("path"), 0)) {
			if (!(zephir_class_exists(&className, zephir_is_true(&__$false)  TSRMLS_CC))) {
				if (!((zephir_file_exists(&path TSRMLS_CC) == SUCCESS))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_16$$17, "phalcon/Cli/Console.zep", 96 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
...
PHP_METHOD(Phalcon_Cli_Console, setArgument) {
	...
	zephir_is_iterable(&arguments, 0, "phalcon/Cli/Console.zep", 175);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arguments), _1)
	{
		...
		if (Z_TYPE_P(&arg) == IS_STRING) {
			...
			ZVAL_STRING(&_2$$5, "--");
			ZVAL_LONG(&_3$$5, 2);
			ZEPHIR_CALL_FUNCTION(&_4$$5, "strncmp", &_5, 112, &arg, &_2$$5, &_3$$5);
			zephir_check_call_status();
			if (ZEPHIR_IS_LONG(&_4$$5, 0)) {
				...
				ZVAL_STRING(&_17$$9, "-");
				ZVAL_LONG(&_18$$9, 1);
				ZEPHIR_CALL_FUNCTION(&_19$$9, "strncmp", &_5, 112, &arg, &_17$$9, &_18$$9);
				zephir_check_call_status();
				if (ZEPHIR_IS_LONG(&_19$$9, 0)) {
					...
					zephir_array_append(&args, &arg, PH_SEPARATE, "phalcon/Cli/Console.zep", 167);
				}
			}
		} else {
			zephir_array_append(&args, &arg, PH_SEPARATE, "phalcon/Cli/Console.zep", 171);
		}
	...
	if (str) {
		...
		zephir_update_property_zval(this_ptr, SL("arguments"), &_22$$13);
	} else {
