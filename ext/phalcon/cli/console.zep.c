...
PHP_METHOD(Phalcon_Cli_Console, addModules) {
	...
	ZEPHIR_OBS_VAR_ONCE(&modules);
	ZVAL_COPY(&modules, modules_param);
...
PHP_METHOD(Phalcon_Cli_Console, setArgument) {
	...
	if (_0) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "array_shift", &_1, 131, &arguments);
		ZEPHIR_UNREF(&arguments);
		zephir_check_call_status();
	}
	zephir_is_iterable(&arguments, 0, "phalcon/cli/console.zep", 199);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arguments), _2)
	{
		...
		if (Z_TYPE_P(&arg) == IS_STRING) {
			...
			ZVAL_STRING(&_3$$5, "--");
			ZVAL_LONG(&_4$$5, 2);
			ZEPHIR_CALL_FUNCTION(&_5$$5, "strncmp", &_6, 132, &arg, &_3$$5, &_4$$5);
			zephir_check_call_status();
			if (ZEPHIR_IS_LONG(&_5$$5, 0)) {
				...
				ZVAL_STRING(&_18$$9, "-");
				ZVAL_LONG(&_19$$9, 1);
				ZEPHIR_CALL_FUNCTION(&_20$$9, "strncmp", &_6, 132, &arg, &_18$$9, &_19$$9);
				zephir_check_call_status();
	...
	if (str) {
		...
		ZEPHIR_CALL_CE_STATIC(&_24$$13, phalcon_cli_router_route_ce, "getdelimiter", &_25, 133);
		zephir_check_call_status();
		...
		if (zephir_fast_count_int(&args TSRMLS_CC)) {
			...
			ZEPHIR_CALL_FUNCTION(&_26$$15, "array_shift", &_1, 131, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
		...
		if (zephir_fast_count_int(&args TSRMLS_CC)) {
			ZEPHIR_MAKE_REF(&args);
			ZEPHIR_CALL_FUNCTION(&_27$$16, "array_shift", &_1, 131, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
