...
PHP_METHOD(Phalcon_Cli_Router, add) {
	...
	ZEPHIR_CALL_METHOD(NULL, &route, "__construct", NULL, 116, &pattern, paths);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cli_Router, handle) {
	...
	if (Z_TYPE_P(arguments) != IS_ARRAY) {
		...
		if (_0$$3) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cli_router_exception_ce, "Arguments must be an array or string", "phalcon/Cli/Router.zep", 244);
			return;
		}
		zephir_read_property(&_1$$3, this_ptr, SL("routes"), PH_NOISY_CC | PH_READONLY);
		zephir_is_iterable(&_1$$3, 0, "phalcon/Cli/Router.zep", 362);
		if (Z_TYPE_P(&_1$$3) == IS_ARRAY) {
			ZEND_HASH_REVERSE_FOREACH_VAL(Z_ARRVAL_P(&_1$$3), _2$$3)
			{
				...
				zephir_check_call_status();
				if (zephir_memnstr_str(&pattern, SL("^"), "phalcon/Cli/Router.zep", 254)) {
				...
				if (zephir_is_true(&routeFound)) {
					...
					if (Z_TYPE_P(&beforeMatch) != IS_NULL) {
						if (!(zephir_is_callable(&beforeMatch TSRMLS_CC))) {
							ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cli_router_exception_ce, "Before-Match callback is not callable in matched route", "phalcon/Cli/Router.zep", 274);
							return;
						}
						ZEPHIR_INIT_NVAR(&_4$$9);
						zephir_create_array(&_4$$9, 3, 0 TSRMLS_CC);
				...
				if (zephir_is_true(&routeFound)) {
					...
					if (Z_TYPE_P(&matches) == IS_ARRAY) {
						...
						zephir_check_call_status();
						zephir_is_iterable(&paths, 0, "phalcon/Cli/Router.zep", 349);
									...
									if (zephir_array_isset_fetch(&matchPosition, &matches, &position, 0 TSRMLS_CC)) {
										if (Z_TYPE_P(&converters) == IS_ARRAY) {
											...
											if (zephir_array_isset_fetch(&converter, &converters, &part, 0 TSRMLS_CC)) {
												...
												zephir_create_array(&_14$$23, 1, 0 TSRMLS_CC);
										...
										if (Z_TYPE_P(&converters) == IS_ARRAY) {
											...
											if (zephir_array_isset_fetch(&converter, &converters, &part, 0 TSRMLS_CC)) {
												...
												zephir_create_array(&_16$$26, 1, 0 TSRMLS_CC);
				...
				zephir_check_call_status();
					...
					if (zephir_is_true(&routeFound)) {
						...
						if (Z_TYPE_P(&beforeMatch) != IS_NULL) {
							...
							zephir_create_array(&_17$$31, 3, 0 TSRMLS_CC);
									...
									if (zephir_array_isset_fetch(&matchPosition, &matches, &position, 0 TSRMLS_CC)) {
										if (Z_TYPE_P(&converters) == IS_ARRAY) {
											...
											if (zephir_array_isset_fetch(&converter, &converters, &part, 0 TSRMLS_CC)) {
												...
												zephir_create_array(&_23$$38, 1, 0 TSRMLS_CC);
										...
										if (Z_TYPE_P(&converters) == IS_ARRAY) {
											...
											if (zephir_array_isset_fetch(&converter, &converters, &part, 0 TSRMLS_CC)) {
												...
												zephir_create_array(&_25$$41, 1, 0 TSRMLS_CC);
										...
										if (zephir_array_isset_fetch(&matchPosition, &matches, &position, 0 TSRMLS_CC)) {
											if (Z_TYPE_P(&converters) == IS_ARRAY) {
												...
												if (zephir_array_isset_fetch(&converter, &converters, &part, 0 TSRMLS_CC)) {
													...
													zephir_create_array(&_27$$45, 1, 0 TSRMLS_CC);
											...
											if (Z_TYPE_P(&converters) == IS_ARRAY) {
												...
												if (zephir_array_isset_fetch(&converter, &converters, &part, 0 TSRMLS_CC)) {
													...
													zephir_create_array(&_29$$48, 1, 0 TSRMLS_CC);
		...
		if (zephir_is_true(&routeFound)) {
			...
			zephir_read_property(&_30$$50, this_ptr, SL("defaultModule"), PH_NOISY_CC | PH_READONLY);
			zephir_update_property_zval(this_ptr, SL("module"), &_30$$50);
			zephir_read_property(&_31$$50, this_ptr, SL("defaultTask"), PH_NOISY_CC | PH_READONLY);
			zephir_update_property_zval(this_ptr, SL("task"), &_31$$50);
			zephir_read_property(&_32$$50, this_ptr, SL("defaultAction"), PH_NOISY_CC | PH_READONLY);
			zephir_update_property_zval(this_ptr, SL("action"), &_32$$50);
			zephir_read_property(&_33$$50, this_ptr, SL("defaultParams"), PH_NOISY_CC | PH_READONLY);
			zephir_update_property_zval(this_ptr, SL("params"), &_33$$50);
			RETURN_THIS();
		}
	} else {
	...
	if (zephir_array_isset_string_fetch(&params, &parts, SL("params"), 0)) {
		if (Z_TYPE_P(&params) != IS_ARRAY) {
			...
			if (zephir_is_true(&strParams)) {
				...
				zephir_fast_explode(&params, &_36$$60, &strParams, LONG_MAX TSRMLS_CC);
			} else {
				ZEPHIR_INIT_NVAR(&params);
				array_init(&params);
			}
	...
	if (zephir_fast_count_int(&params TSRMLS_CC)) {
		...
		zephir_fast_array_merge(&_38$$62, &params, &parts TSRMLS_CC);
