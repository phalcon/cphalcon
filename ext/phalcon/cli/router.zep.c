...
PHP_METHOD(Phalcon_Cli_Router, __construct) {
	...
	if (defaultRoutes == 1) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0, "__construct", &_3, 121, _2, _1);
		...
		ZEPHIR_CALL_METHOD(NULL, _2, "__construct", &_3, 121, _5, _4);
		...
		if (zephir_array_isset_string_fetch(&params, parts, SS("params"), 0 TSRMLS_CC)) {
			if (Z_TYPE_P(params) != IS_ARRAY) {
				...
				if (zephir_is_true(strParams)) {
					ZEPHIR_CALL_CE_STATIC(&_17, phalcon_cli_router_route_ce, "getdelimiter", &_18, 119);
					zephir_check_call_status();
