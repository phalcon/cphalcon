...
PHP_METHOD(Phalcon_Cli_Router_Route, beforeMatch)
{
	...
	if (UNEXPECTED(!(zephir_is_callable(callback)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 435, &_1$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cli_Router_Route, reConfigure)
{
		...
		if (Z_TYPE_P(&taskName) != IS_NULL) {
			if (zephir_memnstr_str(&taskName, SL("\\"), "phalcon/Cli/Router/Route.zep", 458)) {
				...
				if (UNEXPECTED(_1$$10)) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_2$$11, "__construct", NULL, 436, &pattern);
					zephir_check_call_status();
	...
	if (UNEXPECTED(Z_TYPE_P(&routePaths) != IS_ARRAY)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_4$$16, "__construct", NULL, 436, &pattern);
		zephir_check_call_status();
