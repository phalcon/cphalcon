...
ZEPHIR_INIT_CLASS(Phalcon_Auth_Micro_AuthMicroListener)
{
...

	{
		zval _zc0;
		ZVAL_UNDEF(&_zc0);
		zephir_declare_typed_property(phalcon_auth_micro_authmicrolistener_ce, SL("componentName"), &_zc0, ZEND_ACC_PROTECTED, MAY_BE_STRING, NULL, 0);
	}
...
PHP_METHOD(Phalcon_Auth_Micro_AuthMicroListener, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 469, &componentName_zv);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Auth_Micro_AuthMicroListener, beforeExecuteRoute)
{
	...
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 469, PH_NOISY_CC);
