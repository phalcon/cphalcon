...
PHP_METHOD(Phalcon_Auth_Access_AbstractAccess, isAllowed)
{
	...
	if (!(ZEPHIR_IS_EMPTY(&_0))) {
		...
		if (!(_1$$3)) {
			zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_0, 102, PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_FUNCTION(&_3$$3, "in_array", NULL, 89, &actionName_zv, &_2$$3, &__$true);
			zephir_check_call_status();
	...
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_1, 103, PH_NOISY_CC);
	if (!(ZEPHIR_IS_EMPTY(&_4))) {
		_5$$4 = zephir_is_true(&allowed);
		if (_5$$4) {
			zephir_read_property_cached(&_6$$4, this_ptr, _zephir_prop_1, 103, PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_FUNCTION(&_7$$4, "in_array", NULL, 89, &actionName_zv, &_6$$4, &__$true);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Auth_Access_AbstractAccess, setExceptActions)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 102, &exceptActions);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Auth_Access_AbstractAccess, setOnlyActions)
{
	...
	static zend_string *_zephir_prop_0 = NULL;
	if (UNEXPECTED(!_zephir_prop_0)) {
		_zephir_prop_0 = zend_string_init("onlyActions", 11, 1);
	}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 103, &onlyActions);
	ZEPHIR_MM_RESTORE();
}
