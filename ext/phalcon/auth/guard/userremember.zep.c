...
PHP_METHOD(Phalcon_Auth_Guard_UserRemember, __construct)
{
	...
	/* try_start_1: */
		...
		if (Z_TYPE_P(payload) == IS_STRING) {
			...
			ZVAL_BOOL(&_2$$4, 1);
			ZEPHIR_CALL_METHOD(&data, &_1$$4, "__invoke", NULL, 404, payload, &_2$$4);
			zephir_check_call_status_or_jump(try_end_1);
		} else {
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 459, &_5);
	ZEPHIR_INIT_VAR(&_7);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 460, &_7);
	ZEPHIR_INIT_VAR(&_10);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 461, &_10);
	ZEPHIR_MM_RESTORE();
}
