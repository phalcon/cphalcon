...
PHP_METHOD(Phalcon_Auth_Guard_Token, fromOptions)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 327, &_5, &_6);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 328, adapter, &_0, &_2);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Auth_Guard_Token, getTokenForRequest)
{
	...
	if (_9) {
		...
		ZEPHIR_CALL_FUNCTION(&bearer, "mb_substr", NULL, 329, &header, &_10$$4, &__$null, &_11$$4);
		zephir_check_call_status();
