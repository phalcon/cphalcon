...
PHP_METHOD(Phalcon_Auth_Guard_Token, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 451, request);
...
PHP_METHOD(Phalcon_Auth_Guard_Token, getTokenForRequest)
{
	...
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 451, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_7);
...
PHP_METHOD(Phalcon_Auth_Guard_Token, user)
{
	...
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_2, 452, PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&found) != IS_NULL) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 453, &found);
	}
	RETURN_MM_MEMBER(getThis(), "user");
}
