...
PHP_METHOD(Phalcon_Auth_Guard_Config_SessionGuardConfig, __construct)
{
	...
	zval suffix_zv, name_zv, rememberName_zv, *rememberTtl = NULL, rememberTtl_sub, __$null, _0, _1, _2, _3, _4, _5;
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validatenonempty", NULL, 319, &_0, &suffix_zv);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "name");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validatenonempty", NULL, 319, &_0, &name_zv);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "rememberName");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validatenonempty", NULL, 319, &_0, &rememberName_zv);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	if (Z_TYPE_P(&name_zv) != IS_NULL) {
		...
		ZEPHIR_INIT_NVAR(&_0);
		ZVAL_STRING(&_0, "auth");
		ZEPHIR_CALL_METHOD(&_1, this_ptr, "derive", NULL, 320, &_0, &suffix_zv);
		zephir_check_call_status();
	}
	...
	if (Z_TYPE_P(&rememberName_zv) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(&_2, this_ptr, "derive", NULL, 320, &_0, &suffix_zv);
		zephir_check_call_status();
	}
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("rememberTtl"), &_3);
	zephir_read_property(&_4, this_ptr, ZEND_STRL("name"), PH_NOISY_CC | PH_READONLY);
	zephir_read_property(&_5, this_ptr, ZEND_STRL("rememberName"), PH_NOISY_CC | PH_READONLY);
	if (ZEPHIR_IS_IDENTICAL(&_4, &_5)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_auth_exception_ce, "Session guard 'name' and 'rememberName' must differ", "phalcon/Auth/Guard/Config/SessionGuardConfig.zep", 69);
		return;
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Auth_Guard_Config_SessionGuardConfig, validateNonEmpty)
{
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&value_zv, "")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$4, "__construct", NULL, 304, &_1$$4, &param_zv);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$4, "phalcon/Auth/Guard/Config/SessionGuardConfig.zep", 103);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
