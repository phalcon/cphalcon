...
ZEPHIR_INIT_CLASS(Phalcon_Auth_Guard_Config_SessionGuardConfig)
{
	 ...
	 * @var int
	 */
	zend_declare_property_null(phalcon_auth_guard_config_sessionguardconfig_ce, SL("rememberTtl"), ZEND_ACC_PRIVATE);
	/**
	 ...
	 * @var int
	 */
	zephir_declare_class_constant_long(phalcon_auth_guard_config_sessionguardconfig_ce, SL("DEFAULT_REMEMBER_TTL"), 31536000);
...
PHP_METHOD(Phalcon_Auth_Guard_Config_SessionGuardConfig, __construct)
{
	...
	zval suffix_zv, name_zv, rememberName_zv, *rememberTtl = NULL, rememberTtl_sub, __$null, _0, _1, _2, _3, _4, _5, _6$$3;
	...
	if (UNEXPECTED(!_zephir_prop_1)) {
		_zephir_prop_1 = zend_string_init("rememberName", 12, 1);
	}
	...
	ZEND_PARSE_PARAMETERS_START(0, 4)
		...
		Z_PARAM_ZVAL_OR_NULL(rememberTtl)
	ZEND_PARSE_PARAMETERS_END();
	ZEPHIR_METHOD_GLOBALS_PTR = pecalloc(1, sizeof(zephir_method_globals), 0);
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	if (ZEND_NUM_ARGS() > 3) {
		rememberTtl = ZEND_CALL_ARG(execute_data, 4);
	}
	if (!suffix) {
	...
	if (!rememberTtl) {
		rememberTtl = &rememberTtl_sub;
		rememberTtl = &__$null;
	}
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "suffix");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validatenonempty", NULL, 378, &_0, &suffix_zv);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "name");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validatenonempty", NULL, 378, &_0, &name_zv);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "rememberName");
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validatenonempty", NULL, 378, &_0, &rememberName_zv);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	if (Z_TYPE_P(&name_zv) != IS_NULL) {
		...
		ZEPHIR_INIT_NVAR(&_0);
		ZVAL_STRING(&_0, "auth");
		ZEPHIR_CALL_METHOD(&_1, this_ptr, "derive", NULL, 379, &_0, &suffix_zv);
		zephir_check_call_status();
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 426, &_1);
	ZEPHIR_INIT_VAR(&_2);
	if (Z_TYPE_P(&rememberName_zv) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(&_2, this_ptr, "derive", NULL, 379, &_0, &suffix_zv);
		zephir_check_call_status();
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 427, &_2);
	ZEPHIR_INIT_VAR(&_3);
	if (Z_TYPE_P(rememberTtl) != IS_NULL) {
		ZEPHIR_INIT_NVAR(&_3);
		ZVAL_LONG(&_3, zephir_get_intval(rememberTtl));
	} else {
		ZEPHIR_INIT_NVAR(&_3);
		ZVAL_LONG(&_3, 31536000);
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 428, &_3);
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_0, 426, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_5, this_ptr, _zephir_prop_1, 427, PH_NOISY_CC | PH_READONLY);
	if (ZEPHIR_IS_IDENTICAL(&_4, &_5)) {
		ZEPHIR_INIT_VAR(&_6$$3);
		object_init_ex(&_6$$3, phalcon_auth_exceptions_sessionnamesmustdiffer_ce);
		ZEPHIR_CALL_METHOD(NULL, &_6$$3, "__construct", NULL, 380);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_6$$3, "phalcon/Auth/Guard/Config/SessionGuardConfig.zep", 68);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Auth_Guard_Config_SessionGuardConfig, getRememberTtl)
{
	...
	RETURN_MEMBER_TYPED(getThis(), "rememberTtl", IS_LONG);
}
...
PHP_METHOD(Phalcon_Auth_Guard_Config_SessionGuardConfig, validateNonEmpty)
{
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&value_zv, "")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$4, "__construct", NULL, 364, &_1$$4, &param_zv);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$4, "phalcon/Auth/Guard/Config/SessionGuardConfig.zep", 102);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
