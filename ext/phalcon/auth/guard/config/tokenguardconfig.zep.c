...
PHP_METHOD(Phalcon_Auth_Guard_Config_TokenGuardConfig, __construct)
{
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&inputKey_zv, "")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 377, &_1$$3, &_2$$3);
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&storageKey_zv, "")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$4, "__construct", NULL, 377, &_4$$4, &_5$$4);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$4, "phalcon/Auth/Guard/Config/TokenGuardConfig.zep", 45);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 431, &inputKey_zv);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 432, &storageKey_zv);
	ZEPHIR_MM_RESTORE();
}
