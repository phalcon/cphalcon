...
PHP_METHOD(Phalcon_Auth_ManagerFactory, __construct)
{
	...
	if (Z_TYPE_P(adapterLocator) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 329, container);
		zephir_check_call_status();
	}
	...
	if (Z_TYPE_P(guardLocator) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 329, container);
		zephir_check_call_status();
	}
	...
	if (Z_TYPE_P(accessLocator) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 329, container);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Auth_ManagerFactory, load)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &manager, "__construct", NULL, 330, &_3);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&guards) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&guards), _5, _6, _4)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &manager, "addguard", &_22, 331, &_19$$5, &guard, &_21$$5);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &manager, "addguard", &_22, 331, &_33$$6, &guard, &_35$$6);
				zephir_check_call_status();
		}
	}
	ZEPHIR_INIT_NVAR(&gconf);
	...
	if (!(ZEPHIR_IS_EMPTY(&accessList))) {
		ZEPHIR_CALL_METHOD(NULL, &manager, "addaccesslist", NULL, 332, &accessList);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Auth_ManagerFactory, buildAdapter)
{
	...
	if (!zephir_is_true(&_2)) {
		...
		ZEPHIR_CALL_FUNCTION(&_5$$3, "sprintf", NULL, 130, &_4$$3, &name);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Auth_ManagerFactory, buildGuard)
{
	...
	if (!zephir_is_true(&_0)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "sprintf", NULL, 130, &_2$$3, &type_zv);
		zephir_check_call_status();
