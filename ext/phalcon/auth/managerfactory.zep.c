...
PHP_METHOD(Phalcon_Auth_ManagerFactory, __construct)
{
	...
	if (Z_TYPE_P(adapterLocator) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 325, container);
		zephir_check_call_status();
	}
	...
	if (Z_TYPE_P(guardLocator) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 325, container);
		zephir_check_call_status();
	}
	...
	if (Z_TYPE_P(accessLocator) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 325, container);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Auth_ManagerFactory, load)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &manager, "__construct", NULL, 326, &_3);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&guards) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&guards), _5, _6, _4)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &manager, "addguard", &_17, 327, &_14$$5, &guard, &_16$$5);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &manager, "addguard", &_17, 327, &_25$$6, &guard, &_27$$6);
				zephir_check_call_status();
		}
	}
	ZEPHIR_INIT_NVAR(&gconf);
	...
	if (!(ZEPHIR_IS_EMPTY(&accessList))) {
		ZEPHIR_CALL_METHOD(NULL, &manager, "addaccesslist", NULL, 328, &accessList);
		zephir_check_call_status();
	}
