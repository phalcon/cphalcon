...
PHP_METHOD(Phalcon_Auth_ManagerFactory, load)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &manager, "__construct", NULL, 422, &_1);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&guards) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&guards), _3, _4, _2)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &manager, "addguard", &_20, 423, &_17$$3, &guard, &_19$$3);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &manager, "addguard", &_20, 423, &_31$$4, &guard, &_33$$4);
				zephir_check_call_status();
		}
	}
	ZEPHIR_INIT_NVAR(&gconf);
