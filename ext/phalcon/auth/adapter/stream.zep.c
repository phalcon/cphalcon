...
PHP_METHOD(Phalcon_Auth_Adapter_Stream, fromOptions)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 320, hasher, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Auth_Adapter_Stream, loadUsers)
{
	...
	if (!zephir_is_true(&_1)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 321, &path);
		zephir_check_call_status();
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&contents)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$4, "__construct", NULL, 322, &path);
		zephir_check_call_status();
	...
	/* try_start_1: */
		...
		ZVAL_BOOL(&_5$$5, 1);
		ZEPHIR_CALL_METHOD(&data, &_4$$5, "__invoke", NULL, 323, &contents, &_5$$5);
		zephir_check_call_status_or_jump(try_end_1);
	...
	if (EG(exception)) {
		...
		if (zephir_is_instance_of(&_6, SL("InvalidArgumentException"))) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_7$$6, "__construct", NULL, 324, &path, &ex);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&data) != IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_8$$7, "__construct", NULL, 325, &path);
		zephir_check_call_status();
