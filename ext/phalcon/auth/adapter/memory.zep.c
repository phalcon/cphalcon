...
PHP_METHOD(Phalcon_Auth_Adapter_Memory, fromOptions)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 350, &_1, &_4);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 351, hasher, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Auth_Adapter_Memory, retrieveById)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 418, PH_NOISY_CC | PH_READONLY);
	if (!(zephir_array_isset_value(&_1, id))) {
		RETURN_MM_NULL();
	}
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 418, PH_NOISY_CC | PH_READONLY);
