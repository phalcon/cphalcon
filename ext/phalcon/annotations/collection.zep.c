...
PHP_METHOD(Phalcon_Annotations_Collection, __construct)
{
	...
	if (Z_TYPE_P(&reflectionData) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&reflectionData), _0)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", &_2, 336, &annotationData);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_5$$4, "__construct", &_2, 336, &annotationData);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Annotations_Collection, get)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &_6, "__construct", NULL, 338, &name_zv);
	zephir_check_call_status();
