...
PHP_METHOD(Phalcon_Annotations_Reflection, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 351, &reflectionData);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Annotations_Reflection, getClassAnnotations)
{
	...
	if (Z_TYPE_P(&_0) == IS_NULL) {
		...
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_1, 351, PH_NOISY_CC | PH_READONLY);
		if (zephir_array_isset_string_fetch(&reflectionClass, &_1$$3, SL("class"), 0)) {
			...
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 352, &_2$$4);
		}
	}
