...
PHP_METHOD(Phalcon_Annotations_Reader, parse)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 169, &className_zv);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&comment, &reflection, "getdoccomment", NULL, 175);
	zephir_check_call_status();
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&comment)) {
		ZEPHIR_INIT_VAR(&classAnnotations);
		ZEPHIR_CALL_METHOD(&_0$$3, &reflection, "getfilename", NULL, 176);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_1$$3, &reflection, "getstartline", NULL, 177);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&constants, &reflection, "getconstants", NULL, 178);
	zephir_check_call_status();
	if (zephir_fast_count_int(&constants)) {
		...
		if (Z_TYPE_P(&arrayKeys) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arrayKeys), _2$$5)
			{
				...
				ZEPHIR_CALL_METHOD(&constantReflection, &reflection, "getreflectionconstant", &_4, 179, &constant);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&comment, &constantReflection, "getdoccomment", NULL, 0);
				zephir_check_call_status();
				if (!ZEPHIR_IS_FALSE_IDENTICAL(&comment)) {
					ZEPHIR_INIT_NVAR(&constantAnnotations);
					ZEPHIR_CALL_METHOD(&_5$$7, &reflection, "getfilename", NULL, 176);
					zephir_check_call_status();
					ZVAL_LONG(&_6$$7, line);
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (!ZEPHIR_IS_FALSE_IDENTICAL(&comment)) {
						ZEPHIR_INIT_NVAR(&constantAnnotations);
						ZEPHIR_CALL_METHOD(&_7$$10, &reflection, "getfilename", NULL, 176);
						zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&properties, &reflection, "getproperties", NULL, 180);
	zephir_check_call_status();
		...
		if (Z_TYPE_P(&properties) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&properties), _9$$13)
			{
				...
				if (!ZEPHIR_IS_FALSE_IDENTICAL(&comment)) {
					...
					ZEPHIR_CALL_METHOD(&_11$$15, &reflection, "getfilename", NULL, 176);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (!ZEPHIR_IS_FALSE_IDENTICAL(&comment)) {
						ZEPHIR_INIT_NVAR(&propertyAnnotations);
						ZEPHIR_CALL_METHOD(&_14$$18, &reflection, "getfilename", NULL, 176);
						zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&methods, &reflection, "getmethods", NULL, 181);
	zephir_check_call_status();
