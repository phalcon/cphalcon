...
PHP_METHOD(Phalcon_Annotations_Reader, parse) {
	...
	zval *className_param = NULL, reflection, comment, properties, methods, property, method, classAnnotations, annotationsProperties, propertyAnnotations, annotationsMethods, methodAnnotations, _0$$3, _1$$3, *_2$$5, _3$$7, _4$$7, _5$$8, *_6$$10, _7$$12, _8$$12, _9$$13;
	zval className;
	...
	ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 120, &className);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&comment, &reflection, "getdoccomment", NULL, 121);
	zephir_check_call_status();
	if (Z_TYPE_P(&comment) == IS_STRING) {
		ZEPHIR_INIT_VAR(&classAnnotations);
		ZEPHIR_CALL_METHOD(&_0$$3, &reflection, "getfilename", NULL, 122);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_1$$3, &reflection, "getstartline", NULL, 123);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&properties, &reflection, "getproperties", NULL, 124);
	zephir_check_call_status();
	if (zephir_fast_count_int(&properties TSRMLS_CC)) {
		line = 1;
		ZEPHIR_INIT_VAR(&annotationsProperties);
		array_init(&annotationsProperties);
		zephir_is_iterable(&properties, 0, "phalcon/Annotations/Reader.zep", 94);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&properties), _2$$5)
		{
			...
			if (Z_TYPE_P(&comment) == IS_STRING) {
				...
				ZEPHIR_CALL_METHOD(&_3$$7, &reflection, "getfilename", NULL, 122);
				zephir_check_call_status();
				ZVAL_LONG(&_4$$7, line);
	...
	ZEPHIR_CALL_METHOD(&methods, &reflection, "getmethods", NULL, 125);
	zephir_check_call_status();
	if (zephir_fast_count_int(&methods TSRMLS_CC)) {
		ZEPHIR_INIT_VAR(&annotationsMethods);
		array_init(&annotationsMethods);
		zephir_is_iterable(&methods, 0, "phalcon/Annotations/Reader.zep", 129);
