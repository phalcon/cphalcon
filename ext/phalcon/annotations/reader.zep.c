...
PHP_METHOD(Phalcon_Annotations_Reader, parse)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 174, &className);
	...
	ZEPHIR_CALL_METHOD(&properties, &reflection, "getproperties", NULL, 178);
	zephir_check_call_status();
		...
		if (Z_TYPE_P(&properties) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&properties), _2$$5)
			{
				...
				if (Z_TYPE_P(&comment) == IS_STRING) {
					...
					ZEPHIR_CALL_METHOD(&_4$$7, &reflection, "getfilename", NULL, 176);
					zephir_check_call_status();
					ZVAL_LONG(&_5$$7, line);
	...
	ZEPHIR_CALL_METHOD(&methods, &reflection, "getmethods", NULL, 179);
	zephir_check_call_status();
