...
PHP_METHOD(Phalcon_Annotations_Adapter_Stream, read)
{
	...
	ZVAL_LONG(&_5, 2);
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 235, &_4, &_5);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_6, "unserialize", NULL, 28, &contents);
	zephir_check_call_status();
	...
	if (UNEXPECTED(ZEPHIR_GLOBAL(warning).enable)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_7$$5, "__construct", NULL, 268);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Annotations_Adapter_Stream, write)
{
	...
	ZEPHIR_CONCAT_VVS(&_3, &_0, &_1, ".php");
	zephir_get_strval(&path, &_3);
	ZEPHIR_CALL_FUNCTION(&code, "serialize", NULL, 23, data);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_4);
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&_4))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_5$$3, "__construct", NULL, 269);
		zephir_check_call_status();
