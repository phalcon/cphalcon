...
ZEPHIR_INIT_CLASS(Phalcon_Annotations_Adapter_AbstractAdapter) {
	...
	zend_declare_property_null(phalcon_annotations_adapter_abstractadapter_ce, SL("annotations"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_annotations_adapter_abstractadapter_ce, SL("reader"), ZEND_ACC_PROTECTED TSRMLS_CC);
	...
	zend_class_implements(phalcon_annotations_adapter_abstractadapter_ce TSRMLS_CC, 1, phalcon_annotations_adapter_adapterinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Phalcon_Annotations_Adapter_AbstractAdapter, get) {
	...
	if (zephir_array_isset(&_0, &realClassName)) {
		...
		zephir_array_fetch(&_2$$5, &_1$$5, &realClassName, PH_NOISY | PH_READONLY, "phalcon/Annotations/Adapter/AbstractAdapter.zep", 50 TSRMLS_CC);
		RETURN_CTOR(&_2$$5);
	}
	...
	if (_3) {
		...
		ZEPHIR_CALL_METHOD(NULL, &classAnnotations, "__construct", NULL, 19, &parsedAnnotations);
		zephir_check_call_status();
		zephir_update_property_array(this_ptr, SL("annotations"), &realClassName, &classAnnotations TSRMLS_CC);
...
PHP_METHOD(Phalcon_Annotations_Adapter_AbstractAdapter, getMethod) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *className_param = NULL, *methodName_param = NULL, classAnnotations, methods, method, methodKey, *_0$$3, _3$$4;
	...
	if (Z_TYPE_P(&methods) == IS_ARRAY) {
		zephir_is_iterable(&methods, 0, "phalcon/Annotations/Adapter/AbstractAdapter.zep", 93);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&methods), _1$$3, _2$$3, _0$$3)
		{
			...
			ZEPHIR_CALL_FUNCTION(&_3$$4, "strcasecmp", &_4, 20, &methodKey, &methodName);
			zephir_check_call_status();
