...
PHP_METHOD(Phalcon_Annotations_Annotation, getExpression) {
	...
	do {
		...
		if (ZEPHIR_IS_LONG(type, 308)) {
			...
			for (
			  ...
			  ; zephir_hash_move_forward_ex(_2, &_1)
				...
				ZEPHIR_CALL_METHOD(&resolvedItem, this_ptr, "getexpression", &_5, 9, _4);
				zephir_check_call_status();
		...
		if (ZEPHIR_IS_LONG(type, 300)) {
			...
			ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", &_6, 10, expr);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZEPHIR_INIT_VAR(_7);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_7, "phalcon/annotations/annotation.zep", 134 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	} while(0);
