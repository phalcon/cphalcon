...
PHP_METHOD(Phalcon_Annotations_Annotation, __construct) {
	...
	if (zephir_array_isset_string_fetch(&exprArguments, &reflectionData, SL("arguments"), 0)) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&exprArguments), _1$$3)
		{
			...
			zephir_array_fetch_string(&_2$$4, &argument, SL("expr"), PH_NOISY | PH_READONLY, "phalcon/Annotations/Annotation.zep", 56 TSRMLS_CC);
	...
	do {
		...
		if (ZEPHIR_IS_LONG(&type, 308)) {
			...
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0$$7), _1$$7)
			{
				...
				ZVAL_COPY(&item, _1$$7);
				zephir_array_fetch_string(&_2$$8, &item, SL("expr"), PH_NOISY | PH_READONLY, "phalcon/Annotations/Annotation.zep", 137 TSRMLS_CC);
				ZEPHIR_CALL_METHOD(&resolvedItem, this_ptr, "getexpression", &_3, 95, &_2$$8);
				zephir_check_call_status();
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_4$$12, "phalcon/Annotations/Annotation.zep", 150 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	} while(0);
