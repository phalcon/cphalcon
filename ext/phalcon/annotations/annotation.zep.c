...
PHP_METHOD(Phalcon_Annotations_Annotation, __construct)
{
	...
	if (zephir_array_isset_string_fetch(&name, &reflectionData, SL("name"), 0)) {
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 395, &_0$$3);
	}
	...
	if (zephir_array_isset_string_fetch(&exprArguments, &reflectionData, SL("arguments"), 0)) {
		...
		ZEPHIR_INIT_NVAR(&argument);
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 396, &arguments);
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 397, &exprArguments);
	}
	ZEPHIR_MM_RESTORE();
}
