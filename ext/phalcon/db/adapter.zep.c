...
PHP_METHOD(Phalcon_Db_Adapter, describeIndexes) {
	...
	if (Z_TYPE_P(&indexes) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&indexes), _10, _11, _8)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_12$$9, "__construct", &_13, 16, &name, &indexColumns);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_14$$10, "__construct", &_13, 16, &name, &indexColumns);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter, describeReferences) {
	...
	if (Z_TYPE_P(&references) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&references), _22, _23, _20)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_24$$9, "__construct", &_27, 17, &name, &_25$$9);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_28$$10, "__construct", &_27, 17, &name, &_29$$10);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(values), _2, _3, _0)
		{
			...
			if (_5$$3) {
				...
				zephir_array_append(&placeholders, &_7$$5, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1163);
			} else {
				if (Z_TYPE_P(&value) == IS_OBJECT) {
					zephir_get_strval(&_8$$7, &value);
					ZEPHIR_CPY_WRT(&value, &_8$$7);
				...
				if (Z_TYPE_P(&value) == IS_NULL) {
					...
					zephir_array_append(&placeholders, &_9$$8, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1170);
				} else {
					...
					zephir_array_append(&placeholders, &_10$$9, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1184);
				}
			}
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$12) {
					...
					zephir_array_append(&placeholders, &_13$$14, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1163);
				} else {
					if (Z_TYPE_P(&value) == IS_OBJECT) {
					...
					if (Z_TYPE_P(&value) == IS_NULL) {
						...
						zephir_array_append(&placeholders, &_15$$17, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1170);
					} else {
						zephir_array_append(&updateValues, &value, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1172);
						if (Z_TYPE_P(dataTypes) == IS_ARRAY) {
							ZEPHIR_OBS_NVAR(&bindType);
						...
						zephir_array_append(&placeholders, &_16$$18, PH_SEPARATE, "phalcon/Db/Adapter.zep", 1184);
					}
				}
			ZEPHIR_CALL_METHOD(NULL, values, "next", NULL, 0);
			zephir_check_call_status();
