...
{
	ZEPHIR_REGISTER_CLASS_EX(Phalcon\\Db\\Geometry, LineString, phalcon, db_geometry_linestring, phalcon_db_geometry_abstractgeometry_ce, phalcon_db_geometry_linestring_method_entry, 0);

	/**
	 * @var array
	 */
	zend_declare_property_null(phalcon_db_geometry_linestring_ce, SL("points"), ZEND_ACC_PROTECTED);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon_Db_Geometry_LineString, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 616, &points);
	ZVAL_UNDEF(&_0);
	ZVAL_LONG(&_0, srid);
