...
PHP_METHOD(Phalcon_Db_Geometry_WkbParser, parse)
{
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "ctype_xdigit", NULL, 442, &raw_zv);
	...
	if (_0) {
		...
		ZVAL_STRING(&_5$$5, "V");
		ZEPHIR_CALL_FUNCTION(&arr, "unpack", NULL, 443, &_5$$5, &_4$$5);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Geometry_WkbParser, readGeometry)
{
	...
	do {
		...
		if (ZEPHIR_IS_LONG(&baseType, 2)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 444, &_5$$9, &srid);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_LONG(&baseType, 3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 445, &_9$$10, &srid);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_LONG(&baseType, 4) || ZEPHIR_IS_LONG(&baseType, 5) || ZEPHIR_IS_LONG(&baseType, 6) || ZEPHIR_IS_LONG(&baseType, 7)) {
			...
			while (1) {
				...
				ZEPHIR_CALL_METHOD(&_14$$12, this_ptr, "readgeometry", &_15, 446, &srid);
				zephir_check_call_status();
			...
			if (ZEPHIR_IS_LONG_IDENTICAL(&baseType, 5)) {
				...
				ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 448, &items, &srid);
				zephir_check_call_status();
				RETURN_MM();
			}
			if (ZEPHIR_IS_LONG_IDENTICAL(&baseType, 6)) {
				object_init_ex(return_value, phalcon_db_geometry_multipolygon_ce);
				ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 449, &items, &srid);
				zephir_check_call_status();
				RETURN_MM();
			}
		...
		ZEPHIR_CALL_METHOD(NULL, &_16$$16, "__construct", NULL, 451, &_17$$16);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Geometry_WkbParser, readPoint)
{
	...
	ZVAL_LONG(&_5, srid);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 452, &x, &y, &_5);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Db_Geometry_WkbParser, readPointList)
{
	...
	while (1) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_8$$3, "__construct", &_9, 452, &x, &y);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Geometry_WkbParser, readByte)
{
	...
	ZEPHIR_INIT_VAR(&_6);
	ZVAL_STRING(&_6, "C");
	ZEPHIR_CALL_FUNCTION(&arr, "unpack", NULL, 443, &_6, &_5);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Geometry_WkbParser, readUint32)
{
	...
	ZEPHIR_CALL_FUNCTION(&arr, "unpack", NULL, 443, &fmt, &_5);
	zephir_check_call_status();
