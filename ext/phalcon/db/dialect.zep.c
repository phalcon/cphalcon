...
PHP_METHOD(Phalcon_Db_Dialect, escape) {
	...
	zend_ulong _7;
	zval _5;
	zval *str_param = NULL, *escapeChar_param = NULL, parts, key, part, newParts, _3, _4, *_6, _0$$4, _11$$7;
	zval str, escapeChar, _1$$4;
	ZEPHIR_INIT_THIS();
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&parts), _7, _8, _6)
	{
...
PHP_METHOD(Phalcon_Db_Dialect, getColumnList) {
	...
	ZEPHIR_OBS_VAR_ONCE(&columnList);
	ZVAL_COPY(&columnList, columnList_param);
	if (!escapeChar_param) {
		ZEPHIR_INIT_VAR(&escapeChar);
		ZVAL_STRING(&escapeChar, "");
	} else {
	...
	array_init(&columns);
	zephir_is_iterable(&columnList, 0, "phalcon/db/dialect.zep", 177);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&columnList), _0)
	{
		ZEPHIR_INIT_NVAR(&column);
		ZVAL_COPY(&column, _0);
		ZEPHIR_CALL_METHOD(&_1$$3, this_ptr, "getsqlcolumn", &_2, 30, &column, &escapeChar, bindCounts);
		zephir_check_call_status();
		zephir_array_append(&columns, &_1$$3, PH_SEPARATE, "phalcon/db/dialect.zep", 174);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&column);
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpression) {
	...
	ZEPHIR_OBS_VAR_ONCE(&expression);
	ZVAL_COPY(&expression, expression_param);
	if (!escapeChar_param) {
		ZEPHIR_INIT_VAR(&escapeChar);
		ZVAL_STRING(&escapeChar, "");
	} else {
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpressionList) {
	...
	if (_1) {
		zephir_is_iterable(&values, 0, "phalcon/db/dialect.zep", 679);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _2$$4)
		{
			...
			zephir_check_call_status();
			zephir_array_append(&items, &_3$$5, PH_SEPARATE, "phalcon/db/dialect.zep", 676);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&item);
		_5$$4 = zephir_array_isset_string(&expression, SL("parentheses"));
		if (_5$$4) {
			zephir_array_fetch_string(&_6$$4, &expression, SL("parentheses"), PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 679 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpressionCase) {
	...
	zephir_is_iterable(&_3, 0, "phalcon/db/dialect.zep", 747);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3), _4)
	{
		ZEPHIR_INIT_NVAR(&whenClause);
		ZVAL_COPY(&whenClause, _4);
		zephir_array_fetch_string(&_5$$3, &whenClause, SL("type"), PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 737 TSRMLS_CC);
		if (ZEPHIR_IS_STRING(&_5$$3, "when")) {
			zephir_array_fetch_string(&_7$$4, &whenClause, SL("expr"), PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 739 TSRMLS_CC);
			ZEPHIR_CALL_METHOD(&_6$$4, this_ptr, "getsqlexpression", &_2, 36, &_7$$4, &escapeChar, bindCounts);
			zephir_check_call_status();
			zephir_array_fetch_string(&_9$$4, &whenClause, SL("then"), PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 741 TSRMLS_CC);
			...
			zephir_array_fetch_string(&_12$$5, &whenClause, SL("expr"), PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 743 TSRMLS_CC);
			ZEPHIR_CALL_METHOD(&_11$$5, this_ptr, "getsqlexpression", &_2, 36, &_12$$5, &escapeChar, bindCounts);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpressionFrom) {
	...
	if (Z_TYPE_P(expression) == IS_ARRAY) {
		...
		array_init(&tables);
		zephir_is_iterable(expression, 0, "phalcon/db/dialect.zep", 765);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(expression), _0$$3)
		{
			ZEPHIR_INIT_NVAR(&table);
			ZVAL_COPY(&table, _0$$3);
			ZEPHIR_CALL_METHOD(&_1$$4, this_ptr, "getsqltable", &_2, 52, &table, &escapeChar);
			zephir_check_call_status();
			zephir_array_append(&tables, &_1$$4, PH_SEPARATE, "phalcon/db/dialect.zep", 762);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&table);
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpressionJoins) {
	...
	zephir_is_iterable(expression, 0, "phalcon/db/dialect.zep", 813);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(expression), _0)
	{
		...
		if (_1$$3) {
			if (!(zephir_array_isset_long(&joinConditionsArray, 0))) {
				...
				zephir_is_iterable(&joinConditionsArray, 0, "phalcon/db/dialect.zep", 798);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joinConditionsArray), _3$$6)
				{
					ZEPHIR_INIT_NVAR(&condition);
					ZVAL_COPY(&condition, _3$$6);
					ZEPHIR_CALL_METHOD(&_4$$7, this_ptr, "getsqlexpression", &_2, 36, &condition, &escapeChar, bindCounts);
					zephir_check_call_status();
					zephir_array_append(&joinCondition, &_4$$7, PH_SEPARATE, "phalcon/db/dialect.zep", 795);
				} ZEND_HASH_FOREACH_END();
				ZEPHIR_INIT_NVAR(&condition);
		...
		zephir_array_fetch_string(&_7$$3, &join, SL("source"), PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 808 TSRMLS_CC);
		ZEPHIR_CALL_METHOD(&joinTable, this_ptr, "getsqltable", &_8, 52, &_7$$3, &escapeChar);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpressionGroupBy) {
	...
	if (Z_TYPE_P(expression) == IS_ARRAY) {
		...
		array_init(&fields);
		zephir_is_iterable(expression, 0, "phalcon/db/dialect.zep", 851);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(expression), _0$$3)
		{
			...
			zephir_check_call_status();
			zephir_array_append(&fields, &_1$$4, PH_SEPARATE, "phalcon/db/dialect.zep", 848);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&field);
		ZEPHIR_INIT_VAR(&_3$$3);
...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpressionOrderBy) {
	...
	if (Z_TYPE_P(expression) == IS_ARRAY) {
		ZEPHIR_INIT_VAR(&fields);
		array_init(&fields);
		zephir_is_iterable(expression, 0, "phalcon/db/dialect.zep", 897);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(expression), _0$$3)
		{
			ZEPHIR_INIT_NVAR(&field);
			ZVAL_COPY(&field, _0$$3);
			...
			zephir_array_fetch_long(&_1$$4, &field, 0, PH_NOISY | PH_READONLY, "phalcon/db/dialect.zep", 885 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Db_Dialect, prepareTable) {
	...
	if (!ZEPHIR_IS_STRING(&schema, "")) {
		ZEPHIR_CALL_METHOD(&_2$$3, this_ptr, "escapeschema", NULL, 53, &schema, &escapeChar);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_3$$3);
