...
ZEPHIR_INIT_CLASS(Phalcon_Db_Result_Pdo) {
	...
	zend_declare_property_null(phalcon_db_result_pdo_ce, SL("bindParams"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_db_result_pdo_ce, SL("bindTypes"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_db_result_pdo_ce, SL("connection"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Active fetch mode
	 */
	zend_declare_property_long(phalcon_db_result_pdo_ce, SL("fetchMode"), 5, ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var \PDOStatement
	 */
	zend_declare_property_null(phalcon_db_result_pdo_ce, SL("pdoStatement"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_db_result_pdo_ce, SL("result"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_bool(phalcon_db_result_pdo_ce, SL("rowCount"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_db_result_pdo_ce, SL("sqlStatement"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, dataSeek) {
	...
	zephir_read_property(&_0, this_ptr, SL("sqlStatement"), PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&bindParams) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&statement) == IS_OBJECT) {
			zephir_read_property(&_2$$4, this_ptr, SL("bindTypes"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, execute) {
	...
	zephir_read_property(&_0, this_ptr, SL("pdoStatement"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, numRows) {
	...
	zephir_read_property(&_0, this_ptr, SL("rowCount"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&rowCount, &_0);
	if (ZEPHIR_IS_FALSE_IDENTICAL(&rowCount)) {
		zephir_read_property(&_1$$3, this_ptr, SL("connection"), PH_NOISY_CC | PH_READONLY);
		...
		if (_2$$3) {
			zephir_read_property(&_3$$4, this_ptr, SL("pdoStatement"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CPY_WRT(&pdoStatement, &_3$$4);
			ZEPHIR_CALL_METHOD(&rowCount, &pdoStatement, "rowcount", NULL, 0);
			zephir_check_call_status();
		}
		if (ZEPHIR_IS_FALSE_IDENTICAL(&rowCount)) {
			zephir_read_property(&_4$$5, this_ptr, SL("sqlStatement"), PH_NOISY_CC | PH_READONLY);
			...
			if (!(zephir_start_with_str(&sqlStatement, SL("SELECT COUNT(*) ")))) {
				...
				if (zephir_is_true(&_6$$6)) {
					...
					zephir_read_property(&_11$$7, this_ptr, SL("bindTypes"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, setFetchMode) {
	...
	zval *fetchMode_param = NULL, *colNoOrClassNameOrObject = NULL, colNoOrClassNameOrObject_sub, *ctorargs = NULL, ctorargs_sub, __$null, pdoStatement, _0, _10, _1$$3, _2$$3, _3$$4, _4$$5, _5$$5, _6$$6, _7$$7, _8$$7, _9$$8, _11$$9;
	...
	ZEPHIR_CPY_WRT(&pdoStatement, &_0);
	if (fetchMode == 8) {
		ZVAL_LONG(&_2$$3, fetchMode);
		ZEPHIR_CALL_METHOD(&_1$$3, &pdoStatement, "setfetchmode", NULL, 0, &_2$$3, colNoOrClassNameOrObject, ctorargs);
		zephir_check_call_status();
		if (zephir_is_true(&_1$$3)) {
			ZEPHIR_INIT_ZVAL_NREF(_3$$4);
			ZVAL_LONG(&_3$$4, fetchMode);
			zephir_update_property_zval(this_ptr, SL("fetchMode"), &_3$$4);
			RETURN_MM_BOOL(1);
		}
		RETURN_MM_BOOL(0);
	}
	if (fetchMode == 9) {
		...
		if (zephir_is_true(&_4$$5)) {
			...
			ZVAL_LONG(&_6$$6, fetchMode);
			zephir_update_property_zval(this_ptr, SL("fetchMode"), &_6$$6);
			RETURN_MM_BOOL(1);
		}
		RETURN_MM_BOOL(0);
	}
	if (fetchMode == 7) {
		ZVAL_LONG(&_8$$7, fetchMode);
		ZEPHIR_CALL_METHOD(&_7$$7, &pdoStatement, "setfetchmode", NULL, 0, &_8$$7, colNoOrClassNameOrObject);
		zephir_check_call_status();
		if (zephir_is_true(&_7$$7)) {
			ZEPHIR_INIT_ZVAL_NREF(_9$$8);
			ZVAL_LONG(&_9$$8, fetchMode);
			zephir_update_property_zval(this_ptr, SL("fetchMode"), &_9$$8);
			RETURN_MM_BOOL(1);
		}
		RETURN_MM_BOOL(0);
	}
	ZVAL_LONG(&_0, fetchMode);
	ZEPHIR_CALL_METHOD(&_10, &pdoStatement, "setfetchmode", NULL, 0, &_0);
	zephir_check_call_status();
	if (zephir_is_true(&_10)) {
		ZEPHIR_INIT_ZVAL_NREF(_11$$9);
		ZVAL_LONG(&_11$$9, fetchMode);
		zephir_update_property_zval(this_ptr, SL("fetchMode"), &_11$$9);
		RETURN_MM_BOOL(1);
	}
	RETURN_MM_BOOL(0);
