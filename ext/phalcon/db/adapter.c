	...
	if ((Z_TYPE_P(fields) == IS_ARRAY)) {
		if (ZEPHIR_GLOBAL(db).escape_identifiers) {
			...
			for (
				...
				ZEPHIR_INIT_NVAR(_0);
				zephir_call_method_p1_cache(_0, this_ptr, "escapeidentifier", &_9, field);
...
PHP_METHOD(Phalcon_Db_Adapter, update) {
	...
	HashTable *_1;
	HashPosition _0;
	zval *table, *fields, *values, *whereCondition = NULL, *dataTypes = NULL, *placeholders, *updateValues, *position = NULL, *value = NULL, *field, *bindDataTypes = NULL, *escapedField = NULL, *bindType, *escapedTable = NULL, *setClause, *updateSql, *conditions, *whereBind, *whereTypes, **_2, *_4 = NULL, *_5 = NULL;
...
PHP_METHOD(Phalcon_Db_Adapter, limit) {
	...
	if (Z_TYPE_P(sqlQuery_param) != IS_STRING) {
		if (Z_TYPE_P(sqlQuery_param) != IS_NULL) {
...
PHP_METHOD(Phalcon_Db_Adapter, tableExists) {
	...
	if (Z_TYPE_P(tableName_param) != IS_STRING) {
		if (Z_TYPE_P(tableName_param) != IS_NULL) {
...
PHP_METHOD(Phalcon_Db_Adapter, viewExists) {
	...
	if (Z_TYPE_P(viewName_param) != IS_STRING) {
		if (Z_TYPE_P(viewName_param) != IS_NULL) {
...
PHP_METHOD(Phalcon_Db_Adapter, createTable) {
	...
	if (Z_TYPE_P(schemaName_param) != IS_STRING) {
		if (Z_TYPE_P(schemaName_param) != IS_NULL) {
...
PHP_METHOD(Phalcon_Db_Adapter, dropForeignKey) {
	...
	if (Z_TYPE_P(referenceName_param) != IS_STRING) {
		if (Z_TYPE_P(referenceName_param) != IS_NULL) {
...
PHP_METHOD(Phalcon_Db_Adapter, describeIndexes) {
	...
	if (Z_TYPE_P(table_param) != IS_STRING) {
		if (Z_TYPE_P(table_param) != IS_NULL) {
