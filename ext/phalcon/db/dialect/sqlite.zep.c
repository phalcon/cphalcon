...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, addForeignKey) {
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Adding a foreign key constraint to an existing table is not supported by SQLite", "phalcon/Db/Dialect/Sqlite.zep", 67);
	return;
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, addPrimaryKey) {
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Adding a primary key after table has been created is not supported by SQLite", "phalcon/Db/Dialect/Sqlite.zep", 100);
	return;
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, createTable) {
	...
	if (!(zephir_array_isset_string_fetch(&columns, &definition, SL("columns"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The index 'columns' is required in the definition array", "phalcon/Db/Dialect/Sqlite.zep", 120);
		return;
	}
	ZEPHIR_INIT_VAR(&sql);
	...
	zephir_is_iterable(&columns, 0, "phalcon/Db/Dialect/Sqlite.zep", 178);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&columns), _0)
	{
		...
		if (zephir_is_true(&_8$$7)) {
			...
			if (zephir_memnstr_str(&_9$$10, SL("CURRENT_TIMESTAMP"), "phalcon/Db/Dialect/Sqlite.zep", 158)) {
				zephir_concat_self_str(&columnLine, SL(" DEFAULT CURRENT_TIMESTAMP") TSRMLS_CC);
			} else {
				ZEPHIR_INIT_NVAR(&_10$$12);
				ZVAL_STRING(&_10$$12, "\"");
				ZEPHIR_CALL_FUNCTION(&_11$$12, "addcslashes", &_12, 141, &defaultValue, &_10$$12);
				zephir_check_call_status();
		...
		zephir_array_append(&createLines, &columnLine, PH_SEPARATE, "phalcon/Db/Dialect/Sqlite.zep", 172);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&column);
	ZEPHIR_OBS_VAR(&indexes);
	if (zephir_array_isset_string_fetch(&indexes, &definition, SL("indexes"), 0)) {
		zephir_is_iterable(&indexes, 0, "phalcon/Db/Dialect/Sqlite.zep", 194);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&indexes), _15$$14)
		{
			...
			if (_17$$15) {
				...
				_17$$15 = zephir_memnstr_str(&_18$$15, SL("UNIQUE"), "phalcon/Db/Dialect/Sqlite.zep", 190);
			}
			if (_16$$15) {
				ZEPHIR_CALL_METHOD(&_20$$16, &index, "getcolumns", NULL, 0);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_19$$16, this_ptr, "getcolumnlist", &_21, 38, &_20$$16);
				zephir_check_call_status();
				ZEPHIR_INIT_LNVAR(_22$$16);
				ZEPHIR_CONCAT_SVS(&_22$$16, "PRIMARY KEY (", &_19$$16, ")");
				zephir_array_append(&createLines, &_22$$16, PH_SEPARATE, "phalcon/Db/Dialect/Sqlite.zep", 189);
			} else if (_17$$15) {
				ZEPHIR_CALL_METHOD(&_24$$17, &index, "getcolumns", NULL, 0);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_23$$17, this_ptr, "getcolumnlist", &_21, 38, &_24$$17);
				zephir_check_call_status();
				ZEPHIR_INIT_LNVAR(_25$$17);
				ZEPHIR_CONCAT_SVS(&_25$$17, "UNIQUE (", &_23$$17, ")");
				zephir_array_append(&createLines, &_25$$17, PH_SEPARATE, "phalcon/Db/Dialect/Sqlite.zep", 191);
			}
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&index);
	}
	...
	if (zephir_array_isset_string_fetch(&references, &definition, SL("references"), 0)) {
		zephir_is_iterable(&references, 0, "phalcon/Db/Dialect/Sqlite.zep", 216);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&references), _26$$18)
		{
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_28$$19, this_ptr, "getcolumnlist", &_21, 38, &_29$$19);
			zephir_check_call_status();
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_31$$19, this_ptr, "getcolumnlist", &_21, 38, &_32$$19);
			zephir_check_call_status();
			...
			zephir_array_append(&createLines, &referenceSql, PH_SEPARATE, "phalcon/Db/Dialect/Sqlite.zep", 214);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&reference);
	}
	ZEPHIR_INIT_VAR(&_35);
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, createView) {
	...
	if (!(zephir_array_isset_string_fetch(&viewSql, &definition, SL("sql"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The index 'sql' is required in the definition array", "phalcon/Db/Dialect/Sqlite.zep", 231);
		return;
	}
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, dropColumn) {
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Dropping DB column is not supported by SQLite", "phalcon/Db/Dialect/Sqlite.zep", 280);
	return;
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, dropForeignKey) {
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Dropping a foreign key constraint is not supported by SQLite", "phalcon/Db/Dialect/Sqlite.zep", 288);
	return;
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, dropPrimaryKey) {
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Removing a primary key after table has been created is not supported by SQLite", "phalcon/Db/Dialect/Sqlite.zep", 307);
	return;
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, getColumnDefinition) {
	...
	do {
		...
		if (ZEPHIR_IS_EMPTY(&columnSql)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_5$$38, "phalcon/Db/Dialect/Sqlite.zep", 474 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		...
		if (!(ZEPHIR_IS_EMPTY(&typeValues))) {
			if (Z_TYPE_P(&typeValues) == IS_ARRAY) {
				...
				zephir_is_iterable(&typeValues, 0, "phalcon/Db/Dialect/Sqlite.zep", 485);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&typeValues), _8$$40)
				{
					...
					ZVAL_STRING(&_9$$41, "\"");
					ZEPHIR_CALL_FUNCTION(&_10$$41, "addcslashes", &_11, 141, &value$$40, &_9$$41);
					zephir_check_call_status();
				...
				ZVAL_STRING(&_17$$42, "\"");
				ZEPHIR_CALL_FUNCTION(&_18$$42, "addcslashes", &_11, 141, &typeValues, &_17$$42);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, modifyColumn) {
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Altering a DB column is not supported by SQLite", "phalcon/Db/Dialect/Sqlite.zep", 544);
	return;
