...
PHP_METHOD(Phalcon_Db_Dialect_Postgresql, createTable) {
	...
	if (!(zephir_array_isset_string_fetch(&columns, &definition, SL("columns"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The index 'columns' is required in the definition array", "phalcon/Db/Dialect/Postgresql.zep", 124);
		return;
	}
	...
	zephir_is_iterable(&columns, 0, "phalcon/Db/Dialect/Postgresql.zep", 173);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&columns), _0)
	{
		...
		if (zephir_is_true(&_8$$7)) {
			...
			zephir_check_call_status();
			zephir_array_append(&primaryColumns, &_9$$10, PH_SEPARATE, "phalcon/Db/Dialect/Postgresql.zep", 168);
		}
		zephir_array_append(&createLines, &columnLine, PH_SEPARATE, "phalcon/Db/Dialect/Postgresql.zep", 171);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&column);
	if (!(ZEPHIR_IS_EMPTY(&primaryColumns))) {
		ZEPHIR_CALL_METHOD(&_10$$11, this_ptr, "getcolumnlist", NULL, 38, &primaryColumns);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_11$$11);
		ZEPHIR_CONCAT_SVS(&_11$$11, "PRIMARY KEY (", &_10$$11, ")");
		zephir_array_append(&createLines, &_11$$11, PH_SEPARATE, "phalcon/Db/Dialect/Postgresql.zep", 174);
	}
	...
	if (zephir_array_isset_string_fetch(&indexes, &definition, SL("indexes"), 0)) {
		zephir_is_iterable(&indexes, 0, "phalcon/Db/Dialect/Postgresql.zep", 208);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&indexes), _12$$12)
		{
			...
			if (ZEPHIR_IS_STRING(&indexName, "PRIMARY")) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_13$$14, this_ptr, "getcolumnlist", NULL, 38, &_14$$14);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&indexSql);
				...
				if (!(ZEPHIR_IS_EMPTY(&indexType))) {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_15$$16, this_ptr, "getcolumnlist", NULL, 38, &_16$$16);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&indexSql);
					...
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_20$$17, this_ptr, "getcolumnlist", NULL, 38, &_21$$17);
					zephir_check_call_status();
			...
			if (!(ZEPHIR_IS_EMPTY(&indexSql))) {
				zephir_array_append(&createLines, &indexSql, PH_SEPARATE, "phalcon/Db/Dialect/Postgresql.zep", 205);
			}
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&index);
	}
	...
	if (zephir_array_isset_string_fetch(&references, &definition, SL("references"), 0)) {
		zephir_is_iterable(&references, 0, "phalcon/Db/Dialect/Postgresql.zep", 233);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&references), _23$$19)
		{
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_25$$20, this_ptr, "getcolumnlist", NULL, 38, &_26$$20);
			zephir_check_call_status();
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_29$$20, this_ptr, "getcolumnlist", NULL, 38, &_30$$20);
			zephir_check_call_status();
			...
			zephir_array_append(&createLines, &referenceSql, PH_SEPARATE, "phalcon/Db/Dialect/Postgresql.zep", 231);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&reference);
	}
	ZEPHIR_INIT_VAR(&_34);
...
PHP_METHOD(Phalcon_Db_Dialect_Postgresql, createView) {
	...
	if (!(zephir_array_isset_string_fetch(&viewSql, &definition, SL("sql"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The index 'sql' is required in the definition array", "phalcon/Db/Dialect/Postgresql.zep", 252);
		return;
	}
...
PHP_METHOD(Phalcon_Db_Dialect_Postgresql, getColumnDefinition) {
	...
	do {
		...
		if (ZEPHIR_IS_EMPTY(&columnSql)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_5$$34, "phalcon/Db/Dialect/Postgresql.zep", 471 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		...
		if (!(ZEPHIR_IS_EMPTY(&typeValues))) {
			if (Z_TYPE_P(&typeValues) == IS_ARRAY) {
				...
				zephir_is_iterable(&typeValues, 0, "phalcon/Db/Dialect/Postgresql.zep", 482);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&typeValues), _8$$36)
				{
					...
					ZVAL_STRING(&_9$$37, "\\'");
					ZEPHIR_CALL_FUNCTION(&_10$$37, "addcslashes", &_11, 141, &value$$36, &_9$$37);
					zephir_check_call_status();
				...
				ZVAL_STRING(&_17$$38, "\\'");
				ZEPHIR_CALL_FUNCTION(&_18$$38, "addcslashes", &_11, 141, &typeValues, &_17$$38);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Dialect_Postgresql, modifyColumn) {
	...
	if (!ZEPHIR_IS_IDENTICAL(&_17, &_18)) {
		...
		if (zephir_is_true(&_24$$9)) {
			...
			if (zephir_memnstr_str(&_25$$11, SL("BOOLEAN"), "phalcon/Db/Dialect/Postgresql.zep", 561)) {
...
PHP_METHOD(Phalcon_Db_Dialect_Postgresql, castDefault) {
	...
	if (zephir_memnstr_str(&_0, SL("BOOLEAN"), "phalcon/Db/Dialect/Postgresql.zep", 644)) {
		RETURN_CCTOR(&defaultValue);
	}
	ZEPHIR_INIT_VAR(&_1);
	zephir_fast_strtoupper(&_1, &defaultValue);
	if (zephir_memnstr_str(&_1, SL("CURRENT_TIMESTAMP"), "phalcon/Db/Dialect/Postgresql.zep", 648)) {
	...
	if (_5) {
		...
		ZVAL_STRING(&_7$$6, "\\'");
		ZEPHIR_CALL_FUNCTION(&_8$$6, "addcslashes", NULL, 141, &defaultValue, &_7$$6);
		zephir_check_call_status();
