...
PHP_METHOD(Phalcon_Db_Dialect, getSqlExpression) {
	...
	if (ZEPHIR_IS_STRING(type, "binary-op")) {
		...
		zephir_call_internal_method_p2(left, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), _2, escapeChar);
		zephir_array_fetch_string(&_3, expression, SL("right"), PH_NOISY | PH_READONLY TSRMLS_CC);
		ZEPHIR_INIT_VAR(right);
		zephir_call_internal_method_p2(right, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), _3, escapeChar);
	...
	if (ZEPHIR_IS_STRING(type, "unary-op")) {
		...
		if (zephir_array_isset_string_fetch(&left, expression, SS("left"), 0 TSRMLS_CC)) {
			ZEPHIR_INIT_VAR(_5);
			zephir_call_internal_method_p2(_5, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), left, escapeChar);
		...
		if (zephir_array_isset_string_fetch(&right, expression, SS("right"), 0 TSRMLS_CC)) {
			ZEPHIR_INIT_NVAR(_5);
			zephir_call_internal_method_p2(_5, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), right, escapeChar);
			ZEPHIR_CONCAT_VV(return_value, _5, operator);
			RETURN_MM();
		}
	}
	...
	if (ZEPHIR_IS_STRING(type, "parentheses")) {
		ZEPHIR_INIT_NVAR(_5);
		zephir_array_fetch_string(&_2, expression, SL("left"), PH_NOISY | PH_READONLY TSRMLS_CC);
		zephir_call_internal_method_p2(_5, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), _2, escapeChar);
		...
		if (zephir_array_isset_string_fetch(&arguments, expression, SS("arguments"), 0 TSRMLS_CC)) {
			...
			for (
			  ...
			  ; zephir_hash_move_forward_ex(_7, &_6)
				...
				ZEPHIR_INIT_NVAR(_9);
				zephir_call_internal_method_p2(_9, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), argument, escapeChar);
	...
	if (ZEPHIR_IS_STRING(type, "list")) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_11, &_10)
			...
			ZEPHIR_INIT_NVAR(_5);
			zephir_call_internal_method_p2(_5, this_ptr, "getsqlexpression", ZEND_MN(Phalcon\\Db_Dialect_getSqlExpression), item, escapeChar);
