...
PHP_METHOD(Phalcon_Db_Check, __construct)
{
	...
	if (UNEXPECTED(_1)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$4, "__construct", NULL, 463);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$4, "phalcon/Db/Check.zep", 76);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 541, &name_zv);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 542, &expression);
	ZEPHIR_MM_RESTORE();
}
