...
PHP_METHOD(Phalcon_Db_Check, __construct)
{
	...
	if (UNEXPECTED(!(zephir_array_isset_string_fetch(&expression, &definition, SL("expression"), 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 493);
	...
	if (UNEXPECTED(_1)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_2$$4, "__construct", NULL, 494);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$4, "phalcon/Db/Check.zep", 76);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 578, &name_zv);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 579, &expression);
	ZEPHIR_MM_RESTORE();
}
