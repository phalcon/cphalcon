...
PHP_METHOD(Phalcon_Db_Profiler, startProfile)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 637, &activeProfile);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Db_Profiler, stopProfile)
{
	...
	zephir_check_call_status();
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_1, 636, PH_NOISY_CC | PH_READONLY);
	_2 = ZEPHIR_GT_LONG(&_0, 0);
	if (_2) {
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_2, 635, PH_NOISY_CC | PH_READONLY);
		zephir_read_property_cached(&_4, this_ptr, _zephir_prop_1, 636, PH_NOISY_CC | PH_READONLY);
		_2 = ZEPHIR_LE_LONG(&_4, zephir_fast_count_int(&_3));
	}
	if (_2) {
		zephir_read_property_cached(&_5$$3, this_ptr, _zephir_prop_2, 635, PH_NOISY_CC | PH_READONLY);
		...
		if (Z_TYPE_P(&firstKey) != IS_NULL) {
			...
			zephir_read_property_cached(&_6$$4, this_ptr, _zephir_prop_2, 635, PH_NOISY_CC | PH_READONLY);
			zephir_array_unset(&_6$$4, &firstKey, PH_SEPARATE);
		}
	}
	zephir_read_property_cached(&_7, this_ptr, _zephir_prop_3, 638, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_8, &activeProfile, "gettotalelapsednanoseconds", NULL, 0);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_9);
	zephir_add_function(&_9, &_7, &_8);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 638, &_9);
