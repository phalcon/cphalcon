...
PHP_METHOD(Phalcon_Db_RawValue, __construct)
{
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(value, "")) {
		...
		ZVAL_STRING(&_0$$3, "''");
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 637, &_0$$3);
	} else if (Z_TYPE_P(value) == IS_NULL) {
		ZEPHIR_INIT_VAR(&_1$$4);
		ZEPHIR_INIT_NVAR(&_1$$4);
		ZVAL_STRING(&_1$$4, "NULL");
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 637, &_1$$4);
	} else {
		zephir_cast_to_string(&_2$$5, value);
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 637, &_2$$5);
	}
	ZEPHIR_MM_RESTORE();
}
