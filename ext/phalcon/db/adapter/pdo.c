...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, __construct) {
	...
	if (Z_TYPE_P(descriptor) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "The descriptor must be an array", "phalcon/db/adapter/pdo.zep", 67);
		return;
	}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, connect) {
	...
	ZEPHIR_INIT_VAR(_4);
	ZVAL_LONG(_4, 2);
	zephir_array_update_long(&options, 3, &_4, PH_COPY | PH_SEPARATE, "phalcon/db/adapter/pdo.zep", 141);
	if (zephir_array_isset_string_fetch(&persistent, descriptor, SS("persistent"), 1 TSRMLS_CC)) {
		if (zephir_is_true(persistent)) {
			zephir_array_update_long(&options, 12, &ZEPHIR_GLOBAL(global_true), PH_COPY | PH_SEPARATE, "phalcon/db/adapter/pdo.zep", 148);
		}
	}
	ZEPHIR_INIT_LNVAR(_3);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, executePrepared) {
	...
	if (Z_TYPE_P(placeholders) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Placeholders must be an array", "phalcon/db/adapter/pdo.zep", 195);
		return;
	}
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		if (Z_TYPE_P(wildcard) == IS_LONG) {
			...
			if (Z_TYPE_P(wildcard) == IS_STRING) {
				...
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Invalid bind parameter (1)", "phalcon/db/adapter/pdo.zep", 206);
				return;
			}
		}
		if (Z_TYPE_P(dataTypes) == IS_ARRAY) {
			...
			if (!(zephir_array_isset_fetch(&type, dataTypes, wildcard, 0 TSRMLS_CC))) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Invalid bind type parameter (2)", "phalcon/db/adapter/pdo.zep", 213);
				return;
			}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, begin) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *nesting_param = NULL, *pdo, *transactionLevel, *eventsManager = NULL, *savepointName = NULL, *_0, *_1 = NULL, *_2 = NULL;
	zend_bool nesting;
	...
	if (ZEPHIR_IS_LONG(transactionLevel, 1)) {
		...
		if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
			ZEPHIR_INIT_VAR(_1);
			ZVAL_STRING(_1, "db:beginTransaction", 0);
		...
		if (zephir_is_true(transactionLevel)) {
			if (nesting) {
				ZEPHIR_CALL_METHOD(&_2, this_ptr, "isnestedtransactionswithsavepoints",  NULL);
				zephir_check_call_status();
				if (zephir_is_true(_2)) {
					...
					if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
						ZEPHIR_INIT_NVAR(_1);
						ZVAL_STRING(_1, "db:createSavepoint", 0);
						ZEPHIR_CALL_METHOD(NULL, eventsManager, "fire", NULL, _1, this_ptr, savepointName);
						zephir_check_temp_parameter(_1);
						zephir_check_call_status();
					}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, rollback) {
	...
	zephir_read_property_this(&transactionLevel, this_ptr, SL("_transactionLevel"), PH_NOISY_CC);
	if (!(zephir_is_true(transactionLevel))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/db/adapter/pdo.zep", 537);
		return;
	}
	if (ZEPHIR_IS_LONG(transactionLevel, 1)) {
		_0 = zephir_fetch_nproperty_this(this_ptr, SL("_eventsManager"), PH_NOISY_CC);
		...
		if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
			ZEPHIR_INIT_VAR(_1);
			ZVAL_STRING(_1, "db:rollbackTransaction", 0);
			ZEPHIR_CALL_METHOD(NULL, eventsManager, "fire", NULL, _1, this_ptr);
			zephir_check_temp_parameter(_1);
			zephir_check_call_status();
		}
		...
		if (zephir_is_true(transactionLevel)) {
			if (nesting) {
				...
				if (zephir_is_true(_2)) {
					...
					if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
						ZEPHIR_INIT_NVAR(_1);
						ZVAL_STRING(_1, "db:rollbackSavepoint", 0);
						ZEPHIR_CALL_METHOD(NULL, eventsManager, "fire", NULL, _1, this_ptr, savepointName);
						zephir_check_temp_parameter(_1);
						zephir_check_call_status();
					}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, commit) {
	...
	zephir_read_property_this(&transactionLevel, this_ptr, SL("_transactionLevel"), PH_NOISY_CC);
	if (!(zephir_is_true(transactionLevel))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/db/adapter/pdo.zep", 616);
		return;
	}
	if (ZEPHIR_IS_LONG(transactionLevel, 1)) {
		_0 = zephir_fetch_nproperty_this(this_ptr, SL("_eventsManager"), PH_NOISY_CC);
		...
		if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
			ZEPHIR_INIT_VAR(_1);
			ZVAL_STRING(_1, "db:commitTransaction", 0);
			ZEPHIR_CALL_METHOD(NULL, eventsManager, "fire", NULL, _1, this_ptr);
			zephir_check_temp_parameter(_1);
			zephir_check_call_status();
		}
		...
		if (zephir_is_true(transactionLevel)) {
			if (nesting) {
				...
				if (zephir_is_true(_2)) {
					...
					if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
						ZEPHIR_INIT_NVAR(_1);
						ZVAL_STRING(_1, "db:releaseSavepoint", 0);
						ZEPHIR_CALL_METHOD(NULL, eventsManager, "fire", NULL, _1, this_ptr, savepointName);
						zephir_check_temp_parameter(_1);
						zephir_check_call_status();
					}
