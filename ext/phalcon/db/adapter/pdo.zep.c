...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, connect) {
	...
	zend_ulong _1$$13;
	int ZEPHIR_LAST_CALL_STATUS;
	zval *descriptor = NULL, descriptor_sub, __$true, __$null, username, password, dsnParts, dsnAttributes, persistent, options, key, value, _4, _5, _6, _7, *_0$$13, _3$$14;
	ZEPHIR_INIT_THIS();
	...
	if (!(zephir_array_isset_string_fetch(&dsnAttributes, descriptor, SL("dsn"), 0))) {
		...
		array_init(&dsnParts);
		zephir_is_iterable(descriptor, 0, "phalcon/db/adapter/pdo.zep", 147);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(descriptor), _1$$13, _2$$13, _0$$13)
		{
			ZEPHIR_INIT_NVAR(&key);
			if (_2$$13 != NULL) { 
				ZVAL_STR_COPY(&key, _2$$13);
			} else {
				ZVAL_LONG(&key, _1$$13);
			}
			ZEPHIR_INIT_NVAR(&value);
			ZVAL_COPY(&value, _0$$13);
	...
	zephir_read_property(&_6, this_ptr, SL("_type"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_7);
	ZEPHIR_CONCAT_VSV(&_7, &_6, ":", &dsnAttributes);
	ZEPHIR_CALL_METHOD(NULL, &_5, "__construct", NULL, 0, &_7, &username, &password, &options);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, SL("_pdo"), &_5);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, executePrepared) {
	...
	ZEPHIR_OBS_VAR_ONCE(&placeholders);
	ZVAL_COPY(&placeholders, placeholders_param);


	zephir_is_iterable(&placeholders, 0, "phalcon/db/adapter/pdo.zep", 273);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&placeholders), _1, _2, _0)
	{
		...
		if (_3$$3) {
			...
			if (Z_TYPE_P(&castValue) != IS_ARRAY) {
				...
				zephir_is_iterable(&castValue, 0, "phalcon/db/adapter/pdo.zep", 261);
			...
			if (Z_TYPE_P(&value) != IS_ARRAY) {
				...
				zephir_is_iterable(&value, 0, "phalcon/db/adapter/pdo.zep", 269);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, escapeIdentifier) {
	...
	zval *identifier, identifier_sub, _0$$3, _1$$3;
	ZEPHIR_INIT_THIS();
	...
	if (Z_TYPE_P(identifier) == IS_ARRAY) {
		zephir_array_fetch_long(&_0$$3, identifier, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo.zep", 425 TSRMLS_CC);
		zephir_array_fetch_long(&_1$$3, identifier, 1, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo.zep", 425 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, convertBoundParams) {
	...
	if (zephir_is_true(&_1)) {
		zephir_is_iterable(&matches, 0, "phalcon/db/adapter/pdo.zep", 474);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&matches), _2$$3)
		{
			ZEPHIR_INIT_NVAR(&placeMatch);
			ZVAL_COPY(&placeMatch, _2$$3);
			ZEPHIR_OBS_NVAR(&value);
			zephir_array_fetch_long(&_3$$4, &placeMatch, 1, PH_READONLY, "phalcon/db/adapter/pdo.zep", 461 TSRMLS_CC);
			if (!(zephir_array_isset_fetch(&value, &params, &_3$$4, 0 TSRMLS_CC))) {
				if (zephir_array_isset_long(&placeMatch, 2)) {
					ZEPHIR_OBS_NVAR(&value);
					zephir_array_fetch_long(&_4$$6, &placeMatch, 2, PH_READONLY, "phalcon/db/adapter/pdo.zep", 463 TSRMLS_CC);
					if (!(zephir_array_isset_fetch(&value, &params, &_4$$6, 0 TSRMLS_CC))) {
						ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Matched parameter wasn't found in parameters list", "phalcon/db/adapter/pdo.zep", 464);
						return;
					}
				} else {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Matched parameter wasn't found in parameters list", "phalcon/db/adapter/pdo.zep", 467);
					return;
				}
			}
			zephir_array_append(&placeHolders, &value, PH_SEPARATE, "phalcon/db/adapter/pdo.zep", 471);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&placeMatch);
		ZEPHIR_INIT_VAR(&_5$$3);
		ZVAL_STRING(&_5$$3, "?");
		ZEPHIR_CALL_FUNCTION(&boundSql, "preg_replace", NULL, 29, &bindPattern, &_5$$3, &sql);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, rollback) {
	...
	transactionLevel = zephir_get_intval(&_0);
	if (!(transactionLevel)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/db/adapter/pdo.zep", 588);
		return;
	}
	if (transactionLevel == 1) {
