...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, commit) {
	...
	if (UNEXPECTED(!transactionLevel)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/Db/Adapter/Pdo.zep", 152);
		return;
	}
	if (transactionLevel == 1) {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, connect) {
	...
	if (zephir_array_isset_string_fetch(&dsnAttributesCustomRaw, &descriptor, SL("dsn"), 0)) {
		zephir_array_append(&dsnParts, &dsnAttributesCustomRaw, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 281);
	...
	zephir_is_iterable(&dsnAttributesMap, 0, "phalcon/Db/Adapter/Pdo.zep", 301);
	if (Z_TYPE_P(&dsnAttributesMap) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&dsnAttributesMap), _6, _7, _4)
		{
			...
			zephir_array_append(&dsnParts, &_8$$10, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 297);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_append(&dsnParts, &_9$$11, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 297);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, convertBoundParams) {
	...
	if (zephir_is_true(&_1)) {
		zephir_is_iterable(&matches, 0, "phalcon/Db/Adapter/Pdo.zep", 357);
		if (Z_TYPE_P(&matches) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&matches), _3$$3)
			{
				...
				zephir_array_fetch_long(&_5$$4, &placeMatch, 1, PH_READONLY, "phalcon/Db/Adapter/Pdo.zep", 340 TSRMLS_CC);
				if (!(zephir_array_isset_fetch(&value, &params, &_5$$4, 0 TSRMLS_CC))) {
					...
					ZEPHIR_OBS_NVAR(&value);
					zephir_array_fetch_long(&_6$$5, &placeMatch, 2, PH_READONLY, "phalcon/Db/Adapter/Pdo.zep", 347 TSRMLS_CC);
				...
				zephir_array_append(&placeHolders, &value, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 354);
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_OBS_NVAR(&value);
					zephir_array_fetch_long(&_7$$8, &placeMatch, 1, PH_READONLY, "phalcon/Db/Adapter/Pdo.zep", 340 TSRMLS_CC);
					if (!(zephir_array_isset_fetch(&value, &params, &_7$$8, 0 TSRMLS_CC))) {
						...
						ZEPHIR_OBS_NVAR(&value);
						zephir_array_fetch_long(&_8$$9, &placeMatch, 2, PH_READONLY, "phalcon/Db/Adapter/Pdo.zep", 347 TSRMLS_CC);
		...
		ZVAL_STRING(&_9$$3, "?");
		ZEPHIR_CALL_FUNCTION(&boundSql, "preg_replace", NULL, 38, &bindPattern, &_9$$3, &sql);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, executePrepared) {
	...
	zephir_is_iterable(&placeholders, 0, "phalcon/Db/Adapter/Pdo.zep", 570);
	if (Z_TYPE_P(&placeholders) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&placeholders), _2, _3, _0)
		{
			...
			if (Z_TYPE_P(&wildcard) == IS_LONG) {
				...
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Invalid bind parameter (1)", "phalcon/Db/Adapter/Pdo.zep", 490);
				return;
			}
			...
			if (_4$$3) {
						...
						if (Z_TYPE_P(&value) != IS_ARRAY) {
							do {
								if (ZEPHIR_IS_LONG(&type, 1)) {
									ZVAL_LONG(&_6$$12, 10);
									ZEPHIR_CALL_FUNCTION(&castValue, "intval", &_7, 39, &value, &_6$$12);
									zephir_check_call_status();
									break;
								}
				...
				if (Z_TYPE_P(&castValue) != IS_ARRAY) {
					...
					zephir_is_iterable(&castValue, 0, "phalcon/Db/Adapter/Pdo.zep", 558);
					if (Z_TYPE_P(&castValue) == IS_ARRAY) {
				...
				if (Z_TYPE_P(&value) != IS_ARRAY) {
					...
					zephir_is_iterable(&value, 0, "phalcon/Db/Adapter/Pdo.zep", 566);
					if (Z_TYPE_P(&value) == IS_ARRAY) {
						...
						if (ZEPHIR_GLOBAL(db).force_casting) {
							if (Z_TYPE_P(&value) != IS_ARRAY) {
								do {
									if (ZEPHIR_IS_LONG(&type, 1)) {
										ZVAL_LONG(&_26$$43, 10);
										ZEPHIR_CALL_FUNCTION(&castValue, "intval", &_7, 39, &value, &_26$$43);
										zephir_check_call_status();
										break;
									}
									if (ZEPHIR_IS_LONG(&type, 2)) {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, rollback) {
	...
	transactionLevel = zephir_get_intval(&_1);
	if (UNEXPECTED(!transactionLevel)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/Db/Adapter/Pdo.zep", 779);
		return;
	}
	if (transactionLevel == 1) {
		zephir_read_property(&_2$$5, this_ptr, SL("eventsManager"), PH_NOISY_CC | PH_READONLY);
