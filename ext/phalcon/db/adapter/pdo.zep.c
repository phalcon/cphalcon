...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, commit) {
	...
	transactionLevel = zephir_get_intval(&_1);
	if (!(transactionLevel)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/Db/Adapter/Pdo.zep", 154);
		return;
	}
	if (transactionLevel == 1) {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, connect) {
	...
	if (zephir_array_isset_string_fetch(&dsnAttributesCustomRaw, &descriptor, SL("dsn"), 0)) {
		zephir_array_append(&dsnParts, &dsnAttributesCustomRaw, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 285);
	...
	zephir_is_iterable(&dsnAttributesMap, 0, "phalcon/Db/Adapter/Pdo.zep", 299);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&dsnAttributesMap), _5, _6, _4)
	{
		...
		zephir_array_append(&dsnParts, &_7$$10, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 295);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, convertBoundParams) {
	...
	if (zephir_is_true(&_1)) {
		zephir_is_iterable(&matches, 0, "phalcon/Db/Adapter/Pdo.zep", 346);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&matches), _3$$3)
		{
			ZEPHIR_INIT_NVAR(&placeMatch);
			ZVAL_COPY(&placeMatch, _3$$3);
			ZEPHIR_OBS_NVAR(&value);
			zephir_array_fetch_long(&_4$$4, &placeMatch, 1, PH_READONLY, "phalcon/Db/Adapter/Pdo.zep", 333 TSRMLS_CC);
			if (!(zephir_array_isset_fetch(&value, &params, &_4$$4, 0 TSRMLS_CC))) {
				if (zephir_array_isset_long(&placeMatch, 2)) {
					ZEPHIR_OBS_NVAR(&value);
					zephir_array_fetch_long(&_5$$6, &placeMatch, 2, PH_READONLY, "phalcon/Db/Adapter/Pdo.zep", 335 TSRMLS_CC);
					if (!(zephir_array_isset_fetch(&value, &params, &_5$$6, 0 TSRMLS_CC))) {
						ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Matched parameter wasn't found in parameters list", "phalcon/Db/Adapter/Pdo.zep", 336);
						return;
					}
				} else {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Matched parameter wasn't found in parameters list", "phalcon/Db/Adapter/Pdo.zep", 339);
					return;
				}
			}
			zephir_array_append(&placeHolders, &value, PH_SEPARATE, "phalcon/Db/Adapter/Pdo.zep", 343);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&placeMatch);
		ZEPHIR_INIT_VAR(&_6$$3);
		ZVAL_STRING(&_6$$3, "?");
		ZEPHIR_CALL_FUNCTION(&boundSql, "preg_replace", NULL, 21, &bindPattern, &_6$$3, &sql);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, executePrepared) {
	...
	zephir_is_iterable(&placeholders, 0, "phalcon/Db/Adapter/Pdo.zep", 543);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&placeholders), _1, _2, _0)
	{
		...
		if (Z_TYPE_P(&wildcard) == IS_LONG) {
			...
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Invalid bind parameter (1)", "phalcon/Db/Adapter/Pdo.zep", 469);
			return;
		}
		...
		if (_3$$3) {
					...
					if (Z_TYPE_P(&value) != IS_ARRAY) {
						do {
							if (ZEPHIR_IS_LONG(&type, 1)) {
								ZVAL_LONG(&_5$$12, 10);
								ZEPHIR_CALL_FUNCTION(&castValue, "intval", &_6, 22, &value, &_5$$12);
								zephir_check_call_status();
								break;
							}
			...
			if (Z_TYPE_P(&castValue) != IS_ARRAY) {
				...
				zephir_is_iterable(&castValue, 0, "phalcon/Db/Adapter/Pdo.zep", 531);
			...
			if (Z_TYPE_P(&value) != IS_ARRAY) {
				...
				zephir_is_iterable(&value, 0, "phalcon/Db/Adapter/Pdo.zep", 539);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, query) {
	...
	if (Z_TYPE_P(&statement) == IS_OBJECT) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Cannot prepare statement", "phalcon/Db/Adapter/Pdo.zep", 703);
		return;
	}
	if (Z_TYPE_P(&statement) == IS_OBJECT) {
		if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, rollback) {
	...
	transactionLevel = zephir_get_intval(&_1);
	if (!(transactionLevel)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "There is no active transaction", "phalcon/Db/Adapter/Pdo.zep", 736);
		return;
	}
	if (transactionLevel == 1) {
		zephir_read_property(&_2$$5, this_ptr, SL("eventsManager"), PH_NOISY_CC | PH_READONLY);
