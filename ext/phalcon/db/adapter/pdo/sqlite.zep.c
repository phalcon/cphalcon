...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Sqlite, connect)
{
	...
	if (ZEPHIR_IS_EMPTY(&descriptor)) {
		zephir_read_property_cached(&_0$$3, this_ptr, _zephir_prop_0, 576, PH_NOISY_CC | PH_READONLY);
	...
	if (zephir_array_isset_string_fetch(&dbname, &descriptor, SL("dbname"), 0)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$5, "__construct", NULL, 497);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Sqlite, describeColumns)
{
	...
	if (Z_TYPE_P(&fields) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fields), _3)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_4$$3, "__construct", &_46, 496, &columnName, &definition);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_50$$32, "__construct", &_46, 496, &columnName, &definition);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Sqlite, describeIndexes)
{
	...
	array_init(&indexes);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 577, PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _4)
		{
			...
			zephir_read_property_cached(&_8$$3, this_ptr, _zephir_prop_0, 577, PH_NOISY_CC | PH_READONLY);
			...
			zephir_read_property_cached(&_10$$3, this_ptr, _zephir_prop_0, 577, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_read_property_cached(&_31$$13, this_ptr, _zephir_prop_0, 577, PH_NOISY_CC | PH_READONLY);
				...
				ZEPHIR_INIT_NVAR(&describeIndex);
				zephir_array_update_multi(&indexes, &columns, SL("zs"), 3, &keyName, SL("columns"));
				zephir_read_property_cached(&_33$$13, this_ptr, _zephir_prop_0, 577, PH_NOISY_CC | PH_READONLY);
