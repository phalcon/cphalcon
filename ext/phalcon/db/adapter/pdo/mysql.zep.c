...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Mysql, addForeignKey)
{
	...
	if (UNEXPECTED(!zephir_is_true(&_2))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$3, "__construct", NULL, 459);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$3, "phalcon/Db/Adapter/Pdo/Mysql.zep", 65);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_0, 536, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Mysql, describeColumns)
{
	...
	if (Z_TYPE_P(&fields) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fields), _3)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_71$$3, "__construct", &_72, 460, &columnName, &definition);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_143$$62, "__construct", &_72, 460, &columnName, &definition);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Mysql, describeIndexes)
{
	...
	array_init(&indexes);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 536, PH_NOISY_CC | PH_READONLY);
