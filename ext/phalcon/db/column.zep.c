...
PHP_METHOD(Phalcon_Db_Column, __construct) {
	...
	if (UNEXPECTED(!(zephir_array_isset_string_fetch(&type, &definition, SL("type"), 0)))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Column type is required", "/home/nikos/Work/niden/cphalcon/phalcon/Db/Column.zep", 320);
		return;
	}
	...
	if (zephir_array_isset_string_fetch(&scale, &definition, SL("scale"), 0)) {
		do {
			...
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Column type does not support scale parameter", "/home/nikos/Work/niden/cphalcon/phalcon/Db/Column.zep", 370);
			return;
		} while(0);
	...
	if (zephir_array_isset_string_fetch(&autoIncrement, &definition, SL("autoIncrement"), 0)) {
		if (!(zephir_is_true(&autoIncrement))) {
			...
			do {
				...
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_db_exception_ce, "Column type cannot be auto-increment", "/home/nikos/Work/niden/cphalcon/phalcon/Db/Column.zep", 414);
				return;
			} while(0);
