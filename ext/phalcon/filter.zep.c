...
PHP_METHOD(Phalcon_Filter, sanitize) {
	...
	if (Z_TYPE_P(filters) == IS_ARRAY) {
		if (Z_TYPE_P(value) != IS_NULL) {
			zephir_is_iterable(filters, 0, "phalcon/filter.zep", 107);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(filters), _0$$4)
			{
				...
				if (_1$$5) {
					...
					zephir_is_iterable(value, 0, "phalcon/filter.zep", 102);
	...
	if (_8) {
		...
		zephir_is_iterable(value, 0, "phalcon/filter.zep", 119);
...
PHP_METHOD(Phalcon_Filter, _sanitize) {
	...
	do {
		if (ZEPHIR_IS_STRING(&filter, "email")) {
			...
			zephir_check_call_status();
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 200, value, &_5$$5);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "int")) {
			ZVAL_LONG(&_7$$6, 519);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 200, value, &_7$$6);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "int!")) {
		...
		if (ZEPHIR_IS_STRING(&filter, "absint")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("abs", NULL, 201, &_8$$8);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "string")) {
			ZVAL_LONG(&_9$$9, 513);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 200, value, &_9$$9);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "float")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_6, 200, value, &_11$$10, &_10$$10);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "float!")) {
		...
		if (ZEPHIR_IS_STRING(&filter, "striptags")) {
			ZEPHIR_RETURN_CALL_FUNCTION("strip_tags", NULL, 202, value);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "lower")) {
			if ((zephir_function_exists_ex(SL("mb_strtolower") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_RETURN_CALL_FUNCTION("mb_strtolower", NULL, 203, value);
				zephir_check_call_status();
				RETURN_MM();
			}
		...
		if (ZEPHIR_IS_STRING(&filter, "upper")) {
			if ((zephir_function_exists_ex(SL("mb_strtoupper") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_RETURN_CALL_FUNCTION("mb_strtoupper", NULL, 204, value);
				zephir_check_call_status();
				RETURN_MM();
			}
