...
PHP_METHOD(Phalcon_Queue_Adapter_Beanstalk_BeanstalkSubscriptionConsumer, consume)
{
	...
	while (1) {
		...
		zephir_is_iterable(&_3$$4, 0, "phalcon/Queue/Adapter/Beanstalk/BeanstalkSubscriptionConsumer.zep", 84);
		if (Z_TYPE_P(&_3$$4) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3$$4), _4$$4)
			{
				...
				zephir_array_fetch_long(&consumer, &subscription, 0, PH_NOISY, "phalcon/Queue/Adapter/Beanstalk/BeanstalkSubscriptionConsumer.zep", 71);
				ZEPHIR_OBS_NVAR(&callback);
				zephir_array_fetch_long(&callback, &subscription, 1, PH_NOISY, "phalcon/Queue/Adapter/Beanstalk/BeanstalkSubscriptionConsumer.zep", 72);
