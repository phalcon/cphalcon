...
PHP_METHOD(Phalcon_Queue_Adapter_Beanstalk_BeanstalkProducer, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1244, context);
}
...
PHP_METHOD(Phalcon_Queue_Adapter_Beanstalk_BeanstalkProducer, send)
{
	...
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 1245, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_2) == IS_NULL) {
		ZEPHIR_INIT_NVAR(&_1);
		ZVAL_LONG(&_1, 100);
	} else {
		zephir_memory_observe(&_3);
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 1245, PH_NOISY_CC);
		ZEPHIR_INIT_NVAR(&_1);
		ZVAL_LONG(&_1, zephir_get_intval(&_3));
	}
	priority = zephir_get_numberval(&_1);
	ZEPHIR_INIT_VAR(&_4);
	zephir_read_property_cached(&_5, this_ptr, _zephir_prop_1, 1246, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_5) == IS_NULL) {
		ZEPHIR_INIT_NVAR(&_4);
		ZVAL_LONG(&_4, 0);
	} else {
		zephir_read_property_cached(&_6, this_ptr, _zephir_prop_1, 1246, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_INIT_NVAR(&_4);
		ZVAL_LONG(&_4, (int) (zephir_safe_div_zval_long(&_6, 1000)));
	}
	delay = zephir_get_numberval(&_4);
	zephir_read_property_cached(&_7, this_ptr, _zephir_prop_2, 1244, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_8, destination, "getqueuename", NULL, 0);
	zephir_check_call_status();
	zephir_read_property_cached(&_9, this_ptr, _zephir_prop_2, 1244, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Queue_Adapter_Beanstalk_BeanstalkProducer, setDeliveryDelay)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1246, &_0);
	RETURN_THIS();
}
