...
PHP_METHOD(Phalcon_Queue_Adapter_Beanstalk_BeanstalkConsumer, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1246, connection);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1247, queue);
...
PHP_METHOD(Phalcon_Queue_Adapter_Beanstalk_BeanstalkConsumer, reject)
{
	...
	zephir_check_call_status();
	if (requeue) {
		zephir_read_property_cached(&_0$$3, this_ptr, _zephir_prop_0, 1246, PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_1$$3, 100);
		ZVAL_LONG(&_2$$3, 0);
