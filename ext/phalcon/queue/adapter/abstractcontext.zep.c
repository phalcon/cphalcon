...
PHP_METHOD(Phalcon_Queue_Adapter_AbstractContext, createQueue)
{
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 72, &queueName_zv);
...
PHP_METHOD(Phalcon_Queue_Adapter_AbstractContext, createTemporaryQueue)
{
	...
	ZEPHIR_CALL_FUNCTION(&_1, "uniqid", NULL, 73, &_0, &__$true);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Queue_Adapter_AbstractContext, createTopic)
{
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 74, &topicName_zv);
	zephir_check_call_status();
	RETURN_MM();
}
