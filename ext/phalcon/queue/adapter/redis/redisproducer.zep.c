...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisProducer, send)
{
	...
	if (Z_TYPE_P(&_2) == IS_NULL) {
		...
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 1283, PH_NOISY_CC);
		ZEPHIR_INIT_NVAR(&_1);
		ZVAL_LONG(&_1, zephir_get_intval(&_3));
	}
	delay = zephir_get_numberval(&_1);
	zephir_read_property_cached(&_4, this_ptr, _zephir_prop_1, 1282, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisProducer, setDeliveryDelay)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1283, &_0);
	RETURN_THIS();
}
