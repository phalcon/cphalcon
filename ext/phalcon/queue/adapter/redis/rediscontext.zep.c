...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisContext, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1259, redis);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1260, &prefix_zv);
	ZVAL_UNDEF(&_0);
	ZVAL_LONG(&_0, pollInterval);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1261, &_0);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisContext, pushMessage)
{
	...
	if (delay > 0) {
		...
		zephir_read_property_cached(&_3$$3, this_ptr, _zephir_prop_0, 1259, PH_NOISY_CC | PH_READONLY);
	...
	zephir_read_property_cached(&_6, this_ptr, _zephir_prop_0, 1259, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisContext, promote)
{
	...
	if (Z_TYPE_P(&due) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&due), _3)
		{
			...
			ZVAL_COPY(&member, _3);
			zephir_read_property_cached(&_4$$4, this_ptr, _zephir_prop_0, 1259, PH_NOISY_CC | PH_READONLY);
			...
			if (ZEPHIR_GT_LONG(&_5$$4, 0)) {
				...
				zephir_read_property_cached(&_8$$5, this_ptr, _zephir_prop_0, 1259, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				zephir_read_property_cached(&_11$$6, this_ptr, _zephir_prop_0, 1259, PH_NOISY_CC | PH_READONLY);
				...
				if (ZEPHIR_GT_LONG(&_12$$6, 0)) {
					...
					zephir_read_property_cached(&_15$$7, this_ptr, _zephir_prop_0, 1259, PH_NOISY_CC | PH_READONLY);
