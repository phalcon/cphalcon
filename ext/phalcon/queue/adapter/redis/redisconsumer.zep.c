...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisConsumer, receive)
{
	...
	while (1) {
		zephir_read_property_cached(&_2$$4, this_ptr, _zephir_prop_1, 1248, PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_3$$4, 1);
...
PHP_METHOD(Phalcon_Queue_Adapter_Redis_RedisConsumer, reject)
{
	...
	if (requeue) {
		...
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_1, 1249, PH_NOISY_CC | PH_READONLY);
