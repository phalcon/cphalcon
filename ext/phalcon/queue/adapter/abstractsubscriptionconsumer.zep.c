...
ZEPHIR_INIT_CLASS(Phalcon_Queue_Adapter_AbstractSubscriptionConsumer)
{
	 ...
	 * @var int
	 */
	zend_declare_property_long(phalcon_queue_adapter_abstractsubscriptionconsumer_ce, SL("pollInterval"), 200, ZEND_ACC_PROTECTED);
	/**
...
PHP_METHOD(Phalcon_Queue_Adapter_AbstractSubscriptionConsumer, consume)
{
	...
	while (1) {
		...
		if (Z_TYPE_P(&_3$$4) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3$$4), _4$$4)
			{
				...
				if (Z_TYPE_P(&message) != IS_NULL) {
					ZEPHIR_CALL_FUNCTION(&result, "call_user_func", &_5, 79, &callback, &message, &consumer);
					zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(NULL, "usleep", &_11, 72, &_10$$4);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Queue_Adapter_AbstractSubscriptionConsumer, subscribe)
{
	...
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "resolvequeuename", NULL, 80, consumer);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Queue_Adapter_AbstractSubscriptionConsumer, unsubscribe)
{
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "resolvequeuename", NULL, 80, consumer);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "resolvequeuename", NULL, 80, consumer);
	zephir_check_call_status();
