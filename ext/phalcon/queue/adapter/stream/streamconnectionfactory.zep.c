...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamConnectionFactory, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1300, &options);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamConnectionFactory, createContext)
{
	...
	if (zephir_array_isset_value_string(&_0, SL("storageDir"))) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 1300, PH_NOISY_CC | PH_READONLY);
	...
	pollInterval = 200;
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 1300, PH_NOISY_CC | PH_READONLY);
