...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamContext, popMessage)
{
	...
	ZVAL_LONG(&_2, 2);
	ZEPHIR_CALL_FUNCTION(&_3, "flock", NULL, 280, &pointer, &_2);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&lines, "array_filter", NULL, 0, &_1);
	zephir_check_call_status();
	if (ZEPHIR_IS_EMPTY(&lines)) {
		ZVAL_LONG(&_5$$6, 3);
		ZEPHIR_CALL_FUNCTION(NULL, "flock", NULL, 280, &pointer, &_5$$6);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "phpfclose", NULL, 0, &pointer);
		zephir_check_call_status();
		RETURN_MM_NULL();
	...
	zephir_check_call_status();
	ZVAL_LONG(&_2, 3);
	ZEPHIR_CALL_FUNCTION(NULL, "flock", NULL, 280, &pointer, &_2);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "phpfclose", NULL, 0, &pointer);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamContext, ensureDir)
{
	...
	ZEPHIR_CALL_FUNCTION(&_1, "is_dir", NULL, 283, &_0);
	zephir_check_call_status();
	if (!(zephir_is_true(&_1))) {
		zephir_read_property(&_2$$3, this_ptr, ZEND_STRL("storageDir"), PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_3$$3, 0777);
		ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 284, &_2$$3, &_3$$3, &__$true);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
}
