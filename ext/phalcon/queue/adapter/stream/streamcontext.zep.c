...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamContext, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1280, &_2);
	ZVAL_UNDEF(&_3);
	ZVAL_LONG(&_3, pollInterval);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1281, &_3);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamContext, ensureDir)
{
	...
	zephir_check_call_status();
	if (!(zephir_is_true(&_1))) {
		zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_0, 1280, PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_3$$3, 0777);
