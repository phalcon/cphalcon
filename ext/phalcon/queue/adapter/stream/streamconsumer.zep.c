...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamConsumer, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1289, queue);
	ZVAL_UNDEF(&_0);
	ZVAL_LONG(&_0, pollInterval);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1290, &_0);
}
...
PHP_METHOD(Phalcon_Queue_Adapter_Stream_StreamConsumer, reject)
{
	...
	if (requeue) {
		zephir_read_property_cached(&_0$$3, this_ptr, _zephir_prop_0, 1288, PH_NOISY_CC | PH_READONLY);
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_1, 1289, PH_NOISY_CC | PH_READONLY);
