...
PHP_METHOD(Phalcon_Queue_Beanstalk, connect) {
	...
	ZVAL_LONG(&_9, -1);
	ZEPHIR_CALL_FUNCTION(NULL, "stream_set_timeout", NULL, 414, &connection, &_9, &__$null);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Queue_Beanstalk, read) {
	...
	if (length) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$5, "stream_get_meta_data", NULL, 420, &connection);
		zephir_check_call_status();
		...
		ZVAL_STRING(&_6$$8, "\r\n");
		ZEPHIR_CALL_FUNCTION(&data, "stream_get_line", NULL, 419, &connection, &_5$$8, &_6$$8);
		zephir_check_call_status();
	}
