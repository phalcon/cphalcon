...
PHP_METHOD(Phalcon_Queue_Beanstalk, connect) {
	...
	ZVAL_LONG(&_9, -1);
	ZEPHIR_CALL_FUNCTION(NULL, "stream_set_timeout", NULL, 407, &connection, &_9, &__$null);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Queue_Beanstalk, read) {
	...
	if (length) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$5, "stream_get_meta_data", NULL, 413, &connection);
		zephir_check_call_status();
		...
		ZVAL_STRING(&_7$$8, "\r\n");
		ZEPHIR_CALL_FUNCTION(&data, "stream_get_line", &_2, 412, &connection, &_6$$8, &_7$$8);
		zephir_check_call_status();
	}
