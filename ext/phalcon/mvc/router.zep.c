...
PHP_METHOD(Phalcon_Mvc_Router, __construct) {
	...
	if (defaultRoutes) {
		...
		zephir_array_append(&routes, &_0$$3, PH_SEPARATE, "phalcon/Mvc/Router.zep", 92);
		...
		zephir_check_call_status();
		zephir_array_append(&routes, &_2$$3, PH_SEPARATE, "phalcon/Mvc/Router.zep", 98);
	}
...
PHP_METHOD(Phalcon_Mvc_Router, attach) {
	...
	do {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_router_exception_ce, "Invalid route position", "phalcon/Mvc/Router.zep", 238);
		return;
	} while(0);
...
PHP_METHOD(Phalcon_Mvc_Router, getRouteById) {
	...
	if (zephir_array_isset_fetch(&key, &_0, id, 0 TSRMLS_CC)) {
		...
		zephir_array_fetch(&_2$$3, &_1$$3, &key, PH_NOISY | PH_READONLY, "phalcon/Mvc/Router.zep", 332 TSRMLS_CC);
		RETURN_CTOR(&_2$$3);
	}
	zephir_read_property(&_3, this_ptr, SL("routes"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_3, 0, "phalcon/Mvc/Router.zep", 343);
...
PHP_METHOD(Phalcon_Mvc_Router, getRouteByName) {
	...
	if (zephir_array_isset_fetch(&key, &_0, &name, 0 TSRMLS_CC)) {
		zephir_read_property(&_1$$3, this_ptr, SL("routes"), PH_NOISY_CC | PH_READONLY);
		zephir_array_fetch(&_2$$3, &_1$$3, &key, PH_NOISY | PH_READONLY, "phalcon/Mvc/Router.zep", 354 TSRMLS_CC);
		RETURN_CTOR(&_2$$3);
	}
	zephir_read_property(&_3, this_ptr, SL("routes"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_3, 0, "phalcon/Mvc/Router.zep", 367);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_3), _5, _6, _4)
	{
		ZEPHIR_INIT_NVAR(&key);
		if (_6 != NULL) { 
...
PHP_METHOD(Phalcon_Mvc_Router, handle) {
	...
	zephir_read_property(&_3, this_ptr, SL("routes"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_3, 0, "phalcon/Mvc/Router.zep", 634);
	ZEND_HASH_REVERSE_FOREACH_VAL(Z_ARRVAL_P(&_3), _5)
	{
		...
		if (Z_TYPE_P(&methods) != IS_NULL) {
			if (Z_TYPE_P(&request) == IS_NULL) {
				...
				if (Z_TYPE_P(&container) != IS_OBJECT) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_7$$9, "phalcon/Mvc/Router.zep", 439 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
		...
		if (Z_TYPE_P(&hostname) != IS_NULL) {
			if (Z_TYPE_P(&request) == IS_NULL) {
				...
				if (Z_TYPE_P(&container) != IS_OBJECT) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_17$$13, "phalcon/Mvc/Router.zep", 466 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			...
			if (zephir_memnstr_str(&hostname, SL("("), "phalcon/Mvc/Router.zep", 489)) {
				if (!(zephir_memnstr_str(&hostname, SL("#"), "phalcon/Mvc/Router.zep", 490))) {
					ZEPHIR_INIT_NVAR(&regexHostName);
					ZEPHIR_CONCAT_SV(&regexHostName, "#^", &hostname);
					if (!(zephir_memnstr_str(&hostname, SL(":"), "phalcon/Mvc/Router.zep", 492))) {
		...
		zephir_check_call_status();
		if (zephir_memnstr_str(&pattern, SL("^"), "phalcon/Mvc/Router.zep", 518)) {
		...
		if (zephir_is_true(&routeFound)) {
			...
			if (Z_TYPE_P(&beforeMatch) != IS_NULL) {
				if (!(zephir_is_callable(&beforeMatch TSRMLS_CC))) {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_router_exception_ce, "Before-Match callback is not callable in matched route", "phalcon/Mvc/Router.zep", 540);
					return;
				}
		...
		if (zephir_is_true(&routeFound)) {
			...
			if (Z_TYPE_P(&matches) == IS_ARRAY) {
				...
				zephir_check_call_status();
				zephir_is_iterable(&paths, 0, "phalcon/Mvc/Router.zep", 623);
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&paths), _31$$32, _32$$32, _30$$32)
				{
					...
					if (Z_TYPE_P(&part) != IS_STRING) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_33$$34, "phalcon/Mvc/Router.zep", 576 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
...
PHP_METHOD(Phalcon_Mvc_Router, mount) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *group, group_sub, groupRoutes, beforeMatch, hostname, routes, route, eventsManager, _0, _1$$3, *_2$$5, *_3$$7, _4$$9;
	zval *this_ptr = getThis();
	...
	if (!(zephir_fast_count_int(&groupRoutes TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_router_exception_ce, "The group of routes does not contain any routes", "phalcon/Mvc/Router.zep", 750);
		return;
	}
	ZEPHIR_CALL_METHOD(&beforeMatch, group, "getbeforematch", NULL, 0);
	zephir_check_call_status();
	if (Z_TYPE_P(&beforeMatch) != IS_NULL) {
		zephir_is_iterable(&groupRoutes, 0, "phalcon/Mvc/Router.zep", 762);
	...
	zephir_check_call_status();
	if (Z_TYPE_P(&hostname) != IS_NULL) {
		zephir_is_iterable(&groupRoutes, 0, "phalcon/Mvc/Router.zep", 771);
	...
	if (Z_TYPE_P(&routes) == IS_ARRAY) {
		...
		zephir_fast_array_merge(&_4$$9, &routes, &groupRoutes TSRMLS_CC);
		zephir_update_property_zval(this_ptr, SL("routes"), &_4$$9);
	} else {
...
PHP_METHOD(Phalcon_Mvc_Router, notFound) {
	...
	if (_0) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalcon_mvc_router_exception_ce, "The not-found paths must be an array or string", "phalcon/Mvc/Router.zep", 790);
		return;
	}
