...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Micro_LazyLoader) {
	...
	zend_declare_property_null(phalcon_mvc_micro_lazyloader_ce, SL("handler"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_mvc_micro_lazyloader_ce, SL("definition"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Mvc_Micro_LazyLoader, __construct) {
	...
	if (UNEXPECTED(Z_TYPE_P(definition_param) != IS_STRING && Z_TYPE_P(definition_param) != IS_NULL)) {
		zephir_throw_exception_string(spl_ce_InvalidArgumentException, SL("Parameter 'definition' must be of the type string") TSRMLS_CC);
		RETURN_MM_NULL();
	}
...
PHP_METHOD(Phalcon_Mvc_Micro_LazyLoader, callMethod) {
	...
	zval *method_param = NULL, *arguments = NULL, arguments_sub, *modelBinder = NULL, modelBinder_sub, __$null, handler, definition, bindCacheKey, _0, _1$$3, _3$$4;
	zval method;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(modelBinder) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(&_3$$4, modelBinder, "bindtohandler", NULL, 0, &handler, arguments, &bindCacheKey, &method);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(arguments, &_3$$4);
	}
	ZEPHIR_INIT_VAR(&_4);
