...
PHP_METHOD(Phalcon_Mvc_Micro_Collection, setHandler)
{
	...
	if (isLazy) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 961, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 961, &__$false);
	}
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Mvc_Micro_Collection, setPrefix)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 962, &prefix_zv);
	RETURN_THISW();
}
