...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Micro_Collection) {
	...
	zend_declare_property_null(phalcon_mvc_micro_collection_ce, SL("handler"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_mvc_micro_collection_ce, SL("handlers"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Mvc_Micro_Collection, setHandler) {
	...
	if (lazy) {
		zephir_update_property_zval(this_ptr, SL("lazy"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, SL("lazy"), &__$false);
	}
	RETURN_THISW();
