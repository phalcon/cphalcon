...
PHP_METHOD(Phalcon_Mvc_Collection_Behavior_SoftDelete, notify) {
	...
	if (ZEPHIR_IS_STRING(&type, "beforeDelete")) {
		...
		if (!(zephir_array_isset_string_fetch(&value, &options, SL("value"), 0))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_collection_exception_ce, "The option 'value' is required", "phalcon/Mvc/Collection/Behavior/SoftDelete.zep", 40);
			return;
		}
		ZEPHIR_OBS_VAR(&field);
		if (!(zephir_array_isset_string_fetch(&field, &options, SL("field"), 0))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_collection_exception_ce, "The option 'field' is required", "phalcon/Mvc/Collection/Behavior/SoftDelete.zep", 47);
			return;
		}
		ZVAL_BOOL(&_0$$3, 1);
		...
		if (!ZEPHIR_IS_EQUAL(&_1$$3, &value)) {
			...
			if (!(zephir_is_true(&_2$$6))) {
				...
				zephir_check_call_status();
				zephir_is_iterable(&_3$$7, 0, "phalcon/Mvc/Collection/Behavior/SoftDelete.zep", 79);
