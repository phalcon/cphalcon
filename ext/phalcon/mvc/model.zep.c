...
PHP_METHOD(Phalcon_Mvc_Model, __construct) {
	...
	if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "A dependency injector container is required to obtain the services related to the ORM", "phalcon/mvc/model.zep", 142);
		return;
	}
...
PHP_METHOD(Phalcon_Mvc_Model, getModelsMetaData) {
	...
	if (Z_TYPE_P(&metaData) != IS_OBJECT) {
		...
		if (Z_TYPE_P(&metaData) != IS_OBJECT) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The injected service 'modelsMetadata' is not valid", "phalcon/mvc/model.zep", 228);
			return;
		}
...
PHP_METHOD(Phalcon_Mvc_Model, assign) {
	...
	if (Z_TYPE_P(dataColumnMap) == IS_ARRAY) {
		...
		zephir_is_iterable(&data, 0, "phalcon/mvc/model.zep", 464);
	...
	zephir_check_call_status();
	zephir_is_iterable(&_3, 0, "phalcon/mvc/model.zep", 513);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3), _4)
	{
		...
		if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
			...
			if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &attribute, 0 TSRMLS_CC))) {
				if (!(ZEPHIR_GLOBAL(orm).ignore_unknown_columns)) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_5$$13, "phalcon/mvc/model.zep", 486 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				} else {
					continue;
		...
		if (zephir_array_isset_fetch(&value, &dataMapped, &attributeField, 0 TSRMLS_CC)) {
			...
			ZEPHIR_CALL_METHOD(&_8$$16, this_ptr, "_possiblesetter", &_9, 316, &attributeField, &value);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model, cloneResultMap) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&data, 0, "phalcon/mvc/model.zep", 615);
		...
		if (Z_TYPE_P(&key) == IS_STRING) {
			...
			if (!(zephir_array_isset_fetch(&attribute, columnMap, &key, 0 TSRMLS_CC))) {
				if (!(ZEPHIR_GLOBAL(orm).ignore_unknown_columns)) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_4$$7, "phalcon/mvc/model.zep", 558 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				} else {
					continue;
			...
			if (_7$$4) {
				zephir_array_fetch_long(&_8$$10, &attribute, 1, PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 570 TSRMLS_CC);
				do {
				...
				zephir_array_fetch_long(&_11$$15, &attribute, 1, PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 591 TSRMLS_CC);
				do {
			...
			zephir_array_fetch_long(&attributeName, &attribute, 0, PH_NOISY, "phalcon/mvc/model.zep", 607 TSRMLS_CC);
		...
		if (Z_TYPE_P(columnMap) == IS_ARRAY) {
			...
			if (!(zephir_array_isset_fetch(&attribute, columnMap, &key, 0 TSRMLS_CC))) {
				if (!(ZEPHIR_GLOBAL(orm).ignore_unknown_columns)) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_3$$11, "phalcon/mvc/model.zep", 671 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				} else {
					continue;
			...
			if (Z_TYPE_P(&attribute) == IS_ARRAY) {
				ZEPHIR_OBS_NVAR(&attributeName);
				zephir_array_fetch_long(&attributeName, &attribute, 0, PH_NOISY, "phalcon/mvc/model.zep", 681 TSRMLS_CC);
			} else {
...
PHP_METHOD(Phalcon_Mvc_Model, cloneResult) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _2, _3, _1)
	{
		...
		ZVAL_COPY(&value, _1);
		if (Z_TYPE_P(&key) != IS_STRING) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Invalid key in array data provided to dumpResult()", "phalcon/mvc/model.zep", 742);
			return;
		}
		zephir_update_property_zval_zval(&instance, &key, &value TSRMLS_CC);
	} ZEND_HASH_FOREACH_END();
...
PHP_METHOD(Phalcon_Mvc_Model, find) {
	...
	if (Z_TYPE_P(parameters) != IS_ARRAY) {
		...
		if (Z_TYPE_P(parameters) != IS_NULL) {
			zephir_array_append(&params, parameters, PH_SEPARATE, "phalcon/mvc/model.zep", 807);
		}
	} else {
...
PHP_METHOD(Phalcon_Mvc_Model, findFirst) {
	...
	if (Z_TYPE_P(parameters) != IS_ARRAY) {
		...
		array_init(&params);
		if (Z_TYPE_P(parameters) != IS_NULL) {
			zephir_array_append(&params, parameters, PH_SEPARATE, "phalcon/mvc/model.zep", 902);
		}
	} else {
		ZEPHIR_CPY_WRT(&params, parameters);
	}
...
PHP_METHOD(Phalcon_Mvc_Model, query) {
	...
	if (zephir_instance_of_ev(dependencyInjector, phalcon_diinterface_ce TSRMLS_CC)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &criteria, "setdi", NULL, 317, dependencyInjector);
		zephir_check_call_status();
	}
	ZEPHIR_INIT_VAR(&_3);
	zephir_get_called_class(&_3 TSRMLS_CC);
	ZEPHIR_CALL_METHOD(NULL, &criteria, "setmodelname", NULL, 318, &_3);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model, _exists) {
	...
	if (Z_TYPE_P(&uniqueKey) == IS_NULL) {
		...
		zephir_is_iterable(&primaryKeys, 0, "phalcon/mvc/model.zep", 1073);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&primaryKeys), _0$$3)
		{
			...
			if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &field, 0 TSRMLS_CC))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_1$$9, "phalcon/mvc/model.zep", 1037 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			} else {
			...
			if (zephir_fetch_property_zval(&value, this_ptr, &attributeField, PH_SILENT_CC)) {
				...
				zephir_array_append(&uniqueParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1055);
			} else {
				zephir_array_append(&uniqueParams, &__$null, PH_SEPARATE, "phalcon/mvc/model.zep", 1058);
				numberEmpty++;
			}
			...
			if (!(zephir_array_isset_fetch(&type, &bindDataTypes, &field, 0 TSRMLS_CC))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_5$$14, "phalcon/mvc/model.zep", 1063 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			zephir_array_append(&uniqueTypes, &type, PH_SEPARATE, "phalcon/mvc/model.zep", 1066);
			ZEPHIR_CALL_METHOD(&_7$$7, connection, "escapeidentifier", &_8, 0, &field);
			zephir_check_call_status();
			ZEPHIR_INIT_LNVAR(_9$$7);
			ZEPHIR_CONCAT_VS(&_9$$7, &_7$$7, " = ?");
			zephir_array_append(&wherePk, &_9$$7, PH_SEPARATE, "phalcon/mvc/model.zep", 1067);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&field);
	...
	zephir_check_call_status();
	zephir_array_fetch_string(&_14, &num, SL("rowcount"), PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 1123 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Mvc_Model, _groupResult) {
	...
	if (Z_TYPE_P(parameters) != IS_ARRAY) {
		...
		array_init(&params);
		if (Z_TYPE_P(parameters) != IS_NULL) {
			zephir_array_append(&params, parameters, PH_SEPARATE, "phalcon/mvc/model.zep", 1153);
		}
	} else {
		ZEPHIR_CPY_WRT(&params, parameters);
	}
...
PHP_METHOD(Phalcon_Mvc_Model, validate) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *validator, validator_sub, messages, message, _0, _2$$4, _3$$4, _4$$4, _5$$4;
	ZEPHIR_INIT_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model, getMessages) {
	...
	if (_0) {
		...
		zephir_is_iterable(&_1$$3, 0, "phalcon/mvc/model.zep", 1591);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_1$$3), _2$$3)
		{
			...
			if (ZEPHIR_IS_EQUAL(&_3$$4, filter)) {
				zephir_array_append(&filtered, &message, PH_SEPARATE, "phalcon/mvc/model.zep", 1588);
			}
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&message);
...
PHP_METHOD(Phalcon_Mvc_Model, _checkForeignKeysRestrict) {
	...
	zephir_check_call_status();
	error = 0;
	zephir_is_iterable(&belongsTo, 0, "phalcon/mvc/model.zep", 1736);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&belongsTo), _1)
	{
		...
		if (Z_TYPE_P(&foreignKey) == IS_ARRAY) {
			if (zephir_array_isset_string(&foreignKey, SL("action"))) {
				...
				zephir_array_fetch_string(&_2$$6, &foreignKey, SL("action"), PH_NOISY, "phalcon/mvc/model.zep", 1638 TSRMLS_CC);
		...
		if (Z_TYPE_P(&fields) == IS_ARRAY) {
			zephir_is_iterable(&fields, 0, "phalcon/mvc/model.zep", 1676);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&fields), _6$$8, _7$$8, _5$$8)
			{
				...
				zephir_array_fetch(&_8$$9, &referencedFields, &position, PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 1669 TSRMLS_CC);
				ZEPHIR_INIT_LNVAR(_9$$9);
				ZEPHIR_CONCAT_SVSV(&_9$$9, "[", &_8$$9, "] = ?", &position);
				zephir_array_append(&conditions, &_9$$9, PH_SEPARATE, "phalcon/mvc/model.zep", 1669);
				zephir_array_append(&bindParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1670);
			...
			zephir_array_append(&conditions, &_10$$11, PH_SEPARATE, "phalcon/mvc/model.zep", 1681);
			zephir_array_append(&bindParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1682);
			if (Z_TYPE_P(&value) == IS_NULL) {
				validateWithNulls = 1;
			}
		}
		...
		if (zephir_array_isset_string_fetch(&extraConditions, &foreignKey, SL("conditions"), 0)) {
			zephir_array_append(&conditions, &extraConditions, PH_SEPARATE, "phalcon/mvc/model.zep", 1693);
		}
		...
		if (_11$$3) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_16$$17, "__construct", &_18, 8, &message, &fields, &_17$$17);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model, _checkForeignKeysReverseCascade) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&relations, 0, "phalcon/mvc/model.zep", 1849);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&relations), _1)
	{
		...
		if (Z_TYPE_P(&foreignKey) == IS_ARRAY) {
			if (zephir_array_isset_string(&foreignKey, SL("action"))) {
				ZEPHIR_OBS_NVAR(&_2$$6);
				zephir_array_fetch_string(&_2$$6, &foreignKey, SL("action"), PH_NOISY, "phalcon/mvc/model.zep", 1788 TSRMLS_CC);
				action = zephir_get_intval(&_2$$6);
			}
		}
		if (action != 2) {
		...
		array_init(&bindParams);
		if (Z_TYPE_P(&fields) == IS_ARRAY) {
			zephir_is_iterable(&fields, 0, "phalcon/mvc/model.zep", 1818);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&fields), _6$$8, _7$$8, _5$$8)
			{
				ZEPHIR_INIT_NVAR(&position);
				if (_7$$8 != NULL) { 
				...
				ZEPHIR_OBS_NVAR(&value);
				zephir_fetch_property_zval(&value, this_ptr, &field, PH_SILENT_CC);
				zephir_array_fetch(&_8$$9, &referencedFields, &position, PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 1815 TSRMLS_CC);
				ZEPHIR_INIT_LNVAR(_9$$9);
				ZEPHIR_CONCAT_SVSV(&_9$$9, "[", &_8$$9, "] = ?", &position);
				zephir_array_append(&conditions, &_9$$9, PH_SEPARATE, "phalcon/mvc/model.zep", 1815);
				zephir_array_append(&bindParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1816);
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&field);
			ZEPHIR_INIT_NVAR(&position);
		} else {
			...
			zephir_array_append(&conditions, &_10$$10, PH_SEPARATE, "phalcon/mvc/model.zep", 1820);
			zephir_array_append(&bindParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1821);
		}
		ZEPHIR_OBS_NVAR(&extraConditions);
		if (zephir_array_isset_string_fetch(&extraConditions, &foreignKey, SL("conditions"), 0)) {
			zephir_array_append(&conditions, &extraConditions, PH_SEPARATE, "phalcon/mvc/model.zep", 1828);
		}
...
PHP_METHOD(Phalcon_Mvc_Model, _checkForeignKeysReverseRestrict) {
	...
	zephir_check_call_status();
	error = 0;
	zephir_is_iterable(&relations, 0, "phalcon/mvc/model.zep", 1967);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&relations), _1)
	{
		ZEPHIR_INIT_NVAR(&relation);
		ZVAL_COPY(&relation, _1);
		...
		if (Z_TYPE_P(&foreignKey) == IS_ARRAY) {
			if (zephir_array_isset_string(&foreignKey, SL("action"))) {
				ZEPHIR_OBS_NVAR(&_2$$6);
				zephir_array_fetch_string(&_2$$6, &foreignKey, SL("action"), PH_NOISY, "phalcon/mvc/model.zep", 1895 TSRMLS_CC);
				action = zephir_get_intval(&_2$$6);
			}
		}
		if (action != 1) {
		...
		array_init(&bindParams);
		if (Z_TYPE_P(&fields) == IS_ARRAY) {
			zephir_is_iterable(&fields, 0, "phalcon/mvc/model.zep", 1929);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&fields), _5$$8, _6$$8, _4$$8)
			{
				...
				ZEPHIR_OBS_NVAR(&value);
				zephir_fetch_property_zval(&value, this_ptr, &field, PH_SILENT_CC);
				zephir_array_fetch(&_7$$9, &referencedFields, &position, PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 1925 TSRMLS_CC);
				ZEPHIR_INIT_LNVAR(_8$$9);
				ZEPHIR_CONCAT_SVSV(&_8$$9, "[", &_7$$9, "] = ?", &position);
				zephir_array_append(&conditions, &_8$$9, PH_SEPARATE, "phalcon/mvc/model.zep", 1925);
				zephir_array_append(&bindParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1926);
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&field);
			ZEPHIR_INIT_NVAR(&position);
		} else {
			...
			zephir_array_append(&conditions, &_9$$10, PH_SEPARATE, "phalcon/mvc/model.zep", 1931);
			zephir_array_append(&bindParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 1932);
		}
		ZEPHIR_OBS_NVAR(&extraConditions);
		if (zephir_array_isset_string_fetch(&extraConditions, &foreignKey, SL("conditions"), 0)) {
			zephir_array_append(&conditions, &extraConditions, PH_SEPARATE, "phalcon/mvc/model.zep", 1939);
		}
		ZEPHIR_INIT_NVAR(&_11$$3);
		zephir_create_array(&_11$$3, 2, 0 TSRMLS_CC);
		ZEPHIR_INIT_NVAR(&_12$$3);
		...
		if (zephir_is_true(&_10$$3)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_13$$12, "__construct", &_15, 8, &message, &fields, &_14$$12);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model, _preSave) {
	...
	if (ZEPHIR_GLOBAL(orm).virtual_foreign_keys) {
		ZEPHIR_CALL_METHOD(&_7$$9, this_ptr, "_checkforeignkeysrestrict", NULL, 319);
		zephir_check_call_status();
	...
	if (ZEPHIR_GLOBAL(orm).not_null_validations) {
		...
		if (Z_TYPE_P(&notNull) == IS_ARRAY) {
			...
			zephir_check_call_status();
			error = 0;
			zephir_is_iterable(&notNull, 0, "phalcon/mvc/model.zep", 2133);
				...
				if (!(zephir_array_isset(&automaticAttributes, &field))) {
					...
					if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
						...
						if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &field, 0 TSRMLS_CC))) {
							...
							zephir_check_call_status();
							zephir_throw_exception_debug(&_9$$20, "phalcon/mvc/model.zep", 2068 TSRMLS_CC);
							ZEPHIR_MM_RESTORE();
							return;
						}
					} else {
...
PHP_METHOD(Phalcon_Mvc_Model, _doLowInsert) {
	...
	zephir_is_iterable(&attributes, 0, "phalcon/mvc/model.zep", 2317);
		...
		if (!(zephir_array_isset(&automaticAttributes, &field))) {
			if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &field, 0 TSRMLS_CC))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_1$$8, "phalcon/mvc/model.zep", 2271 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			} else {
			...
			if (!ZEPHIR_IS_EQUAL(&field, identityField)) {
				...
				if (zephir_fetch_property_zval(&value, this_ptr, &attributeField, PH_SILENT_CC)) {
					...
					if (!(zephir_array_isset_fetch(&bindType, &bindDataTypes, &field, 0 TSRMLS_CC))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_6$$13, "phalcon/mvc/model.zep", 2295 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
					zephir_array_append(&fields, &field, PH_SEPARATE, "phalcon/mvc/model.zep", 2298);
					zephir_array_append(&values, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 2298);
					zephir_array_append(&bindTypes, &bindType, PH_SEPARATE, "phalcon/mvc/model.zep", 2298);
				} else {
					if (zephir_array_isset(&defaultValues, &field)) {
						ZEPHIR_CALL_METHOD(&_8$$15, connection, "getdefaultvalue", &_9, 0);
						zephir_check_call_status();
						zephir_array_append(&values, &_8$$15, PH_SEPARATE, "phalcon/mvc/model.zep", 2303);
					} else {
						zephir_array_append(&values, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 2305);
					}
					zephir_array_append(&fields, &field, PH_SEPARATE, "phalcon/mvc/model.zep", 2308);
					zephir_array_append(&bindTypes, &bindSkip, PH_SEPARATE, "phalcon/mvc/model.zep", 2308);
				}
			}
		}
	} ZEND_HASH_FOREACH_END();
	...
	if (!ZEPHIR_IS_FALSE_IDENTICAL(identityField)) {
		...
		if (useExplicitIdentity) {
			zephir_array_append(&fields, identityField, PH_SEPARATE, "phalcon/mvc/model.zep", 2326);
		}
		if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
			ZEPHIR_OBS_NVAR(&attributeField);
			if (!(zephir_array_isset_fetch(&attributeField, &columnMap, identityField, 0 TSRMLS_CC))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_11$$20, "phalcon/mvc/model.zep", 2334 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} else {
		...
		if (zephir_fetch_property_zval(&value, this_ptr, &attributeField, PH_SILENT_CC)) {
			...
			if (_13$$22) {
				if (useExplicitIdentity) {
					zephir_array_append(&values, &defaultValue, PH_SEPARATE, "phalcon/mvc/model.zep", 2347);
					zephir_array_append(&bindTypes, &bindSkip, PH_SEPARATE, "phalcon/mvc/model.zep", 2347);
				}
			} else {
				if (!(useExplicitIdentity)) {
				...
				if (!(zephir_array_isset_fetch(&bindType, &bindDataTypes, identityField, 0 TSRMLS_CC))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_14$$27, "phalcon/mvc/model.zep", 2362 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
				zephir_array_append(&values, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 2365);
				zephir_array_append(&bindTypes, &bindType, PH_SEPARATE, "phalcon/mvc/model.zep", 2365);
			}
		} else {
			if (useExplicitIdentity) {
				zephir_array_append(&values, &defaultValue, PH_SEPARATE, "phalcon/mvc/model.zep", 2369);
				zephir_array_append(&bindTypes, &bindSkip, PH_SEPARATE, "phalcon/mvc/model.zep", 2369);
			}
		}
	}
...
PHP_METHOD(Phalcon_Mvc_Model, _doLowUpdate) {
	...
	zephir_is_iterable(&nonPrimary, 0, "phalcon/mvc/model.zep", 2576);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&nonPrimary), _2)
	{
		...
		if (!(zephir_array_isset(&automaticAttributes, &field))) {
			...
			if (!(zephir_array_isset_fetch(&bindType, &bindDataTypes, &field, 0 TSRMLS_CC))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_3$$9, "phalcon/mvc/model.zep", 2470 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &field, 0 TSRMLS_CC))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_6$$11, "phalcon/mvc/model.zep", 2478 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			} else {
			...
			if (zephir_fetch_property_zval(&value, this_ptr, &attributeField, PH_SILENT_CC)) {
				if (!(useDynamicUpdate)) {
					zephir_array_append(&fields, &field, PH_SEPARATE, "phalcon/mvc/model.zep", 2494);
					zephir_array_append(&values, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 2494);
					zephir_array_append(&bindTypes, &bindType, PH_SEPARATE, "phalcon/mvc/model.zep", 2495);
				} else {
						...
						if (Z_TYPE_P(&value) == IS_NULL) {
							...
							if (Z_TYPE_P(&snapshotValue) == IS_NULL) {
								...
								if (!(zephir_array_isset_fetch(&dataType, &dataTypes, &field, 0 TSRMLS_CC))) {
									...
									ZEPHIR_CALL_METHOD(NULL, &_8$$22, "__construct", &_5, 9, &_9$$22);
									zephir_check_call_status();
									zephir_throw_exception_debug(&_8$$22, "phalcon/mvc/model.zep", 2520 TSRMLS_CC);
									ZEPHIR_MM_RESTORE();
									return;
								}
								do {
									...
									if (ZEPHIR_IS_LONG(&dataType, 3) || ZEPHIR_IS_LONG(&dataType, 7)) {
										ZEPHIR_CALL_FUNCTION(&_10$$25, "floatval", &_11, 320, &snapshotValue);
										zephir_check_call_status();
										ZEPHIR_CALL_FUNCTION(&_12$$25, "floatval", &_11, 320, &value);
										zephir_check_call_status();
					...
					if (changed) {
						zephir_array_append(&fields, &field, PH_SEPARATE, "phalcon/mvc/model.zep", 2562);
						zephir_array_append(&values, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 2562);
						zephir_array_append(&bindTypes, &bindType, PH_SEPARATE, "phalcon/mvc/model.zep", 2563);
					}
				}
			} else {
				zephir_array_append(&fields, &field, PH_SEPARATE, "phalcon/mvc/model.zep", 2568);
				zephir_array_append(&values, &__$null, PH_SEPARATE, "phalcon/mvc/model.zep", 2568);
				zephir_array_append(&bindTypes, &bindSkip, PH_SEPARATE, "phalcon/mvc/model.zep", 2568);
			}
		}
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&field);
	...
	if (Z_TYPE_P(&uniqueParams) != IS_ARRAY) {
		...
		if (!(zephir_fast_count_int(&primaryKeys TSRMLS_CC))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "A primary key must be defined in the model in order to perform the operation", "phalcon/mvc/model.zep", 2595);
			return;
		}
		ZEPHIR_INIT_NVAR(&uniqueParams);
		array_init(&uniqueParams);
		zephir_is_iterable(&primaryKeys, 0, "phalcon/mvc/model.zep", 2618);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&primaryKeys), _15$$31)
		{
			...
			if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &field, 0 TSRMLS_CC))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_16$$35, "__construct", &_5, 9, &_17$$35);
					zephir_check_call_status();
					zephir_throw_exception_debug(&_16$$35, "phalcon/mvc/model.zep", 2606 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			} else {
			...
			ZEPHIR_OBS_NVAR(&value);
			if (zephir_fetch_property_zval(&value, this_ptr, &attributeField, PH_SILENT_CC)) {
				zephir_array_append(&uniqueParams, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 2613);
			} else {
				zephir_array_append(&uniqueParams, &__$null, PH_SEPARATE, "phalcon/mvc/model.zep", 2615);
			}
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&field);
	}
	...
	ZEPHIR_RETURN_CALL_METHOD(connection, "update", NULL, 0, table, &fields, &values, &_18, &bindTypes);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Model, _preSaveRelatedRecords) {
	...
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&manager, &_1);
	zephir_is_iterable(related, 0, "phalcon/mvc/model.zep", 2724);
		...
		if (Z_TYPE_P(&relation) == IS_OBJECT) {
			...
			if (ZEPHIR_IS_LONG(&type, 0)) {
				if (Z_TYPE_P(&record) != IS_OBJECT) {
					...
					zephir_check_call_status();
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Only objects can be stored as part of belongs-to relations", "phalcon/mvc/model.zep", 2673);
					return;
				}
				...
				if (Z_TYPE_P(&columns) == IS_ARRAY) {
					...
					zephir_check_call_status();
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Not implemented", "phalcon/mvc/model.zep", 2682);
					return;
				}
				...
				if (!(zephir_is_true(&_11$$5))) {
					...
					zephir_check_call_status();
					zephir_is_iterable(&_12$$8, 0, "phalcon/mvc/model.zep", 2711);
...
PHP_METHOD(Phalcon_Mvc_Model, _postSaveRelatedRecords) {
	...
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&manager, &_0);
	zephir_is_iterable(related, 0, "phalcon/mvc/model.zep", 2915);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(related), _2, _3, _1)
	{
		...
		if (Z_TYPE_P(&relation) == IS_OBJECT) {
			...
			if (_7$$4) {
				...
				zephir_check_call_status();
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Only objects/arrays can be stored as part of has-many/has-one/has-many-to-many relations", "phalcon/mvc/model.zep", 2763);
				return;
			}
			ZEPHIR_CALL_METHOD(&columns, &relation, "getfields", NULL, 0);
			zephir_check_call_status();
			...
			if (Z_TYPE_P(&columns) == IS_ARRAY) {
				...
				zephir_check_call_status();
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Not implemented", "phalcon/mvc/model.zep", 2772);
				return;
			}
			if (Z_TYPE_P(&record) == IS_OBJECT) {
			...
			if (!(zephir_fetch_property_zval(&value, this_ptr, &columns, PH_SILENT_CC))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_14$$10, "phalcon/mvc/model.zep", 2786 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			zephir_is_iterable(&relatedRecords, 0, "phalcon/mvc/model.zep", 2901);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&relatedRecords), _18$$4)
			{
				...
				if (!(zephir_is_true(&_19$$12))) {
					...
					zephir_check_call_status();
					zephir_is_iterable(&_20$$14, 0, "phalcon/mvc/model.zep", 2843);
				...
				if (isThrough) {
					...
					if (!(zephir_is_true(&_27$$17))) {
						...
						zephir_check_call_status();
						zephir_is_iterable(&_28$$18, 0, "phalcon/mvc/model.zep", 2895);
			...
			if (Z_TYPE_P(&record) != IS_ARRAY) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_34$$22, "phalcon/mvc/model.zep", 2907 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
...
PHP_METHOD(Phalcon_Mvc_Model, save) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&_7)) {
		...
		if (ZEPHIR_GLOBAL(orm).exception_on_failed_save) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, &_9$$12, "__construct", NULL, 321, this_ptr, &_10$$12);
			zephir_check_call_status();
			zephir_throw_exception_debug(&_9$$12, "phalcon/mvc/model.zep", 3029 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		RETURN_MM_BOOL(0);
...
PHP_METHOD(Phalcon_Mvc_Model, delete) {
	...
	if (ZEPHIR_GLOBAL(orm).virtual_foreign_keys) {
		ZEPHIR_CALL_METHOD(&_2$$3, this_ptr, "_checkforeignkeysreverserestrict", NULL, 322);
		zephir_check_call_status();
	...
	if (!(zephir_fast_count_int(&primaryKeys TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "A primary key must be defined in the model in order to perform the operation", "phalcon/mvc/model.zep", 3230);
		return;
	}
	zephir_is_iterable(&primaryKeys, 0, "phalcon/mvc/model.zep", 3273);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&primaryKeys), _3)
	{
		...
		if (!(zephir_array_isset_fetch(&bindType, &bindDataTypes, &primaryKey, 0 TSRMLS_CC))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_4$$9, "phalcon/mvc/model.zep", 3242 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
			...
			if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &primaryKey, 0 TSRMLS_CC))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_7$$11, "phalcon/mvc/model.zep", 3250 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} else {
		...
		if (!(zephir_fetch_property_zval(&value, this_ptr, &attributeField, PH_SILENT_CC))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_9$$13, "phalcon/mvc/model.zep", 3262 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		zephir_array_append(&values, &value, PH_SEPARATE, "phalcon/mvc/model.zep", 3268);
		...
		zephir_array_append(&conditions, &_13$$8, PH_SEPARATE, "phalcon/mvc/model.zep", 3269);
		zephir_array_append(&bindTypes, &bindType, PH_SEPARATE, "phalcon/mvc/model.zep", 3270);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&primaryKey);
	if (ZEPHIR_GLOBAL(orm).events) {
		if (0) {
	...
	zephir_check_call_status();
	if (ZEPHIR_GLOBAL(orm).virtual_foreign_keys) {
		ZEPHIR_CALL_METHOD(&_18$$20, this_ptr, "_checkforeignkeysreversecascade", NULL, 323);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model, refresh) {
	...
	if (!ZEPHIR_IS_LONG(&_0, 0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The record cannot be refreshed because it does not exist or is deleted", "phalcon/mvc/model.zep", 3348);
		return;
	}
	ZEPHIR_CALL_METHOD(&metaData, this_ptr, "getmodelsmetadata", NULL, 0);
	zephir_check_call_status();
	...
	if (!(zephir_is_true(&uniqueKey))) {
		...
		if (!(zephir_is_true(&_1$$6))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The record cannot be refreshed because it does not exist or is deleted", "phalcon/mvc/model.zep", 3370);
			return;
		}
		ZEPHIR_OBS_NVAR(&uniqueKey);
		zephir_read_property(&uniqueKey, this_ptr, SL("_uniqueKey"), PH_NOISY_CC);
	...
	zephir_read_property(&uniqueParams, this_ptr, SL("_uniqueParams"), PH_NOISY_CC);
	if (Z_TYPE_P(&uniqueParams) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The record cannot be refreshed because it does not exist or is deleted", "phalcon/mvc/model.zep", 3378);
		return;
	}
	ZEPHIR_INIT_VAR(&fields);
	array_init(&fields);
	...
	zephir_check_call_status();
	zephir_is_iterable(&_2, 0, "phalcon/mvc/model.zep", 3392);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_2), _3)
	{
		...
		zephir_array_append(&fields, &_4$$9, PH_SEPARATE, "phalcon/mvc/model.zep", 3386);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&attribute);
...
PHP_METHOD(Phalcon_Mvc_Model, skipAttributesOnCreate) {
	...
	zephir_is_iterable(&attributes, 0, "phalcon/mvc/model.zep", 3502);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&attributes), _0)
	{
		ZEPHIR_INIT_NVAR(&attribute);
...
PHP_METHOD(Phalcon_Mvc_Model, allowEmptyStringValues) {
	...
	ZEPHIR_INIT_VAR(&keysAttributes);
	array_init(&keysAttributes);
	zephir_is_iterable(&attributes, 0, "phalcon/mvc/model.zep", 3566);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&attributes), _0)
	{
		ZEPHIR_INIT_NVAR(&attribute);
		ZVAL_COPY(&attribute, _0);
...
PHP_METHOD(Phalcon_Mvc_Model, setSnapshotData) {
	...
	if (Z_TYPE_P(columnMap) == IS_ARRAY) {
		...
		array_init(&snapshot);
		zephir_is_iterable(&data, 0, "phalcon/mvc/model.zep", 3791);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _1$$3, _2$$3, _0$$3)
		{
			ZEPHIR_INIT_NVAR(&key);
			if (_2$$3 != NULL) { 
			...
			if (!(zephir_array_isset_fetch(&attribute, columnMap, &key, 0 TSRMLS_CC))) {
				if (!(ZEPHIR_GLOBAL(orm).ignore_unknown_columns)) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_3$$7, "phalcon/mvc/model.zep", 3773 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				} else {
					continue;
			...
			if (Z_TYPE_P(&attribute) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_long_fetch(&attribute, &attribute, 0, 0 TSRMLS_CC))) {
					if (!(ZEPHIR_GLOBAL(orm).ignore_unknown_columns)) {
						...
						ZEPHIR_CALL_METHOD(NULL, &_6$$11, "__construct", &_5, 9, &_7$$11);
						zephir_check_call_status();
						zephir_throw_exception_debug(&_6$$11, "phalcon/mvc/model.zep", 3782 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					} else {
						continue;
...
PHP_METHOD(Phalcon_Mvc_Model, getChangedFields) {
	...
	zephir_read_property(&snapshot, this_ptr, SL("_snapshot"), PH_NOISY_CC);
	if (Z_TYPE_P(&snapshot) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The record doesn't have a valid data snapshot", "phalcon/mvc/model.zep", 3849);
		return;
	}
	zephir_read_property(&_0, this_ptr, SL("_dirtyState"), PH_NOISY_CC | PH_READONLY);
	if (!ZEPHIR_IS_LONG(&_0, 0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Change checking cannot be performed because the object has not been persisted or is deleted", "phalcon/mvc/model.zep", 3856);
		return;
	}
	ZEPHIR_CALL_METHOD(&metaData, this_ptr, "getmodelsmetadata", NULL, 0);
	zephir_check_call_status();
	...
	array_init(&changed);
	ZEPHIR_INIT_VAR(&_1);
	zephir_is_iterable(&allAttributes, 0, "phalcon/mvc/model.zep", 3910);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&allAttributes), _3, _4, _2)
	{
		...
		if (!(zephir_array_isset(&snapshot, &name))) {
			zephir_array_append(&changed, &name, PH_SEPARATE, "phalcon/mvc/model.zep", 3889);
			continue;
		}
		ZEPHIR_OBS_NVAR(&value);
		if (!(zephir_fetch_property_zval(&value, this_ptr, &name, PH_SILENT_CC))) {
			zephir_array_append(&changed, &name, PH_SEPARATE, "phalcon/mvc/model.zep", 3897);
			continue;
		}
		zephir_array_fetch(&_5$$7, &snapshot, &name, PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 3904 TSRMLS_CC);
		if (!ZEPHIR_IS_EQUAL(&value, &_5$$7)) {
			zephir_array_append(&changed, &name, PH_SEPARATE, "phalcon/mvc/model.zep", 3905);
			continue;
		}
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&_1);
...
PHP_METHOD(Phalcon_Mvc_Model, getRelated) {
	...
	if (Z_TYPE_P(&relation) != IS_OBJECT) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$3, "phalcon/mvc/model.zep", 3953 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZVAL_NULL(&_0);
...
PHP_METHOD(Phalcon_Mvc_Model, _invokeFinder) {
	...
	if (!(zephir_array_isset_long_fetch(&value, arguments, 0, 0 TSRMLS_CC))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$7, "phalcon/mvc/model.zep", 4059 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (zephir_array_isset(&attributes, &extraMethod)) {
		...
		if (zephir_array_isset(&attributes, &extraMethodFirst)) {
			...
			if (!(zephir_array_isset(&attributes, &field))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_7$$13, "phalcon/mvc/model.zep", 4093 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
...
PHP_METHOD(Phalcon_Mvc_Model, __call) {
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(&_2, "phalcon/mvc/model.zep", 4144 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Phalcon_Mvc_Model, __callStatic) {
	...
	if (Z_TYPE_P(&records) == IS_NULL) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/mvc/model.zep", 4160 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_CCTOR(records);
...
PHP_METHOD(Phalcon_Mvc_Model, __set) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		array_init(&related);
		zephir_is_iterable(value, 0, "phalcon/mvc/model.zep", 4220);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(value), _3$$6, _4$$6, _2$$6)
		{
			...
			if (Z_TYPE_P(&item) == IS_OBJECT) {
				if (zephir_instance_of_ev(&item, phalcon_mvc_modelinterface_ce TSRMLS_CC)) {
					zephir_array_append(&related, &item, PH_SEPARATE, "phalcon/mvc/model.zep", 4207);
				}
			} else {
	...
	if (zephir_is_true(&_11)) {
		...
		if (!(zephir_is_true(&_12$$14))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_13$$15, "phalcon/mvc/model.zep", 4237 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
...
PHP_METHOD(Phalcon_Mvc_Model, serialize) {
	...
	if (zephir_is_true(&_0)) {
		ZEPHIR_OBS_VAR(&snapshot);
		zephir_read_property(&snapshot, this_ptr, SL("_snapshot"), PH_NOISY_CC);
		if (!ZEPHIR_IS_EQUAL(&attributes, &snapshot)) {
			ZEPHIR_INIT_VAR(&_1$$4);
			zephir_create_array(&_1$$4, 2, 0 TSRMLS_CC);
			zephir_array_update_string(&_1$$4, SL("_attributes"), &attributes, PH_COPY | PH_SEPARATE);
			zephir_array_update_string(&_1$$4, SL("_snapshot"), &snapshot, PH_COPY | PH_SEPARATE);
			ZEPHIR_RETURN_CALL_FUNCTION("serialize", &_2, 65, &_1$$4);
			zephir_check_call_status();
			RETURN_MM();
		}
	}
	ZEPHIR_RETURN_CALL_FUNCTION("serialize", &_2, 65, &attributes);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Model, unserialize) {
	...
	if (Z_TYPE_P(&attributes) == IS_ARRAY) {
		...
		zephir_check_call_status();
		if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "A dependency injector container is required to obtain the services related to the ORM", "phalcon/mvc/model.zep", 4391);
			return;
		}
		zephir_update_property_zval(this_ptr, SL("_dependencyInjector"), &dependencyInjector);
		ZEPHIR_INIT_VAR(&_2$$3);
		...
		if (Z_TYPE_P(&manager) != IS_OBJECT) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The injected service 'modelsManager' is not valid", "phalcon/mvc/model.zep", 4404);
			return;
		}
		...
		if (zephir_is_true(&_1$$3)) {
			if (zephir_array_isset_string_fetch(&snapshot, &attributes, SL("_snapshot"), 1)) {
				zephir_update_property_zval(this_ptr, SL("_snapshot"), &snapshot);
				zephir_array_fetch_string(&_3$$7, &attributes, SL("_attributes"), PH_NOISY | PH_READONLY, "phalcon/mvc/model.zep", 4419 TSRMLS_CC);
				ZEPHIR_CPY_WRT(&attributes, &_3$$7);
			} else {
				zephir_update_property_zval(this_ptr, SL("_snapshot"), &attributes);
			}
		}
		zephir_is_iterable(&attributes, 0, "phalcon/mvc/model.zep", 4432);
...
PHP_METHOD(Phalcon_Mvc_Model, toArray) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/mvc/model.zep", 4500);
		...
		if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
			...
			if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &attribute, 0 TSRMLS_CC))) {
				if (!(ZEPHIR_GLOBAL(orm).ignore_unknown_columns)) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_2$$6, "phalcon/mvc/model.zep", 4478 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				} else {
					continue;
