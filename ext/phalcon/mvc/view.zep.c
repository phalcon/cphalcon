...
PHP_METHOD(Phalcon_Mvc_View, _loadTemplateEngines) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&engines)) {
		...
		if (Z_TYPE_P(&registeredEngines) != IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 384, this_ptr, &dependencyInjector);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View, _engineRender) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_1, 0, "phalcon/mvc/view.zep", 696);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_1), _2)
	{
		ZEPHIR_INIT_NVAR(&viewsDir);
		ZVAL_COPY(&viewsDir, _2);
		ZEPHIR_CALL_METHOD(&_3$$3, this_ptr, "_isabsolutepath", &_4, 385, &viewPath);
		zephir_check_call_status();
		...
		zephir_is_iterable(engines, 0, "phalcon/mvc/view.zep", 694);
	...
	if (notExists == 1) {
		...
		if (!(silence)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_23$$21, "__construct", NULL, 9, &_24$$21);
			zephir_check_call_status();
			zephir_throw_exception_debug(&_23$$21, "phalcon/mvc/view.zep", 706 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
...
PHP_METHOD(Phalcon_Mvc_View, exists) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/mvc/view.zep", 753);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _1)
	{
		ZEPHIR_INIT_NVAR(&viewsDir);
		ZVAL_COPY(&viewsDir, _1);
		ZEPHIR_INIT_NVAR(&_2$$4);
		zephir_is_iterable(&engines, 0, "phalcon/mvc/view.zep", 751);
...
PHP_METHOD(Phalcon_Mvc_View, render) {
	...
	if (renderLevel) {
		...
		if (renderLevel >= 2) {
			if (!(zephir_array_isset_long(&disabledLevels, 2))) {
				...
				silence = 0;
				zephir_is_iterable(&templatesBefore, 0, "phalcon/mvc/view.zep", 904);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&templatesBefore), _15$$20)
				{
					ZEPHIR_INIT_NVAR(&templateBefore);
					ZVAL_COPY(&templateBefore, _15$$20);
					ZEPHIR_INIT_LNVAR(_16$$21);
					ZEPHIR_CONCAT_VV(&_16$$21, &layoutsDir, &templateBefore);
					if (silence) {
						ZVAL_BOOL(&_17$$21, 1);
					} else {
						ZVAL_BOOL(&_17$$21, 0);
					...
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "_enginerender", &_13, 0, &engines, &_16$$21, &_17$$21, &_18$$21, &cache);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
				ZEPHIR_INIT_NVAR(&templateBefore);
				silence = 1;
		...
		if (renderLevel >= 3) {
			if (!(zephir_array_isset_long(&disabledLevels, 3))) {
				...
				ZVAL_LONG(&_19$$23, 3);
				zephir_update_property_zval(this_ptr, SL("_currentRenderLevel"), &_19$$23);
				ZEPHIR_INIT_VAR(&_20$$23);
				ZEPHIR_CONCAT_VV(&_20$$23, &layoutsDir, &layoutName);
				if (silence) {
					ZVAL_BOOL(&_19$$23, 1);
				} else {
					ZVAL_BOOL(&_19$$23, 0);
				...
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "_enginerender", &_13, 0, &engines, &_20$$23, &_19$$23, &_21$$23, &cache);
				zephir_check_call_status();
			}
		}
		if (renderLevel >= 4) {
			if (!(zephir_array_isset_long(&disabledLevels, 4))) {
				ZEPHIR_INIT_ZVAL_NREF(_22$$25);
				ZVAL_LONG(&_22$$25, 4);
				zephir_update_property_zval(this_ptr, SL("_currentRenderLevel"), &_22$$25);
				ZEPHIR_OBS_VAR(&templatesAfter);
				zephir_read_property(&templatesAfter, this_ptr, SL("_templatesAfter"), PH_NOISY_CC);
				silence = 0;
				zephir_is_iterable(&templatesAfter, 0, "phalcon/mvc/view.zep", 931);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&templatesAfter), _23$$25)
				{
					ZEPHIR_INIT_NVAR(&templateAfter);
					ZVAL_COPY(&templateAfter, _23$$25);
					ZEPHIR_INIT_LNVAR(_24$$26);
					ZEPHIR_CONCAT_VV(&_24$$26, &layoutsDir, &templateAfter);
					if (silence) {
						ZVAL_BOOL(&_25$$26, 1);
					} else {
						ZVAL_BOOL(&_25$$26, 0);
					...
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "_enginerender", &_13, 0, &engines, &_24$$26, &_25$$26, &_26$$26, &cache);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
				ZEPHIR_INIT_NVAR(&templateAfter);
				silence = 1;
		...
		if (renderLevel >= 5) {
			if (!(zephir_array_isset_long(&disabledLevels, 5))) {
				...
				ZVAL_LONG(&_27$$28, 5);
				zephir_update_property_zval(this_ptr, SL("_currentRenderLevel"), &_27$$28);
				zephir_read_property(&_27$$28, this_ptr, SL("_mainView"), PH_NOISY_CC | PH_READONLY);
				if (silence) {
					ZVAL_BOOL(&_28$$28, 1);
				} else {
					ZVAL_BOOL(&_28$$28, 0);
				...
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "_enginerender", &_13, 0, &engines, &_27$$28, &_28$$28, &_29$$28, &cache);
				zephir_check_call_status();
			}
		}
		...
		if (Z_TYPE_P(&cache) == IS_OBJECT) {
			ZEPHIR_CALL_METHOD(&_31$$29, &cache, "isstarted", NULL, 0);
			zephir_check_call_status();
			_32$$29 = zephir_is_true(&_31$$29);
			if (_32$$29) {
				ZEPHIR_CALL_METHOD(&_33$$29, &cache, "isfresh", NULL, 0);
				zephir_check_call_status();
				_32$$29 = zephir_is_true(&_33$$29);
			}
			if (_32$$29) {
	...
	if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
		ZEPHIR_INIT_VAR(&_34$$32);
		ZVAL_STRING(&_34$$32, "view:afterRender");
		ZEPHIR_CALL_METHOD(NULL, &eventsManager, "fire", NULL, 0, &_34$$32, this_ptr);
		zephir_check_call_status();
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Mvc_View, cache) {
	...
	if (Z_TYPE_P(options) == IS_ARRAY) {
		...
		zephir_is_iterable(options, 0, "phalcon/mvc/view.zep", 1262);
