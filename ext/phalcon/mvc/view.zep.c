...
PHP_METHOD(Phalcon_Mvc_View, _loadTemplateEngines) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(engines)) {
		...
		if (Z_TYPE_P(registeredEngines) != IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(NULL, _1$$4, "__construct", NULL, 350, this_ptr, dependencyInjector);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View, _engineRender) {
	...
	for (
	  ...
	  ; zend_hash_move_forward_ex(_3, &_2)
		...
		ZEPHIR_CALL_METHOD(&_5$$3, this_ptr, "_isabsolutepath", &_6, 351, viewPath);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View, getPartial) {
	...
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("ob_get_clean", NULL, 258);
	zephir_check_call_status();
	RETURN_MM();
