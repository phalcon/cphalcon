...
PHP_METHOD(Phalcon_Mvc_View, partial)
{
	...
	zephir_is_iterable(_7, 0, "phalcon/Mvc/View.zep", 627);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(_7), _9)
	{
		...
		if (_10$$4) {
			zephir_array_append(&segments, &segment, PH_SEPARATE, "phalcon/Mvc/View.zep", 623);
		}
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&segment);
	...
	zephir_check_call_status();
	zephir_read_property_cached(&_12, this_ptr, _zephir_prop_1, 1175, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_13);
...
PHP_METHOD(Phalcon_Mvc_View, pick)
{
	...
	if (Z_TYPE_P(renderView) == IS_ARRAY) {
		...
		ZVAL_NULL(&layout);
		if (zephir_memnstr_str(renderView, SL("/"), "phalcon/Mvc/View.zep", 674)) {
			ZEPHIR_INIT_VAR(&parts);
			zephir_fast_explode_str(&parts, SL("/"), renderView, LONG_MAX);
			ZEPHIR_OBS_NVAR(&layout);
			zephir_array_fetch_long(&layout, &parts, 0, PH_NOISY, "phalcon/Mvc/View.zep", 676);
		}
		...
		if (Z_TYPE_P(&layout) != IS_NULL) {
			zephir_array_append(&pickView, &layout, PH_SEPARATE, "phalcon/Mvc/View.zep", 682);
		}
	}
...
PHP_METHOD(Phalcon_Mvc_View, processRender)
{
	...
	if (Z_TYPE_P(&pickView) == IS_NULL) {
		...
		zephir_array_fetch_long(&renderView, &pickView, 0, PH_NOISY, "phalcon/Mvc/View.zep", 761);
	...
	if (renderLevel) {
		...
		if (renderLevel >= 2) {
			if (!(zephir_array_isset_value_long(&disabledLevels, 2))) {
				...
				zephir_is_iterable(_12$$17, 0, "phalcon/Mvc/View.zep", 837);
		...
		if (renderLevel >= 3) {
			if (!(zephir_array_isset_value_long(&disabledLevels, 3))) {
				ZVAL_UNDEF(&_21$$21);
				ZVAL_LONG(&_21$$21, 3);
				zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1177, &_21$$21);
		...
		if (renderLevel >= 4) {
			if (!(zephir_array_isset_value_long(&disabledLevels, 4))) {
				ZVAL_UNDEF(&_23$$23);
				ZVAL_LONG(&_23$$23, 4);
				zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1177, &_23$$23);
				zephir_read_property_cached(&_23$$23, this_ptr, _zephir_prop_12, 1169, PH_NOISY_CC | PH_READONLY);
				...
				zephir_is_iterable(_24$$23, 0, "phalcon/Mvc/View.zep", 873);
		...
		if (renderLevel >= 5) {
			if (!(zephir_array_isset_value_long(&disabledLevels, 5))) {
				ZVAL_UNDEF(&_33$$27);
				ZVAL_LONG(&_33$$27, 5);
				zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1177, &_33$$27);
				zephir_read_property_cached(&_33$$27, this_ptr, _zephir_prop_13, 1185, PH_NOISY_CC | PH_READONLY);
				if (silence) {
					ZVAL_BOOL(&_34$$27, 1);
				} else {
					ZVAL_BOOL(&_34$$27, 0);
		...
		ZVAL_UNDEF(&_35$$13);
		ZVAL_LONG(&_35$$13, 0);
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1177, &_35$$13);
	}
	_36 = fireEvents;
	if (_36) {
...
PHP_METHOD(Phalcon_Mvc_View, setViewsDir)
{
	...
	if (_0) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/View.zep", 1141);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if (Z_TYPE_P(viewsDir) == IS_STRING) {
		...
		zephir_is_iterable(_3$$5, 0, "phalcon/Mvc/View.zep", 1157);
		if (Z_TYPE_P(_3$$5) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(_3$$5), _6$$5, _7$$5, _5$$5)
			{
				...
				if (Z_TYPE_P(&directory) != IS_STRING) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_8$$7, "phalcon/Mvc/View.zep", 1151);
					ZEPHIR_MM_RESTORE();
					return;
				}
			...
			while (1) {
				...
				zephir_check_call_status();
					if (Z_TYPE_P(&directory) != IS_STRING) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_13$$9, "phalcon/Mvc/View.zep", 1151);
						ZEPHIR_MM_RESTORE();
						return;
					}
...
PHP_METHOD(Phalcon_Mvc_View, engineRender)
{
	...
	zephir_is_iterable(_2, 0, "phalcon/Mvc/View.zep", 1264);
	if (Z_TYPE_P(_2) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(_2), _4)
		{
			...
			zephir_is_iterable(&engines, 0, "phalcon/Mvc/View.zep", 1259);
				...
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&engines), _8$$3, _9$$3, _7$$3)
				{
					...
					if (zephir_is_true(&_10$$6)) {
						if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
							...
							zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 1173, &_12$$8);
					...
					zephir_array_append(&viewEnginePaths, &viewEnginePath, PH_SEPARATE, "phalcon/Mvc/View.zep", 1257);
				} ZEND_HASH_FOREACH_END();
			} else {
					...
					zephir_check_call_status();
						...
						if (zephir_is_true(&_21$$11)) {
							if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
								...
								zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 1173, &_22$$13);
					...
					ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&engines), _33$$16, _34$$16, _32$$16)
					{
						...
						if (zephir_is_true(&_35$$19)) {
							if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
								...
								zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 1173, &_36$$21);
						...
						zephir_check_call_status();
							...
							if (zephir_is_true(&_45$$24)) {
								if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
									...
									zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 1173, &_46$$26);
	...
	if (!(silence)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_54$$30, "phalcon/Mvc/View.zep", 1271);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&engines)) {
		...
		if (ZEPHIR_IS_EMPTY(&registeredEngines)) {
			...
			if (Z_TYPE_P(&di) != IS_OBJECT) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_3$$6, "phalcon/Mvc/View.zep", 1325);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			zephir_is_iterable(_4$$5, 0, "phalcon/Mvc/View.zep", 1360);
				...
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(_4$$5), _7$$5, _8$$5, _6$$5)
				{
					...
					if (Z_TYPE_P(&engineService) == IS_OBJECT) {
						...
						if (Z_TYPE_P(&engineService) != IS_STRING) {
							...
							zephir_check_call_status();
							zephir_throw_exception_debug(&_13$$12, "phalcon/Mvc/View.zep", 1351);
							ZEPHIR_MM_RESTORE();
							return;
						}
					...
					zephir_check_call_status();
						if (Z_TYPE_P(&engineService) == IS_OBJECT) {
							...
							if (Z_TYPE_P(&engineService) != IS_STRING) {
								...
								zephir_check_call_status();
								zephir_throw_exception_debug(&_21$$18, "phalcon/Mvc/View.zep", 1351);
								ZEPHIR_MM_RESTORE();
								return;
							}
