...
PHP_METHOD(Phalcon_Mvc_View_Simple, partial) {
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "internalrender", NULL, 498, &partialPath, &mergedParams);
	zephir_check_call_status();
	if (Z_TYPE_P(params) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Mvc_View_Simple, render) {
	...
	ZEPHIR_INIT_VAR(&mergedParams);
	zephir_fast_array_merge(&mergedParams, &viewParams, &params TSRMLS_CC);
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "internalrender", NULL, 498, &path, &mergedParams);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "ob_end_clean", NULL, 176);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Simple, loadTemplateEngines) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&engines)) {
		...
		if (Z_TYPE_P(&registeredEngines) != IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_2$$4, "__construct", NULL, 483, this_ptr, &di);
			zephir_check_call_status();
			...
			if (Z_TYPE_P(&di) != IS_OBJECT) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_3$$6, "phalcon/Mvc/View/Simple.zep", 380 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			zephir_is_iterable(&registeredEngines, 0, "phalcon/Mvc/View/Simple.zep", 413);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&registeredEngines), _8$$5, _9$$5, _7$$5)
			{
				...
				if (Z_TYPE_P(&engineService) == IS_OBJECT) {
					if (zephir_instance_of_ev(&engineService, zend_ce_closure TSRMLS_CC)) {
						...
						ZEPHIR_CALL_FUNCTION(&engineObject, "call_user_func", &_13, 226, &engineService, this_ptr);
						zephir_check_call_status();
					} else {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_15$$12, "phalcon/Mvc/View/Simple.zep", 408 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
...
PHP_METHOD(Phalcon_Mvc_View_Simple, internalRender) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&engines, 0, "phalcon/Mvc/View/Simple.zep", 501);
	...
	if (notExists) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_18$$13, "phalcon/Mvc/View/Simple.zep", 504 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
