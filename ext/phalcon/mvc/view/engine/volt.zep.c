...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, isIncluded)
{
	...
	if (Z_TYPE_P(haystack) == IS_STRING) {
		...
		if (zephir_is_true(&_0$$5)) {
			ZEPHIR_CALL_FUNCTION(&_2$$6, "mb_strpos", NULL, 199, haystack, needle);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, slice)
{
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("array_slice", NULL, 278, value, &_5$$9, &length);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_INIT_VAR(&_7);
	...
	if (zephir_is_true(&_6)) {
		if (Z_TYPE_P(&length) != IS_NULL) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", NULL, 304, value, &_8$$11, &length);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZVAL_LONG(&_9$$10, start);
		ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", NULL, 304, value, &_9$$10);
		zephir_check_call_status();
		RETURN_MM();
	}
	if (Z_TYPE_P(&length) != IS_NULL) {
