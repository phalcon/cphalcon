...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, getCompiler)
{
	...
	if (Z_TYPE_P(&compiler) != IS_OBJECT) {
		...
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_1, 1185, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_METHOD(NULL, &compiler, "__construct", NULL, 0, &_1$$3);
		zephir_check_call_status();
		zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_2, 1186, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CPY_WRT(&container, &_2$$3);
		if (Z_TYPE_P(&container) == IS_OBJECT) {
			ZEPHIR_CALL_METHOD(NULL, &compiler, "setdi", NULL, 0, &container);
			zephir_check_call_status();
		}
		zephir_read_property_cached(&_2$$3, this_ptr, _zephir_prop_3, 1187, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CPY_WRT(&options, &_2$$3);
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			ZEPHIR_CALL_METHOD(NULL, &compiler, "setoptions", NULL, 0, &options);
			zephir_check_call_status();
		}
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1184, &compiler);
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, render)
{
	...
	if (mustClean) {
		zephir_read_property_cached(&_12$$11, this_ptr, _zephir_prop_1, 1185, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, setEventsManager)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1188, eventsManager);
}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, phpExtensionLoaded)
{
	...
	zephir_memory_observe(&name_zv);
	ZVAL_STR_COPY(&name_zv, name);
	ZEPHIR_RETURN_CALL_FUNCTION("extension_loaded", NULL, 456, &name_zv);
	zephir_check_call_status();
	RETURN_MM();
}
