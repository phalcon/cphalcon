...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, render)
{
	...
	if (mustClean) {
		zephir_read_property_cached(&_12$$11, this_ptr, _zephir_prop_1, 1192, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, slice)
{
	...
	if (zephir_is_true(&_6)) {
		if (Z_TYPE_P(&length) != IS_NULL) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", NULL, 305, value, &_8$$11, &length);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZVAL_LONG(&_9$$10, start);
		ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", NULL, 305, value, &_9$$10);
		zephir_check_call_status();
		RETURN_MM();
	}
	if (Z_TYPE_P(&length) != IS_NULL) {
