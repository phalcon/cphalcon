...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, convertEncoding) {
	...
	if (_0) {
		ZEPHIR_RETURN_CALL_FUNCTION("utf8_encode", NULL, 491, &text);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	if ((zephir_function_exists_ex(SL("iconv") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("iconv", NULL, 492, &from, &to, &text);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, render) {
	...
	if (mustClean) {
		...
		ZEPHIR_CALL_FUNCTION(&_12$$11, "ob_get_contents", NULL, 489);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, sort) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "asort", NULL, 496, &value);
	ZEPHIR_UNREF(&value);
	zephir_check_call_status();
	RETURN_CTOR(&value);
