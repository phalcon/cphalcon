	...
	while (1) {
		...
		if (!(zephir_array_isset_string_fetch(&type, &expr, SL("type"), 0))) {
			...
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&expr), _3$$6)
			{
				...
				ZEPHIR_CALL_METHOD(&singleExprCode, this_ptr, "expression", &_5, 412, &_4$$7);
				zephir_check_call_status();
				ZEPHIR_OBS_NVAR(&name);
		...
		if (zephir_array_isset_string_fetch(&left, &expr, SL("left"), 0)) {
			ZEPHIR_CALL_METHOD(&leftCode, this_ptr, "expression", &_5, 412, &left);
			zephir_check_call_status();
		}
		...
		if (ZEPHIR_IS_LONG(&type, 124)) {
			...
			ZEPHIR_CALL_METHOD(&exprCode, this_ptr, "resolvefilter", &_11, 417, &_10$$13, &leftCode);
			zephir_check_call_status();
			break;
		}
		ZEPHIR_OBS_NVAR(&right);
		if (zephir_array_isset_string_fetch(&right, &expr, SL("right"), 0)) {
			ZEPHIR_CALL_METHOD(&rightCode, this_ptr, "expression", &_5, 412, &right);
			zephir_check_call_status();
		}
		ZEPHIR_INIT_NVAR(&exprCode);
		ZVAL_NULL(&exprCode);
		do {
			...
			if (ZEPHIR_IS_LONG(&type, 365)) {
				...
				if (zephir_array_isset_string_fetch(&start, &expr, SL("start"), 0)) {
					ZEPHIR_CALL_METHOD(&startCode, this_ptr, "expression", &_5, 412, &start);
					zephir_check_call_status();
				} else {
				...
				if (zephir_array_isset_string_fetch(&end, &expr, SL("end"), 0)) {
					ZEPHIR_CALL_METHOD(&endCode, this_ptr, "expression", &_5, 412, &end);
					zephir_check_call_status();
				} else {
			...
			if (ZEPHIR_IS_LONG(&type, 366)) {
				...
				ZEPHIR_CALL_METHOD(&_18$$69, this_ptr, "expression", &_5, 412, &_19$$69);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&exprCode);
			...
			ZEPHIR_CALL_METHOD(NULL, &_20$$73, "__construct", &_24, 414, &_23$$73);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileForeach) {
	...
	ZEPHIR_CALL_METHOD(&code, this_ptr, "_statementlist", NULL, 418, &blockStatements, &_2);
	zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&ifExpr, &statement, SL("if_expr"), 0)) {
		ZEPHIR_CALL_METHOD(&_14$$13, this_ptr, "expression", NULL, 412, &ifExpr);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileSet) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&assignments), _0)
	{
		...
		ZEPHIR_CALL_METHOD(&exprCode, this_ptr, "expression", &_2, 412, &_1$$4);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&target, this_ptr, "expression", &_2, 412, &_3$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, _statementList) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&statements), _1)
	{
		...
		if (!(zephir_array_isset_string(&statement, SL("type")))) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_2$$7, "__construct", &_6, 414, &_5$$7, &statement);
			zephir_check_call_status();
		...
		if (Z_TYPE_P(&extensions) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&tempCompilation, this_ptr, "fireextensionevent", &_9, 413, &_8$$8, &_7$$8);
			zephir_check_call_status();
		...
		do {
			...
			if (ZEPHIR_IS_LONG(&type, 307)) {
				...
				if (zephir_is_true(&blockMode)) {
					...
					if (Z_TYPE_P(&blockStatements) == IS_ARRAY) {
						...
						ZEPHIR_CALL_METHOD(&_30$$24, this_ptr, "_statementlist", &_32, 418, &blockStatements, &_31$$24);
						zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(NULL, &_56$$38, "__construct", &_6, 414, &_59$$38);
			zephir_check_call_status();
