...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, compileEcho)
{
	...
	if (ZEPHIR_IS_LONG(&_2, 350)) {
		ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "istagfactory", NULL, 502, &expr);
		zephir_check_call_status();
		if (ZEPHIR_IS_TRUE_IDENTICAL(&_3$$4)) {
			ZVAL_BOOL(&_4$$5, 1);
			ZEPHIR_CALL_METHOD(&exprCode, this_ptr, "expression", NULL, 499, &expr, &_4$$5);
			zephir_check_call_status();
		}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, expression)
{
	...
	while (1) {
		...
		if (!(zephir_array_isset_string_fetch(&type, &expr, SL("type"), 0))) {
			...
			if (Z_TYPE_P(&expr) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&expr), _4$$6)
				{
					...
					ZEPHIR_CALL_METHOD(&singleExprCode, this_ptr, "expression", &_8, 499, &_6$$7, &_7$$7);
					zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						ZEPHIR_CALL_METHOD(&singleExprCode, this_ptr, "expression", &_8, 499, &_10$$10, &_11$$10);
						zephir_check_call_status();
						ZEPHIR_OBS_NVAR(&name);
						if (zephir_array_isset_string_fetch(&name, &singleExpr, SL("name"), 0)) {
		...
		if (zephir_array_isset_string_fetch(&left, &expr, SL("left"), 0)) {
			...
			ZEPHIR_CALL_METHOD(&leftCode, this_ptr, "expression", &_8, 499, &left, &_14$$14);
			zephir_check_call_status();
		}
		...
		if (ZEPHIR_IS_LONG(&type, 124)) {
			...
			ZEPHIR_CALL_METHOD(&exprCode, this_ptr, "resolvefilter", &_18, 504, &_17$$16, &leftCode);
			zephir_check_call_status();
			break;
		}
		...
		if (zephir_array_isset_string_fetch(&right, &expr, SL("right"), 0)) {
			...
			ZEPHIR_CALL_METHOD(&rightCode, this_ptr, "expression", &_8, 499, &right, &_19$$17);
			zephir_check_call_status();
		}
		ZEPHIR_INIT_NVAR(&exprCode);
		ZVAL_NULL(&exprCode);
		do {
			...
			if (ZEPHIR_IS_LONG(&type, 365)) {
				...
				if (zephir_array_isset_string_fetch(&start, &expr, SL("start"), 0)) {
					...
					ZEPHIR_CALL_METHOD(&startCode, this_ptr, "expression", &_8, 499, &start, &_28$$54);
					zephir_check_call_status();
				} else {
				...
				if (zephir_array_isset_string_fetch(&end, &expr, SL("end"), 0)) {
					...
					ZEPHIR_CALL_METHOD(&endCode, this_ptr, "expression", &_8, 499, &end, &_29$$56);
					zephir_check_call_status();
				} else {
			...
			if (ZEPHIR_IS_LONG(&type, 366)) {
				...
				ZEPHIR_CALL_METHOD(&_30$$74, this_ptr, "expression", &_8, 499, &_31$$74, &_32$$74);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&exprCode);
			...
			ZEPHIR_CALL_METHOD(NULL, &_33$$78, "__construct", &_37, 500, &_36$$78);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, functionCall)
{
	...
	if (zephir_array_isset_string_fetch(&funcArguments, &expr, SL("arguments"), 0)) {
		...
		ZEPHIR_CALL_METHOD(&arguments, this_ptr, "expression", NULL, 499, &funcArguments, &_0$$3);
		zephir_check_call_status();
	} else {
	...
	if (ZEPHIR_IS_LONG(&nameType, 265)) {
		...
		if (Z_TYPE_P(&extensions) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&code, this_ptr, "fireextensionevent", NULL, 503, &_3$$6, &_2$$6);
			zephir_check_call_status();
		...
		if (Z_TYPE_P(&functions) == IS_ARRAY) {
			...
			if (zephir_array_isset_fetch(&definition, &functions, &name, 0)) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_5$$9, "__construct", NULL, 500, &_8$$9);
				zephir_check_call_status();
		...
		if (_15$$5) {
			...
			if (ZEPHIR_IS_TRUE_IDENTICAL(&_21$$28)) {
				...
				if (zephir_array_isset_string_fetch(&funcArguments, &expr, SL("arguments"), 0)) {
					ZVAL_BOOL(&_22$$30, 1);
					ZEPHIR_CALL_METHOD(&arguments, this_ptr, "expression", NULL, 499, &funcArguments, &_22$$30);
					zephir_check_call_status();
				} else {
					ZEPHIR_INIT_NVAR(&arguments);
					ZVAL_STRING(&arguments, "");
	...
	ZEPHIR_CALL_METHOD(&_23, this_ptr, "expression", NULL, 499, &nameExpr, &_24);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt_Compiler, resolveFilter)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &_20, "__construct", NULL, 500, &_23);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&statements) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&statements), _2)
		{
			...
			do {
				...
				ZEPHIR_CALL_METHOD(NULL, &_56$$37, "__construct", &_8, 500, &_59$$37);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (UNEXPECTED(!(zephir_array_isset_string(&statement, SL("type"))))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_60$$40, "__construct", &_8, 500, &_63$$40, &statement);
					zephir_check_call_status();
				...
				do {
					...
					if (ZEPHIR_IS_LONG(&type, 307)) {
						...
						if (zephir_is_true(&blockMode)) {
							...
							if (Z_TYPE_P(&blockStatements) == IS_ARRAY) {
								...
								ZEPHIR_CALL_METHOD(&_80$$57, this_ptr, "statementlist", &_35, 501, &blockStatements, &_81$$57);
								zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(NULL, &_94$$70, "__construct", &_8, 500, &_97$$70);
					zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&name, &expression, SL("name"), 0)) {
		...
		if (zephir_array_isset_string_fetch(&left, &name, SL("left"), 0)) {
			...
			if (zephir_array_isset_string_fetch(&leftValue, &left, SL("value"), 0)) {
				...
				if (_0$$6) {
					ZEPHIR_RETURN_CALL_METHOD(this_ptr, "istagfactory", NULL, 502, &left);
					zephir_check_call_status();
					RETURN_MM();
				}
			}
