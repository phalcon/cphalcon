...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_View_Engine_AbstractEngine)
{
...

	{
		zval _zc0;
		ZVAL_NULL(&_zc0);
		zephir_declare_typed_property(phalcon_mvc_view_engine_abstractengine_ce, SL("eventsManager"), &_zc0, ZEND_ACC_PROTECTED, MAY_BE_NULL, SL("Phalcon\\Events\\ManagerInterface"));
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_AbstractEngine, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 250, view);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 251, container);
}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_AbstractEngine, setEventsManager)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 252, eventsManager);
}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_AbstractEngine, fireManagerEvent)
{
	...
	if (Z_TYPE_P(&_0) != IS_NULL) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 252, PH_NOISY_CC | PH_READONLY);
		if (cancellable) {
			ZVAL_BOOL(&_2$$3, 1);
		} else {
			ZVAL_BOOL(&_2$$3, 0);
