...
PHP_METHOD(Phalcon_Mvc_Micro, mount) {
	...
	HashTable *_3;
	HashPosition _2;
	zephir_fcall_cache_entry *_1 = NULL, *_5 = NULL, *_8 = NULL, *_9 = NULL;
	int ZEPHIR_LAST_CALL_STATUS;
	...
	if (Z_TYPE_P(handlers) == IS_ARRAY) {
		...
		if (zephir_is_true(_0)) {
			...
			ZEPHIR_CALL_METHOD(NULL, lazyHandler, "__construct", &_1, 271, mainHandler);
			zephir_check_call_status();
		} else {
		...
		zephir_check_call_status();
		zephir_is_iterable(handlers, &_3, &_2, 0, 0, "phalcon/mvc/micro.zep", 437);
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_3, &_2)
			...
			ZEPHIR_CALL_METHOD(&route, this_ptr, "map", &_5, 0, prefixedPattern, realHandler);
			zephir_check_call_status();
			...
			if (_7) {
				ZEPHIR_CALL_METHOD(NULL, route, "via", &_8, 0, methods);
				zephir_check_call_status();
			}
			if (Z_TYPE_P(name) == IS_STRING) {
				ZEPHIR_CALL_METHOD(NULL, route, "setname", &_9, 0, name);
				zephir_check_call_status();
			}
		}
	}
...
PHP_METHOD(Phalcon_Mvc_Micro, setService) {
	...
	if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, dependencyInjector, "__construct", &_0, 139);
		zephir_check_call_status();
		zephir_update_property_this(this_ptr, SL("_dependencyInjector"), dependencyInjector TSRMLS_CC);
	}
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "set", &_1, 272, serviceName, definition, (shared ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false)));
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Micro, hasService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "has", &_1, 273, serviceName);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Micro, getService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "get", &_1, 274, serviceName);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Micro, getSharedService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "getshared", &_1, 275, serviceName);
	zephir_check_call_status();
	RETURN_MM();
	...
	/* try_start_1: */
		...
		if (Z_TYPE_P(matchedRoute) == IS_OBJECT) {
			...
			if (Z_TYPE_P(eventsManager) == IS_OBJECT) {
				...
				ZEPHIR_CALL_METHOD(&_5, eventsManager, "fire", NULL, 0, _1, this_ptr);
				zephir_check_temp_parameter(_1);
				zephir_check_call_status_or_jump(try_end_1);
			...
			if (Z_TYPE_P(beforeHandlers) == IS_ARRAY) {
				...
				zephir_is_iterable(beforeHandlers, &_7, &_6, 0, 0, "phalcon/mvc/micro.zep", 687);
				for (
			...
			if (Z_TYPE_P(afterHandlers) == IS_ARRAY) {
				zephir_update_property_this(this_ptr, SL("_stopped"), (0) ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false) TSRMLS_CC);
				zephir_is_iterable(afterHandlers, &_11, &_10, 0, 0, "phalcon/mvc/micro.zep", 742);
				for (
				  ...
				  ; zephir_hash_move_forward_ex(_11, &_10)
					...
					if (!(zephir_is_callable(after TSRMLS_CC))) {
						...
						ZEPHIR_CALL_METHOD(NULL, _3, "__construct", &_4, 2, _14);
		...
		if (Z_TYPE_P(finishHandlers) == IS_ARRAY) {
			...
			ZVAL_NULL(params);
			zephir_is_iterable(finishHandlers, &_16, &_15, 0, 0, "phalcon/mvc/micro.zep", 832);
			for (
