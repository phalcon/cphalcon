...
PHP_METHOD(Phalcon_Mvc_Micro, mount) {
	...
	if (Z_TYPE_P(handlers) == IS_ARRAY) {
		...
		if (zephir_is_true(_0)) {
			...
			ZEPHIR_CALL_METHOD(NULL, lazyHandler, "__construct", NULL, 295, mainHandler);
			zephir_check_call_status();
		} else {
...
PHP_METHOD(Phalcon_Mvc_Micro, setService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "set", NULL, 296, serviceName, definition, (shared ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false)));
	zephir_check_call_status();
	RETURN_MM();
