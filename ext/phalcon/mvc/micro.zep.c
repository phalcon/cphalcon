...
PHP_METHOD(Phalcon_Mvc_Micro, mount) {
	...
	if (Z_TYPE_P(&handlers) == IS_ARRAY) {
		...
		if (zephir_is_true(&_0$$5)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &lazyHandler, "__construct", NULL, 308, &mainHandler);
			zephir_check_call_status();
		} else {
...
PHP_METHOD(Phalcon_Mvc_Micro, setService) {
	...
	if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &dependencyInjector, "__construct", NULL, 309);
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, SL("_dependencyInjector"), &dependencyInjector);
	}
	if (shared) {
...
PHP_METHOD(Phalcon_Mvc_Micro, offsetUnset) {
	...
	ZEPHIR_CALL_METHOD(NULL, &dependencyInjector, "remove", NULL, 314, alias);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
