...
PHP_METHOD(Phalcon_Mvc_Micro, mount) {
	...
	if (Z_TYPE_P(&handlers) == IS_ARRAY) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&handlers, 0, "phalcon/mvc/micro.zep", 437);
...
PHP_METHOD(Phalcon_Mvc_Micro, setService) {
	...
	if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &dependencyInjector, "__construct", NULL, 183);
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, SL("_dependencyInjector"), &dependencyInjector);
	}
	if (shared) {
	...
	/* try_start_1: */
		...
		if (Z_TYPE_P(&matchedRoute) == IS_OBJECT) {
			...
			if (Z_TYPE_P(&beforeHandlers) == IS_ARRAY) {
				...
				zephir_is_iterable(&beforeHandlers, 0, "phalcon/mvc/micro.zep", 688);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&beforeHandlers), _10$$12)
				{
					...
					if (!(zephir_is_callable(&before TSRMLS_CC))) {
						...
						zephir_check_call_status_or_jump(try_end_1);
						zephir_throw_exception_debug(&_12$$17, "phalcon/mvc/micro.zep", 671 TSRMLS_CC);
						goto try_end_1;
			...
			if (Z_TYPE_P(&afterHandlers) == IS_ARRAY) {
				...
				zephir_is_iterable(&afterHandlers, 0, "phalcon/mvc/micro.zep", 751);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&afterHandlers), _20$$22)
				{
					...
					if (!(zephir_is_callable(&after TSRMLS_CC))) {
						...
						zephir_check_call_status_or_jump(try_end_1);
						zephir_throw_exception_debug(&_22$$27, "phalcon/mvc/micro.zep", 746 TSRMLS_CC);
						goto try_end_1;
			...
			if (!(zephir_is_callable(&notFoundHandler TSRMLS_CC))) {
				...
				zephir_check_call_status_or_jump(try_end_1);
				zephir_throw_exception_debug(&_26$$31, "phalcon/mvc/micro.zep", 770 TSRMLS_CC);
				goto try_end_1;
		...
		if (Z_TYPE_P(&finishHandlers) == IS_ARRAY) {
			...
			ZVAL_NULL(&params);
			zephir_is_iterable(&finishHandlers, 0, "phalcon/mvc/micro.zep", 841);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&finishHandlers), _29$$33)
			{
				...
				if (!(zephir_is_callable(&finish TSRMLS_CC))) {
					...
					zephir_check_call_status_or_jump(try_end_1);
					zephir_throw_exception_debug(&_31$$38, "phalcon/mvc/micro.zep", 822 TSRMLS_CC);
					goto try_end_1;
