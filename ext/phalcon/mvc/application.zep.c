...
PHP_METHOD(Phalcon_Mvc_Application, registerModules) {
	...
	if (Z_TYPE_P(modules) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_application_exception_ce, "Modules must be an Array", "phalcon/mvc/application.zep", 134);
		return;
	}
	if (merge == 0) {
...
PHP_METHOD(Phalcon_Mvc_Application, handle) {
	...
	if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_application_exception_ce, "A dependency injection object is required to access internal services", "phalcon/mvc/application.zep", 199);
		return;
	}
	...
	if (zephir_is_true(moduleName)) {
		...
		if (!(zephir_array_isset_fetch(&module, _0, moduleName, 0 TSRMLS_CC))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_2, "phalcon/mvc/application.zep", 245 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		_4 = Z_TYPE_P(module) != IS_ARRAY;
		if (_4) {
			_4 = Z_TYPE_P(module) != IS_OBJECT;
		}
		if (_4) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_application_exception_ce, "Invalid module definition", "phalcon/mvc/application.zep", 252);
			return;
		}
			...
			if (zephir_array_isset_string_fetch(&path, module, SS("path"), 0 TSRMLS_CC)) {
				if (!(zephir_class_exists(className, zephir_is_true(ZEPHIR_GLOBAL(global_false))  TSRMLS_CC))) {
					if ((zephir_file_exists(path TSRMLS_CC) == SUCCESS)) {
						...
						ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, _3);
						zephir_check_call_status();
						zephir_throw_exception_debug(_2, "phalcon/mvc/application.zep", 275 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
				}
