...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, handle)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_2, 1136, PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&handlers) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&handlers), _3)
		{
			...
			if (!(ZEPHIR_IS_EMPTY(&prefix))) {
				...
				ZEPHIR_CALL_METHOD(NULL, &route, "__construct", &_4, 261, &prefix);
				zephir_check_call_status();
			...
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 1137, &__$null);
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, processActionAnnotation)
{
	...
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_1, 1137, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&routePrefix, &_2);
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_2, 1139, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_2) != IS_NULL) {
		zephir_read_property_cached(&_3$$6, this_ptr, _zephir_prop_2, 1139, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_FUNCTION(&_4$$6, "call_user_func", NULL, 81, &_3$$6, &proxyActionName);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&converts) == IS_ARRAY) {
		zephir_is_iterable(&converts, 0, "phalcon/Mvc/Router/Annotations.zep", 389);
		if (Z_TYPE_P(&converts) == IS_ARRAY) {
	...
	zephir_check_call_status();
	if (Z_TYPE_P(&converts) == IS_ARRAY) {
		zephir_is_iterable(&converts, 0, "phalcon/Mvc/Router/Annotations.zep", 400);
		if (Z_TYPE_P(&converts) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, setActionSuffix)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1138, &actionSuffix_zv);
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, setActionPreformatCallback)
{
	...
	if (EXPECTED(zephir_is_callable(callback))) {
		...
		zephir_create_closure_ex(&_0$$4, NULL, phalcon_91__closure_ce, SL("__invoke"));
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1139, &_0$$4);
	} else {
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, setControllerSuffix)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1136, &controllerSuffix_zv);
	RETURN_THISW();
}
