	...
	if (Z_TYPE_P(&handlers) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&handlers), _6)
		{
			...
			if (!(ZEPHIR_IS_EMPTY(&prefix))) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_9$$6, &route, "getcompiledpattern", &_10, 502);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (!(ZEPHIR_IS_EMPTY(&prefix))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &route, "__construct", &_8, 118, &prefix);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_32$$29, &route, "getcompiledpattern", &_10, 502);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, processActionAnnotation)
{
	...
	if (Z_TYPE_P(&_2) != IS_NULL) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$6, "call_user_func", NULL, 263, &_3$$6, &proxyActionName);
		zephir_check_call_status();
