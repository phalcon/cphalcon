...
PHP_METHOD(Phalcon_Mvc_Router_Group, _addRoute) {
	...
	if (Z_TYPE_P(defaultPaths) == IS_ARRAY) {
		if (Z_TYPE_P(paths) == IS_STRING) {
			ZEPHIR_CALL_CE_STATIC(&processedPaths, phalcon_mvc_router_route_ce, "getroutepaths", &_0, 80, paths);
			zephir_check_call_status();
		} else {
	...
	ZEPHIR_CALL_METHOD(NULL, route, "__construct", NULL, 79, _2, mergedPaths, httpMethods);
	zephir_check_call_status();
	zephir_update_property_array_append(this_ptr, SL("_routes"), route TSRMLS_CC);
	ZEPHIR_CALL_METHOD(NULL, route, "setgroup", NULL, 375, this_ptr);
	zephir_check_call_status();
	RETURN_CCTOR(route);
