...
PHP_METHOD(Phalcon_Mvc_Url, get)
{
	...
	if (Z_TYPE_P(uri) == IS_ARRAY) {
		...
		if (UNEXPECTED(Z_TYPE_P(&route) != IS_OBJECT)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_12$$14, "__construct", NULL, 32, &_13$$14);
			zephir_check_call_status();
	...
	if (local) {
		...
		ZVAL_STRING(&_19$$15, "/");
		ZEPHIR_CALL_FUNCTION(uri, "preg_replace", NULL, 40, &_18$$15, &_19$$15, &_17$$15);
		zephir_check_call_status();
	}
	if (zephir_is_true(args)) {
		ZEPHIR_CALL_FUNCTION(&queryString, "http_build_query", NULL, 487, args);
		zephir_check_call_status();
