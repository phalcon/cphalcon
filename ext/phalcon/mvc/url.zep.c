...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Url)
{
	...
	zend_declare_property_null(phalcon_mvc_url_ce, SL("baseUri"), ZEND_ACC_PROTECTED);
	{
		zval _zc0;
		ZVAL_NULL(&_zc0);
		zephir_declare_typed_property(phalcon_mvc_url_ce, SL("router"), &_zc0, ZEND_ACC_PROTECTED, MAY_BE_NULL, SL("Phalcon\\Mvc\\RouterInterface"));
	}
...
PHP_METHOD(Phalcon_Mvc_Url, get)
{
	...
	if (local == 0) {
		...
		if (_0$$3) {
			_1$$3 = zephir_memnstr_str(uri, SL("//"), "phalcon/Mvc/Url.zep", 131);
			if (!(_1$$3)) {
				_1$$3 = zephir_memnstr_str(uri, SL(":"), "phalcon/Mvc/Url.zep", 131);
			}
			_0$$3 = _1$$3;
	...
	if (Z_TYPE_P(uri) == IS_ARRAY) {
		...
		if (UNEXPECTED(!(zephir_array_isset_string_fetch(&routeName, uri, SL("for"), 0)))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_6$$10, "phalcon/Mvc/Url.zep", 148);
			ZEPHIR_MM_RESTORE();
			return;
		}
		zephir_read_property_cached(&_7$$9, this_ptr, _zephir_prop_0, 1176, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CPY_WRT(&router, &_7$$9);
		if (UNEXPECTED(!zephir_is_true(&router))) {
			zephir_read_property_cached(&_8$$11, this_ptr, _zephir_prop_1, 1177, PH_NOISY_CC | PH_READONLY);
			...
			if (UNEXPECTED(Z_TYPE_P(&container) != IS_OBJECT)) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_9$$12, "phalcon/Mvc/Url.zep", 160);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (UNEXPECTED(!zephir_is_true(&_10$$11))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_12$$13, "phalcon/Mvc/Url.zep", 164);
				ZEPHIR_MM_RESTORE();
				return;
			}
		...
		if (UNEXPECTED(Z_TYPE_P(&route) != IS_OBJECT)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_15$$14, "phalcon/Mvc/Url.zep", 177);
			ZEPHIR_MM_RESTORE();
			return;
		}
	...
	if (local) {
		...
		object_init_ex(&_25$$18, phalcon_support_helper_str_reduceslashes_ce);
		if (zephir_has_constructor(&_25$$18)) {
			ZEPHIR_CALL_METHOD(NULL, &_25$$18, "__construct", NULL, 0);
			zephir_check_call_status();
		}

		ZEPHIR_INIT_VAR(&_26$$18);
		ZEPHIR_CONCAT_VV(&_26$$18, baseUri, &strUri);
		ZEPHIR_CALL_METHOD(uri, &_25$$18, "__invoke", NULL, 0, &_26$$18);
		zephir_check_call_status();
	}
	if (zephir_is_true(arguments)) {
		...
		zephir_fast_strpos(&queryPos, uri, &_27$$19, 0 );
		_28$$19 = replaceArgs;
		if (_28$$19) {
			_28$$19 = !ZEPHIR_IS_FALSE_IDENTICAL(&queryPos);
		}
		if (_28$$19) {
...
PHP_METHOD(Phalcon_Mvc_Url, getBaseUri)
{
	...
	if (Z_TYPE_P(&baseUri) == IS_NULL) {
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1178, &baseUri);
	}
...
PHP_METHOD(Phalcon_Mvc_Url, getStaticBaseUri)
{
	...
	static zend_string *_zephir_prop_0 = NULL;
	if (UNEXPECTED(!_zephir_prop_0)) {
		_zephir_prop_0 = zend_string_init("staticBaseUri", 13, 1);
	}
	ZEPHIR_METHOD_GLOBALS_PTR = pecalloc(1, sizeof(zephir_method_globals), 0);
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
...
PHP_METHOD(Phalcon_Mvc_Url, setBasePath)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1180, &basePath_zv);
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Mvc_Url, setBaseUri)
{
	...
	if (UNEXPECTED(!_zephir_prop_1)) {
		_zephir_prop_1 = zend_string_init("staticBaseUri", 13, 1);
	}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1178, &baseUri_zv);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_1, 1179, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_0) == IS_NULL) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1179, &baseUri_zv);
	}
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Mvc_Url, setStaticBaseUri)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1179, &staticBaseUri_zv);
	RETURN_THISW();
}
