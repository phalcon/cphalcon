...
PHP_METHOD(Phalcon_Mvc_Url, get) {
	...
	if (Z_TYPE_P(local) == IS_NULL) {
		...
		if (_0$$3) {
			_1$$3 = zephir_memnstr_str(uri, SL("//"), "phalcon/mvc/url.zep", 181);
			if (!(_1$$3)) {
				_1$$3 = zephir_memnstr_str(uri, SL(":"), "phalcon/mvc/url.zep", 181);
			}
			_0$$3 = _1$$3;
	...
	if (Z_TYPE_P(uri) == IS_ARRAY) {
		...
		if (Z_TYPE_P(route) != IS_OBJECT) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_10$$13, "phalcon/mvc/url.zep", 223 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	...
	if (zephir_is_true(args)) {
		ZEPHIR_CALL_FUNCTION(&queryString, "http_build_query", NULL, 328, args);
		zephir_check_call_status();
