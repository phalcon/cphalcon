...
PHP_METHOD(Phalcon_Mvc_Collection, setId) {
	...
	zend_class_entry *_0;
	zval *id, *modelsManager, *useImplicitIds, *mongoId = NULL;
	...
	if ((Z_TYPE_P(id) != IS_OBJECT)) {
		...
		ZEPHIR_INIT_VAR(useImplicitIds);
		zephir_call_method_p1(useImplicitIds, modelsManager, "isusingimplicitobjectids", this_ptr);
		if (zephir_is_true(useImplicitIds)) {
			ZEPHIR_INIT_VAR(mongoId);
			_0 = zend_fetch_class(SL("MongoId"), ZEND_FETCH_CLASS_AUTO TSRMLS_CC);
			object_init_ex(mongoId, _0);
		} else {
...
PHP_METHOD(Phalcon_Mvc_Collection, _getResultset) {
	...
	HashTable *_2;
	HashPosition _1;
	zval *params, *collection, *connection, *unique, *source, *mongoCollection, *conditions = NULL, *base = NULL, *documentsCursor, *fields, *limit, *sort, *skip, *document = NULL, *collections, *_0, **_3, *_4 = NULL;

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 4, 0, &params, &collection, &connection, &unique);
...
PHP_METHOD(Phalcon_Mvc_Collection, _getGroupResultset) {
	...
	zend_bool simple;
	zval *params, *collection, *connection, *source, *mongoCollection, *conditions = NULL, *documentsCursor, *limit, *sort = NULL;
