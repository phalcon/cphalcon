...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, setDI) {
	...
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "di");
	zephir_update_property_array(this_ptr, SL("_params"), &_0, container TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, limit) {
	...
	if (zephir_is_numeric(offset)) {
		...
		ZEPHIR_CALL_FUNCTION(offset, "abs", NULL, 186, &_2$$4);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_3$$4);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, fromInput) {
	...
	zval *container, container_sub, *modelName_param = NULL, *data_param = NULL, *operator_param = NULL, attribute, conditions, field, value, type, metaData, model, dataTypes, bind, criteria, columnMap, _0$$3, _1$$3, _3$$3, *_4$$3, _9$$9, _10$$9, _11$$8, _12$$10;
	zval *this_ptr = getThis();
	...
	if (zephir_fast_count_int(&conditions TSRMLS_CC)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &criteria, "where", NULL, 389, &_12$$10);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &criteria, "bind", NULL, 390, &bind);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_METHOD(NULL, &criteria, "setmodelname", NULL, 383, &modelName);
	zephir_check_call_status();
