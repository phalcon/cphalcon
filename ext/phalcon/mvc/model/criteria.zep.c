...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, fromInput) {
	...
	if (zephir_fast_count_int(&conditions TSRMLS_CC)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &criteria, "where", NULL, 356, &_12$$10);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &criteria, "bind", NULL, 357, &bind);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_METHOD(NULL, &criteria, "setmodelname", NULL, 350, &modelName);
	zephir_check_call_status();
