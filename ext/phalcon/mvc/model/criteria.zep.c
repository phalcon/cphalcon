...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, limit) {
	...
	if (zephir_is_numeric(offset)) {
		...
		ZEPHIR_CALL_FUNCTION(offset, "abs", NULL, 200, &_2$$4);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_3$$4);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, fromInput) {
	...
	if (zephir_fast_count_int(&conditions TSRMLS_CC)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &criteria, "where", NULL, 364, &_12$$10);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &criteria, "bind", NULL, 365, &bind);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_METHOD(NULL, &criteria, "setmodelname", NULL, 358, &modelName);
	zephir_check_call_status();
