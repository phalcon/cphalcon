...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, bind) {
	...
	if (merge) {
		...
		if (Z_TYPE_P(&bind) == IS_ARRAY) {
			...
			zephir_update_property_array(this_ptr, SL("params"), &_3$$6, &_2$$6);
			ZEPHIR_INIT_NVAR(&_2$$6);
		} else {
			ZEPHIR_INIT_VAR(&_4$$7);
			ZVAL_STRING(&_4$$7, "bind");
			zephir_update_property_array(this_ptr, SL("params"), &_4$$7, &bindParams);
		}
	} else {
		ZEPHIR_INIT_VAR(&_5$$8);
		ZVAL_STRING(&_5$$8, "bind");
		zephir_update_property_array(this_ptr, SL("params"), &_5$$8, &bindParams);
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, inWhere) {
	...
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/Mvc/Model/Criteria.zep", 413);
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _1)
		{
			...
			zephir_array_append(&bindKeys, &queryKey, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 403);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, notInWhere) {
	...
	ZEPHIR_INIT_VAR(&bindKeys);
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/Mvc/Model/Criteria.zep", 453);
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _0)
		{
			...
			zephir_array_append(&bindKeys, &_3$$3, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 443);
			zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
			ZEPHIR_SEPARATE(&hiddenParam);
			zephir_increment(&hiddenParam);
		} ZEND_HASH_FOREACH_END();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_append(&bindKeys, &_5$$4, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 443);
				zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
				ZEPHIR_SEPARATE(&hiddenParam);
				zephir_increment(&hiddenParam);
			ZEPHIR_CALL_METHOD(NULL, &values, "next", NULL, 0);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, limit) {
	...
	if (zephir_is_numeric(&_0)) {
		...
		zephir_update_property_array(this_ptr, SL("params"), &_5$$4, &_4$$4);
		...
		zephir_update_property_array(this_ptr, SL("params"), &_6$$5, &_7$$5);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, fromInput) {
	...
	if (zephir_fast_count_int(&data TSRMLS_CC)) {
		...
		array_init(&bind);
		zephir_is_iterable(&data, 0, "phalcon/Mvc/Model/Criteria.zep", 727);
		if (Z_TYPE_P(&data) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _6$$3, _7$$3, _4$$3)
			{
				...
				if (_8$$4) {
					...
					zephir_array_fetch(&attribute, &columnMap, &field, PH_NOISY, "phalcon/Mvc/Model/Criteria.zep", 702 TSRMLS_CC);
				} else {
				...
				if (zephir_array_isset_fetch(&type, &dataTypes, &attribute, 0 TSRMLS_CC)) {
					...
					if (_9$$7) {
						if (ZEPHIR_IS_LONG(&type, 2)) {
							...
							zephir_array_append(&conditions, &_10$$9, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 713);
						...
						zephir_array_append(&conditions, &_12$$8, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 722);
					...
					if (zephir_array_isset_fetch(&type, &dataTypes, &attribute, 0 TSRMLS_CC)) {
						...
						if (_14$$13) {
							if (ZEPHIR_IS_LONG(&type, 2)) {
								...
								zephir_array_append(&conditions, &_15$$15, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 713);
							...
							zephir_array_append(&conditions, &_17$$14, PH_SEPARATE, "phalcon/Mvc/Model/Criteria.zep", 722);
							zephir_array_update_zval(&bind, &field, &value, PH_COPY | PH_SEPARATE);
						}
					}
	...
	if (zephir_fast_count_int(&conditions TSRMLS_CC)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &criteria, "where", NULL, 418, &_18$$16);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &criteria, "bind", NULL, 419, &bind);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_METHOD(NULL, &criteria, "setmodelname", NULL, 412, &modelName);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, execute) {
	...
	if (Z_TYPE_P(&model) != IS_STRING) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Model name must be string", "phalcon/Mvc/Model/Criteria.zep", 794);
		return;
	}
