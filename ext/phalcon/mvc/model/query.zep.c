...
PHP_METHOD(Phalcon_Mvc_Model_Query, _executeSelect) {
	...
	zephir_is_iterable(columns, &_7, &_6, 0, 0, "phalcon/mvc/model/query.zep", 2303);
	for (
	  ; zephir_hash_get_current_data_ex(_7, (void**) &_8, &_6) == SUCCESS
	  ; zephir_hash_move_forward_ex(_7, &_6)
	) {
		...
		zephir_array_fetch_string(&_9, column, SL("type"), PH_NOISY | PH_READONLY, "phalcon/mvc/model/query.zep", 2290 TSRMLS_CC);
	...
	ZEPHIR_OBS_VAR(metaData);
	zephir_read_property_this(&metaData, this_ptr, SL("_metaData"), PH_NOISY_CC);
	zephir_is_iterable(columns, &_11, &_10, 0, 0, "phalcon/mvc/model/query.zep", 2414);
	for (
	  ; zephir_hash_get_current_data_ex(_11, (void**) &_12, &_10) == SUCCESS
	  ; zephir_hash_move_forward_ex(_11, &_10)
	) {
		...
		zephir_array_fetch_string(&_9, column, SL("type"), PH_NOISY | PH_READONLY, "phalcon/mvc/model/query.zep", 2334 TSRMLS_CC);
		if (ZEPHIR_IS_STRING(_9, "object")) {
			...
			if (isComplex == 1) {
				...
				zephir_is_iterable(attributes, &_17, &_16, 0, 0, "phalcon/mvc/model/query.zep", 2368);
				for (
				  ...
				  ; zephir_hash_move_forward_ex(_17, &_16)
					...
					zephir_array_append(&selectColumns, _19, PH_SEPARATE, "phalcon/mvc/model/query.zep", 2362);
				}
				...
				for (
				  ...
				  ; zephir_hash_move_forward_ex(_24, &_23)
					...
					zephir_array_fast_append(_19, attribute);
					zephir_array_fast_append(_19, sqlColumn);
					zephir_array_append(&selectColumns, _19, PH_SEPARATE, "phalcon/mvc/model/query.zep", 2386);
				}
			}
		} else {
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _executeInsert) {
	...
	zephir_is_iterable(values, &_2, &_1, 0, 0, "phalcon/mvc/model/query.zep", 2645);
	for (
	  ; zephir_hash_get_current_data_ex(_2, (void**) &_3, &_1) == SUCCESS
	  ; zephir_hash_move_forward_ex(_2, &_1)
	) {
		...
		zephir_array_fetch_string(&_4, value, SL("type"), PH_NOISY | PH_READONLY, "phalcon/mvc/model/query.zep", 2590 TSRMLS_CC);
		do {
			...
			if (ZEPHIR_IS_LONG(_4, 273) || ZEPHIR_IS_LONG(_4, 274)) {
				...
				if (!(zephir_array_isset_fetch(&insertValue, bindParams, wildcard, 0 TSRMLS_CC))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(_10, "phalcon/mvc/model/query.zep", 2611 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
				break;
		...
		if (automaticFields == 1) {
			if (Z_TYPE_P(columnMap) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_fetch(&attributeName, columnMap, fieldName, 0 TSRMLS_CC))) {
					...
					ZEPHIR_CALL_METHOD(NULL, _10, "__construct", &_12, _11);
					zephir_check_call_status();
					zephir_throw_exception_debug(_10, "phalcon/mvc/model/query.zep", 2629 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
			} else {
