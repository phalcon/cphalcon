...
PHP_METHOD(Phalcon_Mvc_Model_Query, _getQualified) {
	...
	if (zephir_array_isset_string_fetch(&columnDomain, expr, SS("domain"), 0 TSRMLS_CC)) {
		...
		zephir_is_iterable(_1, &_4, &_3, 0, 0, "phalcon/mvc/model/query.zep", 280);
		for (
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _getExpression) {
	...
	if (zephir_array_isset_long(expr, 0)) {
		...
		zephir_is_iterable(expr, &_10, &_9, 0, 0, "phalcon/mvc/model/query.zep", 625);
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_10, &_9)
			...
			zephir_check_call_status();
			zephir_array_append(&listItems, _12, PH_SEPARATE, "phalcon/mvc/model/query.zep", 623);
		}
		array_init_size(return_value, 3);
