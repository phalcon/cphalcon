	...
	if (zephir_array_isset_long(&expr, 0)) {
		...
		if (Z_TYPE_P(&expr) == IS_ARRAY) {
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_89$$74, this_ptr, "_getexpression", NULL, 456, &exprListItem);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _executeSelect) {
	...
	if (!(isComplex)) {
		...
		ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 474, &simpleColumnMap, &resultObject, &resultData, &cache, &_98$$83);
		zephir_check_call_status();
		RETURN_MM();
	}
