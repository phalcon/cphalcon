...
PHP_METHOD(Phalcon_Mvc_Model_Query, _getQualified) {
	...
	zval *expr_param = NULL, *columnName, *sqlColumnAliases, *metaData, *sqlAliases, *source = NULL, *sqlAliasesModelsInstances, *realColumnName = NULL, *columnDomain, *model = NULL, *models, *columnMap = NULL, *hasModel = NULL, *_0 = NULL, *_1, *_2 = NULL, **_6, *_7 = NULL, *_9, *_10;
	zval *expr = NULL;
	...
	if (zephir_array_isset_string_fetch(&columnDomain, expr, SS("domain"), 0 TSRMLS_CC)) {
		...
		zephir_is_iterable(_1, &_5, &_4, 0, 0, "phalcon/mvc/model/query.zep", 279);
		for (
		...
		if (!(zephir_array_isset_fetch(&source, models, _0, 0 TSRMLS_CC))) {
			...
			ZEPHIR_CALL_METHOD(NULL, _10, "__construct", &_3, _2);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&exprType, expr, SS("type"), 0 TSRMLS_CC)) {
		...
		do {
			...
			if (ZEPHIR_IS_LONG(exprType, 356)) {
				ZEPHIR_INIT_NVAR(exprReturn);
				zephir_create_array(exprReturn, 2, 0);
			...
			if (ZEPHIR_IS_LONG(exprType, 367)) {
				ZEPHIR_INIT_NVAR(exprReturn);
				zephir_create_array(exprReturn, 3, 0);
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _getSingleJoin) {
	...
	if (Z_TYPE_P(fields) != IS_ARRAY) {
		...
		zephir_create_array(sqlJoinConditions, 4, 0);
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _getMultiJoin) {
	...
	if (Z_TYPE_P(fields) == IS_ARRAY) {
		...
		zephir_create_array(sqlJoins, 2, 0);
		ZEPHIR_INIT_NVAR(_8);
		zephir_create_array(_8, 3, 0);
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		if (zephir_array_isset_string_fetch(&alias, selectedModel, SS("alias"), 0 TSRMLS_CC)) {
			...
			if (Z_TYPE_P(completeSource) == IS_ARRAY) {
				...
				ZEPHIR_INIT_NVAR(completeSource);
				zephir_create_array(completeSource, 3, 0);
				zephir_array_fast_append(completeSource, source);
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _prepareInsert) {
	...
	if (zephir_is_true(schema)) {
		...
		zephir_create_array(source, 2, 0);
