	...
	if (zephir_array_isset_string_fetch(&exprType, &expr, SL("type"), 0)) {
		...
		do {
			...
			if (ZEPHIR_IS_LONG(&exprType, 309)) {
				...
				ZVAL_BOOL(&_82$$68, 1);
				ZEPHIR_CALL_METHOD(&_81$$68, this_ptr, "_prepareselect", NULL, 451, &expr, &_82$$68);
				zephir_check_call_status();
	...
	if (zephir_array_isset_long(&expr, 0)) {
		...
		if (Z_TYPE_P(&expr) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&expr), _85$$71)
			{
				...
				ZEPHIR_CALL_METHOD(&_87$$72, this_ptr, "_getexpression", NULL, 446, &exprListItem);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Query, _executeSelect) {
	...
	if (!(isComplex)) {
		...
		ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 464, &simpleColumnMap, &resultObject, &resultData, &cache, &_100$$83);
		zephir_check_call_status();
		RETURN_MM();
	}
