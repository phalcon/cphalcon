...
PHP_METHOD(Phalcon_Mvc_Model_Manager, getRelationRecords) {
	...
	if ((Z_TYPE_P(preConditions) != IS_NULL)) {
		array_init_size(conditions, Resource id #1126);
	...
	array_init_size(findParams, Resource id #1128);
	ZEPHIR_INIT_NVAR(_2);
	...
	if ((Z_TYPE_P(parameters) == IS_ARRAY)) {
		...
		array_init_size(findArguments, Resource id #1130);
	...
	array_init_size(arguments, Resource id #1132);
	...
	ZEPHIR_INIT_NVAR(_2);
	array_init_size(_2, Resource id #1135);
	ZEPHIR_INIT_NVAR(_3);
