...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Strategy_Annotations, getColumnMaps) {
	...
	zval *model, model_sub, *container, container_sub, __$null, annotations, className, reflection, propertiesAnnotations, property, propAnnotations, columnAnnotation, columnName, orderedColumnMap, reversedColumnMap, _0, *_5, _1$$4, _2$$4, _3$$5, _4$$5, _8$$6, _9$$6;
	...
	if (Z_TYPE_P(&reflection) != IS_OBJECT) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$4, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 39 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (!(zephir_fast_count_int(&propertiesAnnotations TSRMLS_CC))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$5, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 47 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	hasReversedColumn = 0;
	zephir_is_iterable(&propertiesAnnotations, 0, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 85);
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Strategy_Annotations, getMetaData) {
	...
	if (Z_TYPE_P(container) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "The dependency injector is invalid", "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 107);
		return;
	}
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "annotations");
	...
	if (Z_TYPE_P(&reflection) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 1, &_2$$4);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$4, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 116 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_METHOD(&propertiesAnnotations, &reflection, "getpropertiesannotations", NULL, 0);
	...
	if (!(zephir_fast_count_int(&propertiesAnnotations TSRMLS_CC))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$5, "__construct", NULL, 1, &_4$$5);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$5, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 125 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	zephir_is_iterable(&propertiesAnnotations, 0, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 379);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&propertiesAnnotations), _6, _7, _5)
	{
		ZEPHIR_INIT_NVAR(&property);
		if (_7 != NULL) { 
		...
		if (zephir_is_true(&_64$$6)) {
			zephir_array_append(&primaryKeys, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 325);
		} else {
			zephir_array_append(&nonPrimaryKeys, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 327);
		}
		ZEPHIR_INIT_NVAR(&_9$$6);
		...
		if (zephir_is_true(&_66$$6)) {
			zephir_array_append(&skipOnInsert, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 341);
		}
		ZEPHIR_INIT_NVAR(&_9$$6);
		...
		if (zephir_is_true(&_67$$6)) {
			zephir_array_append(&skipOnUpdate, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 348);
		}
		ZEPHIR_INIT_NVAR(&_9$$6);
		...
		if (zephir_is_true(&_68$$6)) {
			zephir_array_append(&emptyStringValues, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 355);
		}
		ZEPHIR_INIT_NVAR(&_9$$6);
		...
		if (!(zephir_is_true(&_69$$6))) {
			zephir_array_append(&notNull, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 362);
		}
		ZEPHIR_INIT_NVAR(&_9$$6);
		...
		zephir_array_append(&attributes, &columnName, PH_SEPARATE, "phalcon/Mvc/Model/Metadata/Strategy/Annotations.zep", 373);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&propAnnotations);
	ZEPHIR_INIT_NVAR(&property);
