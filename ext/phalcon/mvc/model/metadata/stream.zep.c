...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Stream, __construct)
{
	...
	if (zephir_array_isset_string_fetch(&metaDataDir, &options, SL("metaDataDir"), 0)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1035, &metaDataDir);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Stream, write)
{
	...
	/* try_start_1: */
		...
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 1035, PH_NOISY_CC | PH_READONLY);
