...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, __construct)
{
	...
	if (Z_TYPE_P(params) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&conditions) == IS_ARRAY) {
			...
			zephir_is_iterable(&conditions, 0, "phalcon/Mvc/Model/Query/Builder.zep", 196);
			if (Z_TYPE_P(&conditions) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&conditions), _0$$7)
				{
					...
					if (Z_TYPE_P(&singleConditionArray) == IS_ARRAY) {
						...
						if (Z_TYPE_P(&singleCondition) == IS_STRING) {
							zephir_array_append(&mergedConditions, &singleCondition, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 183);
						}
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, addFrom)
{
	...
	if (_2) {
		...
		zephir_array_append(&models, &model_zv, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 351);
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, autoescape)
{
	...
	_0 = zephir_memnstr_str(&identifier_zv, SL("["), "phalcon/Mvc/Model/Query/Builder.zep", 425);
	if (!(_0)) {
		_0 = zephir_memnstr_str(&identifier_zv, SL("."), "phalcon/Mvc/Model/Query/Builder.zep", 425);
	}
	_1 = _0;
	if (!(_1)) {
		_1 = zephir_memnstr_str(&identifier_zv, SL("("), "phalcon/Mvc/Model/Query/Builder.zep", 425);
	}
	_2 = _1;
	if (!(_2)) {
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql)
{
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		if (UNEXPECTED(!(zephir_fast_count_int(&models)))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_1$$5, "phalcon/Mvc/Model/Query/Builder.zep", 710);
			ZEPHIR_MM_RESTORE();
			return;
		}
	} else {
		if (UNEXPECTED(!zephir_is_true(&models))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_2$$7, "phalcon/Mvc/Model/Query/Builder.zep", 714);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
	...
	if (zephir_is_numeric(&conditions)) {
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			if (UNEXPECTED(zephir_fast_count_int(&models) > 1)) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_3$$10, "phalcon/Mvc/Model/Query/Builder.zep", 727);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (zephir_array_isset_long_fetch(&firstPrimaryKey, &primaryKeys, 0, 0)) {
				...
				if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
					...
					if (UNEXPECTED(!(zephir_array_isset_fetch(&attributeField, &columnMap, &firstPrimaryKey, 0)))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_8$$17, "phalcon/Mvc/Model/Query/Builder.zep", 764);
						ZEPHIR_MM_RESTORE();
						return;
					}
				} else {
		...
		if (UNEXPECTED(noPrimary)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_11$$19, "phalcon/Mvc/Model/Query/Builder.zep", 786);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
	...
	if (Z_TYPE_P(&columns) != IS_NULL) {
		if (Z_TYPE_P(&columns) == IS_ARRAY) {
			...
			zephir_is_iterable(&columns, 0, "phalcon/Mvc/Model/Query/Builder.zep", 819);
			if (Z_TYPE_P(&columns) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _14$$25, _15$$25, _12$$25)
				{
					...
					if (Z_TYPE_P(&columnAlias) == IS_LONG) {
						zephir_array_append(&selectedColumns, &column, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 813);
					} else {
						ZEPHIR_CALL_METHOD(&_16$$28, this_ptr, "autoescape", NULL, 0, &columnAlias);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&_17$$28);
						ZEPHIR_CONCAT_VSV(&_17$$28, &column, " AS ", &_16$$28);
						zephir_array_append(&selectedColumns, &_17$$28, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 815);
					}
				} ZEND_HASH_FOREACH_END();
			} else {
				...
				while (1) {
					...
					zephir_check_call_status();
						if (Z_TYPE_P(&columnAlias) == IS_LONG) {
							...
							ZEPHIR_CALL_METHOD(&_18$$31, this_ptr, "autoescape", NULL, 0, &columnAlias);
							zephir_check_call_status();
							ZEPHIR_INIT_NVAR(&_19$$31);
							ZEPHIR_CONCAT_VSV(&_19$$31, &column, " AS ", &_18$$31);
							zephir_array_append(&selectedColumns, &_19$$31, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 815);
						}
			...
			zephir_fast_join_str(&_20$$25, SL(", "), &selectedColumns);
			zephir_concat_self(&phql, &_20$$25);
		} else {
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			...
			if (Z_TYPE_P(&models) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _23$$34, _24$$34, _21$$34)
				{
					...
					zephir_array_append(&selectedColumns, &selectedColumn, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 837);
				} ZEND_HASH_FOREACH_END();
			} else {
				...
				while (1) {
					...
					zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedColumn);
						if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
							ZEPHIR_CALL_METHOD(&_27$$39, this_ptr, "autoescape", NULL, 0, &model);
							zephir_check_call_status();
							ZEPHIR_CONCAT_VS(&selectedColumn, &_27$$39, ".*");
						} else {
							ZEPHIR_CALL_METHOD(&_28$$40, this_ptr, "autoescape", NULL, 0, &modelColumnAlias);
							zephir_check_call_status();
			...
			zephir_fast_join_str(&_29$$34, SL(", "), &selectedColumns);
			zephir_concat_self(&phql, &_29$$34);
		} else {
			ZEPHIR_CALL_METHOD(&_30$$41, this_ptr, "autoescape", NULL, 0, &models);
			zephir_check_call_status();
			ZEPHIR_INIT_VAR(&_31$$41);
			ZEPHIR_CONCAT_VS(&_31$$41, &_30$$41, ".*");
			zephir_concat_self(&phql, &_31$$41);
		}
	}
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		ZEPHIR_INIT_VAR(&selectedModels);
		array_init(&selectedModels);
		zephir_is_iterable(&models, 0, "phalcon/Mvc/Model/Query/Builder.zep", 862);
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _34$$42, _35$$42, _32$$42)
			{
				...
				if (Z_TYPE_P(&modelAlias) == IS_STRING) {
					ZEPHIR_CALL_METHOD(&_36$$44, this_ptr, "autoescape", NULL, 0, &model);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_37$$44, this_ptr, "autoescape", NULL, 0, &modelAlias);
					zephir_check_call_status();
				...
				zephir_array_append(&selectedModels, &selectedModel, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 859);
			} ZEND_HASH_FOREACH_END();
		} else {
			ZEPHIR_CALL_METHOD(NULL, &models, "rewind", NULL, 0);
			zephir_check_call_status();
			while (1) {
				...
				zephir_check_call_status();
					if (Z_TYPE_P(&modelAlias) == IS_STRING) {
						ZEPHIR_CALL_METHOD(&_38$$47, this_ptr, "autoescape", NULL, 0, &model);
						zephir_check_call_status();
						ZEPHIR_CALL_METHOD(&_39$$47, this_ptr, "autoescape", NULL, 0, &modelAlias);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedModel);
		...
		zephir_fast_join_str(&_40$$42, SL(", "), &selectedModels);
		ZEPHIR_INIT_VAR(&_41$$42);
		ZEPHIR_CONCAT_SV(&_41$$42, " FROM ", &_40$$42);
		zephir_concat_self(&phql, &_41$$42);
	} else {
		ZEPHIR_CALL_METHOD(&_42$$49, this_ptr, "autoescape", NULL, 0, &models);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_43$$49);
		ZEPHIR_CONCAT_SV(&_43$$49, " FROM ", &_42$$49);
		zephir_concat_self(&phql, &_43$$49);
	}
	...
	if (Z_TYPE_P(&joins) == IS_ARRAY) {
		zephir_is_iterable(&joins, 0, "phalcon/Mvc/Model/Query/Builder.zep", 917);
		if (Z_TYPE_P(&joins) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joins), _44$$50)
			{
				...
				zephir_array_fetch_long(&joinModel, &join, 0, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 877);
				ZEPHIR_OBS_NVAR(&joinConditions);
				zephir_array_fetch_long(&joinConditions, &join, 1, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 882);
				ZEPHIR_OBS_NVAR(&joinAlias);
				zephir_array_fetch_long(&joinAlias, &join, 2, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 887);
				ZEPHIR_OBS_NVAR(&joinType);
				zephir_array_fetch_long(&joinType, &join, 3, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 892);
				if (zephir_is_true(&joinType)) {
					ZEPHIR_CALL_METHOD(&_46$$52, this_ptr, "autoescape", NULL, 0, &joinModel);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_47$$52);
					ZEPHIR_CONCAT_SVSV(&_47$$52, " ", &joinType, " JOIN ", &_46$$52);
					zephir_concat_self(&phql, &_47$$52);
				} else {
					ZEPHIR_CALL_METHOD(&_48$$53, this_ptr, "autoescape", NULL, 0, &joinModel);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_49$$53);
					ZEPHIR_CONCAT_SV(&_49$$53, " JOIN ", &_48$$53);
					zephir_concat_self(&phql, &_49$$53);
				}
				if (zephir_is_true(&joinAlias)) {
					ZEPHIR_CALL_METHOD(&_50$$54, this_ptr, "autoescape", NULL, 0, &joinAlias);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_51$$54);
					ZEPHIR_CONCAT_SV(&_51$$54, " AS ", &_50$$54);
					zephir_concat_self(&phql, &_51$$54);
				}
				if (zephir_is_true(&joinConditions)) {
					ZEPHIR_INIT_NVAR(&_52$$55);
					ZEPHIR_CONCAT_SV(&_52$$55, " ON ", &joinConditions);
					zephir_concat_self(&phql, &_52$$55);
				}
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_array_fetch_long(&joinType, &join, 3, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 892);
					if (zephir_is_true(&joinType)) {
						ZEPHIR_CALL_METHOD(&_53$$57, this_ptr, "autoescape", NULL, 0, &joinModel);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&_54$$57);
						ZEPHIR_CONCAT_SVSV(&_54$$57, " ", &joinType, " JOIN ", &_53$$57);
						zephir_concat_self(&phql, &_54$$57);
					} else {
						ZEPHIR_CALL_METHOD(&_55$$58, this_ptr, "autoescape", NULL, 0, &joinModel);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&_56$$58);
						ZEPHIR_CONCAT_SV(&_56$$58, " JOIN ", &_55$$58);
						zephir_concat_self(&phql, &_56$$58);
					}
					if (zephir_is_true(&joinAlias)) {
						ZEPHIR_CALL_METHOD(&_57$$59, this_ptr, "autoescape", NULL, 0, &joinAlias);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&_58$$59);
						ZEPHIR_CONCAT_SV(&_58$$59, " AS ", &_57$$59);
						zephir_concat_self(&phql, &_58$$59);
					}
					if (zephir_is_true(&joinConditions)) {
						ZEPHIR_INIT_NVAR(&_59$$60);
						ZEPHIR_CONCAT_SV(&_59$$60, " ON ", &joinConditions);
						zephir_concat_self(&phql, &_59$$60);
					}
	...
	if (Z_TYPE_P(&conditions) == IS_STRING) {
		if (!(ZEPHIR_IS_EMPTY(&conditions))) {
			...
			zephir_concat_self(&phql, &_60$$62);
		}
	}
	...
	if (!(ZEPHIR_IS_EMPTY(&group))) {
		...
		zephir_is_iterable(&group, 0, "phalcon/Mvc/Model/Query/Builder.zep", 937);
		if (Z_TYPE_P(&group) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&group), _61$$63)
			{
				ZEPHIR_INIT_NVAR(&groupItem);
				ZVAL_COPY(&groupItem, _61$$63);
				ZEPHIR_CALL_METHOD(&_63$$64, this_ptr, "autoescape", NULL, 0, &groupItem);
				zephir_check_call_status();
				zephir_array_append(&groupItems, &_63$$64, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 934);
			} ZEND_HASH_FOREACH_END();
		} else {
			ZEPHIR_CALL_METHOD(NULL, &group, "rewind", NULL, 0);
			zephir_check_call_status();
			while (1) {
				ZEPHIR_CALL_METHOD(&_62$$63, &group, "valid", NULL, 0);
				zephir_check_call_status();
				if (!zend_is_true(&_62$$63)) {
					break;
				}
				ZEPHIR_CALL_METHOD(&groupItem, &group, "current", NULL, 0);
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_64$$65, this_ptr, "autoescape", NULL, 0, &groupItem);
					zephir_check_call_status();
					zephir_array_append(&groupItems, &_64$$65, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 934);
		...
		zephir_fast_join_str(&_65$$63, SL(", "), &groupItems);
		ZEPHIR_INIT_VAR(&_66$$63);
		ZEPHIR_CONCAT_SV(&_66$$63, " GROUP BY ", &_65$$63);
		zephir_concat_self(&phql, &_66$$63);
	}
	...
	if (Z_TYPE_P(&having) != IS_NULL) {
		if (!(ZEPHIR_IS_EMPTY(&having))) {
			...
			ZEPHIR_CONCAT_SV(&_67$$67, " HAVING ", &having);
			zephir_concat_self(&phql, &_67$$67);
		}
	}
	...
	if (Z_TYPE_P(&order) != IS_NULL) {
		if (Z_TYPE_P(&order) == IS_ARRAY) {
			...
			zephir_is_iterable(&order, 0, "phalcon/Mvc/Model/Query/Builder.zep", 981);
			if (Z_TYPE_P(&order) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&order), _68$$69)
				{
					ZEPHIR_INIT_NVAR(&orderItem);
					ZVAL_COPY(&orderItem, _68$$69);
					if (Z_TYPE_P(&orderItem) == IS_LONG) {
						zephir_array_append(&orderItems, &orderItem, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 964);
						continue;
					}
					if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 969) != 0) {
						...
						zephir_array_fetch_long(&_71$$72, &itemExplode$$72, 0, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Query/Builder.zep", 973);
						ZEPHIR_CALL_METHOD(&_70$$72, this_ptr, "autoescape", NULL, 0, &_71$$72);
						zephir_check_call_status();
						zephir_array_fetch_long(&_72$$72, &itemExplode$$72, 1, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Query/Builder.zep", 973);
						ZEPHIR_INIT_NVAR(&_73$$72);
						ZEPHIR_CONCAT_VSV(&_73$$72, &_70$$72, " ", &_72$$72);
						zephir_array_append(&orderItems, &_73$$72, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 973);
						continue;
					}
					ZEPHIR_CALL_METHOD(&_74$$70, this_ptr, "autoescape", NULL, 0, &orderItem);
					zephir_check_call_status();
					zephir_array_append(&orderItems, &_74$$70, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 978);
				} ZEND_HASH_FOREACH_END();
			} else {
				ZEPHIR_CALL_METHOD(NULL, &order, "rewind", NULL, 0);
				zephir_check_call_status();
				while (1) {
					ZEPHIR_CALL_METHOD(&_69$$69, &order, "valid", NULL, 0);
					zephir_check_call_status();
					if (!zend_is_true(&_69$$69)) {
						break;
					}
					...
					zephir_check_call_status();
						...
						if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 969) != 0) {
							...
							zephir_array_fetch_long(&_76$$75, &itemExplode$$75, 0, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Query/Builder.zep", 973);
							ZEPHIR_CALL_METHOD(&_75$$75, this_ptr, "autoescape", NULL, 0, &_76$$75);
							zephir_check_call_status();
							zephir_array_fetch_long(&_77$$75, &itemExplode$$75, 1, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Query/Builder.zep", 973);
							ZEPHIR_INIT_NVAR(&_78$$75);
							ZEPHIR_CONCAT_VSV(&_78$$75, &_75$$75, " ", &_77$$75);
							zephir_array_append(&orderItems, &_78$$75, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 973);
							continue;
						}
						ZEPHIR_CALL_METHOD(&_79$$73, this_ptr, "autoescape", NULL, 0, &orderItem);
						zephir_check_call_status();
						zephir_array_append(&orderItems, &_79$$73, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 978);
			...
			zephir_fast_join_str(&_80$$69, SL(", "), &orderItems);
			ZEPHIR_INIT_VAR(&_81$$69);
			ZEPHIR_CONCAT_SV(&_81$$69, " ORDER BY ", &_80$$69);
			zephir_concat_self(&phql, &_81$$69);
		} else {
			ZEPHIR_INIT_VAR(&_82$$76);
			ZEPHIR_CONCAT_SV(&_82$$76, " ORDER BY ", &order);
			zephir_concat_self(&phql, &_82$$76);
		}
	}
	...
	if (Z_TYPE_P(&limit) != IS_NULL) {
		...
		if (Z_TYPE_P(&limit) == IS_ARRAY) {
			...
			zephir_array_fetch_string(&number, &limit, SL("number"), PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 995);
			zephir_memory_observe(&offset);
			...
			if (zephir_is_numeric(&limit)) {
				...
				zephir_read_property(&_83$$82, this_ptr, ZEND_STRL("offset"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_CPY_WRT(&offset, &_83$$82);
		...
		if (zephir_is_numeric(&number)) {
			...
			ZVAL_LONG(&_84$$85, 10);
			ZEPHIR_CALL_FUNCTION(&_85$$85, "intval", NULL, 77, &number, &_84$$85);
			zephir_check_call_status();
			ZEPHIR_INIT_VAR(&_86$$85);
			ZVAL_STRING(&_86$$85, "APL0");
			zephir_update_property_array(this_ptr, SL("bindParams"), &_86$$85, &_85$$85);
			ZEPHIR_INIT_VAR(&_87$$85);
			ZVAL_STRING(&_87$$85, "APL0");
			ZEPHIR_INIT_VAR(&_88$$85);
			ZVAL_LONG(&_88$$85, 1);
			zephir_update_property_array(this_ptr, SL("bindTypes"), &_87$$85, &_88$$85);
			_89$$85 = zephir_is_numeric(&offset);
			if (_89$$85) {
				_89$$85 = !ZEPHIR_IS_LONG_IDENTICAL(&offset, 0);
			}
			if (_89$$85) {
				zephir_concat_self_str(&phql, SL(" OFFSET :APL1:"));
				ZVAL_LONG(&_90$$86, 10);
				ZEPHIR_CALL_FUNCTION(&_91$$86, "intval", NULL, 77, &offset, &_90$$86);
				zephir_check_call_status();
				ZEPHIR_INIT_VAR(&_92$$86);
				ZVAL_STRING(&_92$$86, "APL1");
				zephir_update_property_array(this_ptr, SL("bindParams"), &_92$$86, &_91$$86);
				ZEPHIR_INIT_VAR(&_93$$86);
				ZVAL_STRING(&_93$$86, "APL1");
				ZEPHIR_INIT_VAR(&_94$$86);
				ZVAL_LONG(&_94$$86, 1);
				zephir_update_property_array(this_ptr, SL("bindTypes"), &_93$$86, &_94$$86);
			}
		}
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getQuery)
{
	...
	if (UNEXPECTED(Z_TYPE_P(&container) != IS_OBJECT)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Query/Builder.zep", 1050);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_3);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, groupBy)
{
	...
	if (Z_TYPE_P(group) == IS_STRING) {
		if (zephir_memnstr_str(group, SL(","), "phalcon/Mvc/Model/Query/Builder.zep", 1106)) {
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, limit)
{
	...
	ZVAL_LONG(&_0, limit);
	ZEPHIR_CALL_FUNCTION(&_1, "abs", NULL, 495, &_0);
	zephir_check_call_status();
	...
	if (zephir_is_numeric(offset)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$4, "abs", NULL, 495, &_2$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, conditionBetween)
{
	...
	if (UNEXPECTED(_0)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Query/Builder.zep", 1584);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
...
{
	zval _2, _3$$4, _8$$5, _10$$6;
	zend_bool _0;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS, hiddenParam = 0;
	zval values;
	zval clause_zv, operator_zv, expr_zv, *values_param = NULL, key, queryKey, value, bindKeys, bindParams, operatorMethod, _4, *_5, _6, _11, _12, _13, _1$$3, _7$$5, _9$$6;
	zend_string *clause = NULL, *operator = NULL, *expr = NULL;
	zval *this_ptr = getThis();
	...
	if (UNEXPECTED(_0)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 0, &operator_zv);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Query/Builder.zep", 1627);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
	...
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/Mvc/Model/Query/Builder.zep", 1658);
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _5)
		{
			...
			ZVAL_LONG(&_7$$5, hiddenParam);
			ZEPHIR_INIT_NVAR(&_8$$5);
			ZEPHIR_CONCAT_SV(&_8$$5, "AP", &_7$$5);
			ZEPHIR_CPY_WRT(&key, &_8$$5);
			ZEPHIR_INIT_NVAR(&queryKey);
			ZEPHIR_CONCAT_SVS(&queryKey, ":", &key, ":");
			zephir_array_append(&bindKeys, &queryKey, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 1649);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZVAL_LONG(&_9$$6, hiddenParam);
				ZEPHIR_INIT_NVAR(&_10$$6);
				ZEPHIR_CONCAT_SV(&_10$$6, "AP", &_9$$6);
				ZEPHIR_CPY_WRT(&key, &_10$$6);
				ZEPHIR_INIT_NVAR(&queryKey);
				ZEPHIR_CONCAT_SVS(&queryKey, ":", &key, ":");
				zephir_array_append(&bindKeys, &queryKey, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 1649);
				zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
	...
	ZEPHIR_INIT_NVAR(&value);
	ZEPHIR_INIT_VAR(&_11);
	zephir_fast_join_str(&_11, SL(", "), &bindKeys);
	ZEPHIR_INIT_VAR(&_12);
	ZEPHIR_CONCAT_VSVS(&_12, &expr_zv, " IN (", &_11, ")");
	ZEPHIR_CALL_METHOD_ZVAL(NULL, this_ptr, &operatorMethod, NULL, 0, &_12, &bindParams);
	zephir_check_call_status();
	ZVAL_UNDEF(&_13);
	ZVAL_LONG(&_13, hiddenParam);
	zephir_update_property_zval(this_ptr, ZEND_STRL("hiddenParamNumber"), &_13);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, conditionNotBetween)
{
	...
	if (UNEXPECTED(_0)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 0, &operator_zv);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Query/Builder.zep", 1676);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, conditionNotIn)
{
	...
	if (UNEXPECTED(_0)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 0, &operator_zv);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Query/Builder.zep", 1718);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
	...
	ZEPHIR_INIT_VAR(&bindKeys);
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/Mvc/Model/Query/Builder.zep", 1749);
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _5)
		{
			ZEPHIR_INIT_NVAR(&value);
			...
			ZEPHIR_INIT_NVAR(&queryKey);
			ZEPHIR_CONCAT_SVS(&queryKey, ":", &key, ":");
			zephir_array_append(&bindKeys, &queryKey, PH_SEPARATE, "phalcon/Mvc/Model/Query/Builder.zep", 1740);
			zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
			hiddenParam++;
		} ZEND_HASH_FOREACH_END();
	} else {
	...
	zephir_fast_join_str(&_11, SL(", "), &bindKeys);
	ZEPHIR_INIT_VAR(&_12);
	ZEPHIR_CONCAT_VSVS(&_12, &expr_zv, " NOT IN (", &_11, ")");
	ZEPHIR_CALL_METHOD_ZVAL(NULL, this_ptr, &operatorMethod, NULL, 0, &_12, &bindParams);
	zephir_check_call_status();
	ZVAL_UNDEF(&_13);
	ZVAL_LONG(&_13, hiddenParam);
