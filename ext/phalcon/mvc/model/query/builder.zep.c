...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, __construct) {
	...
	if (Z_TYPE_P(params) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&conditions) == IS_ARRAY) {
			...
			zephir_is_iterable(&conditions, 0, "phalcon/mvc/model/query/builder.zep", 145);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&conditions), _0$$7)
			{
				...
				if (Z_TYPE_P(&singleConditionArray) == IS_ARRAY) {
					...
					if (Z_TYPE_P(&singleCondition) == IS_STRING) {
						zephir_array_append(&mergedConditions, &singleCondition, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 132);
					}
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, addFrom) {
	...
	if (Z_TYPE_P(alias) == IS_STRING) {
		...
		zephir_array_append(&models, model, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 394);
	}
	zephir_update_property_zval(this_ptr, SL("_models"), &models);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, where) {
	...
	zval *conditions, conditions_sub, *bindParams = NULL, bindParams_sub, *bindTypes = NULL, bindTypes_sub, __$null, currentBindParams, currentBindTypes, mergedParams, mergedTypes;
	ZEPHIR_INIT_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, andWhere) {
	...
	zval *conditions_param = NULL, *bindParams = NULL, bindParams_sub, *bindTypes = NULL, bindTypes_sub, __$null, currentBindParams, currentBindTypes, mergedParams, mergedTypes, currentConditions, newConditions;
	zval conditions;
	ZEPHIR_INIT_THIS();
	...
	zephir_read_property(&currentConditions, this_ptr, SL("_conditions"), PH_NOISY_CC | PH_READONLY);
	...
	zephir_update_property_zval(this_ptr, SL("_conditions"), &newConditions);
	if (Z_TYPE_P(bindParams) == IS_ARRAY) {
		zephir_read_property(&currentBindParams, this_ptr, SL("_bindParams"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&currentBindParams) == IS_ARRAY) {
			ZEPHIR_INIT_VAR(&mergedParams);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, orWhere) {
	...
	zval *conditions_param = NULL, *bindParams = NULL, bindParams_sub, *bindTypes = NULL, bindTypes_sub, __$null, currentBindParams, currentBindTypes, mergedParams, mergedTypes, currentConditions, _0$$3;
	zval conditions;
	ZEPHIR_INIT_THIS();
	...
	if (zephir_is_true(&currentConditions)) {
		...
		zephir_update_property_zval(this_ptr, SL("_conditions"), &_0$$3);
	} else {
		zephir_update_property_zval(this_ptr, SL("_conditions"), &conditions);
	}
	if (Z_TYPE_P(bindParams) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, limit) {
	...
	zval *limit = NULL, limit_sub, *offset = NULL, offset_sub, __$null, _0$$3;
	ZEPHIR_INIT_THIS();
	...
	if (zephir_is_numeric(offset)) {
		...
		zephir_update_property_zval(this_ptr, SL("_offset"), &_0$$3);
	}
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql) {
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		if (!(zephir_fast_count_int(&models TSRMLS_CC))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "At least one model is required to build the query", "phalcon/mvc/model/query/builder.zep", 1034);
			return;
		}
	} else {
		if (!(zephir_is_true(&models))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "At least one model is required to build the query", "phalcon/mvc/model/query/builder.zep", 1038);
			return;
		}
	}
	ZEPHIR_OBS_VAR(&conditions);
	...
	if (zephir_is_numeric(&conditions)) {
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			if (zephir_fast_count_int(&models TSRMLS_CC) > 1) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Cannot build the query. Invalid condition", "phalcon/mvc/model/query/builder.zep", 1051);
				return;
			}
			ZEPHIR_OBS_VAR(&model);
			zephir_array_fetch_long(&model, &models, 0, PH_NOISY, "phalcon/mvc/model/query/builder.zep", 1053 TSRMLS_CC);
		} else {
			...
			if (zephir_array_isset_long_fetch(&firstPrimaryKey, &primaryKeys, 0, 0 TSRMLS_CC)) {
				...
				if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
					...
					if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &firstPrimaryKey, 0 TSRMLS_CC))) {
						...
						ZEPHIR_CALL_METHOD(NULL, &_5$$17, "__construct", NULL, 9, &_6$$17);
						zephir_check_call_status();
						zephir_throw_exception_debug(&_5$$17, "phalcon/mvc/model/query/builder.zep", 1081 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
				} else {
				...
				ZEPHIR_CALL_METHOD(&_7$$13, this_ptr, "autoescape", &_8, 367, &model);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_9$$13, this_ptr, "autoescape", &_8, 367, &attributeField);
				zephir_check_call_status();
		...
		if (noPrimary == 1) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Source related to this model does not have a primary key defined", "phalcon/mvc/model/query/builder.zep", 1096);
			return;
		}
	}
	ZEPHIR_OBS_VAR(&distinct);
	...
	if (Z_TYPE_P(&columns) != IS_NULL) {
		if (Z_TYPE_P(&columns) == IS_ARRAY) {
			...
			zephir_is_iterable(&columns, 0, "phalcon/mvc/model/query/builder.zep", 1128);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _13$$25, _14$$25, _12$$25)
			{
				...
				if (Z_TYPE_P(&columnAlias) == IS_LONG) {
					zephir_array_append(&selectedColumns, &column, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1122);
				} else {
					ZEPHIR_CALL_METHOD(&_15$$28, this_ptr, "autoescape", &_8, 367, &columnAlias);
					zephir_check_call_status();
					ZEPHIR_INIT_LNVAR(_16$$28);
					ZEPHIR_CONCAT_VSV(&_16$$28, &column, " AS ", &_15$$28);
					zephir_array_append(&selectedColumns, &_16$$28, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1124);
				}
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&column);
			ZEPHIR_INIT_NVAR(&columnAlias);
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			...
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _19$$31, _20$$31, _18$$31)
			{
				...
				if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
					ZEPHIR_CALL_METHOD(&_21$$33, this_ptr, "autoescape", &_8, 367, &model);
					zephir_check_call_status();
					ZEPHIR_CONCAT_VS(&selectedColumn, &_21$$33, ".*");
				} else {
					ZEPHIR_CALL_METHOD(&_22$$34, this_ptr, "autoescape", &_8, 367, &modelColumnAlias);
					zephir_check_call_status();
					ZEPHIR_CONCAT_VS(&selectedColumn, &_22$$34, ".*");
				}
				zephir_array_append(&selectedColumns, &selectedColumn, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1148);
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&model);
			ZEPHIR_INIT_NVAR(&modelColumnAlias);
			...
			ZEPHIR_CALL_METHOD(&_24$$35, this_ptr, "autoescape", &_8, 367, &models);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		...
		zephir_is_iterable(&models, 0, "phalcon/mvc/model/query/builder.zep", 1174);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _27$$36, _28$$36, _26$$36)
		{
			...
			if (Z_TYPE_P(&modelAlias) == IS_STRING) {
				ZEPHIR_CALL_METHOD(&_29$$38, this_ptr, "autoescape", &_8, 367, &model);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_30$$38, this_ptr, "autoescape", &_8, 367, &modelAlias);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&selectedModel);
				ZEPHIR_CONCAT_VSV(&selectedModel, &_29$$38, " AS ", &_30$$38);
			} else {
				ZEPHIR_CALL_METHOD(&selectedModel, this_ptr, "autoescape", &_8, 367, &model);
				zephir_check_call_status();
			}
			zephir_array_append(&selectedModels, &selectedModel, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1171);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&model);
		ZEPHIR_INIT_NVAR(&modelAlias);
		...
		ZEPHIR_CALL_METHOD(&_33$$40, this_ptr, "autoescape", &_8, 367, &models);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&joins) == IS_ARRAY) {
		zephir_is_iterable(&joins, 0, "phalcon/mvc/model/query/builder.zep", 1231);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joins), _35$$41)
		{
			ZEPHIR_INIT_NVAR(&join);
			ZVAL_COPY(&join, _35$$41);
			ZEPHIR_OBS_NVAR(&joinModel);
			zephir_array_fetch_long(&joinModel, &join, 0, PH_NOISY, "phalcon/mvc/model/query/builder.zep", 1191 TSRMLS_CC);
			ZEPHIR_OBS_NVAR(&joinConditions);
			zephir_array_fetch_long(&joinConditions, &join, 1, PH_NOISY, "phalcon/mvc/model/query/builder.zep", 1196 TSRMLS_CC);
			ZEPHIR_OBS_NVAR(&joinAlias);
			zephir_array_fetch_long(&joinAlias, &join, 2, PH_NOISY, "phalcon/mvc/model/query/builder.zep", 1201 TSRMLS_CC);
			ZEPHIR_OBS_NVAR(&joinType);
			zephir_array_fetch_long(&joinType, &join, 3, PH_NOISY, "phalcon/mvc/model/query/builder.zep", 1206 TSRMLS_CC);
			if (zephir_is_true(&joinType)) {
				ZEPHIR_CALL_METHOD(&_36$$43, this_ptr, "autoescape", &_8, 367, &joinModel);
				zephir_check_call_status();
				ZEPHIR_INIT_LNVAR(_37$$43);
				ZEPHIR_CONCAT_SVSV(&_37$$43, " ", &joinType, " JOIN ", &_36$$43);
				zephir_concat_self(&phql, &_37$$43 TSRMLS_CC);
			} else {
				ZEPHIR_CALL_METHOD(&_38$$44, this_ptr, "autoescape", &_8, 367, &joinModel);
				zephir_check_call_status();
			...
			if (zephir_is_true(&joinAlias)) {
				ZEPHIR_CALL_METHOD(&_40$$45, this_ptr, "autoescape", &_8, 367, &joinAlias);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(&group) != IS_NULL) {
		if (Z_TYPE_P(&group) == IS_STRING) {
			if (zephir_memnstr_str(&group, SL(","), "phalcon/mvc/model/query/builder.zep", 1246)) {
		...
		zephir_is_iterable(&group, 0, "phalcon/mvc/model/query/builder.zep", 1258);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&group), _48$$49)
		{
			ZEPHIR_INIT_NVAR(&groupItem);
			ZVAL_COPY(&groupItem, _48$$49);
			ZEPHIR_CALL_METHOD(&_49$$52, this_ptr, "autoescape", &_8, 367, &groupItem);
			zephir_check_call_status();
			zephir_array_append(&groupItems, &_49$$52, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1255);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&groupItem);
	...
	if (Z_TYPE_P(&order) != IS_NULL) {
		if (Z_TYPE_P(&order) == IS_ARRAY) {
			...
			zephir_is_iterable(&order, 0, "phalcon/mvc/model/query/builder.zep", 1278);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&order), _53$$56)
			{
				ZEPHIR_INIT_NVAR(&orderItem);
				ZVAL_COPY(&orderItem, _53$$56);
				ZEPHIR_CALL_METHOD(&_54$$57, this_ptr, "autoescape", &_8, 367, &orderItem);
				zephir_check_call_status();
				zephir_array_append(&orderItems, &_54$$57, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1276);
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&orderItem);
	...
	if (Z_TYPE_P(&limit) != IS_NULL) {
		...
		if (Z_TYPE_P(&limit) == IS_ARRAY) {
			...
			zephir_array_fetch_string(&number, &limit, SL("number"), PH_NOISY, "phalcon/mvc/model/query/builder.zep", 1293 TSRMLS_CC);
			ZEPHIR_OBS_VAR(&offset);
		...
		if (zephir_is_numeric(&number)) {
			...
			ZVAL_LONG(&_58$$67, 10);
			ZEPHIR_CALL_FUNCTION(&_59$$67, "intval", &_60, 26, &number, &_58$$67);
			zephir_check_call_status();
			...
			if (zephir_is_numeric(&offset)) {
				...
				ZVAL_LONG(&_64$$68, 10);
				ZEPHIR_CALL_FUNCTION(&_65$$68, "intval", &_60, 26, &offset, &_64$$68);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getQuery) {
	...
	ZEPHIR_CALL_METHOD(&phql, this_ptr, "getphql", NULL, 368);
	zephir_check_call_status();
	zephir_read_property(&_0, this_ptr, SL("_dependencyInjector"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&dependencyInjector, &_0);
	if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "A dependency injection object is required to access ORM services", "phalcon/mvc/model/query/builder.zep", 1347);
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, autoescape) {
	...
	_0 = zephir_memnstr_str(&identifier, SL("["), "phalcon/mvc/model/query/builder.zep", 1382);
	if (!(_0)) {
		_0 = zephir_memnstr_str(&identifier, SL("."), "phalcon/mvc/model/query/builder.zep", 1382);
	}
	_1 = _0;
