...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql) {
			...
			if (zephir_array_isset_long_fetch(&firstPrimaryKey, &primaryKeys, 0, 0 TSRMLS_CC)) {
				...
				if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
					...
					if (!(zephir_array_isset_fetch(&attributeField, &columnMap, &firstPrimaryKey, 0 TSRMLS_CC))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_5$$17, "phalcon/mvc/model/query/builder.zep", 1085 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
				} else {
				...
				ZEPHIR_CALL_METHOD(&_7$$13, this_ptr, "autoescape", &_8, 368, &model);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_9$$13, this_ptr, "autoescape", &_8, 368, &attributeField);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(&columns) != IS_NULL) {
		if (Z_TYPE_P(&columns) == IS_ARRAY) {
			...
			zephir_is_iterable(&columns, 0, "phalcon/mvc/model/query/builder.zep", 1132);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _13$$25, _14$$25, _12$$25)
			{
				...
				if (Z_TYPE_P(&columnAlias) == IS_LONG) {
					...
					ZEPHIR_CALL_METHOD(&_15$$28, this_ptr, "autoescape", &_8, 368, &columnAlias);
					zephir_check_call_status();
					ZEPHIR_INIT_LNVAR(_16$$28);
					ZEPHIR_CONCAT_VSV(&_16$$28, &column, " AS ", &_15$$28);
					zephir_array_append(&selectedColumns, &_16$$28, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1128);
				}
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&column);
			ZEPHIR_INIT_NVAR(&columnAlias);
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			...
			array_init(&selectedColumns);
			zephir_is_iterable(&models, 0, "phalcon/mvc/model/query/builder.zep", 1155);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _19$$31, _20$$31, _18$$31)
			{
				...
				if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
					ZEPHIR_CALL_METHOD(&_21$$33, this_ptr, "autoescape", &_8, 368, &model);
					zephir_check_call_status();
					ZEPHIR_CONCAT_VS(&selectedColumn, &_21$$33, ".*");
				} else {
					ZEPHIR_CALL_METHOD(&_22$$34, this_ptr, "autoescape", &_8, 368, &modelColumnAlias);
					zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_24$$35, this_ptr, "autoescape", &_8, 368, &models);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		...
		zephir_is_iterable(&models, 0, "phalcon/mvc/model/query/builder.zep", 1178);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _27$$36, _28$$36, _26$$36)
		{
			...
			if (Z_TYPE_P(&modelAlias) == IS_STRING) {
				ZEPHIR_CALL_METHOD(&_29$$38, this_ptr, "autoescape", &_8, 368, &model);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_30$$38, this_ptr, "autoescape", &_8, 368, &modelAlias);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&selectedModel);
				ZEPHIR_CONCAT_VSV(&selectedModel, &_29$$38, " AS ", &_30$$38);
			} else {
				ZEPHIR_CALL_METHOD(&selectedModel, this_ptr, "autoescape", &_8, 368, &model);
				zephir_check_call_status();
			}
		...
		ZEPHIR_CALL_METHOD(&_33$$40, this_ptr, "autoescape", &_8, 368, &models);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&joins) == IS_ARRAY) {
		zephir_is_iterable(&joins, 0, "phalcon/mvc/model/query/builder.zep", 1235);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joins), _35$$41)
		{
			...
			if (zephir_is_true(&joinType)) {
				ZEPHIR_CALL_METHOD(&_36$$43, this_ptr, "autoescape", &_8, 368, &joinModel);
				zephir_check_call_status();
				ZEPHIR_INIT_LNVAR(_37$$43);
				ZEPHIR_CONCAT_SVSV(&_37$$43, " ", &joinType, " JOIN ", &_36$$43);
				zephir_concat_self(&phql, &_37$$43 TSRMLS_CC);
			} else {
				ZEPHIR_CALL_METHOD(&_38$$44, this_ptr, "autoescape", &_8, 368, &joinModel);
				zephir_check_call_status();
			...
			if (zephir_is_true(&joinAlias)) {
				ZEPHIR_CALL_METHOD(&_40$$45, this_ptr, "autoescape", &_8, 368, &joinAlias);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(&group) != IS_NULL) {
		...
		zephir_is_iterable(&group, 0, "phalcon/mvc/model/query/builder.zep", 1262);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&group), _48$$49)
		{
			ZEPHIR_INIT_NVAR(&groupItem);
			ZVAL_COPY(&groupItem, _48$$49);
			ZEPHIR_CALL_METHOD(&_49$$52, this_ptr, "autoescape", &_8, 368, &groupItem);
			zephir_check_call_status();
			zephir_array_append(&groupItems, &_49$$52, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1259);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&groupItem);
	...
	if (Z_TYPE_P(&order) != IS_NULL) {
		if (Z_TYPE_P(&order) == IS_ARRAY) {
			...
			zephir_is_iterable(&order, 0, "phalcon/mvc/model/query/builder.zep", 1282);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&order), _53$$56)
			{
				ZEPHIR_INIT_NVAR(&orderItem);
				ZVAL_COPY(&orderItem, _53$$56);
				ZEPHIR_CALL_METHOD(&_54$$57, this_ptr, "autoescape", &_8, 368, &orderItem);
				zephir_check_call_status();
				zephir_array_append(&orderItems, &_54$$57, PH_SEPARATE, "phalcon/mvc/model/query/builder.zep", 1280);
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&orderItem);
	...
	if (Z_TYPE_P(&limit) != IS_NULL) {
		...
		if (zephir_is_numeric(&number)) {
			...
			ZVAL_LONG(&_58$$67, 10);
			ZEPHIR_CALL_FUNCTION(&_59$$67, "intval", &_60, 36, &number, &_58$$67);
			zephir_check_call_status();
			...
			if (zephir_is_numeric(&offset)) {
				...
				ZVAL_LONG(&_64$$68, 10);
				ZEPHIR_CALL_FUNCTION(&_65$$68, "intval", &_60, 36, &offset, &_64$$68);
				zephir_check_call_status();
