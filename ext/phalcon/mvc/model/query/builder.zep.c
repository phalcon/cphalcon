...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getModels)
{
	...
	if (_1) {
		ZEPHIR_RETURN_CALL_CE_STATIC(phalcon_helper_arr_ce, "first", &_2, 298, &models);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_CCTOR(&models);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, limit)
{
	...
	ZVAL_LONG(&_0, limit);
	ZEPHIR_CALL_FUNCTION(&_1, "abs", NULL, 274, &_0);
	zephir_check_call_status();
	...
	if (zephir_is_numeric(offset)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$4, "abs", NULL, 274, &_2$$4);
		zephir_check_call_status();
