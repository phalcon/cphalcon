...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getModels)
{
	...
	if (_1) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("reset", NULL, 472, &models);
		ZEPHIR_UNREF(&models);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql)
{
	...
	if (zephir_is_numeric(&conditions)) {
		...
		if (zephir_fast_count_int(&primaryKeys)) {
			...
			if (zephir_array_isset_long_fetch(&firstPrimaryKey, &primaryKeys, 0, 0)) {
				...
				ZEPHIR_CALL_METHOD(&_7$$13, this_ptr, "autoescape", NULL, 473, &model);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_8$$13, this_ptr, "autoescape", NULL, 473, &attributeField);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(&columns) != IS_NULL) {
			...
			if (Z_TYPE_P(&columns) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _12$$26, _13$$26, _10$$26)
				{
					...
					if (Z_TYPE_P(&columnAlias) == IS_LONG) {
						...
						ZEPHIR_CALL_METHOD(&_14$$29, this_ptr, "autoescape", NULL, 473, &columnAlias);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						if (Z_TYPE_P(&columnAlias) == IS_LONG) {
							...
							ZEPHIR_CALL_METHOD(&_16$$32, this_ptr, "autoescape", NULL, 473, &columnAlias);
							zephir_check_call_status();
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			...
			if (Z_TYPE_P(&models) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _21$$35, _22$$35, _19$$35)
				{
					...
					if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
						ZEPHIR_CALL_METHOD(&_23$$37, this_ptr, "autoescape", NULL, 473, &model);
						zephir_check_call_status();
						ZEPHIR_CONCAT_VS(&selectedColumn, &_23$$37, ".*");
					} else {
						ZEPHIR_CALL_METHOD(&_24$$38, this_ptr, "autoescape", NULL, 473, &modelColumnAlias);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedColumn);
						if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
							ZEPHIR_CALL_METHOD(&_25$$40, this_ptr, "autoescape", NULL, 473, &model);
							zephir_check_call_status();
							ZEPHIR_CONCAT_VS(&selectedColumn, &_25$$40, ".*");
						} else {
							ZEPHIR_CALL_METHOD(&_26$$41, this_ptr, "autoescape", NULL, 473, &modelColumnAlias);
							zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_28$$42, this_ptr, "autoescape", NULL, 473, &models);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _32$$43, _33$$43, _30$$43)
			{
				...
				if (Z_TYPE_P(&modelAlias) == IS_STRING) {
					ZEPHIR_CALL_METHOD(&_34$$45, this_ptr, "autoescape", NULL, 473, &model);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_35$$45, this_ptr, "autoescape", NULL, 473, &modelAlias);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					if (Z_TYPE_P(&modelAlias) == IS_STRING) {
						ZEPHIR_CALL_METHOD(&_36$$48, this_ptr, "autoescape", NULL, 473, &model);
						zephir_check_call_status();
						ZEPHIR_CALL_METHOD(&_37$$48, this_ptr, "autoescape", NULL, 473, &modelAlias);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedModel);
		...
		ZEPHIR_CALL_METHOD(&_40$$50, this_ptr, "autoescape", NULL, 473, &models);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&joins) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joins), _42$$51)
			{
				...
				if (zephir_is_true(&joinType)) {
					ZEPHIR_CALL_METHOD(&_44$$53, this_ptr, "autoescape", NULL, 473, &joinModel);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_45$$53);
					ZEPHIR_CONCAT_SVSV(&_45$$53, " ", &joinType, " JOIN ", &_44$$53);
					zephir_concat_self(&phql, &_45$$53);
				} else {
					ZEPHIR_CALL_METHOD(&_46$$54, this_ptr, "autoescape", NULL, 473, &joinModel);
					zephir_check_call_status();
				...
				if (zephir_is_true(&joinAlias)) {
					ZEPHIR_CALL_METHOD(&_48$$55, this_ptr, "autoescape", NULL, 473, &joinAlias);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_array_fetch_long(&joinType, &join, 3, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 872);
					if (zephir_is_true(&joinType)) {
						ZEPHIR_CALL_METHOD(&_51$$58, this_ptr, "autoescape", NULL, 473, &joinModel);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&_52$$58);
						ZEPHIR_CONCAT_SVSV(&_52$$58, " ", &joinType, " JOIN ", &_51$$58);
						zephir_concat_self(&phql, &_52$$58);
					} else {
						ZEPHIR_CALL_METHOD(&_53$$59, this_ptr, "autoescape", NULL, 473, &joinModel);
						zephir_check_call_status();
					...
					if (zephir_is_true(&joinAlias)) {
						ZEPHIR_CALL_METHOD(&_55$$60, this_ptr, "autoescape", NULL, 473, &joinAlias);
						zephir_check_call_status();
	...
	if (!(ZEPHIR_IS_EMPTY(&group))) {
		...
		if (Z_TYPE_P(&group) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&group), _59$$64)
			{
				...
				ZEPHIR_CALL_METHOD(&_61$$65, this_ptr, "autoescape", NULL, 473, &groupItem);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_62$$66, this_ptr, "autoescape", NULL, 473, &groupItem);
					zephir_check_call_status();
			...
			if (Z_TYPE_P(&order) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&order), _66$$70)
				{
					...
					if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 949) != 0) {
						...
						ZEPHIR_CALL_METHOD(&_68$$73, this_ptr, "autoescape", NULL, 473, &_69$$73);
						zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_72$$71, this_ptr, "autoescape", NULL, 473, &orderItem);
					zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 949) != 0) {
							...
							ZEPHIR_CALL_METHOD(&_73$$76, this_ptr, "autoescape", NULL, 473, &_74$$76);
							zephir_check_call_status();
						...
						ZEPHIR_CALL_METHOD(&_77$$74, this_ptr, "autoescape", NULL, 473, &orderItem);
						zephir_check_call_status();
