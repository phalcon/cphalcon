...
PHP_METHOD(Phalcon_Mvc_Model_Binder, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 993, cache);
}
...
PHP_METHOD(Phalcon_Mvc_Model_Binder, bindToHandler)
{
	...
	ZEPHIR_INIT_VAR(&_3);
	array_init(&_3);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 995, &_3);
...
PHP_METHOD(Phalcon_Mvc_Model_Binder, getParamsFromReflection)
{
	...
	if (!ZEPHIR_IS_NULL(&methodName_zv)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 238, handler);
		zephir_check_call_status();
	}
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 993, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&cache, &_0);
	ZEPHIR_CALL_METHOD(&methodParams, &reflection, "getparameters", NULL, 239);
	zephir_check_call_status();
