...
PHP_METHOD(Phalcon_Mvc_Model_Binder, bindToHandler) {
	...
	if (_1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "You must specify methodName for handler or pass Closure as handler", "phalcon/Mvc/Model/Binder.zep", 64);
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
	array_init(&_2);
	...
	if (Z_TYPE_P(&paramsCache) == IS_ARRAY) {
		zephir_is_iterable(&paramsCache, 0, "phalcon/Mvc/Model/Binder.zep", 79);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&paramsCache), _4$$4, _5$$4, _3$$4)
		{
			...
			zephir_array_fetch(&paramValue, &params, &paramKey, PH_NOISY, "phalcon/Mvc/Model/Binder.zep", 72 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Mvc_Model_Binder, getParamsFromReflection) {
	...
	if (!ZEPHIR_IS_STRING(&methodName, "")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 387, handler, &methodName);
		zephir_check_call_status();
	} else {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&methodParams), _2, _3, _1)
	{
		...
		if (!(zephir_array_isset(&params, &paramKey))) {
			zephir_array_fetch(&_4$$7, &paramsKeys, &paramKey, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Binder.zep", 155 TSRMLS_CC);
			ZEPHIR_CPY_WRT(&paramKey, &_4$$7);
		}
		ZEPHIR_INIT_NVAR(&boundModel);
		ZVAL_NULL(&boundModel);
		ZEPHIR_OBS_NVAR(&paramValue);
		zephir_array_fetch(&paramValue, &params, &paramKey, PH_NOISY, "phalcon/Mvc/Model/Binder.zep", 159 TSRMLS_CC);
		ZEPHIR_INIT_NVAR(&_5$$5);
		ZVAL_STRING(&_5$$5, "Phalcon\\Mvc\\Model");
		ZEPHIR_CALL_FUNCTION(&_6$$5, "is_subclass_of", &_7, 388, &className, &_5$$5);
		zephir_check_call_status();
		if (ZEPHIR_IS_STRING(&className, "Phalcon\\Mvc\\Model")) {
			if (Z_TYPE_P(&realClasses) == IS_NULL) {
				if (zephir_instance_of_ev(handler, phalcon_mvc_controller_bindmodelinterface_ce TSRMLS_CC)) {
					...
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_mvc_model_exception_ce, "Handler must implement Phalcon\\Mvc\\Model\\Binder\\BindableInterface in order to use Phalcon\\Mvc\\Model as parameter", "phalcon/Mvc/Model/Binder.zep", 169);
					return;
				}
			}
			if (Z_TYPE_P(&realClasses) == IS_ARRAY) {
				...
				if (!(zephir_array_isset_fetch(&className, &realClasses, &paramKey, 0 TSRMLS_CC))) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_9$$14, "phalcon/Mvc/Model/Binder.zep", 175 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
