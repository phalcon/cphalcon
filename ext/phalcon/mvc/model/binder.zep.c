...
PHP_METHOD(Phalcon_Mvc_Model_Binder, getParamsFromReflection) {
	...
	zval *handler, handler_sub, *params_param = NULL, *cacheKey_param = NULL, *methodName_param = NULL, methodParams, reflection, paramKey, methodParam, className, realClasses, boundModel, cache, handlerClass, reflectionClass, paramsKeys, paramValue, _0, *_1, _4$$7, _5$$5, _6$$5, _9$$14, _10$$14;
	zval *this_ptr = getThis();
	...
	if (!ZEPHIR_IS_STRING(&methodName, "")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 435, handler, &methodName);
		zephir_check_call_status();
	} else {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&methodParams), _2, _3, _1)
	{
		...
		if (!(zephir_array_isset(&params, &paramKey))) {
			zephir_array_fetch(&_4$$7, &paramsKeys, &paramKey, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Binder.zep", 164 TSRMLS_CC);
		...
		ZEPHIR_CALL_FUNCTION(&_6$$5, "is_subclass_of", &_7, 436, &className, &_5$$5);
		zephir_check_call_status();
		if (ZEPHIR_IS_STRING(&className, "Phalcon\\Mvc\\Model")) {
			...
			if (Z_TYPE_P(&realClasses) == IS_ARRAY) {
				...
				if (UNEXPECTED(!(zephir_array_isset_fetch(&className, &realClasses, &paramKey, 0 TSRMLS_CC)))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_9$$14, "__construct", &_11, 1, &_10$$14);
					zephir_check_call_status();
					zephir_throw_exception_debug(&_9$$14, "phalcon/Mvc/Model/Binder.zep", 188 TSRMLS_CC);
					ZEPHIR_MM_RESTORE();
					return;
				}
