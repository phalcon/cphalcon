			...
			if (Z_TYPE_P(&data) != IS_NULL) {
				...
				if ((zephir_method_exists_ex(model, SL("metadata") TSRMLS_CC) == SUCCESS)) {
					...
					if (Z_TYPE_P(&modelMetadata) != IS_ARRAY) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_0$$8, "phalcon/mvc/model/metadata.zep", 119 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
				} else {
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, getStrategy) {
	...
	zephir_read_property(&strategy, this_ptr, SL("_strategy"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readMetaDataIndex) {
	...
	zval *model, model_sub, *index_param = NULL, source, schema, key, metaData, _0, _1, _2, _3, _4, _5;
	ZEPHIR_INIT_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, writeMetaDataIndex) {
	...
	zend_bool _0, _1;
	int index, ZEPHIR_LAST_CALL_STATUS;
	zval *model, model_sub, *index_param = NULL, *data, data_sub, metaData, source, schema, key, _2, _3;
	ZEPHIR_INIT_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readColumnMap) {
	...
	if (!(zephir_array_isset_fetch(&data, &_0, &keyName, 0 TSRMLS_CC))) {
		...
		zephir_array_fetch(&data, &_1$$4, &keyName, PH_NOISY, "phalcon/mvc/model/metadata.zep", 329 TSRMLS_CC);
	}
	RETURN_CCTOR(data);
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readColumnMapIndex) {
	...
	if (!(zephir_array_isset_fetch(&columnMapModel, &_0, &keyName, 0 TSRMLS_CC))) {
		...
		zephir_array_fetch(&columnMapModel, &_1$$4, &keyName, PH_NOISY, "phalcon/mvc/model/metadata.zep", 354 TSRMLS_CC);
	}
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, hasAttribute) {
	...
	if (Z_TYPE_P(&columnMap) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(&_0$$4, this_ptr, "readmetadata", NULL, 14, model);
		zephir_check_call_status();
		zephir_array_fetch_long(&_1$$4, &_0$$4, 4, PH_READONLY, "phalcon/mvc/model/metadata.zep", 651 TSRMLS_CC);
