...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readColumnMap) {
	...
	if (!(zephir_array_isset_fetch(&data, &_0, &keyName, 0))) {
		...
		ZVAL_NULL(&_2$$4);
		ZVAL_NULL(&_3$$4);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "initialize", NULL, 27, model, &_1$$4, &_2$$4, &_3$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readMetaData) {
	...
	if (!(zephir_array_isset(&_2, &key))) {
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "initialize", NULL, 27, model, &key, &source, &schema);
		zephir_check_call_status();
	}
			...
			if (Z_TYPE_P(&data) != IS_NULL) {
				...
				if ((zephir_method_exists_ex(model, ZEND_STRL("metadata")) == SUCCESS)) {
					...
					if (UNEXPECTED(Z_TYPE_P(&modelMetadata) != IS_ARRAY)) {
						...
						ZEPHIR_CALL_METHOD(NULL, &_2$$8, "__construct", NULL, 6, &_3$$8);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, throwWriteException) {
	...
	if (zephir_is_true(option)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 6, &message);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$3, "phalcon/Mvc/Model/MetaData.zep", 879);
		ZEPHIR_MM_RESTORE();
		return;
	} else {
		ZEPHIR_CALL_FUNCTION(NULL, "trigger_error", NULL, 5, &message);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
