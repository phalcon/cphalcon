...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readColumnMap)
{
	...
	if (!(zephir_array_isset_fetch(&data, &_0, &keyName, 0))) {
		...
		ZVAL_NULL(&_2$$4);
		ZVAL_NULL(&_3$$4);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "initialize", NULL, 32, model, &_1$$4, &_2$$4, &_3$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, readMetaData)
{
	...
	if (!(zephir_array_isset(&_2, &key))) {
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "initialize", NULL, 32, model, &key, &source, &schema);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, setAutomaticCreateAttributes)
{
	...
	ZVAL_LONG(&_0, 10);
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "writemetadataindex", NULL, 33, model, &_0, &attributes);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData, write)
{
	...
	/* try_start_1: */
		...
		if (ZEPHIR_IS_FALSE_IDENTICAL(&result)) {
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "throwwriteexception", NULL, 34, &option);
			zephir_check_call_status_or_jump(try_end_1);
		}
