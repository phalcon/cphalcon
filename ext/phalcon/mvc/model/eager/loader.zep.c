...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1014, manager);
}
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, apply)
{
	...
	if (Z_TYPE_P(&eagerMap) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&eagerMap), _1, _2, _0)
		{
			...
			ZEPHIR_CALL_FUNCTION(&_10$$3, "in_array", &_11, 88, &__$null, &values, &__$true);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_OBS_NVAR(&related);
				zephir_array_fetch_string(&related, &node, SL("empty"), PH_NOISY, "phalcon/Mvc/Model/Eager/Loader.zep", 75);
				ZEPHIR_CALL_FUNCTION(&_24$$10, "in_array", &_11, 88, &__$null, &values, &__$true);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, loadResultset)
{
	...
	if (!(_0)) {
		ZEPHIR_CALL_METHOD(&_1, resultset, "count", NULL, 212);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, buildMap)
{
	...
	if (Z_TYPE_P(&tree) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&tree), _1, _2, _0)
		{
			...
			ZEPHIR_INIT_NVAR(&node);
			ZVAL_COPY(&node, _0);
			zephir_read_property_cached(&_3$$3, this_ptr, _zephir_prop_0, 1014, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				zephir_read_property_cached(&_11$$5, this_ptr, _zephir_prop_0, 1014, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, buildNode)
{
	...
	array_init(&index);
	position = 0;
	ZEPHIR_CALL_METHOD(NULL, &children, "rewind", NULL, 213);
	zephir_check_call_status();
	while (1) {
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, buildThroughNode)
{
	...
	if (!(ZEPHIR_IS_EMPTY(&keys))) {
		...
		zephir_read_property_cached(&_6$$3, this_ptr, _zephir_prop_0, 1014, PH_NOISY_CC | PH_READONLY);
		...
		ZVAL_LONG(&_13$$3, 0);
		ZEPHIR_CALL_FUNCTION(&_7$$3, "array_column", NULL, 337, &keys, &_13$$3);
		zephir_check_call_status();
	...
	array_init(&index);
	position = 0;
	ZEPHIR_CALL_METHOD(NULL, &referenced, "rewind", NULL, 213);
	zephir_check_call_status();
	while (1) {
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, collectKeys)
{
	...
	if (Z_TYPE_P(&parents) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&parents), _0)
		{
			...
			ZEPHIR_INIT_NVAR(&field);
			ZEPHIR_CALL_FUNCTION(&_7$$3, "in_array", &_8, 88, &__$null, &values, &__$true);
			zephir_check_call_status();
			if (zephir_is_true(&_7$$3)) {
				continue;
			}
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_INIT_NVAR(&field);
				ZEPHIR_CALL_FUNCTION(&_18$$13, "in_array", &_8, 88, &__$null, &values, &__$true);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, fetchReferenced)
{
	...
	if (zephir_fast_count_int(&referencedFields) == 1) {
		...
		ZVAL_LONG(&_6$$4, 0);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "array_column", NULL, 337, &keys, &_6$$4);
		zephir_check_call_status();
