...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1020, manager);
}
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, loadResultset)
{
	...
	ZEND_PARSE_PARAMETERS_START(3, 3)
		Z_PARAM_OBJECT_OF_CLASS(resultset, zephir_get_internal_ce(SL("phalcon\\mvc\\model\\resultset\\simple")))
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, buildMap)
{
	...
	if (Z_TYPE_P(&tree) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&tree), _1, _2, _0)
		{
			...
			ZEPHIR_INIT_NVAR(&node);
			ZVAL_COPY(&node, _0);
			zephir_read_property_cached(&_3$$3, this_ptr, _zephir_prop_0, 1020, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				zephir_read_property_cached(&_11$$5, this_ptr, _zephir_prop_0, 1020, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Eager_Loader, buildThroughNode)
{
	...
	if (!(ZEPHIR_IS_EMPTY(&keys))) {
		...
		zephir_read_property_cached(&_6$$3, this_ptr, _zephir_prop_0, 1020, PH_NOISY_CC | PH_READONLY);
