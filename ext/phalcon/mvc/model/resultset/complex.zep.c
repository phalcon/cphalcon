...
PHP_METHOD(Phalcon_Mvc_Model_Resultset_Complex, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_mvc_model_resultset_complex_ce, this_ptr, "__construct", &_0, 372, result, cache);
...
PHP_METHOD(Phalcon_Mvc_Model_Resultset_Complex, toArray) {
	...
	array_init(records);
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "rewind", NULL, 75);
	zephir_check_call_status();
	while (1) {
		...
		ZEPHIR_CALL_METHOD(&current, this_ptr, "current", &_2, 373);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Resultset_Complex, serialize) {
	...
	ZEPHIR_CALL_FUNCTION(&serialized, "serialize", NULL, 65, _0);
	zephir_check_call_status();
