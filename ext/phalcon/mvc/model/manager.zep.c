...
PHP_METHOD(Phalcon_Mvc_Model_Manager, _getConnection) {
	...
	if (Z_TYPE_P(&container) != IS_OBJECT) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Manager.zep", 467 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, notifyEvent) {
	...
	zephir_read_property(&_3, this_ptr, SL("_customEventsManager"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_5);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, addHasOne) {
	...
	ZVAL_LONG(&_1, 1);
	ZEPHIR_CALL_METHOD(NULL, &relation, "__construct", NULL, 391, &_1, &referencedModel, fields, referencedFields, options);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, addHasMany) {
	...
	object_init_ex(&relation, phalcon_mvc_model_relation_ce);
	ZVAL_LONG(&_0, 2);
	ZEPHIR_CALL_METHOD(NULL, &relation, "__construct", NULL, 391, &_0, &referencedModel, fields, referencedFields, options);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(&alias);
	if (zephir_array_isset_string_fetch(&alias, options, SL("alias"), 0)) {
		if (Z_TYPE_P(&alias) != IS_STRING) {
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, addHasManyToMany) {
	...
	ZEPHIR_CALL_METHOD(NULL, &relation, "__construct", NULL, 391, &_0, &referencedModel, fields, referencedFields, options);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &relation, "setintermediaterelation", NULL, 392, intermediateFields, &intermediateModel, intermediateReferencedFields);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(&alias);
	if (zephir_array_isset_string_fetch(&alias, options, SL("alias"), 0)) {
		if (Z_TYPE_P(&alias) != IS_STRING) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(findParamsOne), _3$$4, _4$$4, _2$$4)
		{
			...
			if (_5$$5) {
				if (!(zephir_array_isset_long(&findParams, 0))) {
					...
					zephir_array_fetch_long(&_6$$8, &findParams, 0, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Manager.zep", 1152 TSRMLS_CC);
	...
	if (Z_TYPE_P(findParamsTwo) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(findParamsTwo), _9$$11, _10$$11, _8$$11)
		{
			...
			if (_11$$12) {
				if (!(zephir_array_isset_long(&findParams, 0))) {
					...
					zephir_array_fetch_long(&_12$$15, &findParams, 0, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Manager.zep", 1173 TSRMLS_CC);
			...
			if (_14$$12) {
				if (!(zephir_array_isset(&findParams, &key))) {
					...
					if (Z_TYPE_P(&value) == IS_ARRAY) {
						...
						zephir_array_fetch(&_16$$20, &findParams, &key, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Manager.zep", 1185 TSRMLS_CC);
		...
		if (Z_TYPE_P(findParamsTwo) == IS_STRING) {
			if (!(zephir_array_isset_long(&findParams, 0))) {
				...
				zephir_array_fetch_long(&_17$$24, &findParams, 0, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Manager.zep", 1198 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, getRelationRecords) {
	...
	if (zephir_is_true(&_0)) {
		...
		ZEPHIR_CALL_METHOD(&_5$$3, this_ptr, "_mergefindparameters", NULL, 393, &extraParameters, parameters);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&fields) != IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_15$$10), _17$$10, _18$$10, _16$$10)
		{
			...
			zephir_array_fetch(&_19$$11, &referencedFields, &refPosition, PH_NOISY | PH_READONLY, "phalcon/Mvc/Model/Manager.zep", 1310 TSRMLS_CC);
			ZEPHIR_INIT_LNVAR(_20$$11);
			ZEPHIR_CONCAT_SVSVS(&_20$$11, "[", &_19$$11, "] = :APR", &refPosition, ":");
			zephir_array_append(&conditions, &_20$$11, PH_SEPARATE, "phalcon/Mvc/Model/Manager.zep", 1310);
	...
	ZEPHIR_CALL_METHOD(&findArguments, this_ptr, "_mergefindparameters", NULL, 393, &findParams, parameters);
	zephir_check_call_status();
	if (Z_TYPE_P(&extraParameters) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&findParams, this_ptr, "_mergefindparameters", NULL, 393, &extraParameters, &findArguments);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, createQuery) {
	...
	if (Z_TYPE_P(&container) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 1, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Manager.zep", 1634 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_6);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, createBuilder) {
	...
	if (Z_TYPE_P(&container) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 1, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Mvc/Model/Manager.zep", 1677 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_6);
