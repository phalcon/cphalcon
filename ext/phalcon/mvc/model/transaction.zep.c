...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1112, &connection);
	if (autoBegin) {
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, rollback)
{
	...
	if (UNEXPECTED(zephir_is_true(&_1))) {
		...
		if (Z_TYPE_P(rollbackRecord) == IS_OBJECT) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1115, rollbackRecord);
		}
		zephir_read_property_cached(&_2$$4, this_ptr, _zephir_prop_3, 1116, PH_NOISY_CC | PH_READONLY);
		if (zephir_is_true(&_2$$4)) {
			ZEPHIR_INIT_VAR(&_3$$7);
			object_init_ex(&_3$$7, phalcon_mvc_model_transaction_failed_ce);
			zephir_read_property_cached(&_4$$7, this_ptr, _zephir_prop_2, 1115, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, setRollbackedRecord)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1115, record);
}
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, setTransactionManager)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1113, manager);
}
