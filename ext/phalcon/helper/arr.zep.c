...
PHP_METHOD(Phalcon_Helper_Arr, chunk) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("array_chunk", NULL, 197, &collection, &_0, &_1);
...
PHP_METHOD(Phalcon_Helper_Arr, first) {
	...
	ZEPHIR_CALL_SELF(&filtered, "filtercollection", &_0, 198, &collection, method);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(&filtered);
	ZEPHIR_RETURN_CALL_FUNCTION("reset", NULL, 199, &filtered);
...
PHP_METHOD(Phalcon_Helper_Arr, firstKey) {
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&filtered, "array_filter", NULL, 200, &collection, method);
		zephir_check_call_status();
	} else {
		ZEPHIR_CPY_WRT(&filtered, &collection);
	}
	ZEPHIR_MAKE_REF(&filtered);
	ZEPHIR_CALL_FUNCTION(NULL, "reset", NULL, 199, &filtered);
	ZEPHIR_UNREF(&filtered);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("key", NULL, 201, &filtered);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Helper_Arr, flatten) {
	...
	array_init(&data);
	zephir_is_iterable(&collection, 0, "phalcon/Helper/Arr.zep", 86);
	if (Z_TYPE_P(&collection) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&collection), _0)
		{
			...
			if (Z_TYPE_P(&item) != IS_ARRAY) {
				zephir_array_append(&data, &item, PH_SEPARATE, "phalcon/Helper/Arr.zep", 76);
			} else {
				if (deep) {
					ZEPHIR_INIT_NVAR(&_2$$6);
					ZVAL_BOOL(&_5$$6, 1);
					ZEPHIR_CALL_SELF(&_3$$6, "flatten", &_4, 202, &item, &_5$$6);
					zephir_check_call_status();
					zephir_fast_array_merge(&_2$$6, &data, &_3$$6 TSRMLS_CC);
					ZEPHIR_CPY_WRT(&data, &_2$$6);
				} else {
					ZEPHIR_INIT_NVAR(&_6$$7);
					ZEPHIR_CALL_FUNCTION(&_7$$7, "array_values", &_8, 115, &item);
					zephir_check_call_status();
			...
			zephir_check_call_status();
				if (Z_TYPE_P(&item) != IS_ARRAY) {
					...
					if (deep) {
						...
						ZVAL_BOOL(&_11$$11, 1);
						ZEPHIR_CALL_SELF(&_10$$11, "flatten", &_4, 202, &item, &_11$$11);
						zephir_check_call_status();
						zephir_fast_array_merge(&_9$$11, &data, &_10$$11 TSRMLS_CC);
						ZEPHIR_CPY_WRT(&data, &_9$$11);
					} else {
						ZEPHIR_INIT_NVAR(&_12$$12);
						ZEPHIR_CALL_FUNCTION(&_13$$12, "array_values", &_8, 115, &item);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Helper_Arr, group) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *collection_param = NULL, *method, method_sub, element, key, filtered, *_0, _1;
	zval collection;
	zval *this_ptr = getThis();
	...
	array_init(&filtered);
	zephir_is_iterable(&collection, 0, "phalcon/Helper/Arr.zep", 126);
	if (Z_TYPE_P(&collection) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&collection), _0)
		{
			...
			if (_3$$3) {
				ZEPHIR_CALL_FUNCTION(&key, "call_user_func", &_4, 203, method, &element);
				zephir_check_call_status();
				...
				ZEPHIR_OBS_NVAR(&key);
				zephir_array_fetch(&key, &element, method, PH_NOISY, "phalcon/Helper/Arr.zep", 121 TSRMLS_CC);
				zephir_array_update_multi(&filtered, &element TSRMLS_CC, SL("za"), 2, &key);
			}
		} ZEND_HASH_FOREACH_END();
	} else {
...
PHP_METHOD(Phalcon_Helper_Arr, isUnique) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "array_unique", NULL, 204, &collection);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Helper_Arr, last) {
	...
	ZEPHIR_MAKE_REF(&filtered);
	ZEPHIR_RETURN_CALL_FUNCTION("end", NULL, 205, &filtered);
	ZEPHIR_UNREF(&filtered);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Helper_Arr, lastKey) {
	...
	ZEPHIR_MAKE_REF(&filtered);
	ZEPHIR_CALL_FUNCTION(NULL, "end", NULL, 205, &filtered);
	ZEPHIR_UNREF(&filtered);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("key", NULL, 201, &filtered);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Helper_Arr, order) {
	...
	array_init(&sorted);
	zephir_is_iterable(&collection, 0, "phalcon/Helper/Arr.zep", 206);
	if (Z_TYPE_P(&collection) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&collection), _0)
		{
			ZEPHIR_INIT_NVAR(&item);
			...
			if (Z_TYPE_P(&item) == IS_OBJECT) {
				...
				ZEPHIR_OBS_NVAR(&key);
				zephir_array_fetch(&key, &item, attribute, PH_NOISY, "phalcon/Helper/Arr.zep", 200 TSRMLS_CC);
			}
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&order, "asc")) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "ksort", NULL, 206, &sorted);
		ZEPHIR_UNREF(&sorted);
		zephir_check_call_status();
	} else {
		ZEPHIR_MAKE_REF(&sorted);
		ZEPHIR_CALL_FUNCTION(NULL, "krsort", NULL, 207, &sorted);
		ZEPHIR_UNREF(&sorted);
		zephir_check_call_status();
	}
	ZEPHIR_RETURN_CALL_FUNCTION("array_values", NULL, 115, &sorted);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Helper_Arr, pluck) {
	...
	ZEPHIR_INIT_VAR(&filtered);
	array_init(&filtered);
	zephir_is_iterable(&collection, 0, "phalcon/Helper/Arr.zep", 233);
	if (Z_TYPE_P(&collection) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&collection), _0)
		{
			ZEPHIR_INIT_NVAR(&item);
			...
			if (_2$$3) {
				...
				zephir_array_append(&filtered, &_4$$4, PH_SEPARATE, "phalcon/Helper/Arr.zep", 227);
			} else if (_3$$3) {
				zephir_array_fetch(&_5$$5, &item, &element, PH_NOISY | PH_READONLY, "phalcon/Helper/Arr.zep", 229 TSRMLS_CC);
				zephir_array_append(&filtered, &_5$$5, PH_SEPARATE, "phalcon/Helper/Arr.zep", 229);
			}
		} ZEND_HASH_FOREACH_END();
	} else {
		ZEPHIR_CALL_METHOD(NULL, &collection, "rewind", NULL, 0);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_6$$6) {
					...
					zephir_array_append(&filtered, &_8$$7, PH_SEPARATE, "phalcon/Helper/Arr.zep", 227);
				} else if (_7$$6) {
					zephir_array_fetch(&_9$$8, &item, &element, PH_NOISY | PH_READONLY, "phalcon/Helper/Arr.zep", 229 TSRMLS_CC);
					zephir_array_append(&filtered, &_9$$8, PH_SEPARATE, "phalcon/Helper/Arr.zep", 229);
				}
			ZEPHIR_CALL_METHOD(NULL, &collection, "next", NULL, 0);
			zephir_check_call_status();
		}
...
PHP_METHOD(Phalcon_Helper_Arr, set) {
	...
	if (Z_TYPE_P(index) == IS_NULL) {
		zephir_array_append(&collection, value, PH_SEPARATE, "phalcon/Helper/Arr.zep", 242);
	} else {
...
PHP_METHOD(Phalcon_Helper_Arr, sliceLeft) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("array_slice", NULL, 208, &collection, &_0, &_1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Helper_Arr, sliceRight) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("array_slice", NULL, 208, &collection, &_0);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Helper_Arr, split) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "array_values", NULL, 115, &collection);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Helper_Arr, validateAll) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "array_filter", NULL, 200, &collection, method);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Helper_Arr, filterCollection) {
	...
	if (_0) {
		ZEPHIR_RETURN_CALL_FUNCTION("array_filter", NULL, 200, &collection, method);
		zephir_check_call_status();
		RETURN_MM();
	} else {
		RETURN_CTOR(&collection);
