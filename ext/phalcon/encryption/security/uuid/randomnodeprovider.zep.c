...
PHP_METHOD(Phalcon_Encryption_Security_Uuid_RandomNodeProvider, getNode)
{
	...
	ZVAL_LONG(&_0, 6);
	ZEPHIR_CALL_FUNCTION(&nodeBytes, "random_bytes", NULL, 497, &_0);
	zephir_check_call_status();
	ZVAL_LONG(&_0, 0);
	ZVAL_LONG(&_1, 1);
	ZEPHIR_INIT_VAR(&_2);
	zephir_substr(&_2, &nodeBytes, 0 , 1 , 0);
	ZEPHIR_CALL_FUNCTION(&_3, "ord", NULL, 462, &_2);
	zephir_check_call_status();
	ZVAL_LONG(&_4, ((int) (zephir_get_numberval(&_3)) | 0x01));
	ZEPHIR_CALL_FUNCTION(&_5, "chr", NULL, 461, &_4);
	zephir_check_call_status();
	ZVAL_LONG(&_4, 1);
	ZEPHIR_INIT_VAR(&_6);
	...
	ZEPHIR_RETURN_CALL_FUNCTION("bin2hex", NULL, 508, &nodeBytes);
	zephir_check_call_status();
	RETURN_MM();
}
