...
PHP_METHOD(Phalcon_Encryption_Security_JWT_Signer_Hmac, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 688, &algo_zv);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Encryption_Security_JWT_Signer_Hmac, getAlgHeader)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 688, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_2);
	ZVAL_STRING(&_2, "sha");
	ZEPHIR_INIT_VAR(&_3);
	ZVAL_STRING(&_3, "");
