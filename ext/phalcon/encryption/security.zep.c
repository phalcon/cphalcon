...
PHP_METHOD(Phalcon_Encryption_Security, hash)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "%02s");
	ZEPHIR_CALL_FUNCTION(&formatted, "sprintf", NULL, 132, &_0, &cost);
	zephir_check_call_status();
	...
	do {
		...
		if (ZEPHIR_IS_LONG(&_1, 5)) {
			...
			ZEPHIR_CALL_FUNCTION(&prefix, "sprintf", NULL, 132, &_2$$6, &formatted);
			zephir_check_call_status();
			break;
		}
		if (ZEPHIR_IS_LONG(&_1, 6)) {
			...
			ZEPHIR_CALL_FUNCTION(&prefix, "sprintf", NULL, 132, &_3$$7, &formatted);
			zephir_check_call_status();
			break;
		}
		legacy = 0;
