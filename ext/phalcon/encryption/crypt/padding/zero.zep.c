...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_Zero, pad)
{
	...
	ZVAL_LONG(&_0, 0);
	ZEPHIR_CALL_FUNCTION(&_1, "chr", NULL, 505, &_0);
	zephir_check_call_status();
	ZVAL_LONG(&_0, paddingSize);
	ZEPHIR_RETURN_CALL_FUNCTION("str_repeat", NULL, 6, &_1, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_Zero, unpad)
{
	...
	ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 508, &input_zv);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&inputArray, "str_split", NULL, 167, &input_zv);
	zephir_check_call_status();
	...
	while (1) {
		...
		if (_0) {
			...
			ZVAL_LONG(&_2, 0);
			ZEPHIR_CALL_FUNCTION(&_3, "chr", &_4, 505, &_2);
			zephir_check_call_status();
