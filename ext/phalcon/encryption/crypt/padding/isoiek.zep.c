...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_IsoIek, pad)
{
	...
	ZVAL_LONG(&_0, 0);
	ZEPHIR_CALL_FUNCTION(&_2, "chr", NULL, 250, &_0);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_IsoIek, unpad)
{
	...
	paddingSize = 0;
	ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 244, &input);
	zephir_check_call_status();
	...
	while (1) {
		...
		if (_0) {
			...
			ZVAL_LONG(&_2, 0);
			ZEPHIR_CALL_FUNCTION(&_3, "chr", &_4, 250, &_2);
			zephir_check_call_status();
	...
	ZVAL_LONG(&_2, 0x80);
	ZEPHIR_CALL_FUNCTION(&_7, "chr", &_4, 250, &_2);
	zephir_check_call_status();
