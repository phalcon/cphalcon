...
PHP_METHOD(Phalcon_Logger_LoggerFactory, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 929, factory);
}
...
PHP_METHOD(Phalcon_Logger_LoggerFactory, load)
{
	...
	if (Z_TYPE_P(&adapters) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&adapters), _5, _6, _4)
		{
			...
			zephir_check_call_status();
			zephir_read_property_cached(&_9$$3, this_ptr, _zephir_prop_0, 929, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_check_call_status();
				zephir_read_property_cached(&_15$$4, this_ptr, _zephir_prop_0, 929, PH_NOISY_CC | PH_READONLY);
