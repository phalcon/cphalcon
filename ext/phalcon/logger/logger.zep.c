...
PHP_METHOD(Phalcon_Logger_Logger, __construct)
{
	...
	if (ZEPHIR_IS_EMPTY(timezone)) {
		ZEPHIR_CALL_FUNCTION(&localTimeZone, "date_default_timezone_get", NULL, 453);
...
PHP_METHOD(Phalcon_Logger_Logger, log)
{
	...
	ZEPHIR_CALL_METHOD(&intLevel, this_ptr, "getlevelnumber", NULL, 454, level);
	zephir_check_call_status();
	zephir_cast_to_string(&_0, message);
...
PHP_METHOD(Phalcon_Logger_Logger, addMessage)
{
	...
	if (ZEPHIR_GE_LONG(&_0, level)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &item, "__construct", NULL, 455, &message, &levelName, &_4$$3, &_2$$3, &context);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&adapters, "array_diff_key", NULL, 279, &registered, &excluded);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Logger_Logger, getLevelNumber)
{
	...
	if (Z_TYPE_P(level) == IS_STRING) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&levels, "array_flip", NULL, 185, &_0$$3);
		zephir_check_call_status();
