...
PHP_METHOD(Phalcon_Logger_Factory, loadClass) {
	...
	if (Z_TYPE_P(config) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_factory_exception_ce, "Config must be array or Phalcon\\Config object", "phalcon/Logger/Factory.zep", 56);
		return;
	}
	ZEPHIR_OBS_VAR(&loggerName);
	if (!(zephir_array_isset_string_fetch(&loggerName, config, SL("name"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_factory_exception_ce, "You must provide the 'name' option in factory config parameter.", "phalcon/Logger/Factory.zep", 62);
		return;
	}
	ZEPHIR_OBS_VAR(&adapters);
	if (!(zephir_array_isset_string_fetch(&adapters, config, SL("adapters"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_factory_exception_ce, "You must provide the 'adapters' option in factory config parameter.", "phalcon/Logger/Factory.zep", 68);
		return;
	}
	zephir_is_iterable(&adapters, 0, "phalcon/Logger/Factory.zep", 91);
	if (Z_TYPE_P(&adapters) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&adapters), _2)
		{
			...
			if (!(zephir_array_isset_string_fetch(&name, &adapter, SL("name"), 0))) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_factory_exception_ce, "The adapters section of your configuration is missing the 'name' option", "phalcon/Logger/Factory.zep", 78);
				return;
			}
			ZEPHIR_OBS_NVAR(&type);
			if (!(zephir_array_isset_string_fetch(&type, &adapter, SL("adapter"), 0))) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_factory_exception_ce, "The adapters section of your configuration is missing the 'adapter' option", "phalcon/Logger/Factory.zep", 84);
				return;
			}
	...
	if (Z_TYPE_P(&adapters) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&adapters), _6, _7, _4)
		{
			...
			ZEPHIR_OBS_NVAR(&name);
			zephir_array_fetch_string(&name, &element, SL("name"), PH_NOISY, "phalcon/Logger/Factory.zep", 94 TSRMLS_CC);
			ZEPHIR_OBS_NVAR(&type);
			zephir_array_fetch_string(&type, &element, SL("adapter"), PH_NOISY, "phalcon/Logger/Factory.zep", 95 TSRMLS_CC);
