...
PHP_METHOD(Phalcon_Logger_Adapter_AbstractAdapter, add)
{
	...
	if (_1) {
		...
		if (Z_TYPE_P(&firstKey) != IS_NULL) {
			...
			zephir_read_property_cached(&_5$$4, this_ptr, _zephir_prop_1, 122, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Logger_Adapter_AbstractAdapter, begin)
{
	...
	if (zephir_is_true(&_0)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 134);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Logger_Adapter_AbstractAdapter, commit)
{
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checktransaction", NULL, 135);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&item);
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "resettransaction", NULL, 136);
	zephir_check_call_status();
	RETURN_THIS();
}
