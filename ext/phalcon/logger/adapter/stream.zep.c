...
PHP_METHOD(Phalcon_Logger_Adapter_Stream, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 949, &name_zv);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 950, &mode);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Logger_Adapter_Stream, process)
{
	...
	if (!(Z_TYPE_P(&_0) == IS_RESOURCE)) {
		...
		if (!(Z_TYPE_P(&fileHandler) == IS_RESOURCE)) {
			...
			zephir_read_property_cached(&_4$$4, this_ptr, _zephir_prop_1, 949, PH_NOISY_CC | PH_READONLY);
			zephir_read_property_cached(&_5$$4, this_ptr, _zephir_prop_2, 950, PH_NOISY_CC | PH_READONLY);
