...
PHP_METHOD(Phalcon_Logger_Adapter_Firephp, logInternal) {
	...
	if (!ZEPHIR_IS_TRUE_IDENTICAL(_2)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_4, 232, &_3$$4);
		zephir_check_call_status();
	...
	ZEPHIR_SINIT_VAR(_8);
	ZVAL_LONG(&_8, 4500);
	ZEPHIR_CALL_FUNCTION(&chunk, "str_split", NULL, 70, format, &_8);
	zephir_check_call_status();
	zephir_is_iterable(chunk, &_10, &_9, 0, 0, "phalcon/logger/adapter/firephp.zep", 102);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_10, &_9)
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_4, 232, content);
		zephir_check_call_status();
