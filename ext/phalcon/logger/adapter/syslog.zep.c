...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, close) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("closelog", NULL, 401);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, process) {
	...
	ZEPHIR_CALL_FUNCTION(&result, "openlog", NULL, 402, &name, &option, &facility);
	zephir_check_call_status();
	if (!zephir_is_true(&result)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 403, &_3$$3);
		zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&level, this_ptr, "logleveltosyslog", NULL, 404, &_4);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 405, &level, &message);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
