...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, process)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&level, this_ptr, "logleveltosyslog", NULL, 408, &_9);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 409, &level, &message);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, openlog)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("openlog", NULL, 410, &ident, &_0, &_1);
	zephir_check_call_status();
	RETURN_MM();
}
