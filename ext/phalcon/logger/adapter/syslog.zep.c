...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 921, &option);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 922, &facility);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, process)
{
	...
	if (!zephir_is_true(&result)) {
		...
		zephir_read_property_cached(&_4$$3, this_ptr, _zephir_prop_0, 920, PH_NOISY_CC | PH_READONLY);
	...
	if (1) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 923, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 923, &__$false);
	}
