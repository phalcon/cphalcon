...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, close) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("closelog", NULL, 402);
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, process) {
	...
	if (Z_TYPE_P(&message) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "The formatted message is not valid", "phalcon/Logger/Adapter/Syslog.zep", 115);
		return;
	}
	...
	ZEPHIR_CALL_FUNCTION(&result, "openlog", NULL, 403, &name, &option, &facility);
	zephir_check_call_status();
	if (!zephir_is_true(&result)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$4, "sprintf", NULL, 175, &_2$$4, &name, &facility);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 404, &_3$$4);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$4, "phalcon/Logger/Adapter/Syslog.zep", 131 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	opened = 1;
	...
	zephir_array_fetch_long(&_4, &message, 1, PH_NOISY | PH_READONLY, "phalcon/Logger/Adapter/Syslog.zep", 136 TSRMLS_CC);
	ZEPHIR_CALL_METHOD(&level, this_ptr, "logleveltosyslog", NULL, 405, &_4);
	zephir_check_call_status();
	zephir_array_fetch_long(&_5, &message, 1, PH_NOISY | PH_READONLY, "phalcon/Logger/Adapter/Syslog.zep", 138 TSRMLS_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 406, &level, &_5);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
