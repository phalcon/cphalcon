...
PHP_METHOD(Phalcon_Logger_Adapter_File, __construct) {
	...
	zval options;
	zval *name_param = NULL, *options_param = NULL, mode, handler, _0$$7;
	zval name, _1$$7;
	zval *this_ptr = getThis();
	...
	if (1 == 1) {
		...
		if (zephir_array_isset_string_fetch(&mode, &options, SL("mode"), 0)) {
			if (zephir_memnstr_str(&mode, SL("r"), "phalcon/logger/adapter/file.zep", 71)) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "Logger must be opened in append or write mode", "phalcon/logger/adapter/file.zep", 72);
				return;
			}
		}
	}
	...
	ZEPHIR_CALL_FUNCTION(&handler, "fopen", NULL, 270, &name, &mode);
	...
	if (Z_TYPE_P(&handler) != IS_RESOURCE) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$7, "phalcon/logger/adapter/file.zep", 86 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Logger_Adapter_File, logInternal) {
	...
	if (Z_TYPE_P(&fileHandler) != IS_RESOURCE) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "Cannot send message to the log because it is invalid", "phalcon/logger/adapter/file.zep", 115);
		return;
	}
...
PHP_METHOD(Phalcon_Logger_Adapter_File, __wakeup) {
	...
	if (Z_TYPE_P(&path) != IS_STRING) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "Invalid data passed to Phalcon\\Logger\\Adapter\\File::__wakeup()", "phalcon/logger/adapter/file.zep", 138);
		return;
	}
	ZEPHIR_OBS_VAR(&mode);
	...
	if (zephir_memnstr_str(&mode, SL("r"), "phalcon/logger/adapter/file.zep", 149)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "Logger must be opened in append or write mode", "phalcon/logger/adapter/file.zep", 150);
		return;
	}
	ZEPHIR_CALL_FUNCTION(&_1, "fopen", NULL, 270, &path, &mode);
	zephir_check_call_status();
