...
ZEPHIR_INIT_CLASS(Phalcon_Firewall_Adapter) {
	...
	ZEPHIR_REGISTER_CLASS(Phalcon\\Firewall, Adapter, phalcon, firewall_adapter, phalcon_firewall_adapter_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	/**
	 * Storing active identity object implementing Phalcon/Acl/RoleAware
	 */
	zend_declare_property_null(phalcon_firewall_adapter_ce, SL("activeIdentity"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Storing active user role
	 */
	zend_declare_property_null(phalcon_firewall_adapter_ce, SL("activeRole"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Should role always be resolved using role callback or just once?
	 * @var bool
	 */
	zend_declare_property_bool(phalcon_firewall_adapter_ce, SL("alwaysResolvingRole"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Cache for caching access
	 * @var <CacheAdapterInterface>
	 */
	zend_declare_property_null(phalcon_firewall_adapter_ce, SL("cache"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Default access
	 * @var int
	 */
	zend_declare_property_long(phalcon_firewall_adapter_ce, SL("defaultAccess"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Events manager
	 * @var mixed
	 */
	zend_declare_property_null(phalcon_firewall_adapter_ce, SL("eventsManager"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Internal cache for caching access during request time
	 * @var mixed
	 */
	zend_declare_property_null(phalcon_firewall_adapter_ce, SL("internalCache"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var mixed
	 */
	zend_declare_property_null(phalcon_firewall_adapter_ce, SL("roleCallback"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(phalcon_firewall_adapter_ce TSRMLS_CC, 1, phalcon_firewall_adapterinterface_ce);
	zend_class_implements(phalcon_firewall_adapter_ce TSRMLS_CC, 1, phalcon_events_eventsawareinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Phalcon_Firewall_Adapter, setRoleCallback) {
	...
	if (!(zephir_is_callable(callback TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalcon_firewall_exception_ce, "Role callback must be function.", "phalcon/Firewall/Adapter.zep", 139);
		return;
	}
...
PHP_METHOD(Phalcon_Firewall_Adapter, callRoleCallback) {
	...
	if (ZEPHIR_IS_EMPTY(&identity)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_firewall_exception_ce, "Function defined as roleCallback must return something.", "phalcon/Firewall/Adapter.zep", 154);
		return;
	}
	if (Z_TYPE_P(&identity) == IS_OBJECT) {
		if (!(zephir_instance_of_ev(&identity, phalcon_acl_roleaware_ce TSRMLS_CC))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_firewall_exception_ce, "Role passed as object must implement 'Phalcon\\Acl\\RoleAware'", "phalcon/Firewall/Adapter.zep", 159);
			return;
		}
...
PHP_METHOD(Phalcon_Firewall_Adapter, throwFirewallException) {
	...
	zephir_throw_exception_debug(&exception, "phalcon/Firewall/Adapter.zep", 258 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
