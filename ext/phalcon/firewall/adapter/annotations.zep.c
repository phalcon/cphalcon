...
PHP_METHOD(Phalcon_Firewall_Adapter_Annotations, beforeExecuteRoute) {
	...
	if (_7) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_8$$10, "phalcon/Firewall/Adapter/Annotations.zep", 103 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (Z_TYPE_P(&_10) == IS_NULL) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_11$$11, "phalcon/Firewall/Adapter/Annotations.zep", 110 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (ZEPHIR_IS_LONG_IDENTICAL(&numberArguments, 1)) {
		...
		if (Z_TYPE_P(&annotationRoles) == IS_ARRAY) {
			...
			if (_1$$4) {
				...
				ZEPHIR_CALL_FUNCTION(&roleIntersect, "array_intersect", NULL, 214, role, &annotationRoles);
				zephir_check_call_status();
