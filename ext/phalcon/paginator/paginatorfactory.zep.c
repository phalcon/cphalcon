...
PHP_METHOD(Phalcon_Paginator_PaginatorFactory, load) {
	...
	ZEPHIR_OBS_VAR(&name);
	zephir_array_fetch_string(&name, config, SL("adapter"), PH_NOISY, "phalcon/Paginator/PaginatorFactory.zep", 55);
	ZEPHIR_INIT_VAR(&_2);
	array_init(&_2);
	ZEPHIR_INIT_VAR(&_3);
...
PHP_METHOD(Phalcon_Paginator_PaginatorFactory, newInstance) {
	...
	if (!(zephir_array_isset(&_0, &name))) {
		...
		zephir_array_fetch(&definition, &_1$$3, &name, PH_NOISY, "phalcon/Paginator/PaginatorFactory.zep", 71);
	...
	zephir_array_fetch(&_5, &_4, &name, PH_NOISY | PH_READONLY, "phalcon/Paginator/PaginatorFactory.zep", 80);
	RETURN_CTOR(&_5);
