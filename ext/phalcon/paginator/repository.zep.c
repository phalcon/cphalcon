...
ZEPHIR_INIT_CLASS(Phalcon_Paginator_Repository) {
	...
	zend_declare_property_null(phalcon_paginator_repository_ce, SL("_properties"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(phalcon_paginator_repository_ce, SL("_aliases"), ZEND_ACC_PROTECTED TSRMLS_CC);

	phalcon_paginator_repository_ce->create_object = zephir_init_properties_Phalcon_Paginator_Repository;

	zend_class_implements(phalcon_paginator_repository_ce TSRMLS_CC, 1, phalcon_paginator_repositoryinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Phalcon_Paginator_Repository, setProperties) {
	...
	zephir_update_property_zval(this_ptr, SL("_properties"), &properties);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Paginator_Repository, setAliases) {
	...
	zephir_update_property_zval(this_ptr, SL("_aliases"), &aliases);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Paginator_Repository, getAliases) {
	...
	RETURN_MEMBER(getThis(), "_aliases");
...
PHP_METHOD(Phalcon_Paginator_Repository, __get) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "trigger_error", NULL, 1, &_3);
	zephir_check_call_status();
	RETURN_MM_NULL();
...
PHP_METHOD(Phalcon_Paginator_Repository, getProperty) {
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_read_property(&_1, this_ptr, SL("_properties"), PH_NOISY_CC | PH_READONLY);
	if (zephir_array_isset(&_1, &property)) {
		zephir_read_property(&_2, this_ptr, SL("_properties"), PH_NOISY_CC | PH_READONLY);
		zephir_array_fetch(&_0, &_2, &property, PH_NOISY, "phalcon/paginator/repository.zep", 137 TSRMLS_CC);
	} else {
...
PHP_METHOD(Phalcon_Paginator_Repository, getRealNameProperty) {
	...
	if (zephir_array_isset(&aliases, &property)) {
		zephir_array_fetch(&_0$$3, &aliases, &property, PH_NOISY | PH_READONLY, "phalcon/paginator/repository.zep", 149 TSRMLS_CC);
		RETURN_CTOR(&_0$$3);
	}
...
zend_object *zephir_init_properties_Phalcon_Paginator_Repository(zend_class_entry *class_type TSRMLS_DC) {
	...
	
	{
		...
		if (Z_TYPE_P(&_2) == IS_NULL) {
			...
			array_init(&_3$$4);
			zephir_update_property_zval(this_ptr, SL("_properties"), &_3$$4);
		}
		ZEPHIR_MM_RESTORE();
