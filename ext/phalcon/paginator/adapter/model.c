...
PHP_METHOD(Phalcon_Paginator_Adapter_Model, getPaginate) {
	...
	zval *config, *items, *pageItems, *page, *valid = NULL, *_0, *_1, _2, *_3, *_4 = NULL, *_7, *_8, *_9, *_10, *_11, *_12, *_13;

	ZEPHIR_MM_GROW();

	ZEPHIR_OBS_VAR(_0);
	zephir_read_property_this(&_0, this_ptr, SL("_limitRows"), PH_NOISY_CC);
	show = zephir_get_numberval(_0);
	...
	pageNumber = zephir_get_numberval(_1);
	if ((pageNumber <= 0)) {
		pageNumber = 1;
	}
	if ((show <= 0)) {
	...
	totalPages = zephir_get_numberval(_3);
	...
	zephir_update_property_zval(page, SL("current"), _10 TSRMLS_CC);
	if (((n % show) != 0)) {
		pagesTotal = (((n / show) + 1));
	} else {
		pagesTotal = ((n / show));
	...
	ZVAL_LONG(_11, pagesTotal);
	zephir_update_property_zval(page, SL("last"), _11 TSRMLS_CC);
	ZEPHIR_INIT_ZVAL_NREF(_12);
	ZVAL_LONG(_12, pagesTotal);
	zephir_update_property_zval(page, SL("total_pages"), _12 TSRMLS_CC);
	ZEPHIR_INIT_ZVAL_NREF(_13);
	ZVAL_LONG(_13, n);
	zephir_update_property_zval(page, SL("total_items"), _13 TSRMLS_CC);
	RETURN_CCTOR(page);
