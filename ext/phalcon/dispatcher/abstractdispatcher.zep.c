...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, dispatch)
{
		...
		if (EG(exception)) {
			...
			if (zephir_is_instance_of(&_7$$4, SL("Exception"))) {
				...
				if (!ZEPHIR_IS_FALSE_IDENTICAL(&_8$$7)) {
					...
					zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 273);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
	...
	while (1) {
		...
		if (hasEventsManager) {
			...
			if (EG(exception)) {
				...
				if (zephir_is_instance_of(&_17$$12, SL("Exception"))) {
					...
					zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 313);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
		...
		ZEPHIR_CALL_FUNCTION(&handlerHash, "spl_object_hash", &_34, 147, &handler);
		zephir_check_call_status();
		...
		if (hasEventsManager) {
			...
			if (EG(exception)) {
				...
				if (zephir_is_instance_of(&_56$$30, SL("Exception"))) {
					...
					zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 452);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
			...
			if ((zephir_method_exists_ex(&handler, ZEND_STRL("initialize")) == SUCCESS)) {
				...
				if (EG(exception)) {
					...
					if (zephir_is_instance_of(&_72$$41, SL("Exception"))) {
						...
						zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 513);
						ZEPHIR_MM_RESTORE();
						return;
					}
				}
			...
			if (zephir_is_true(&eventsManager)) {
				...
				if (EG(exception)) {
					...
					if (zephir_is_instance_of(&_86$$46, SL("Exception"))) {
						...
						zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 541);
						ZEPHIR_MM_RESTORE();
						return;
					}
				}
		...
		if (EG(exception)) {
			...
			if (zephir_is_instance_of(&_102$$10, SL("Exception"))) {
				...
				zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 613);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
		...
		if ((zephir_method_exists_ex(&handler, ZEND_STRL("afterexecuteroute")) == SUCCESS)) {
			...
			if (EG(exception)) {
				...
				if (zephir_is_instance_of(&_119$$67, SL("Exception"))) {
					...
					zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 646);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
		...
		if (hasEventsManager) {
			...
			if (EG(exception)) {
				...
				if (zephir_is_instance_of(&_125$$72, SL("Exception"))) {
					...
					zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 663);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
	...
	if (hasEventsManager) {
		...
		if (EG(exception)) {
			...
			if (zephir_is_instance_of(&_131$$76, SL("Exception"))) {
				...
				zephir_throw_exception_debug(&e, "phalcon/Dispatcher/AbstractDispatcher.zep", 680);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, forward)
{
	...
	if (UNEXPECTED(ZEPHIR_IS_TRUE_IDENTICAL(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 148);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Dispatcher/AbstractDispatcher.zep", 712);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, getActiveMethod)
{
	...
	if (!(zephir_array_isset_fetch(&activeMethodName, &_0, &_1, 0))) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&activeMethodName, "lcfirst", NULL, 138, &_2$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, getHandlerClass)
{
	...
	if (!(zephir_memnstr_str(&handlerName, SL("\\"), "phalcon/Dispatcher/AbstractDispatcher.zep", 989))) {
...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, toCamelCase)
{
	...
	if (!(zephir_array_isset_fetch(&camelCaseInput, &_0, &input_zv, 0))) {
		ZEPHIR_INIT_VAR(&_1$$3);
		ZVAL_STRING(&_1$$3, "/[_-]+/");
		ZEPHIR_CALL_FUNCTION(&_2$$3, "preg_split", NULL, 129, &_1$$3, &input_zv);
		zephir_check_call_status();
