...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, dispatch)
{
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&handlerHash, "spl_object_hash", &_37, 108, &handler);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Dispatcher_AbstractDispatcher, toCamelCase)
{
	...
	if (!(zephir_array_isset_fetch(&camelCaseInput, &_0, &input, 0))) {
		...
		ZVAL_STRING(&_1$$3, "/[_-]+/");
		ZEPHIR_CALL_FUNCTION(&_2$$3, "preg_split", NULL, 109, &_1$$3, &input);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&_1$$3);
		ZVAL_STRING(&_1$$3, "ucfirst");
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_map", NULL, 110, &_1$$3, &_2$$3);
		zephir_check_call_status();
