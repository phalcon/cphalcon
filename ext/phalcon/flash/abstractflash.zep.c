...
ZEPHIR_INIT_CLASS(Phalcon_Flash_AbstractFlash) {
	 ...
	 * @var bool
	 */
	zend_declare_property_bool(phalcon_flash_abstractflash_ce, SL("autoescape"), 1, ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var bool
	 */
	zend_declare_property_bool(phalcon_flash_abstractflash_ce, SL("automaticHtml"), 1, ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var array
	 */
	zend_declare_property_null(phalcon_flash_abstractflash_ce, SL("cssClasses"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, __construct) {
	...
	zval *escaper = NULL, escaper_sub, __$null;
	...
	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &escaper);
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, getEscaperService) {
	...
	if (UNEXPECTED(Z_TYPE_P(&container) != IS_OBJECT)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$4, "phalcon/Flash/AbstractFlash.zep", 131 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&_7);
	...
	if (UNEXPECTED(zephir_is_true(&_6))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_10$$6, "phalcon/Flash/AbstractFlash.zep", 139 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, outputMessage) {
	...
	if (Z_TYPE_P(message) == IS_ARRAY) {
		...
		zephir_is_iterable(message, 0, "phalcon/Flash/AbstractFlash.zep", 279);
		if (Z_TYPE_P(message) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(message), _1$$3)
			{
				ZEPHIR_INIT_NVAR(&msg);
				ZVAL_COPY(&msg, _1$$3);
				ZEPHIR_CALL_METHOD(&preparedMsg, this_ptr, "prepareescapedmessage", &_3, 94, &msg);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&htmlMessage, this_ptr, "preparehtmlmessage", &_4, 95, &type, &preparedMsg);
				zephir_check_call_status();
				if (implicitFlush) {
		...
		ZEPHIR_CALL_METHOD(&preparedMsg, this_ptr, "prepareescapedmessage", &_3, 94, message);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&htmlMessage, this_ptr, "preparehtmlmessage", &_4, 95, &type, &preparedMsg);
		zephir_check_call_status();
		if (implicitFlush) {
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, prepareHtmlMessage) {
	...
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "gettemplate", NULL, 96, &cssClasses);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_2);
