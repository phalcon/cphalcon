...
PHP_METHOD(Phalcon_Flash_AbstractFlash, getEscaperService)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &_5, "__construct", NULL, 161);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, outputMessage)
{
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 162);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(message) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(message), _3)
		{
			...
			ZVAL_COPY(&item, _3);
			ZEPHIR_CALL_METHOD(&prepared, this_ptr, "prepareescapedmessage", &_4, 163, &item);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&html, this_ptr, "preparehtmlmessage", &_5, 164, &type_zv, &prepared);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, prepareHtmlMessage)
{
	...
	ZEPHIR_CALL_METHOD(&cssClasses, this_ptr, "checkclasses", NULL, 165, &_1, &type_zv);
	zephir_check_call_status();
	zephir_read_property(&_2, this_ptr, ZEND_STRL("cssIconClasses"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&cssIconClasses, this_ptr, "checkclasses", NULL, 165, &_2, &type_zv);
	zephir_check_call_status();
	zephir_read_property(&_3, this_ptr, ZEND_STRL("interpolator"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "gettemplate", NULL, 166, &cssClasses, &cssIconClasses);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_5);
