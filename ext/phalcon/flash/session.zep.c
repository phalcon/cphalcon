...
PHP_METHOD(Phalcon_Flash_Session, output)
{
	...
	ZEPHIR_INIT_NVAR(&message);
	ZEPHIR_INIT_NVAR(&type);
	zephir_read_property_cached(&_7, this_ptr, _zephir_prop_0, 767, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Flash_Session, getSessionMessages)
{
	...
	if (1 != ZEPHIR_IS_EMPTY(&type_zv)) {
		...
		if (zephir_array_isset_fetch(&returnMessages, &messages, &type_zv, 0)) {
			if (remove) {
				...
				zephir_read_property_cached(&_1$$6, this_ptr, _zephir_prop_0, 766, PH_NOISY_CC | PH_READONLY);
	...
	if (remove) {
		zephir_read_property_cached(&_2$$7, this_ptr, _zephir_prop_0, 766, PH_NOISY_CC | PH_READONLY);
