...
PHP_METHOD(Phalcon_Flash_Session, getSessionMessages)
{
	...
	if (1 != ZEPHIR_IS_EMPTY(&type_zv)) {
		...
		if (zephir_array_isset_fetch(&returnMessages, &messages, &type_zv, 0)) {
			if (remove) {
				...
				zephir_read_property_cached(&_1$$6, this_ptr, _zephir_prop_0, 751, PH_NOISY_CC | PH_READONLY);
	...
	if (remove) {
		zephir_read_property_cached(&_2$$7, this_ptr, _zephir_prop_0, 751, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Flash_Session, getSessionService)
{
	...
	if (_2) {
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_1, 754, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_INIT_VAR(&_5);
	...
	if (_2) {
		zephir_read_property_cached(&_6$$4, this_ptr, _zephir_prop_1, 754, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_INIT_VAR(&_8$$4);
		ZVAL_STRING(&_8$$4, "session");
		ZEPHIR_CALL_METHOD(&_7$$4, &_6$$4, "getshared", NULL, 0, &_8$$4);
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 753, &_7$$4);
		RETURN_MM_MEMBER(getThis(), "sessionService");
	}
	ZEPHIR_INIT_NVAR(&_5);
