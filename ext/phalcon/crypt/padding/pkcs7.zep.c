...
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Crypt\\Padding, Pkcs7, phalcon, crypt_padding_pkcs7, phalcon_crypt_padding_pkcs7_method_entry, 0);

	zend_class_implements(phalcon_crypt_padding_pkcs7_ce, 1, phalcon_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon_Crypt_Padding_Pkcs7, unpad)
{
	...
	if (ZEPHIR_LE_LONG(&ord, blockSize)) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "chr", NULL, 211, &paddingSize);
		zephir_check_call_status();
