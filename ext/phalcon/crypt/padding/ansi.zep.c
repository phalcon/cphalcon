 ...
 */
ZEPHIR_INIT_CLASS(Phalcon_Crypt_Padding_Ansi)
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Crypt\\Padding, Ansi, phalcon, crypt_padding_ansi, phalcon_crypt_padding_ansi_method_entry, 0);

	zend_class_implements(phalcon_crypt_padding_ansi_ce, 1, phalcon_crypt_padding_padinterface_ce);
	return SUCCESS;
}
 ...
 * @return string
 */
PHP_METHOD(Phalcon_Crypt_Padding_Ansi, pad)
{
	...
	ZEPHIR_CALL_FUNCTION(&_3, "chr", NULL, 211, &_0);
	zephir_check_call_status();
 ...
 * @return int
 */
PHP_METHOD(Phalcon_Crypt_Padding_Ansi, unpad)
{
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	...
	ZEPHIR_CALL_FUNCTION(&ord, "ord", NULL, 212, &last);
	zephir_check_call_status();
	if (ZEPHIR_LE_LONG(&ord, blockSize)) {
		...
		if (ZEPHIR_GT_LONG(&paddingSize, 1)) {
			ZVAL_LONG(&_2$$4, 0);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "chr", NULL, 211, &_2$$4);
			zephir_check_call_status();
