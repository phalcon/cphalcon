...
PHP_METHOD(Phalcon_Container_Resolver_Lazy_CsEnv, resolve)
{
	...
	ZEPHIR_INIT_VAR(&_3);
	ZVAL_STRING(&_3, "\\");
	ZEPHIR_CALL_FUNCTION(&values, "str_getcsv", NULL, 485, &_0, &_1, &_2, &_3);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&_4) != IS_NULL) {
		...
		if (Z_TYPE_P(&values) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&values), _6$$3, _7$$3, _5$$3)
			{
				...
				zephir_read_property_cached(&_8$$4, this_ptr, _zephir_prop_0, 544, PH_NOISY_CC | PH_READONLY);
			...
			while (1) {
				...
				zephir_check_call_status();
					zephir_read_property_cached(&_12$$5, this_ptr, _zephir_prop_0, 544, PH_NOISY_CC | PH_READONLY);
					ZEPHIR_MAKE_REF(&value);
