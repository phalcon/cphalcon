...
PHP_METHOD(Phalcon_Container_Resolver_Lazy_FunctionCall, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 537, &functionName_zv);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 538, &arguments);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Container_Resolver_Lazy_FunctionCall, resolve)
{
	...
	if (UNEXPECTED(!_zephir_prop_1)) {
		_zephir_prop_1 = zend_string_init("functionName", 12, 1);
	}
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 538, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&arguments, this_ptr, "resolvearguments", NULL, 0, ioc, &_0);
	zephir_check_call_status();
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 537, PH_NOISY_CC | PH_READONLY);
