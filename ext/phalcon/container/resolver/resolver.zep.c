...
PHP_METHOD(Phalcon_Container_Resolver_Resolver, isResolvableClass)
{
	...
	ZEPHIR_RETURN_CALL_METHOD(&_0, "isinstantiable", NULL, 497);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Container_Resolver_Resolver, resolveClass)
{
	...
	if (Z_TYPE_P(&constructor) == IS_NULL) {
		...
		array_init(&_0$$3);
		ZEPHIR_RETURN_CALL_METHOD(&reflection, "newinstanceargs", NULL, 478, &_0$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	ZEPHIR_CALL_METHOD(&resolved, this_ptr, "resolveparameters", NULL, 0, ioc, &params, &arguments);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_METHOD(&reflection, "newinstanceargs", NULL, 478, &resolved);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Container_Resolver_Resolver, resolveParameter)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_6, "__construct", NULL, 498, &_7, &declaringName);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&parameters) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&parameters), _1, _2, _0)
		{
			...
			if (zephir_array_key_exists(&arguments, &position)) {
				...
				ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "resolvearg", &_5, 499, ioc, &_4$$4);
				zephir_check_call_status();
			...
			if (zephir_array_key_exists(&arguments, &name)) {
				...
				ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "resolvearg", &_5, 499, ioc, &_7$$5);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (zephir_array_key_exists(&arguments, &position)) {
					...
					ZEPHIR_CALL_METHOD(&_12$$7, this_ptr, "resolvearg", &_5, 499, ioc, &_13$$7);
					zephir_check_call_status();
				...
				if (zephir_array_key_exists(&arguments, &name)) {
					...
					ZEPHIR_CALL_METHOD(&_14$$8, this_ptr, "resolvearg", &_5, 499, ioc, &_15$$8);
					zephir_check_call_status();
