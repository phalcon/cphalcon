	...
	if (Z_TYPE_P(&namespaces) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&namespaces), _2, _3, _1)
		{
			...
			if (ZEPHIR_IS_TRUE_IDENTICAL(&_5$$3)) {
				zephir_read_property_cached(&_7$$5, this_ptr, _zephir_prop_1, 484, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (ZEPHIR_IS_TRUE_IDENTICAL(&_13$$6)) {
					zephir_read_property_cached(&_14$$8, this_ptr, _zephir_prop_1, 484, PH_NOISY_CC | PH_READONLY);
