...
PHP_METHOD(Phalcon_Autoload_Loader, __construct)
{
	...
	if (isDebug) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 467, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 467, &__$false);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Autoload_Loader, autoload)
{
	...
	if (ZEPHIR_IS_LONG_IDENTICAL(&_0, 0)) {
		...
		array_init(&_1$$3);
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 470, &_1$$3);
	}
	result = 1;
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 469, PH_NOISY_CC | PH_READONLY);
	ZVAL_UNDEF(&_3);
	ZVAL_LONG(&_3, (zephir_get_numberval(&_2) + 1));
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 469, &_3);
	ZEPHIR_INIT_VAR(&_4);
	...
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_6)) {
		...
		if (!ZEPHIR_IS_TRUE_IDENTICAL(&_8$$4)) {
			...
			zephir_check_call_status();
			zephir_read_property_cached(&_11$$5, this_ptr, _zephir_prop_2, 471, PH_NOISY_CC | PH_READONLY);
			ZVAL_BOOL(&_12$$5, 1);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 469, &_15);
...
PHP_METHOD(Phalcon_Autoload_Loader, setFileCheckingCallback)
{
	...
	if (1 == zephir_is_callable(method)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 476, method);
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 476, &_0$$4);
	} else {
...
PHP_METHOD(Phalcon_Autoload_Loader, requireFile)
{
	...
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&_1)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 477, &file_zv);
		ZEPHIR_INIT_VAR(&_2$$3);
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&directories), _1)
		{
			...
			if (Z_TYPE_P(&extensions) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&extensions), _3$$3)
				{
					...
					zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 478, &filePath);
	...
	if (Z_TYPE_P(&namespaces) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&namespaces), _2, _3, _1)
		{
			...
			if (ZEPHIR_IS_TRUE_IDENTICAL(&_5$$3)) {
				zephir_read_property_cached(&_7$$5, this_ptr, _zephir_prop_1, 478, PH_NOISY_CC | PH_READONLY);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (ZEPHIR_IS_TRUE_IDENTICAL(&_13$$6)) {
					zephir_read_property_cached(&_14$$8, this_ptr, _zephir_prop_1, 478, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Autoload_Loader, setEventsManager)
{
	...
	if (_2) {
		...
		zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 479, PH_NOISY_CC);
		_2 = Z_TYPE_P(&_3) != IS_NULL;
	}
	if (_2) {
		zephir_memory_observe(&_4$$3);
		zephir_read_property_cached(&_4$$3, this_ptr, _zephir_prop_0, 479, PH_NOISY_CC);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 480, eventsManager);
	ZEPHIR_MM_RESTORE();
}
