...
PHP_METHOD(Phalcon_Url, get) {
	...
	if (local == 0) {
		...
		if (_0$$3) {
			_1$$3 = zephir_memnstr_str(uri, SL("//"), "/home/nikos/Work/niden/cphalcon/phalcon/Url.zep", 107);
			if (!(_1$$3)) {
				_1$$3 = zephir_memnstr_str(uri, SL(":"), "/home/nikos/Work/niden/cphalcon/phalcon/Url.zep", 107);
			}
			_0$$3 = _1$$3;
	...
	if (Z_TYPE_P(uri) == IS_ARRAY) {
		...
		if (UNEXPECTED(!(zephir_array_isset_string_fetch(&routeName, uri, SL("for"), 0)))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_url_exception_ce, "It's necessary to define the route name with the parameter 'for'", "/home/nikos/Work/niden/cphalcon/phalcon/Url.zep", 126);
			return;
		}
		...
		if (UNEXPECTED(!zephir_is_true(&router))) {
			...
			if (UNEXPECTED(Z_TYPE_P(&container) != IS_OBJECT)) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_8$$12, "/home/nikos/Work/niden/cphalcon/phalcon/Url.zep", 142);
				ZEPHIR_MM_RESTORE();
				return;
			}
			...
			if (UNEXPECTED(!zephir_is_true(&_12$$11))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_14$$13, "/home/nikos/Work/niden/cphalcon/phalcon/Url.zep", 150);
				ZEPHIR_MM_RESTORE();
				return;
			}
		...
		if (UNEXPECTED(Z_TYPE_P(&route) != IS_OBJECT)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_19$$14, "/home/nikos/Work/niden/cphalcon/phalcon/Url.zep", 165);
			ZEPHIR_MM_RESTORE();
			return;
		}
	...
	if (local) {
		...
		ZVAL_STRING(&_26$$15, "/");
		ZEPHIR_CALL_FUNCTION(uri, "preg_replace", NULL, 48, &_25$$15, &_26$$15, &_24$$15);
		zephir_check_call_status();
	}
