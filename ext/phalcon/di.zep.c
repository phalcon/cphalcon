...
PHP_METHOD(Phalcon_Di, __call) {
	...
	if (zephir_start_with_str(&method, SL("set"))) {
		...
		if (zephir_array_isset_long_fetch(&definition, &arguments, 0, 0)) {
			...
			ZEPHIR_CALL_FUNCTION(&_5$$6, "lcfirst", NULL, 91, &_4$$6);
			zephir_check_call_status();
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(&_6, "phalcon/Di.zep", 143);
	ZEPHIR_MM_RESTORE();
	return;
		...
		if (Z_TYPE_P(&service) != IS_NULL) {
			...
			if (EG(exception)) {
				...
				if (zephir_instance_of_ev(&_8$$7, phalcon_di_exception_serviceresolutionexception_ce)) {
					...
					zephir_check_call_status();
					zephir_throw_exception_debug(&_10$$9, "phalcon/Di.zep", 206);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
			...
			if (UNEXPECTED(!(zephir_class_exists(&name, 1)))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_12$$12, "phalcon/Di.zep", 221);
				ZEPHIR_MM_RESTORE();
				return;
			}
...
PHP_METHOD(Phalcon_Di, getRaw) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&service, &_0, &name, 0)))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Di.zep", 287);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Di, getService) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&service, &_0, &name, 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 6, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Di.zep", 303);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Di, loadFromConfig) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&services, 0, "phalcon/Di.zep", 353);
	if (Z_TYPE_P(&services) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&services), _2, _3, _0)
		{
			...
			if (_4$$3) {
				...
				zephir_array_fetch_string(&_5$$3, &service, SL("shared"), PH_NOISY, "phalcon/Di.zep", 351);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_8$$4) {
					...
					zephir_array_fetch_string(&_9$$4, &service, SL("shared"), PH_NOISY, "phalcon/Di.zep", 351);
...
PHP_METHOD(Phalcon_Di, loadFromPhp) {
	...
	ZEPHIR_CALL_METHOD(NULL, &services, "__construct", NULL, 93, &filePath);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di, loadFromYaml) {
	...
	ZEPHIR_CALL_METHOD(NULL, &services, "__construct", NULL, 94, &filePath, &callbacks);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "loadfromconfig", NULL, 0, &services);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
