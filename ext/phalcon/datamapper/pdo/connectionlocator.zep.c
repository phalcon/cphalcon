...
PHP_METHOD(Phalcon_DataMapper_Pdo_ConnectionLocator, setMaster)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 564, callableObject);
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_DataMapper_Pdo_ConnectionLocator, getConnection)
{
	...
	if (ZEPHIR_IS_IDENTICAL(&_1, &requested)) {
		ZEPHIR_CALL_FUNCTION(&requested, "array_rand", NULL, 504, &collection);
		zephir_check_call_status();
	}
	...
	if (!(zephir_array_isset_value(&instances, &instanceName))) {
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 565, &instances);
	}
