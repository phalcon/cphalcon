...
PHP_METHOD(Phalcon_DataMapper_Pdo_ConnectionLocator, getMaster)
{
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "applyeventsmanager", NULL, 511, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_DataMapper_Pdo_ConnectionLocator, getConnection)
{
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "applyeventsmanager", NULL, 511, &_6);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_DataMapper_Pdo_ConnectionLocator, fireManagerEvent)
{
	...
	if (Z_TYPE_P(&_0) != IS_NULL) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 570, PH_NOISY_CC | PH_READONLY);
		if (cancellable) {
			ZVAL_BOOL(&_2$$3, 1);
		} else {
			ZVAL_BOOL(&_2$$3, 0);
