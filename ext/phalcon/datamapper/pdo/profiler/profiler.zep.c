...
PHP_METHOD(Phalcon_DataMapper_Pdo_Profiler_Profiler, __construct)
{
	...
	ZEND_PARSE_PARAMETERS_START(0, 1)
		Z_PARAM_OPTIONAL
		Z_PARAM_OBJECT_OF_CLASS_OR_NULL(logger, zephir_get_internal_ce(SL("psrext\\log\\loggerinterface")))
...
PHP_METHOD(Phalcon_DataMapper_Pdo_Profiler_Profiler, finish)
{
	...
	if (UNEXPECTED(zephir_is_true(&_0))) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&finish, "hrtime", NULL, 222, &__$true);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_1$$3, &ex, "gettraceasstring", NULL, 223);
		zephir_check_call_status();
		...
		if (ZEPHIR_IS_EMPTY(&values)) {
			...
			ZEPHIR_CALL_METHOD(&_5$$3, this_ptr, "encode", NULL, 224, &values);
			zephir_check_call_status();
		}
...
PHP_METHOD(Phalcon_DataMapper_Pdo_Profiler_Profiler, start)
{
	...
	if (UNEXPECTED(zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "hrtime", NULL, 222, &__$true);
		zephir_check_call_status();
