...
PHP_METHOD(Phalcon_Di_Service, getParameter)
{
	...
	if (UNEXPECTED(Z_TYPE_P(&_0) != IS_ARRAY)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 465);
	...
	if (Z_TYPE_P(&definition) == IS_STRING) {
		...
		if (Z_TYPE_P(&definition) == IS_OBJECT) {
			...
			if (Z_TYPE_P(&definition) == IS_ARRAY) {
				...
				ZEPHIR_CALL_METHOD(&instance, &builder, "build", NULL, 466, container, &definition, parameters);
				zephir_check_call_status();
			} else {
				found = 0;
			}
...
PHP_METHOD(Phalcon_Di_Service, setParameter)
{
	...
	if (UNEXPECTED(Z_TYPE_P(&_0) != IS_ARRAY)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 467);
		zephir_check_call_status();
