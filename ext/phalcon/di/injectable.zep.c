...
PHP_METHOD(Phalcon_Di_Injectable, __get)
{
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&propertyName_zv, "di")) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 13, &container);
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&propertyName_zv, "persistent")) {
		...
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 14, &_1$$4);
		zephir_memory_observe(&_4$$4);
		zephir_read_property_cached(&_4$$4, this_ptr, _zephir_prop_1, 14, PH_NOISY_CC);
		RETURN_CCTOR(&_4$$4);
	}
	...
	ZEPHIR_CALL_FUNCTION(NULL, "trigger_error", NULL, 11, &_6);
	zephir_check_call_status();
	RETURN_MM_NULL();
}
...
PHP_METHOD(Phalcon_Di_Injectable, getDI)
{
	...
	if (Z_TYPE_P(&container) == IS_NULL) {
		...
		if (UNEXPECTED(Z_TYPE_P(&container) != IS_OBJECT)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 12);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Injectable, setDI)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 15, container);
}
