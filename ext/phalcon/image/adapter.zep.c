...
PHP_METHOD(Phalcon_Image_Adapter, resize) {
	...
	ZVAL_LONG(&_25, 1);
	ZEPHIR_CALL_FUNCTION(&_26, "max", NULL, 64, &_23, &_25);
	...
	ZVAL_LONG(&_28, 1);
	ZEPHIR_CALL_FUNCTION(&_29, "max", NULL, 64, &_27, &_28);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Image_Adapter, text) {
	...
	ZEPHIR_CALL_FUNCTION(&colors, "array_map", NULL, 60, &_11, &_10);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Image_Adapter, render) {
	...
	if (!(!(Z_TYPE_P(&ext) == IS_UNDEF) && Z_STRLEN_P(&ext))) {
		...
		ZVAL_LONG(&_1$$3, 4);
		ZEPHIR_CALL_FUNCTION(&_2$$3, "pathinfo", NULL, 66, &_0$$3, &_1$$3);
		zephir_check_call_status();
