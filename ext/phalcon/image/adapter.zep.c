...
PHP_METHOD(Phalcon_Image_Adapter, resize) {
	...
	ZVAL_LONG(&_25, 1);
	ZEPHIR_CALL_FUNCTION(&_26, "max", NULL, 79, &_23, &_25);
	...
	ZVAL_LONG(&_28, 1);
	ZEPHIR_CALL_FUNCTION(&_29, "max", NULL, 79, &_27, &_28);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Image_Adapter, text) {
	...
	ZVAL_LONG(&_9, 2);
	ZEPHIR_CALL_FUNCTION(&_10, "str_split", NULL, 80, &color, &_9);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_11);
...
PHP_METHOD(Phalcon_Image_Adapter, render) {
	...
	if (!(!(Z_TYPE_P(&ext) == IS_UNDEF) && Z_STRLEN_P(&ext))) {
		...
		ZVAL_LONG(&_1$$3, 4);
		ZEPHIR_CALL_FUNCTION(&_2$$3, "pathinfo", NULL, 81, &_0$$3, &_1$$3);
		zephir_check_call_status();
