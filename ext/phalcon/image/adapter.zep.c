...
PHP_METHOD(Phalcon_Image_Adapter, resize) {
	...
	ZEPHIR_INIT_VAR(_10);
	ZVAL_LONG(_10, 1);
	ZEPHIR_CALL_FUNCTION(&_11, "max", &_12, 67, _8, _10);
	...
	ZEPHIR_INIT_VAR(_14);
	ZVAL_LONG(_14, 1);
	ZEPHIR_CALL_FUNCTION(&_15, "max", &_12, 67, _10, _14);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Image_Adapter, text) {
	...
	ZVAL_LONG(&_4, 2);
	ZEPHIR_CALL_FUNCTION(&_7, "str_split", NULL, 68, color, &_4);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_4);
...
PHP_METHOD(Phalcon_Image_Adapter, render) {
	...
	if (!(ext && Z_STRLEN_P(ext))) {
		...
		ZEPHIR_SINIT_VAR(_1);
		ZVAL_LONG(&_1, 4);
		ZEPHIR_CALL_FUNCTION(&_2, "pathinfo", NULL, 70, _0, &_1);
		zephir_check_call_status();
