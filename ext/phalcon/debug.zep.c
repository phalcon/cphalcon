...
PHP_METHOD(Phalcon_Debug, debugVar) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "debug_backtrace", NULL, 167);
...
PHP_METHOD(Phalcon_Debug, listenExceptions) {
	...
	zephir_array_fast_append(&_0, &_1);
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 168, &_0);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, listenLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 168, &_2);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, onUncaughtException) {
	...
	ZEPHIR_CALL_FUNCTION(&obLevel, "ob_get_level", NULL, 169);
	zephir_check_call_status();
	...
	if (zephir_is_true(&showBackTrace)) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_11$$5), _13$$5, _14$$5, _12$$5)
		{
			...
			ZEPHIR_CALL_METHOD(&_15$$7, this_ptr, "showtraceitem", &_16, 170, &n, &traceItem);
			zephir_check_call_status();
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(_REQUEST), _22$$5, _23$$5, _21$$5)
		{
			...
			if (1 != zephir_array_isset(&blacklist, &_24$$8)) {
				if (Z_TYPE_P(&value) != IS_ARRAY) {
					...
					ZEPHIR_CALL_FUNCTION(&_26$$11, "print_r", &_27, 171, &value, &__$true);
					zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_38$$5, "get_included_files", NULL, 172);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_43$$5, "memory_get_usage", NULL, 173, &__$true);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "error_reporting", NULL, 174);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_1);
	...
	if (zephir_is_true(&_1)) {
		...
		ZVAL_LONG(&_3$$3, 0);
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 175, message, &_3$$3, severity, file, line);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 176, &_0$$3, &_3$$3, &_4$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Debug, getArrayDump) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&argument), _2, _3, _1)
	{
		...
		if (ZEPHIR_IS_STRING(&v, "")) {
			...
			ZEPHIR_CALL_METHOD(&_5$$8, this_ptr, "getarraydump", &_7, 177, &v, &_6$$8);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&varDump);
...
PHP_METHOD(Phalcon_Debug, getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_1$$11, this_ptr, "getarraydump", NULL, 177, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, &trace, SL("class"), 0)) {
		...
		if (zephir_is_true(&_4$$3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8$$5, &classReflection, "isinternal", NULL, 178);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string(&trace, SL("class"))) {
		...
		if ((zephir_function_exists(&functionName TSRMLS_CC)  == SUCCESS)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &functionReflection, "__construct", NULL, 91, &functionName);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_14$$10, &functionReflection, "isinternal", NULL, 179);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&filez, &trace, SL("file"), 0)) {
		...
		if (zephir_is_true(&showFiles)) {
			ZEPHIR_CALL_FUNCTION(&lines, "file", NULL, 180, &filez);
			zephir_check_call_status();
			...
			while (1) {
				...
				if (_39$$24) {
					...
					ZEPHIR_CALL_FUNCTION(&_45$$29, "htmlentities", &_46, 176, &_40$$29, &_43$$29, &_44$$29);
					zephir_check_call_status();
