...
PHP_METHOD(Phalcon_Debug, _escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 148, _0, &_3, &_4);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Debug, _getArrayDump) {
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		ZEPHIR_CALL_FUNCTION(&_4, "is_scalar", &_5, 149, v);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(v) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&_6, this_ptr, "_getarraydump", &_9, 150, v, _8);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, _getVarDump) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "is_scalar", NULL, 149, variable);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_1, this_ptr, "_getarraydump", &_2, 150, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string(trace, SS("class"))) {
		...
		if (zephir_is_true(_3)) {
			...
			ZEPHIR_CALL_METHOD(NULL, classReflection, "__construct", NULL, 60, className);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_6, classReflection, "isinternal", NULL, 152);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string(trace, SS("class"))) {
		...
		if ((zephir_function_exists(functionName TSRMLS_CC)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_6, functionReflection, "isinternal", NULL, 154);
			zephir_check_call_status();
			if (zephir_is_true(_6)) {
			...
			while (1) {
				...
				if (ZEPHIR_IS_STRING(currentLine, "\n")) {
					...
					if (ZEPHIR_IS_STRING(currentLine, "\r\n")) {
						...
						ZEPHIR_CALL_FUNCTION(&_6, "htmlentities", &_20, 148, _19, entCompat, utf8);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	if (zephir_is_true(&_1)) {
		...
		ZEPHIR_INIT_VAR(_3);
		ZVAL_LONG(_3, 0);
		ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, 157, message, _3, severity, file, line);
		zephir_check_call_status();
	...
	if (zephir_is_true(showBackTrace)) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_11, &_10)
			...
			ZEPHIR_CALL_METHOD(&_13, this_ptr, "showtraceitem", &_14, 159, n, traceItem);
			zephir_check_call_status();
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_16, &_15)
			...
			if (Z_TYPE_P(value) != IS_ARRAY) {
				...
				ZEPHIR_CALL_FUNCTION(&_13, "print_r", &_19, 160, value, ZEPHIR_GLOBAL(global_true));
				zephir_check_call_status();
				ZEPHIR_INIT_LNVAR(_18);
		...
		ZEPHIR_CALL_FUNCTION(&_13, "get_included_files", NULL, 161);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_13, "memory_get_usage", NULL, 162, ZEPHIR_GLOBAL(global_true));
		zephir_check_call_status();
		ZEPHIR_INIT_LNVAR(_26);
