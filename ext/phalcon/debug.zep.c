...
PHP_METHOD(Phalcon_Debug, _escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 153, _0$$3, &_3$$3, &_4$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		if (Z_TYPE_P(v) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&_6$$9, this_ptr, "_getarraydump", &_8, 154, v, _7$$9);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, _getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_2$$11, this_ptr, "_getarraydump", &_1, 154, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, trace, SS("class"), 0 TSRMLS_CC)) {
		...
		if (zephir_is_true(_3$$3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8$$5, classReflection, "isinternal", NULL, 156);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string(trace, SS("class"))) {
		...
		if ((zephir_function_exists(functionName TSRMLS_CC)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_16$$10, functionReflection, "isinternal", NULL, 158);
			zephir_check_call_status();
		...
		if (zephir_is_true(showFiles)) {
			...
			while (1) {
				...
				if (_45$$26) {
					...
					ZEPHIR_CALL_FUNCTION(&_51$$31, "htmlentities", &_52, 153, _46$$31, &_49$$31, &_50$$31);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *severity, *message, *file, *line, *_0 = NULL, _1, *_2$$3, *_3$$3;
	...
	if (zephir_is_true(showBackTrace)) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_12$$5, &_11$$5)
			...
			ZEPHIR_CALL_METHOD(&_14$$7, this_ptr, "showtraceitem", &_15, 163, n, traceItem);
			zephir_check_call_status();
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_17$$5, &_16$$5)
			...
			if (Z_TYPE_P(value) != IS_ARRAY) {
				...
				ZEPHIR_CALL_FUNCTION(&_20$$10, "print_r", &_21, 164, value, ZEPHIR_GLOBAL(global_true));
				zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_29$$5, "get_included_files", NULL, 165);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_34$$5, "memory_get_usage", NULL, 166, ZEPHIR_GLOBAL(global_true));
		zephir_check_call_status();
