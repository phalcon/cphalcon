...
PHP_METHOD(Phalcon_Debug, onUncaughtException) {
	...
	ZEPHIR_CALL_FUNCTION(&obLevel, "ob_get_level", NULL, 153);
	zephir_check_call_status();
	...
	if (zephir_is_true(&showBackTrace)) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_11$$5, 0, "phalcon/debug.zep", 256);
		if (Z_TYPE_P(&_11$$5) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_11$$5), _14$$5, _15$$5, _12$$5)
			{
				...
				ZEPHIR_CALL_METHOD(&_16$$7, this_ptr, "showtraceitem", &_17, 154, &n, &traceItem);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_18$$8, this_ptr, "showtraceitem", &_17, 154, &n, &traceItem);
					zephir_check_call_status();
		...
		ZEPHIR_CALL_CE_STATIC(&blacklist, phalcon_helper_arr_ce, "get", &_19, 155, &_20$$5, &_22$$5, &_21$$5);
		zephir_check_call_status();
			...
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_REQUEST), _25$$5, _26$$5, _23$$5)
			{
				...
				if (1 != zephir_array_isset(&blacklist, &_27$$9)) {
					if (Z_TYPE_P(&value) != IS_ARRAY) {
						...
						ZEPHIR_CALL_FUNCTION(&_29$$12, "print_r", &_30, 156, &value, &__$true);
						zephir_check_call_status();
				...
				zephir_check_call_status();
					...
					if (1 != zephir_array_isset(&blacklist, &_32$$13)) {
						if (Z_TYPE_P(&value) != IS_ARRAY) {
							...
							ZEPHIR_CALL_FUNCTION(&_34$$16, "print_r", &_30, 156, &value, &__$true);
							zephir_check_call_status();
		...
		ZEPHIR_CALL_CE_STATIC(&blacklist, phalcon_helper_arr_ce, "get", &_19, 155, &_36$$5, &_37$$5, &_22$$5);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_49$$5, "get_included_files", NULL, 157);
		zephir_check_call_status();
		zephir_is_iterable(&_49$$5, 0, "phalcon/debug.zep", 296);
		...
		ZEPHIR_CALL_FUNCTION(&_56$$5, "memory_get_usage", NULL, 158, &__$true);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&dataVars) == IS_ARRAY) {
			...
			zephir_concat_self_str(&html, SL("<tr><th>Key</th><th>Value</th></tr>") TSRMLS_CC);
			zephir_is_iterable(&dataVars, 0, "phalcon/debug.zep", 314);
			if (Z_TYPE_P(&dataVars) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&dataVars), _60$$23, _61$$23, _58$$23)
				{
					...
					zephir_array_fetch_long(&_63$$24, &dataVar, 0, PH_NOISY | PH_READONLY, "phalcon/debug.zep", 312 TSRMLS_CC);
				...
				while (1) {
					...
					zephir_check_call_status();
						zephir_array_fetch_long(&_66$$25, &dataVar, 0, PH_NOISY | PH_READONLY, "phalcon/debug.zep", 312 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	if (zephir_is_true(&_1)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$3, "phalcon/debug.zep", 344 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Debug, setBlacklist) {
	...
	ZEPHIR_CALL_CE_STATIC(&area, phalcon_helper_arr_ce, "get", &_0, 155, &blacklist, &_2, &_1);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&subArray);
	array_init(&subArray);
	ZEPHIR_INIT_VAR(&result);
	array_init(&result);
	zephir_is_iterable(&area, 0, "phalcon/debug.zep", 364);
	...
	ZEPHIR_CALL_CE_STATIC(&area, phalcon_helper_arr_ce, "get", &_0, 155, &blacklist, &_7, &_2);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&subArray);
	array_init(&subArray);
	zephir_is_iterable(&area, 0, "phalcon/debug.zep", 373);
	if (Z_TYPE_P(&area) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Debug, escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 161, &_0$$3, &_3$$3, &_4$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Debug, getArrayDump) {
	...
	array_init(&dump);
	zephir_is_iterable(&argument, 0, "phalcon/debug.zep", 464);
	if (Z_TYPE_P(&argument) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&argument), _3, _4, _1)
		{
			...
			if (ZEPHIR_IS_STRING(&v, "")) {
				...
				ZEPHIR_CALL_METHOD(&_6$$8, this_ptr, "getarraydump", &_8, 162, &v, &_7$$8);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&varDump);
			...
			zephir_array_append(&dump, &_10$$5, PH_SEPARATE, "phalcon/debug.zep", 461);
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				if (ZEPHIR_IS_STRING(&v, "")) {
					...
					ZEPHIR_CALL_METHOD(&_11$$15, this_ptr, "getarraydump", &_8, 162, &v, &_12$$15);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&varDump);
				...
				zephir_array_append(&dump, &_14$$12, PH_SEPARATE, "phalcon/debug.zep", 461);
...
PHP_METHOD(Phalcon_Debug, getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_1$$11, this_ptr, "getarraydump", NULL, 162, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, &trace, SL("class"), 0)) {
		...
		if (zephir_is_true(&_4$$3)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &classReflection, "__construct", NULL, 81, &className);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8$$5, &classReflection, "isinternal", NULL, 163);
			zephir_check_call_status();
		...
		zephir_array_fetch_string(&_13$$3, &trace, SL("type"), PH_NOISY | PH_READONLY, "phalcon/debug.zep", 603 TSRMLS_CC);
		zephir_concat_self(&html, &_13$$3 TSRMLS_CC);
	}
	ZEPHIR_OBS_VAR(&functionName);
	zephir_array_fetch_string(&functionName, &trace, SL("function"), PH_NOISY, "phalcon/debug.zep", 609 TSRMLS_CC);
	if (zephir_array_isset_string(&trace, SL("class"))) {
		...
		if ((zephir_function_exists(&functionName TSRMLS_CC)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_14$$10, &functionReflection, "isinternal", NULL, 164);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&traceArgs, &trace, SL("args"), 0)) {
		...
		zephir_is_iterable(&traceArgs, 0, "phalcon/debug.zep", 658);
		if (Z_TYPE_P(&traceArgs) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&traceArgs), _18$$14)
			{
				...
				zephir_array_append(&arguments, &_22$$15, PH_SEPARATE, "phalcon/debug.zep", 652);
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_array_append(&arguments, &_24$$16, PH_SEPARATE, "phalcon/debug.zep", 652);
	...
	if (zephir_array_isset_string_fetch(&filez, &trace, SL("file"), 0)) {
		...
		zephir_array_fetch_string(&_27$$17, &trace, SL("line"), PH_NOISY, "phalcon/debug.zep", 666 TSRMLS_CC);
		...
		if (zephir_is_true(&showFiles)) {
			...
			while (1) {
				...
				zephir_array_fetch(&currentLine, &lines, &linePosition, PH_NOISY, "phalcon/debug.zep", 738 TSRMLS_CC);
				if (zephir_is_true(&showFileFragment)) {
				...
				if (_42$$25) {
					...
					ZEPHIR_CALL_FUNCTION(&_48$$30, "htmlentities", &_49, 161, &_43$$30, &_46$$30, &_47$$30);
					zephir_check_call_status();
