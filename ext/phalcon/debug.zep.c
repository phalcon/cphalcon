...
PHP_METHOD(Phalcon_Debug, debugVar) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "debug_backtrace", NULL, 204);
...
PHP_METHOD(Phalcon_Debug, listenExceptions) {
	...
	zephir_array_fast_append(&_0, &_1);
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 205, &_0);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, listenLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 205, &_2);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, onUncaughtException) {
	...
	ZEPHIR_CALL_FUNCTION(&obLevel, "ob_get_level", NULL, 206);
	zephir_check_call_status();
	if (ZEPHIR_GT_LONG(&obLevel, 0)) {
		ZEPHIR_CALL_FUNCTION(NULL, "ob_end_clean", NULL, 207);
		zephir_check_call_status();
	}
	...
	zephir_read_property(&_8, this_ptr, ZEND_STRL("showBackTrace"), PH_NOISY_CC | PH_READONLY);
	...
	if (zephir_is_true(&showBackTrace)) {
		...
		if (Z_TYPE_P(&_10$$5) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_10$$5), _13$$5, _14$$5, _11$$5)
			{
				...
				ZEPHIR_CALL_METHOD(&_15$$7, this_ptr, "showtraceitem", &_16, 208, &n, &traceItem);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_17$$8, this_ptr, "showtraceitem", &_16, 208, &n, &traceItem);
					zephir_check_call_status();
		...
		zephir_read_property(&_20$$5, this_ptr, ZEND_STRL("blacklist"), PH_NOISY_CC | PH_READONLY);
			...
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_REQUEST), _25$$5, _26$$5, _23$$5)
			{
				...
				if (1 != zephir_array_isset(&blacklist, &_27$$9)) {
					if (Z_TYPE_P(&value) != IS_ARRAY) {
						...
						ZEPHIR_CALL_FUNCTION(&_29$$12, "print_r", &_30, 209, &value, &__$true);
						zephir_check_call_status();
				...
				zephir_check_call_status();
					...
					if (1 != zephir_array_isset(&blacklist, &_32$$13)) {
						if (Z_TYPE_P(&value) != IS_ARRAY) {
							...
							ZEPHIR_CALL_FUNCTION(&_34$$16, "print_r", &_30, 209, &value, &__$true);
							zephir_check_call_status();
		...
		zephir_read_property(&_37$$5, this_ptr, ZEND_STRL("blacklist"), PH_NOISY_CC | PH_READONLY);
		...
		ZEPHIR_CALL_FUNCTION(&_51$$5, "get_included_files", NULL, 210);
		zephir_check_call_status();
		...
		ZEPHIR_INIT_NVAR(&keyFile);
		zephir_concat_self_str(&html, SL("</table></div>"));
		ZEPHIR_CALL_FUNCTION(&_58$$5, "memory_get_usage", NULL, 211, &__$true);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "error_reporting", NULL, 212);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	...
	if (UNEXPECTED(zephir_is_true(&_1))) {
		...
		ZVAL_LONG(&_3$$3, 0);
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 213, message, &_3$$3, severity, file, line);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 214, &_0$$3, &_3$$3, &_4$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	if (Z_TYPE_P(&argument) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&argument), _3, _4, _1)
		{
			...
			if (ZEPHIR_IS_STRING(&v, "")) {
				...
				ZEPHIR_CALL_METHOD(&_6$$8, this_ptr, "getarraydump", &_8, 215, &v, &_7$$8);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&varDump);
		...
		while (1) {
			...
			zephir_check_call_status();
				if (ZEPHIR_IS_STRING(&v, "")) {
					...
					ZEPHIR_CALL_METHOD(&_11$$15, this_ptr, "getarraydump", &_8, 215, &v, &_12$$15);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&varDump);
...
PHP_METHOD(Phalcon_Debug, getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_1$$10, this_ptr, "getarraydump", NULL, 215, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, &trace, SL("class"), 0)) {
		...
		if (zephir_is_true(&_4$$3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8$$5, &classReflection, "isinternal", NULL, 216);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string(&trace, SL("class"))) {
		...
		if ((zephir_function_exists(&functionName)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_14$$10, &functionReflection, "isinternal", NULL, 217);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&filez, &trace, SL("file"), 0)) {
		...
		if (zephir_is_true(&showFiles)) {
			ZEPHIR_CALL_FUNCTION(&lines, "file", NULL, 218, &filez);
			zephir_check_call_status();
			...
			while (1) {
				...
				if (_42$$25) {
					...
					ZEPHIR_CALL_FUNCTION(&_48$$30, "htmlentities", &_49, 214, &_43$$30, &_46$$30, &_47$$30);
					zephir_check_call_status();
