...
PHP_METHOD(Phalcon_Debug, debugVar) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "debug_backtrace", NULL, 173);
...
PHP_METHOD(Phalcon_Debug, onUncaughtException) {
	...
	ZEPHIR_CALL_FUNCTION(&obLevel, "ob_get_level", NULL, 175);
	zephir_check_call_status();
	if (ZEPHIR_GT_LONG(&obLevel, 0)) {
		ZEPHIR_CALL_FUNCTION(NULL, "ob_end_clean", NULL, 176);
		zephir_check_call_status();
	}
	...
	if (zephir_is_true(&showBackTrace)) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_11$$5), _13$$5, _14$$5, _12$$5)
		{
			...
			ZEPHIR_CALL_METHOD(&_15$$7, this_ptr, "showtraceitem", &_16, 177, &n, &traceItem);
			zephir_check_call_status();
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(_REQUEST), _22$$5, _23$$5, _21$$5)
		{
			...
			if (1 != zephir_array_isset(&blacklist, &_24$$8)) {
				if (Z_TYPE_P(&value) != IS_ARRAY) {
					...
					ZEPHIR_CALL_FUNCTION(&_26$$11, "print_r", &_27, 178, &value, &__$true);
					zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_38$$5, "get_included_files", NULL, 179);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_43$$5, "memory_get_usage", NULL, 180, &__$true);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "error_reporting", NULL, 181);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_1);
	...
	if (UNEXPECTED(zephir_is_true(&_1))) {
		...
		ZVAL_LONG(&_3$$3, 0);
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 182, message, &_3$$3, severity, file, line);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 183, &_0$$3, &_3$$3, &_4$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Debug, getArrayDump) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&argument), _2, _3, _1)
	{
		...
		if (ZEPHIR_IS_STRING(&v, "")) {
			...
			ZEPHIR_CALL_METHOD(&_5$$8, this_ptr, "getarraydump", &_7, 184, &v, &_6$$8);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&varDump);
...
PHP_METHOD(Phalcon_Debug, getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_1$$10, this_ptr, "getarraydump", NULL, 184, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, &trace, SL("class"), 0)) {
		...
		if (zephir_is_true(&_4$$3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8$$5, &classReflection, "isinternal", NULL, 185);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string(&trace, SL("class"))) {
		...
		if ((zephir_function_exists(&functionName TSRMLS_CC)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_14$$10, &functionReflection, "isinternal", NULL, 186);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&filez, &trace, SL("file"), 0)) {
		...
		if (zephir_is_true(&showFiles)) {
			ZEPHIR_CALL_FUNCTION(&lines, "file", NULL, 187, &filez);
			zephir_check_call_status();
			...
			while (1) {
				...
				if (_39$$24) {
					...
					ZEPHIR_CALL_FUNCTION(&_45$$29, "htmlentities", &_46, 183, &_40$$29, &_43$$29, &_44$$29);
					zephir_check_call_status();
