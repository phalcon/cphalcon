...
PHP_METHOD(Phalcon_Debug, listenExceptions) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 159, &_0);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, listenLowSeverity) {
	...
	zephir_array_fast_append(&_0, &_1);
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 160, &_0);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_2);
	...
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 159, &_2);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, debugVar) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "debug_backtrace", NULL, 161);
	zephir_check_call_status();
	zephir_array_fast_append(&_0, &_1);
	ZEPHIR_INIT_VAR(&_2);
	zephir_time(&_2);
...
PHP_METHOD(Phalcon_Debug, _getArrayDump) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&argument), _2, _3, _1)
	{
		...
		if (Z_TYPE_P(&v) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&_6$$9, this_ptr, "_getarraydump", &_8, 163, &v, &_7$$9);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, _getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_2$$11, this_ptr, "_getarraydump", &_1, 163, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, getMajorVersion) {
	...
	ZEPHIR_CALL_CE_STATIC(&_0, phalcon_version_ce, "get", &_1, 164);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, getVersion) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(&_1, phalcon_version_ce, "get", &_2, 164);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	ZEPHIR_OBS_VAR_ONCE(&trace);
	ZVAL_COPY(&trace, trace_param);
	...
	if (zephir_array_isset_string(&trace, SL("class"))) {
		...
		if ((zephir_function_exists(&functionName TSRMLS_CC)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_16$$10, &functionReflection, "isinternal", NULL, 166);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string_fetch(&traceArgs, &trace, SL("args"), 0)) {
		if (zephir_fast_count_int(&traceArgs TSRMLS_CC)) {
			...
			zephir_is_iterable(&traceArgs, 0, "phalcon/debug.zep", 439);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&traceArgs), _22$$15)
			{
				...
				zephir_array_append(&arguments, &_25$$16, PH_SEPARATE, "phalcon/debug.zep", 433);
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&argument);
	...
	if (zephir_array_isset_string_fetch(&filez, &trace, SL("file"), 0)) {
		...
		zephir_array_fetch_string(&_28$$18, &trace, SL("line"), PH_NOISY, "phalcon/debug.zep", 450 TSRMLS_CC);
		...
		if (zephir_is_true(&showFiles)) {
			...
			while (1) {
				...
				if (_43$$26) {
					...
					ZEPHIR_CALL_FUNCTION(&_49$$31, "htmlentities", &_50, 162, &_44$$31, &_47$$31, &_48$$31);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtException) {
	...
	if (zephir_is_true(&showBackTrace)) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_29$$5, 0, "phalcon/debug.zep", 694);
