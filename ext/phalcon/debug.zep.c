...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string(trace, SS("class"))) {
		...
		zephir_array_fetch_string(&_7, trace, SL("type"), PH_NOISY | PH_READONLY, "phalcon/debug.zep", 443 TSRMLS_CC);
	...
	if (zephir_array_isset_string(trace, SS("args"))) {
		...
		if (zephir_fast_count_int(traceArgs TSRMLS_CC)) {
			...
			zephir_is_iterable(traceArgs, &_10, &_9, 0, 0, "phalcon/debug.zep", 498);
			for (
	...
	if (zephir_array_isset_string_fetch(&filez, trace, SS("file"), 0 TSRMLS_CC)) {
		ZEPHIR_OBS_VAR(_13);
		zephir_array_fetch_string(&_13, trace, SL("line"), PH_NOISY, "phalcon/debug.zep", 509 TSRMLS_CC);
		zephir_get_strval(_14, _13);
		ZEPHIR_CPY_WRT(line, _14);
		ZEPHIR_INIT_LNVAR(_3);
		...
		if (zephir_is_true(showFiles)) {
			ZEPHIR_CALL_FUNCTION(&lines, "file", &_15, filez);
			zephir_check_call_status();
			...
			if (zephir_is_true(showFileFragment)) {
				...
				ZVAL_LONG(&_17, firstLine);
				ZEPHIR_INIT_VAR(_18);
				ZEPHIR_CONCAT_SVSVSVS(_18, "<pre class=\"prettyprint highlight:", &_16, ":", line, " linenums:", &_17, "\">");
				...
				ZVAL_LONG(&_16, firstLine);
				ZEPHIR_INIT_LNVAR(_6);
				ZEPHIR_CONCAT_SVSVS(_6, "<pre class=\"prettyprint highlight:", &_16, ":", line, " linenums error-scroll\">");
				zephir_concat_self(&html, _6 TSRMLS_CC);
			}
			...
			while (1) {
				...
				if (zephir_is_true(showFileFragment)) {
					if (i == firstLine) {
						...
						if (zephir_is_true(_1)) {
							...
							zephir_fast_str_replace(_19, comment, space, currentLine);
				...
				if (ZEPHIR_IS_STRING(currentLine, "\n")) {
					...
					if (ZEPHIR_IS_STRING(currentLine, "\r\n")) {
						...
						ZEPHIR_INIT_NVAR(_19);
						zephir_fast_str_replace(_19, tab, twoSpaces, currentLine);
						ZEPHIR_CALL_FUNCTION(&_5, "htmlentities", &_20, _19, entCompat, utf8);
						zephir_check_call_status();
