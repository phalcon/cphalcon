...
ZEPHIR_INIT_CLASS(Phalcon_Time_Clock_SystemClock)
{
...

	{
		zval _zc0;
		ZVAL_UNDEF(&_zc0);
		zephir_declare_typed_property(phalcon_time_clock_systemclock_ce, SL("timezone"), &_zc0, ZEND_ACC_PRIVATE, 0, SL("DateTimeZone"));
	}
...
PHP_METHOD(Phalcon_Time_Clock_SystemClock, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1400, timezone);
}
...
PHP_METHOD(Phalcon_Time_Clock_SystemClock, now)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 1400, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "now");
