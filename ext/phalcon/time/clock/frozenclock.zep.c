...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1377, now);
}
...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, fromSystemTimezone)
{
	...
	ZEPHIR_CALL_FUNCTION(&_2, "date_default_timezone_get", NULL, 250);
...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, adjust)
{
	...
	if (zephir_is_true(&_3)) {
		...
		/* try_start_1: */
			...
			zephir_read_property_cached(&_4$$4, this_ptr, _zephir_prop_0, 1377, PH_NOISY_CC | PH_READONLY);
		...
		if (EG(exception)) {
			...
			if (zephir_is_instance_of(&_5$$3, SL("Throwable"))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_6$$5, "phalcon/Time/Clock/FrozenClock.zep", 74);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
		...
		ZVAL_LONG(&_8$$6, 2);
		ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 295, &_7$$6, &_8$$6);
		zephir_check_call_status();
		zephir_read_property_cached(&_8$$6, this_ptr, _zephir_prop_0, 1377, PH_NOISY_CC | PH_READONLY);
	...
	if (UNEXPECTED(_9)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_10$$7, "phalcon/Time/Clock/FrozenClock.zep", 103);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1377, &modified);
	RETURN_THIS();
}
