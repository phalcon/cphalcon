...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1368, now);
}
...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, adjust)
{
	...
	if (zephir_is_true(&_3)) {
		...
		/* try_start_1: */
			...
			zephir_read_property_cached(&_4$$4, this_ptr, _zephir_prop_0, 1368, PH_NOISY_CC | PH_READONLY);
		...
		zephir_check_call_status();
		zephir_read_property_cached(&_8$$6, this_ptr, _zephir_prop_0, 1368, PH_NOISY_CC | PH_READONLY);
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1368, &modified);
	RETURN_THIS();
}
