...
ZEPHIR_INIT_CLASS(Phalcon_Time_Clock_FrozenClock)
{
...

	{
		zval _zc0;
		ZVAL_UNDEF(&_zc0);
		zephir_declare_typed_property(phalcon_time_clock_frozenclock_ce, SL("now"), &_zc0, ZEND_ACC_PRIVATE, 0, SL("DateTimeImmutable"));
	}
...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1383, now);
}
...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, fromSystemTimezone)
{
	...
	ZEPHIR_CALL_FUNCTION(&_2, "date_default_timezone_get", NULL, 256);
...
PHP_METHOD(Phalcon_Time_Clock_FrozenClock, adjust)
{
	...
	if (zephir_is_true(&_3)) {
		...
		/* try_start_1: */
			...
			zephir_read_property_cached(&_4$$4, this_ptr, _zephir_prop_0, 1383, PH_NOISY_CC | PH_READONLY);
		...
		if (EG(exception)) {
			...
			if (zephir_is_instance_of(&_5$$3, SL("Throwable"))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_6$$5, "phalcon/Time/Clock/FrozenClock.zep", 71);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
		...
		zephir_create_closure_ex(&_7$$6, NULL, phalcon_100__closure_ce, SL("__invoke"));
		ZVAL_LONG(&_8$$6, 2);
		ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 301, &_7$$6, &_8$$6);
		zephir_check_call_status();
		zephir_read_property_cached(&_8$$6, this_ptr, _zephir_prop_0, 1383, PH_NOISY_CC | PH_READONLY);
	...
	if (UNEXPECTED(_9)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_10$$7, "phalcon/Time/Clock/FrozenClock.zep", 100);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1383, &modified);
	RETURN_THIS();
}
