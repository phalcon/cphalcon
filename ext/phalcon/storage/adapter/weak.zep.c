...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 322, &_1);
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "");
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 323, &_0);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 324, &options);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, clear)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	array_init(&_0);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 325, &_0);
	RETURN_MM_BOOL(1);
}
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, getKeys)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 325, PH_NOISY_CC | PH_READONLY);
	...
	if (UNEXPECTED(!ZEPHIR_IS_IDENTICAL(&_1, &prefix_zv))) {
		...
		array_init(&results);
		zephir_is_iterable(&keys, 0, "phalcon/Storage/Adapter/Weak.zep", 92);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&keys), _2$$3)
		{
			ZEPHIR_INIT_NVAR(&key);
			ZVAL_COPY(&key, _2$$3);
			if (zephir_start_with(&key, &prefix_zv, NULL)) {
				zephir_array_append(&results, &key, PH_SEPARATE, "phalcon/Storage/Adapter/Weak.zep", 89);
			}
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, doDelete)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 325, PH_NOISY_CC | PH_READONLY);
	exists = zephir_array_isset_value(&_1, &key_zv);
	zephir_unset_property_array(this_ptr, ZEND_STRL("weakList"), &key_zv);
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_1, 325, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, doGet)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 326, &key_zv);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_1, 325, PH_NOISY_CC | PH_READONLY);
	if (0 == zephir_array_isset_value(&_0, &key_zv)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 326, &__$null);
		RETVAL_ZVAL(defaultValue, 1, 0);
		RETURN_MM();
	}
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_1, 325, PH_NOISY_CC | PH_READONLY);
	zephir_memory_observe(&wr);
	zephir_array_fetch(&wr, &_1, &key_zv, PH_NOISY, "phalcon/Storage/Adapter/Weak.zep", 178);
