...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 326, &_1);
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "");
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 327, &_0);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 328, &options);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, getKeys)
{
	...
	if (UNEXPECTED(!ZEPHIR_IS_IDENTICAL(&_1, &prefix_zv))) {
		...
		array_init(&results);
		zephir_is_iterable(&keys, 0, "phalcon/Storage/Adapter/Weak.zep", 75);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&keys), _2$$3)
		{
			ZEPHIR_INIT_NVAR(&key);
			ZVAL_COPY(&key, _2$$3);
			if (zephir_start_with(&key, &prefix_zv, NULL)) {
				zephir_array_append(&results, &key, PH_SEPARATE, "phalcon/Storage/Adapter/Weak.zep", 72);
			}
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, doDelete)
{
	...
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_1, 329, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Adapter_Weak, doGet)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 330, &key_zv);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_1, 329, PH_NOISY_CC | PH_READONLY);
	...
	zephir_array_fetch(&wr, &_1, &key_zv, PH_NOISY, "phalcon/Storage/Adapter/Weak.zep", 141);
