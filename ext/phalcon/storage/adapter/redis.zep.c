...
PHP_METHOD(Phalcon_Storage_Adapter_Redis, getAdapter)
{
	...
	if (Z_TYPE_P(&_0) == IS_NULL) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_1$$3, this_ptr, "checkconnect", NULL, 109, &connection);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_2$$3, &_1$$3, "checkauth", NULL, 110, &connection);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "checkindex", NULL, 111, &connection);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Redis, checkConnect)
{
	...
	if (!(zephir_is_true(&result))) {
		...
		ZEPHIR_CALL_FUNCTION(&_6$$5, "sprintf", NULL, 113, &_5$$5, &host, &port);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Redis, setSerializer)
{
	...
	ZEPHIR_CALL_FUNCTION(&_7, "defined", NULL, 114, &_0);
	zephir_check_call_status();
	if (zephir_is_true(&_7)) {
		ZEPHIR_INIT_VAR(&_8$$5);
		ZVAL_STRING(&_8$$5, "\\Redis::SERIALIZER_JSON");
		ZEPHIR_CALL_FUNCTION(&_9$$5, "constant", NULL, 115, &_8$$5);
		zephir_check_call_status();
