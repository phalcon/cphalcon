...
PHP_METHOD(Phalcon_Storage_Adapter_Libmemcached, getAdapter)
{
	...
	if (!(zephir_is_true(&_0))) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_1, 308, PH_NOISY_CC | PH_READONLY);
		...
		zephir_check_call_status();
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_2, 309, PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_4$$3, -1002);
		...
		if (ZEPHIR_IS_EMPTY(&serverList)) {
			...
			ZEPHIR_CALL_FUNCTION(&_9$$4, "array_replace", NULL, 268, &failover, &client);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(&client, &_9$$4);
			ZEPHIR_CALL_METHOD(&_9$$4, this_ptr, "setoptions", NULL, 269, &connection, &client);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_10$$4, &_9$$4, "setservers", NULL, 270, &connection, &servers);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, &_10$$4, "setsasl", NULL, 271, &connection, &saslUser, &saslPass);
			zephir_check_call_status();
		}
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "setserializer", NULL, 272, &connection);
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 307, &connection);
	}
...
PHP_METHOD(Phalcon_Storage_Adapter_Libmemcached, setSerializer)
{
	...
	if (zephir_array_isset_value(&map, &serializer)) {
		...
		ZVAL_STRING(&_1$$3, "");
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 310, &_1$$3);
