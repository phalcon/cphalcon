...
PHP_METHOD(Phalcon_Storage_Serializer_Php, unserialize)
{
	...
	if (UNEXPECTED(_5)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1274, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1274, &__$false);
		}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1273, &result);
	ZEPHIR_MM_RESTORE();
}
