...
PHP_METHOD(Phalcon_Storage_Serializer_Php, serialize)
{
	...
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 1332, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Serializer_Php, unserialize)
{
	...
	ZVAL_LONG(&_4, (8 | 2));
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 297, &_3, &_4);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&result, this_ptr, "phpunserialize", NULL, 0, data);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "restore_error_handler", NULL, 298);
	zephir_check_call_status();
	...
	if (UNEXPECTED(_5)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1333, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1333, &__$false);
		}
