...
PHP_METHOD(Phalcon_Storage_Serializer_Php, setAllowedClasses)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1341, allowedClasses);
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Storage_Serializer_Php, unserialize)
{
	...
	zephir_read_property_cached(&_6, this_ptr, _zephir_prop_1, 1341, PH_NOISY_CC);
	...
	if (UNEXPECTED(_8)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1342, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1342, &__$false);
		}
