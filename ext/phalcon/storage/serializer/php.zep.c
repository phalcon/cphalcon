...
PHP_METHOD(Phalcon_Storage_Serializer_Php, serialize)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 1277, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Serializer_Php, unserialize)
{
	...
	zephir_check_call_status();
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1277, data);
		RETURN_MM_NULL();
	}
	ZEPHIR_INIT_VAR(&_1);
	...
	if (UNEXPECTED(_5)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1278, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1278, &__$false);
		}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1277, &result);
	ZEPHIR_MM_RESTORE();
}
