...
PHP_METHOD(Phalcon_Storage_Serializer_Php, setAllowedClasses)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1357, allowedClasses);
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Storage_Serializer_Php, unserialize)
{
	...
	if (UNEXPECTED(_8)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1358, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 1358, &__$false);
		}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1356, &result);
	ZEPHIR_MM_RESTORE();
}
