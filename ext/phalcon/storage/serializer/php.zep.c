...
PHP_METHOD(Phalcon_Storage_Serializer_Php, serialize)
{
	...
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 1336, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Serializer_Php, unserialize)
{
	...
	ZVAL_LONG(&_4, (8 | 2));
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 295, &_3, &_4);
	zephir_check_call_status();
	...
	if (UNEXPECTED(_5)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1337, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1337, &__$false);
		}
