...
PHP_METHOD(Phalcon_Storage_Serializer_Base64, serialize)
{
	...
	if (Z_TYPE_P(&_0) != IS_STRING) {
		...
		zephir_throw_exception_debug(&_1$$3, "phalcon/Storage/Serializer/Base64.zep", 29);
		ZEPHIR_MM_RESTORE();
		return;
	}
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 1308, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Serializer_Base64, unserialize)
{
	...
	if (!ZEPHIR_IS_STRING_IDENTICAL(&_0, "string")) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 0);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/Storage/Serializer/Base64.zep", 47);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZVAL_BOOL(&_2, 1);
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&result))) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1309, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1309, &__$false);
		}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1308, &result);
	ZEPHIR_MM_RESTORE();
}
