...
PHP_METHOD(Phalcon_Storage_Serializer_Base64, unserialize)
{
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&result))) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1267, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1267, &__$false);
		}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 1266, &result);
	ZEPHIR_MM_RESTORE();
}
