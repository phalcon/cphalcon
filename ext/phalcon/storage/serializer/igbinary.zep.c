...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, unserialize)
{
	...
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		...
		ZVAL_LONG(&_2$$4, 2);
		ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 238, &_1$$4, &_2$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, doUnserialize)
{
	...
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 1, 0, &value);
	ZEPHIR_RETURN_CALL_FUNCTION("igbinary_unserialize", NULL, 244, value);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, phpIgbinarySerialize)
{
	...
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 1, 0, &value);
	ZEPHIR_RETURN_CALL_FUNCTION("igbinary_serialize", NULL, 245, value);
	zephir_check_call_status();
	RETURN_MM();
}
