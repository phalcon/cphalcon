...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, serialize)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 333, PH_NOISY_CC | PH_READONLY);
	...
	if (UNEXPECTED(Z_TYPE_P(&result) == IS_NULL)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 334, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 334, &__$false);
		}
...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, unserialize)
{
	...
	zephir_check_call_status();
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 333, data);
	} else {
		...
		ZVAL_LONG(&_2$$4, 2);
		ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 299, &_1$$4, &_2$$4);
		zephir_check_call_status();
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 333, &result);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, phpIgbinarySerialize)
{
	...
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 1, 0, &value);
	ZEPHIR_RETURN_CALL_FUNCTION("igbinary_serialize", NULL, 305, value);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, phpIgbinaryUnserialize)
{
	...
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 1, 0, &value);
	ZEPHIR_RETURN_CALL_FUNCTION("igbinary_unserialize", NULL, 306, value);
	zephir_check_call_status();
	RETURN_MM();
}
