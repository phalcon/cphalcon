...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, serialize)
{
	...
	if (UNEXPECTED(Z_TYPE_P(&result) == IS_NULL)) {
		if (0) {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 329, &__$true);
		} else {
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 329, &__$false);
		}
...
PHP_METHOD(Phalcon_Storage_Serializer_Igbinary, unserialize)
{
	...
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 328, &result);
	}
	ZEPHIR_MM_RESTORE();
}
