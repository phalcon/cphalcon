...
PHP_METHOD(Phalcon_Storage_Serializer_Json, serialize)
{
	...
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_1, 1322, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_0, 1324, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Storage_Serializer_Json, unserialize)
{
	...
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		...
		zephir_read_property_cached(&_1$$4, this_ptr, _zephir_prop_1, 1323, PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_METHOD(&_2$$4, &_1$$4, "__invoke", NULL, 0, data);
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 1324, &_2$$4);
	}
	ZEPHIR_MM_RESTORE();
}
