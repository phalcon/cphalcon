...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$23, "optionsfromresultset", &_9, 508, &options, &using, &value, &_11$$23);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$25, "optionsfromarray", &_13, 509, &options, &value, &_15$$25);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _2, _3, _0)
		{
			...
			if (Z_TYPE_P(&optionText) == IS_ARRAY) {
				...
				ZEPHIR_CALL_SELF(&_6$$4, "optionsfromarray", &_7, 509, &optionText, value, &closeOption);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&optionText) == IS_ARRAY) {
					...
					ZEPHIR_CALL_SELF(&_17$$12, "optionsfromarray", &_7, 509, &optionText, value, &closeOption);
					zephir_check_call_status();
