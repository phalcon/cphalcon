...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$24, "_optionsfromresultset", &_9, 459, &options, &using, &value, &_11$$24);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$26, "_optionsfromarray", &_13, 460, &options, &value, &_15$$26);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Tag_Select, _optionsFromArray) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _1, _2, _0)
	{
		...
		if (Z_TYPE_P(&optionText) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_5$$4, "_optionsfromarray", &_6, 460, &optionText, value, &closeOption);
			zephir_check_call_status();
