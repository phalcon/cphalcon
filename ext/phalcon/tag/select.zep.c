...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		if (UNEXPECTED(!(zephir_array_isset_string_fetch(&using, &params, SL("using"), 0)))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_tag_exception_ce, "The 'using' parameter is required", "phalcon/Tag/Select.zep", 95);
			return;
		}
		...
		if (UNEXPECTED(_2$$19)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_tag_exception_ce, "The 'using' parameter should be an array", "phalcon/Tag/Select.zep", 101);
			return;
		}
	}
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		zephir_check_call_status();
		zephir_concat_self(&code, &_8$$23 TSRMLS_CC);
	} else {
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			zephir_check_call_status();
			zephir_concat_self(&code, &_12$$25 TSRMLS_CC);
		}
	}
...
PHP_METHOD(Phalcon_Tag_Select, optionsFromArray) {
	...
	ZEPHIR_INIT_VAR(&code);
	ZVAL_STRING(&code, "");
	zephir_is_iterable(&data, 0, "phalcon/Tag/Select.zep", 180);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _1, _2, _0)
	{
		...
		ZEPHIR_CALL_FUNCTION(&escaped, "htmlspecialchars", &_3, 198, &optionValue);
		zephir_check_call_status();
		if (Z_TYPE_P(&optionText) == IS_ARRAY) {
			ZEPHIR_INIT_NVAR(&_4$$4);
			ZEPHIR_GET_CONSTANT(&_4$$4, "PHP_EOL");
			ZEPHIR_CALL_SELF(&_5$$4, "optionsfromarray", &_6, 0, &optionText, value, &closeOption);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Tag_Select, optionsFromResultset) {
	...
	if (Z_TYPE_P(using) == IS_ARRAY) {
		if (UNEXPECTED(zephir_fast_count_int(using TSRMLS_CC) != 2)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_tag_exception_ce, "Parameter 'using' requires two values", "phalcon/Tag/Select.zep", 203);
			return;
		}
		ZEPHIR_OBS_VAR(&usingZero);
		zephir_array_fetch_long(&usingZero, using, 0, PH_NOISY, "phalcon/Tag/Select.zep", 206 TSRMLS_CC);
		ZEPHIR_OBS_VAR(&usingOne);
		zephir_array_fetch_long(&usingOne, using, 1, PH_NOISY, "phalcon/Tag/Select.zep", 207 TSRMLS_CC);
	}
		...
		if (Z_TYPE_P(using) == IS_ARRAY) {
			if (Z_TYPE_P(&option) == IS_OBJECT) {
				...
				if (UNEXPECTED(Z_TYPE_P(&option) != IS_ARRAY)) {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_tag_exception_ce, "Resultset returned an invalid value", "phalcon/Tag/Select.zep", 226);
					return;
				}
				ZEPHIR_OBS_NVAR(&optionValue);
				zephir_array_fetch(&optionValue, &option, &usingZero, PH_NOISY, "phalcon/Tag/Select.zep", 229 TSRMLS_CC);
				ZEPHIR_OBS_NVAR(&optionText);
				zephir_array_fetch(&optionText, &option, &usingOne, PH_NOISY, "phalcon/Tag/Select.zep", 230 TSRMLS_CC);
			}
			...
			if (Z_TYPE_P(using) == IS_OBJECT) {
				...
				zephir_check_call_status();
				zephir_concat_self(&code, &_12$$19 TSRMLS_CC);
			}
		}
	}
