...
PHP_METHOD(Phalcon_Http_Request, getBasicAuth)
{
	...
	if (!(_2)) {
		...
		ZEPHIR_CALL_METHOD(&_3, this_ptr, "hasserver", NULL, 339, &_1);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request, getPatchPut)
{
	...
	if (Z_TYPE_P(&cached) == IS_NULL) {
		...
		if (_0$$3) {
			...
			ZEPHIR_CALL_FUNCTION(NULL, "parse_str", NULL, 353, &_4$$6, &cached);
			ZEPHIR_UNREF(&cached);
			zephir_check_call_status();
		}
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "gethelper", NULL, 337, &cached, &name, filters, defaultValue, &_5, &_6);
	zephir_check_call_status();
	RETURN_MM();
}
