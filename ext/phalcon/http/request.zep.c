...
PHP_METHOD(Phalcon_Http_Request, getAttributes)
{
	...
	if (Z_TYPE_P(&_0) == IS_NULL) {
		...
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 863, &_1$$3);
	}
...
PHP_METHOD(Phalcon_Http_Request, getClientAddress)
{
	...
	if (trustForwardedHeader) {
		zephir_read_property_cached(&_0$$4, this_ptr, _zephir_prop_0, 864, PH_NOISY_CC | PH_READONLY);
		if (!ZEPHIR_IS_STRING_IDENTICAL(&_0$$4, "")) {
			zephir_memory_observe(&trustedProxyHeaderIp);
			zephir_read_property_cached(&_1$$5, this_ptr, _zephir_prop_0, 864, PH_NOISY_CC | PH_READONLY);
		...
		zephir_read_property_cached(&_2$$4, this_ptr, _zephir_prop_1, 865, PH_NOISY_CC);
		...
		if (!(ZEPHIR_IS_EMPTY(&forwarded))) {
			...
			if (Z_TYPE_P(&reverseForwardedIps) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&reverseForwardedIps), _7$$8)
				{
					...
					zephir_read_property_cached(&_8$$9, this_ptr, _zephir_prop_1, 865, PH_NOISY_CC);
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						zephir_read_property_cached(&_14$$12, this_ptr, _zephir_prop_1, 865, PH_NOISY_CC);
	...
	if (Z_TYPE_P(&server) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&server), _1, _2, _0)
		{
			...
			if (zephir_start_with_str(&name, SL("HTTP_"))) {
				...
				ZEPHIR_CALL_FUNCTION(&name, "ucwords", &_9, 332, &_3$$4);
				zephir_check_call_status();
			...
			if (zephir_array_isset_value(&contentHeaders, &name)) {
				...
				ZEPHIR_CALL_FUNCTION(&name, "ucwords", &_9, 332, &_14$$5);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				if (zephir_start_with_str(&name, SL("HTTP_"))) {
					...
					ZEPHIR_CALL_FUNCTION(&name, "ucwords", &_9, 332, &_23$$7);
					zephir_check_call_status();
				...
				if (zephir_array_isset_value(&contentHeaders, &name)) {
					...
					ZEPHIR_CALL_FUNCTION(&name, "ucwords", &_9, 332, &_33$$8);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request, getHttpHost)
{
	...
	zephir_read_property_cached(&strict, this_ptr, _zephir_prop_0, 867, PH_NOISY_CC);
	ZEPHIR_INIT_VAR(&_0);
...
PHP_METHOD(Phalcon_Http_Request, getJsonRawBody)
{
	...
	ZEPHIR_RETURN_CALL_METHOD(&_0, "__invoke", NULL, 383, &rawBody, &_1);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Http_Request, getMethod)
{
	...
	if (ZEPHIR_IS_IDENTICAL(&_0, &returnMethod)) {
		...
		if (!(ZEPHIR_IS_EMPTY(&overridedMethod))) {
			...
			zephir_read_property_cached(&_2$$5, this_ptr, _zephir_prop_0, 868, PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Http_Request, getPatch)
{
	...
	zephir_read_property_cached(&_1, this_ptr, _zephir_prop_0, 869, PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "getpostdata", NULL, 0, &_1);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 869, &_0);
	zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 869, PH_NOISY_CC | PH_READONLY);
	if (notAllowEmpty) {
		ZVAL_BOOL(&_3, 1);
	} else {
		ZVAL_BOOL(&_3, 0);
...
PHP_METHOD(Phalcon_Http_Request, getRawBody)
{
	...
	if (ZEPHIR_IS_EMPTY(&rawBody)) {
		...
		zephir_check_call_status();
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 870, &contents);
...
PHP_METHOD(Phalcon_Http_Request, setHttpMethodParameterOverride)
{
	...
	if (override) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 868, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 868, &__$false);
	}
	RETURN_THISW();
}
...
PHP_METHOD(Phalcon_Http_Request, setTrustedProxyHeader)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 864, &trustedHeader);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Http_Request, isIpAddressInCIDR)
{
	...
	ZEPHIR_CALL_FUNCTION(&_9, "strncmp", NULL, 335, &ipBits, &subnetBits, &_8);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request, getFilterService)
{
	...
	if (Z_TYPE_P(&filterService) != IS_OBJECT) {
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 871, &filterService);
	}
