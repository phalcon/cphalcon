...
ZEPHIR_INIT_CLASS(Phalcon_Http_JWT_Signer_Hmac) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Phalcon\\Http\\JWT\\Signer, Hmac, phalcon, http_jwt_signer_hmac, phalcon_http_jwt_signer_abstractsigner_ce, phalcon_http_jwt_signer_hmac_method_entry, 0);
...
PHP_METHOD(Phalcon_Http_JWT_Signer_Hmac, __construct) {
	...
	if (!(zephir_array_isset(&supported, &algo))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_unsupportedalgorithmexception_ce, "Unsupported HMAC algorithm", "phalcon/Http/JWT/Signer/Hmac.zep", 40);
		return;
	}
...
PHP_METHOD(Phalcon_Http_JWT_Signer_Hmac, sign) {
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "gethash", NULL, 293, &payload, &passphrase);
...
PHP_METHOD(Phalcon_Http_JWT_Signer_Hmac, verify) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "gethash", NULL, 293, &payload, &passphrase);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_JWT_Signer_Hmac, getHash) {
	...
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("hash_hmac", NULL, 184, &_0, &payload, &passphrase, &__$true);
	zephir_check_call_status();
	RETURN_MM();
