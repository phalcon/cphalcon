...
ZEPHIR_INIT_CLASS(Phalcon_Http_JWT_Validator) {
	...
	ZEPHIR_REGISTER_CLASS(Phalcon\\Http\\JWT, Validator, phalcon, http_jwt_validator, phalcon_http_jwt_validator_method_entry, 0);

	/**
	 * @var int
	 */
	zend_declare_property_long(phalcon_http_jwt_validator_ce, SL("timeShift"), 0, ZEND_ACC_PRIVATE);

	/**
	 * @var Token
	 */
	zend_declare_property_null(phalcon_http_jwt_validator_ce, SL("token"), ZEND_ACC_PRIVATE);
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateAudience) {
	...
	if (!(zephir_fast_in_array(&audience, &_2))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: audience not allowed", "phalcon/Http/JWT/Validator.zep", 71);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateExpiration) {
	...
	if (_4) {
		...
		ZEPHIR_CALL_METHOD(&_5, this_ptr, "gettimestamp", NULL, 299, &_6);
		zephir_check_call_status();
	...
	if (_4) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: the token has expired", "phalcon/Http/JWT/Validator.zep", 91);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateId) {
	...
	if (!ZEPHIR_IS_IDENTICAL(&id, &_4)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: incorrect Id", "phalcon/Http/JWT/Validator.zep", 108);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateIssuedAt) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "gettimestamp", NULL, 299, &_1);
	zephir_check_call_status();
	...
	if (ZEPHIR_LE_LONG(&_0, zephir_get_intval(&_3))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: the token cannot be used yet (future)", "phalcon/Http/JWT/Validator.zep", 125);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateIssuer) {
	...
	if (!ZEPHIR_IS_IDENTICAL(&issuer, &_4)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: incorrect issuer", "phalcon/Http/JWT/Validator.zep", 142);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateNotBefore) {
	...
	zephir_check_call_status();
	if (ZEPHIR_LE_LONG(&_0, zephir_get_intval(&_3))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: the token cannot be used yet (not before)", "phalcon/Http/JWT/Validator.zep", 159);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_JWT_Validator, validateSignature) {
	...
	if (!zephir_is_true(&_0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_jwt_exceptions_validatorexception_ce, "Validation: the signature does not match", "phalcon/Http/JWT/Validator.zep", 183);
		return;
	}
	RETURN_THIS();
