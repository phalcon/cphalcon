...
PHP_METHOD(Phalcon_Http_Request_File, __construct)
{
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 871, &name);
		...
		if (zephir_is_true(&_1$$3)) {
			...
			zephir_check_call_status();
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 872, &_3$$4);
		}
	}
	ZEPHIR_INIT_VAR(&_5);
	...
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 874, &_6);
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "type");
	ZEPHIR_CALL_METHOD(&_7, this_ptr, "getarrval", NULL, 0, &file, &_5);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_4, 875, &_7);
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "error");
	ZEPHIR_CALL_METHOD(&_8, this_ptr, "getarrval", NULL, 0, &file, &_5);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_5, 876, &_8);
	if (!(ZEPHIR_IS_EMPTY(&key_zv))) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_6, 877, &key_zv);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 878, PH_NOISY_CC);
	if (ZEPHIR_IS_EMPTY(&_0)) {
		ZVAL_LONG(&_1$$3, 16);
		ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 0, &_1$$3);
		zephir_check_call_status();
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&finfo)) {
			zephir_read_property_cached(&_2$$4, this_ptr, _zephir_prop_1, 873, PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 0, &finfo, &_2$$4);
			zephir_check_call_status();
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 878, &mime);
		}
	}
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile)
{
	...
	zephir_memory_observe(&name);
	zephir_read_property_cached(&name, this_ptr, _zephir_prop_0, 873, PH_NOISY_CC);
