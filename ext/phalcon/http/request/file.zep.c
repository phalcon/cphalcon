...
PHP_METHOD(Phalcon_Http_Request_File, __construct)
{
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 908, &name);
		...
		if (zephir_is_true(&_1$$3)) {
			...
			zephir_check_call_status();
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 909, &_3$$4);
		}
	}
	ZEPHIR_INIT_VAR(&_5);
	...
	if (!(ZEPHIR_IS_EMPTY(&key_zv))) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_6, 914, &key_zv);
	}
	ZEPHIR_MM_RESTORE();
}
	...
	if (ZEPHIR_IS_EMPTY(&_0)) {
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&finfo)) {
			...
			if (!ZEPHIR_IS_FALSE_IDENTICAL(&mime)) {
				zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 915, &mime);
			}
		}
	}
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile)
{
	...
	zephir_memory_observe(&name);
	zephir_read_property_cached(&name, this_ptr, _zephir_prop_0, 910, PH_NOISY_CC);
