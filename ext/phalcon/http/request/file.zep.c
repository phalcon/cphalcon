...
PHP_METHOD(Phalcon_Http_Request_File, __construct)
{
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 891, &name);
	...
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 893, &_4);
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "size");
	ZEPHIR_CALL_METHOD(&_6, this_ptr, "getarrval", NULL, 0, &file, &_5);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 894, &_6);
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "type");
	ZEPHIR_CALL_METHOD(&_7, this_ptr, "getarrval", NULL, 0, &file, &_5);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_4, 895, &_7);
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "error");
	ZEPHIR_CALL_METHOD(&_8, this_ptr, "getarrval", NULL, 0, &file, &_5);
	zephir_check_call_status();
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_5, 896, &_8);
	if (!(ZEPHIR_IS_EMPTY(&key_zv))) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_6, 897, &key_zv);
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType)
{
	...
	if (ZEPHIR_IS_EMPTY(&_0)) {
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&finfo)) {
			zephir_read_property_cached(&_2$$4, this_ptr, _zephir_prop_1, 893, PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 0, &finfo, &_2$$4);
			zephir_check_call_status();
			if (!ZEPHIR_IS_FALSE_IDENTICAL(&mime)) {
				zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 898, &mime);
			}
		}
	}
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile)
{
	...
	zephir_memory_observe(&name);
	zephir_read_property_cached(&name, this_ptr, _zephir_prop_0, 893, PH_NOISY_CC);
