...
PHP_METHOD(Phalcon_Http_Request_File, __construct)
{
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "defined", NULL, 138, &_0$$3);
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType)
{
	...
	ZVAL_LONG(&_0, 16);
	ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 400, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(&finfo) != IS_RESOURCE) {
		RETURN_MM_STRING("");
	}
	zephir_read_property(&_0, this_ptr, ZEND_STRL("tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 401, &finfo, &_0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 402, &finfo);
	zephir_check_call_status();
	RETURN_CCTOR(&mime);
}
...
PHP_METHOD(Phalcon_Http_Request_File, moveTo)
{
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 373, &_0, &destination);
	zephir_check_call_status();
	RETURN_MM();
}
