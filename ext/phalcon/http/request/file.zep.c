...
PHP_METHOD(Phalcon_Http_Request_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "defined", NULL, 116, &_0$$3);
		zephir_check_call_status();
		if (zephir_is_true(&_1$$3)) {
			ZVAL_LONG(&_2$$4, 4);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "pathinfo", NULL, 101, &name, &_2$$4);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType) {
	...
	ZVAL_LONG(&_0, 16);
	ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 350, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(&finfo) != IS_RESOURCE) {
		RETURN_MM_STRING("");
	}
	zephir_read_property(&_0, this_ptr, SL("tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 351, &finfo, &_0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 352, &finfo);
	zephir_check_call_status();
	RETURN_CCTOR(&mime);
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile) {
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_uploaded_file", NULL, 14, &tmp);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, moveTo) {
	...
	zephir_read_property(&_0, this_ptr, SL("tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 323, &_0, &destination);
	zephir_check_call_status();
	RETURN_MM();
