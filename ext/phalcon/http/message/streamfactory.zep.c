...
PHP_METHOD(Phalcon_Http_Message_StreamFactory, createStream) {
	...
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "r+");
	ZEPHIR_CALL_FUNCTION(&tempResource, "fopen", NULL, 225, &_0, &_1);
	zephir_check_call_status();
	zephir_fwrite(NULL, &tempResource, &content TSRMLS_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "rewind", NULL, 226, &tempResource);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Message_StreamFactory, createStreamFromResource) {
	...
	if (!(_0)) {
		ZEPHIR_CALL_FUNCTION(&_1, "get_resource_type", NULL, 227, phpResource);
		zephir_check_call_status();
		ZEPHIR_SINIT_VAR(_2);
