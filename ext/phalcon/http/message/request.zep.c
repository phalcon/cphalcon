...
PHP_METHOD(Phalcon_Http_Message_Request, getHeaderValue) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(values), _0)
	{
		...
		ZVAL_COPY(&value, _0);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkheadervalue", &_1, 273, &value);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Message_Request, processHeaders) {
	...
	if (Z_TYPE_P(headers) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(headers), _1$$3, _2$$3, _0$$3)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkheadername", &_3, 267, &name);
			zephir_check_call_status();
			zephir_get_strval(&_4$$4, &name);
			ZEPHIR_CPY_WRT(&name, &_4$$4);
			ZEPHIR_CALL_METHOD(&_5$$4, this_ptr, "getheadervalue", &_6, 268, &value);
			zephir_check_call_status();
		...
		if (_10$$3) {
			...
			ZEPHIR_CALL_METHOD(&host, this_ptr, "geturihost", NULL, 271, &_14$$5);
			zephir_check_call_status();
