	...
	if (Z_TYPE_P(&files) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&files), _0)
		{
			...
			if (Z_TYPE_P(&file) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkuploadedfiles", &_2, 291, &file);
				zephir_check_call_status();
			} else {
...
PHP_METHOD(Phalcon_Http_Message_ServerRequest, getHeaderValue) {
	...
	if (Z_TYPE_P(values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(values), _1)
		{
			...
			ZVAL_COPY(&value, _1);
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkheadervalue", &_3, 301, &value);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(headers) == IS_ARRAY) {
		...
		if (Z_TYPE_P(headers) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(headers), _2$$3, _3$$3, _0$$3)
			{
				...
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkheadername", &_4, 297, &name);
				zephir_check_call_status();
				zephir_get_strval(&_5$$4, &name);
				ZEPHIR_CPY_WRT(&name, &_5$$4);
				ZEPHIR_CALL_METHOD(&_6$$4, this_ptr, "getheadervalue", &_7, 298, &value);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(&value, &_6$$4);
				ZEPHIR_CALL_METHOD(NULL, &collection, "set", &_8, 275, &name, &value);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_10$$5, this_ptr, "getheadervalue", &_7, 298, &value);
					zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&_11$$3, &collection, "has", NULL, 276, &_12$$3);
		zephir_check_call_status();
		...
		if (_13$$3) {
			...
			ZEPHIR_CALL_METHOD(&host, this_ptr, "geturihost", NULL, 300, &_17$$6);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(NULL, &collection, "set", &_8, 275, &_19$$6, &_18$$6);
			zephir_check_call_status();
		}
	} else {
