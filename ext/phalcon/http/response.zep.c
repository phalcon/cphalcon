...
PHP_METHOD(Phalcon_Http_Response, setStatusCode) {
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(message, "")) {
		...
		zephir_array_fetch_long(&defaultMessage, _12$$6, code, PH_NOISY, "phalcon/http/response.zep", 207 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Http_Response, redirect) {
	...
	if (_10) {
		...
		zephir_array_fetch_long(&message, _11$$13, 302, PH_NOISY, "phalcon/http/response.zep", 481 TSRMLS_CC);
	} else {
