...
PHP_METHOD(Phalcon_Http_Response, redirect) {
	...
	if (externalRedirect) {
		...
		if (_0$$5) {
			...
			ZVAL_STRING(&_1$$5, "://");
			ZEPHIR_CALL_FUNCTION(&_2$$5, "strstr", NULL, 254, location, &_1$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, send) {
	...
	if (Z_TYPE_P(&content) != IS_NULL) {
		...
		if (_1$$5) {
			ZEPHIR_CALL_FUNCTION(NULL, "readfile", NULL, 255, &file);
			zephir_check_call_status();
		}
	}
	if (1) {
	...
	if (Z_TYPE_P(&currentHeadersRaw) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&currentHeadersRaw), _2$$3, _3$$3, _1$$3)
		{
			...
			if (_4$$4) {
				...
				ZEPHIR_CALL_FUNCTION(&_6$$4, "strstr", &_7, 254, &key, &_5$$4);
				zephir_check_call_status();
