...
PHP_METHOD(Phalcon_Http_Response, redirect) {
	...
	if (externalRedirect) {
		...
		if (_0$$5) {
			...
			ZVAL_STRING(&_1$$5, "://");
			ZEPHIR_CALL_FUNCTION(&_2$$5, "strstr", NULL, 267, location, &_1$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, send) {
	...
	if (UNEXPECTED(zephir_is_true(&_0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_response_exception_ce, "Response was already sent", "phalcon/Http/Response.zep", 324);
		return;
	}
...
PHP_METHOD(Phalcon_Http_Response, setFileToSend) {
	...
	if (zephir_is_true(attachment)) {
		if ((zephir_function_exists_ex(ZEND_STRL("mb_detect_encoding")) == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(&basePathEncoding, "mb_detect_encoding", NULL, 218, &basePath, &_1$$6, &__$true);
			zephir_check_call_status();
		}
		...
		if (!ZEPHIR_IS_STRING(&basePathEncoding, "ASCII")) {
			ZEPHIR_CALL_FUNCTION(&_3$$7, "rawurlencode", NULL, 221, &basePath);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(&_7$$8, "addcslashes", NULL, 187, &basePath, &_6$$8);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, setHeaders) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&data, 0, "phalcon/Http/Response.zep", 620);
...
PHP_METHOD(Phalcon_Http_Response, setStatusCode) {
	...
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_0);
	zephir_is_iterable(&currentHeadersRaw, 0, "phalcon/Http/Response.zep", 719);
	if (Z_TYPE_P(&currentHeadersRaw) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&currentHeadersRaw), _3, _4, _1)
		{
			...
			if (_5$$3) {
				...
				ZEPHIR_CALL_FUNCTION(&_7$$3, "strstr", &_8, 267, &key, &_6$$3);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_10$$5) {
					...
					ZEPHIR_CALL_FUNCTION(&_12$$5, "strstr", &_8, 267, &key, &_11$$5);
					zephir_check_call_status();
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&message, "")) {
		...
		if (UNEXPECTED(!(zephir_array_isset_long(&statusCodes, code)))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_http_response_exception_ce, "Non-standard statuscode given without a message", "phalcon/Http/Response.zep", 796);
			return;
		}
		ZEPHIR_OBS_VAR(&defaultMessage);
		zephir_array_fetch_long(&defaultMessage, &statusCodes, code, PH_NOISY, "phalcon/Http/Response.zep", 799);
