...
PHP_METHOD(Phalcon_Http_Response_Headers, remove)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 912, &headers);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Http_Response_Headers, send)
{
	...
	ZEPHIR_INIT_NVAR(&header);
	if (1) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 913, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 913, &__$false);
	}
	RETURN_MM_BOOL(1);
}
