...
PHP_METHOD(Phalcon_Http_Response_Headers, remove)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 893, &headers);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Http_Response_Headers, send)
{
	...
	if (!(_1)) {
		zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 894, PH_NOISY_CC | PH_READONLY);
		_1 = ZEPHIR_IS_TRUE_IDENTICAL(&_2);
	}
	if (_1) {
		RETURN_MM_BOOL(0);
	}
	zephir_read_property_cached(&_3, this_ptr, _zephir_prop_1, 893, PH_NOISY_CC | PH_READONLY);
