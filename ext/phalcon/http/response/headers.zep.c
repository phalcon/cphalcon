...
PHP_METHOD(Phalcon_Http_Response_Headers, remove)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 930, &headers);
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Http_Response_Headers, reset)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	array_init(&_0);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 930, &_0);
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Phalcon_Http_Response_Headers, send)
{
	...
	if (!(_1)) {
		zephir_read_property_cached(&_2, this_ptr, _zephir_prop_0, 931, PH_NOISY_CC | PH_READONLY);
	...
	ZEPHIR_INIT_NVAR(&header);
	if (1) {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 931, &__$true);
	} else {
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 931, &__$false);
	}
	RETURN_MM_BOOL(1);
}
