...
PHP_METHOD(Phalcon_Tag, friendlyTitle) {
	...
	ZEPHIR_INIT_NVAR(&_0);
	ZVAL_STRING(&_0, "iconv");
	ZEPHIR_CALL_FUNCTION(&_14, "extension_loaded", NULL, 141, &_0);
	zephir_check_call_status();
	if (zephir_is_true(&_14)) {
		ZVAL_LONG(&_15$$10, 6);
		ZEPHIR_CALL_FUNCTION(NULL, "setlocale", NULL, 250, &_15$$10, &locale);
		zephir_check_call_status();
	}
...
PHP_METHOD(Phalcon_Tag, getTitle) {
	...
	if (prepend) {
		...
		if (!(ZEPHIR_IS_EMPTY(&documentPrependTitle))) {
			ZEPHIR_CALL_FUNCTION(&tmp$$5, "array_reverse", NULL, 253, &documentPrependTitle);
			zephir_check_call_status();
