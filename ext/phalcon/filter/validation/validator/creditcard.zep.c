...
PHP_METHOD(Phalcon_Filter_Validation_Validator_CreditCard, verifyByLuhnAlgorithm)
{
	...
	ZEPHIR_CALL_FUNCTION(&_1, "str_split", NULL, 206, &number_zv);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&digits, "array_reverse", NULL, 264, &_1);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&digit);
	ZEPHIR_INIT_NVAR(&position);
	ZEPHIR_CALL_FUNCTION(&_9, "str_split", NULL, 206, &hash);
	zephir_check_call_status();
