...
PHP_METHOD(Phalcon_Filter_Validation_Validator_Callback, validate)
{
	...
	if (zephir_is_callable(&callback)) {
		...
		zephir_read_property_cached(&savedTemplate, this_ptr, _zephir_prop_0, 754, PH_NOISY_CC);
		...
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_1, 755, &savedChanged);
		zephir_update_property_zval_cached(this_ptr, _zephir_prop_2, 756, &savedTemplates);
