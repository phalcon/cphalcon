...
PHP_METHOD(Phalcon_Filter_Validation_Validator_File_MimeType, validate)
{
	...
	zval *validation, validation_sub, *field, field_sub, fieldTypes, mime, replacePairs, tmp, types, value, _0, _1, _2$$5, _3$$6, _4$$7, _5$$9, _6$$9;
	...
	if (UNEXPECTED(Z_TYPE_P(&types) != IS_ARRAY)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$5, "phalcon/Filter/Validation/Validator/File/MimeType.zep", 95);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if ((zephir_function_exists_ex(ZEND_STRL("finfo_open")) == SUCCESS)) {
		...
		if (zephir_is_true(&tmp)) {
			zephir_array_fetch_string(&_4$$7, &value, SL("tmp_name"), PH_NOISY | PH_READONLY, "phalcon/Filter/Validation/Validator/File/MimeType.zep", 101);
	...
	if (!zephir_is_true(&mime)) {
		...
		zephir_array_fetch_string(&mime, &value, SL("type"), PH_NOISY, "phalcon/Filter/Validation/Validator/File/MimeType.zep", 107);
	}
	if (!(zephir_fast_in_array(&mime, &types))) {
		ZEPHIR_INIT_VAR(&replacePairs);
		zephir_create_array(&replacePairs, 1, 0);
		ZEPHIR_INIT_VAR(&_5$$9);
		zephir_fast_join_str(&_5$$9, SL(", "), &types);
		zephir_array_update_string(&replacePairs, SL(":types"), &_5$$9, PH_COPY | PH_SEPARATE);
