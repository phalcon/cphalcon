...
PHP_METHOD(Phalcon_Filter_FilterLocator, sanitize) {
	...
	if (Z_TYPE_P(sanitizers) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(sanitizers), _1$$3, _2$$3, _0$$3)
		{
			...
			if (_3$$5) {
				ZEPHIR_CALL_METHOD(&_4$$8, this_ptr, "processarrayvalues", &_5, 200, value, &sanitizerName, &sanitizerParams);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(value, &_4$$8);
			} else {
				ZEPHIR_CALL_METHOD(&_6$$9, this_ptr, "sanitizer", &_7, 201, value, &sanitizerName, &sanitizerParams);
				zephir_check_call_status();
	...
	if (_8) {
		ZEPHIR_RETURN_CALL_METHOD(this_ptr, "processarrayvalues", &_5, 200, value, sanitizers);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "sanitizer", &_7, 201, value, sanitizers);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Filter_FilterLocator, processArrayValues) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&values), _1, _2, _0)
	{
		...
		ZEPHIR_CALL_METHOD(&_3$$3, this_ptr, "sanitizer", &_4, 201, &itemValue, &sanitizerName, &sanitizerParams);
		zephir_check_call_status();
