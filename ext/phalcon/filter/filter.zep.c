	...
	if (Z_TYPE_P(&sanitizers) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&sanitizers), _2, _3, _0)
		{
			...
			if (Z_TYPE_P(value) == IS_ARRAY) {
				...
				ZEPHIR_CALL_METHOD(&_5$$5, this_ptr, "processvalueisarray", &_7, 275, value, &sanitizerName, &sanitizerParams, &_6$$5);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(value, &_5$$5);
			} else {
				ZEPHIR_CALL_METHOD(&_8$$6, this_ptr, "processvalueisnotarray", &_9, 276, value, &sanitizerName, &sanitizerParams);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(value) == IS_ARRAY) {
					...
					ZEPHIR_CALL_METHOD(&_10$$8, this_ptr, "processvalueisarray", &_7, 275, value, &sanitizerName, &sanitizerParams, &_11$$8);
					zephir_check_call_status();
					ZEPHIR_CPY_WRT(value, &_10$$8);
				} else {
					ZEPHIR_CALL_METHOD(&_12$$9, this_ptr, "processvalueisnotarray", &_9, 276, value, &sanitizerName, &sanitizerParams);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Filter_Filter, processValueIsArray)
{
	...
	if (noRecursive) {
		...
		ZEPHIR_CALL_METHOD(&_1$$4, this_ptr, "processarrayvalues", NULL, 272, value, &sanitizerName, &sanitizerParams);
		zephir_check_call_status();
