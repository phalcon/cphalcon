...
PHP_METHOD(Phalcon_Filter_Filter, processArraySanitizers)
{
	...
	if (Z_TYPE_P(&sanitizers) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&sanitizers), _2, _3, _0)
		{
			...
			ZEPHIR_CALL_METHOD(&_5$$4, this_ptr, "processvalueisarray", &_7, 279, value, &sanitizerName, &sanitizerParams, &_6$$4);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(value, &_5$$4);
			ZEPHIR_CALL_METHOD(&_5$$4, this_ptr, "processvalueisnotarray", &_8, 280, value, &sanitizerName, &sanitizerParams);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(value, &_5$$4);
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(&_9$$5, this_ptr, "processvalueisarray", &_7, 279, value, &sanitizerName, &sanitizerParams, &_10$$5);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(value, &_9$$5);
				ZEPHIR_CALL_METHOD(&_9$$5, this_ptr, "processvalueisnotarray", &_8, 280, value, &sanitizerName, &sanitizerParams);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(value, &_9$$5);
...
PHP_METHOD(Phalcon_Filter_Filter, processValueIsArray)
{
	...
	if (_0) {
		ZEPHIR_CALL_METHOD(&_1$$3, this_ptr, "processarrayvalues", NULL, 276, value, &sanitizerName, &sanitizerParams);
		zephir_check_call_status();
