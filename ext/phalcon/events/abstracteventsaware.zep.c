...
PHP_METHOD(Phalcon_Events_AbstractEventsAware, setEventsManager)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 56, eventsManager);
}
...
PHP_METHOD(Phalcon_Events_AbstractEventsAware, fireManagerEvent)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 56, PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&_0) != IS_NULL) {
		zephir_read_property_cached(&_1$$3, this_ptr, _zephir_prop_0, 56, PH_NOISY_CC | PH_READONLY);
		if (cancellable) {
			ZVAL_BOOL(&_2$$3, 1);
		} else {
			ZVAL_BOOL(&_2$$3, 0);
