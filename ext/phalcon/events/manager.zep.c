...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, _0, eventType, 0 TSRMLS_CC))) {
		...
		if (zephir_is_true(_1$$4)) {
			...
			ZVAL_LONG(_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, priorityQueue, "setextractflags", NULL, 191, _2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, priorityQueue, "insert", NULL, 192, handler, _3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, _0, eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, newPriorityQueue, "setextractflags", NULL, 191, _1$$5);
			zephir_check_call_status();
			...
			while (1) {
				...
				if (!ZEPHIR_IS_IDENTICAL(_5$$6, handler)) {
					...
					ZEPHIR_CALL_METHOD(NULL, newPriorityQueue, "insert", &_8, 192, _6$$7, _7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), eventType, newPriorityQueue TSRMLS_CC);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 193, handler, priorityQueue, ZEPHIR_GLOBAL(global_true));
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fireQueue) {
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue TSRMLS_CC))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 194, &_2$$5, _1$$5);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(queue) == IS_OBJECT) {
		...
		zephir_is_iterable(queue, &_12$$21, &_11$$21, 0, 0, "phalcon/events/manager.zep", 356);
		for (
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	if (!(zephir_memnstr_str(eventType, SL(":"), "phalcon/events/manager.zep", 384))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_0$$4, "phalcon/events/manager.zep", 385 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	ZEPHIR_OBS_VAR(type);
	zephir_array_fetch_long(&type, eventParts, 0, PH_NOISY, "phalcon/events/manager.zep", 389 TSRMLS_CC);
	ZEPHIR_OBS_VAR(eventName);
	zephir_array_fetch_long(&eventName, eventParts, 1, PH_NOISY, "phalcon/events/manager.zep", 390 TSRMLS_CC);
	ZEPHIR_INIT_VAR(status);
	ZVAL_NULL(status);
	...
	if (zephir_array_isset_fetch(&fireEvents, events, type, 0 TSRMLS_CC)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, event, "__construct", &_5, 195, eventName, source, data, _4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_6, 196, fireEvents, event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, events, eventType, 0 TSRMLS_CC)) {
		...
		if (_7$$8) {
			if (Z_TYPE_P(event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, event, "__construct", &_5, 195, eventName, source, data, _8$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_6, 196, fireEvents, event);
			zephir_check_call_status();
