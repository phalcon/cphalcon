...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
			...
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 193, handler, &priorityQueue, &__$true);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0 TSRMLS_CC)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 194, &eventName, source, data, &_4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 195, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &eventType, 0 TSRMLS_CC)) {
		...
		if (_5$$8) {
			if (Z_TYPE_P(&event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 194, &eventName, source, data, &_6$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 195, &fireEvents, &event);
			zephir_check_call_status();
