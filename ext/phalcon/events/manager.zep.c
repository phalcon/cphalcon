...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC))) {
		zephir_read_property(&_1$$4, this_ptr, SL("enablePriorities"), PH_NOISY_CC | PH_READONLY);
		...
		if (zephir_is_true(&_1$$4)) {
			...
			ZVAL_LONG(&_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "setextractflags", NULL, 177, &_2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "insert", NULL, 178, handler, &_3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (Z_TYPE_P(handler) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_events_exception_ce, "Event handler must be an Object", "phalcon/Events/Manager.zep", 112);
		return;
	}
	ZEPHIR_OBS_VAR(&priorityQueue);
	zephir_read_property(&_0, this_ptr, SL("events"), PH_NOISY_CC | PH_READONLY);
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(&_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "setextractflags", NULL, 177, &_1$$5);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$5, 3);
			...
			while (1) {
				...
				zephir_check_call_status();
				zephir_array_fetch_string(&_5$$6, &data, SL("data"), PH_NOISY | PH_READONLY, "phalcon/Events/Manager.zep", 129 TSRMLS_CC);
				if (!ZEPHIR_IS_IDENTICAL(&_5$$6, handler)) {
					zephir_array_fetch_string(&_6$$7, &data, SL("data"), PH_NOISY | PH_READONLY, "phalcon/Events/Manager.zep", 130 TSRMLS_CC);
					zephir_array_fetch_string(&_7$$7, &data, SL("priority"), PH_NOISY | PH_READONLY, "phalcon/Events/Manager.zep", 130 TSRMLS_CC);
					ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "insert", &_8, 178, &_6$$7, &_7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("events"), &eventType, &newPriorityQueue TSRMLS_CC);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 179, handler, &priorityQueue, &__$true);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	zval *eventType_param = NULL, *source, source_sub, *data = NULL, data_sub, *cancelable_param = NULL, __$null, events, eventParts, type, eventName, event, status, fireEvents, _0, _1$$4, _4$$7, _6$$10;
	zval eventType, _2$$4;
	zval *this_ptr = getThis();
	...
	if (!(zephir_memnstr_str(&eventType, SL(":"), "phalcon/Events/Manager.zep", 188))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$4, "phalcon/Events/Manager.zep", 189 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	ZEPHIR_OBS_VAR(&type);
	zephir_array_fetch_long(&type, &eventParts, 0, PH_NOISY, "phalcon/Events/Manager.zep", 193 TSRMLS_CC);
	ZEPHIR_OBS_VAR(&eventName);
	zephir_array_fetch_long(&eventName, &eventParts, 1, PH_NOISY, "phalcon/Events/Manager.zep", 194 TSRMLS_CC);
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0 TSRMLS_CC)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 180, &eventName, source, data, &_4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 181, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &eventType, 0 TSRMLS_CC)) {
		...
		if (_5$$8) {
			if (Z_TYPE_P(&event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 180, &eventName, source, data, &_6$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 181, &fireEvents, &event);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fireQueue) {
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue TSRMLS_CC))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 133, &_2$$5, &_1$$5);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, &_0$$5, "__construct", NULL, 1, &_3$$5);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_0$$5, "phalcon/Events/Manager.zep", 255 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} else {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_events_exception_ce, "The queue is not valid", "phalcon/Events/Manager.zep", 258);
			return;
		}
	}
	ZEPHIR_INIT_VAR(&status);
	...
	if (Z_TYPE_P(&eventName) != IS_STRING) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_events_exception_ce, "The event type not valid", "phalcon/Events/Manager.zep", 267);
		return;
	}
	...
	ZEPHIR_OBS_VAR(&_5);
	zephir_read_property(&_5, this_ptr, SL("collect"), PH_NOISY_CC);
	...
	if (Z_TYPE_P(queue) == IS_OBJECT) {
		...
		zephir_is_iterable(queue, 0, "phalcon/Events/Manager.zep", 403);
...
PHP_METHOD(Phalcon_Events_Manager, getListeners) {
	...
	zval *type_param = NULL, events, fireEvents, _0;
	zval type;
	zval *this_ptr = getThis();
