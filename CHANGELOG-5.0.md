...
- Added `Phalcon\Traits\Support\Helper\Arr\GetTrait`, exposing the protected `static getArrVal()` helper (returns an array element by key, falling back to a default value and optionally casting the result via `settype()`), and refactored the classes that each carried their own private/protected copy of this method - `Phalcon\Http\Cookie`, `Phalcon\Http\Request\File`, `Phalcon\Image\ImageFactory`, `Phalcon\Logger\LoggerFactory`, `Phalcon\Mvc\Model\MetaData`, `Phalcon\Session\Adapter\AbstractAdapter`, `Phalcon\Session\Adapter\Stream`, `Phalcon\Session\Manager` and `Phalcon\Storage\Adapter\AbstractAdapter` - to `use` the shared trait, removing the duplicated `getArrVal()` definitions. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\FileTrait`, exposing protected static, overridable wrappers around PHP's filesystem functions - `phpFclose()`, `phpFgetCsv()`, `phpFileExists()`, `phpFileGetContents()`, `phpFilePutContents()`, `phpFopen()`, `phpFwrite()` (now with an optional `$length`), `phpIsWritable()` and `phpUnlink()` - so filesystem access can be substituted in tests, and routed the framework's direct file-function calls through the shared trait. The duplicated wrapper definitions previously carried by `Phalcon\Assets\Asset`, `Phalcon\Auth\Adapter\Stream`, `Phalcon\Forms\Loader\JsonLoader`, `Phalcon\Session\Adapter\Stream`, `Phalcon\Storage\Adapter\Stream` and `Phalcon\Translate\Adapter\Csv` were removed in favor of it. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\IniTrait`, exposing protected static, overridable wrappers around PHP's ini functions - `phpIniGet()`, `phpIniGetBool()` and `phpIniGetInt()` (each returning `ini_get()` coerced to the requested type, or a default when the option is unset) and `phpParseIniFile()`. `Phalcon\Session\Adapter\Stream` and `Phalcon\Config\Adapter\Ini` were switched to the trait, removing their local `phpIniGet()`/`phpParseIniFile()` wrappers. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\InfoTrait`, exposing protected static, overridable wrappers around PHP's runtime-inspection functions - `phpExtensionLoaded()` (`extension_loaded()`) and `phpFunctionExists()` (`function_exists()`) - so extension/function availability checks can be substituted in tests, and routed the framework's direct calls through the shared trait. The duplicated wrapper definitions previously carried by `Phalcon\Config\Adapter\Yaml`, `Phalcon\Encryption\Crypt` and `Phalcon\Translate\Adapter\Gettext` were removed in favor of it. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\DirFromFileTrait`, exposing the protected `static toDirFromFile()` helper that turns a file name into a calculated nested directory path (e.g. for spreading cache files across sub-directories). An optional `$filesystemSafe` flag replaces `.` with `-` in the generated segments so a key can never produce a `..` path that escapes its base directory. `Phalcon\Support\Helper\Str\DirFromFile` (default behavior) and `Phalcon\Storage\Adapter\Stream` (with `$filesystemSafe` enabled) now `use` the trait, removing their local implementations. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\DirSeparatorTrait`, exposing the protected `static toDirSeparator()` helper that trims any trailing separators from a directory string and appends exactly one `DIRECTORY_SEPARATOR`. `Phalcon\Support\Helper\Str\DirSeparator`, `Phalcon\Session\Adapter\Stream`, `Phalcon\Storage\Adapter\Stream`, `Phalcon\Mvc\View` and `Phalcon\Mvc\View\Simple` now `use` the trait, removing the four duplicated private `getDirSeparator()` methods (and the helper's inline copy) in favor of it. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\EndsWithTrait`, exposing the protected `static toEndsWith()` helper that reports whether a string ends with a given substring - case-insensitive by default and multibyte-aware (it lowercases both sides via `mb_strtolower()` before delegating to the native Zephir `ends_with()`), returning `false` for an empty haystack. `Phalcon\Support\Helper\Str\AbstractStr` (the base for the `Str\EndsWith` and `Str\Concat` helpers) now `use`s the trait, removing its inline `toEndsWith()` implementation. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\StartsWithTrait`, exposing the protected `static toStartsWith()` helper that reports whether a string starts with a given substring - case-insensitive by default and multibyte-aware (it lowercases both sides via `mb_strtolower()` before delegating to the native Zephir `starts_with()`), returning `false` for an empty haystack. `Phalcon\Support\Helper\Str\AbstractStr` (the base for the `Str\StartsWith` and `Str\Concat` helpers) now `use`s the trait, removing its inline `toStartsWith()` implementation. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\InterpolateTrait`, exposing the protected `static toInterpolate()` helper that substitutes `context` values into `%placeholder%`-style tokens in a string (PSR-3 message interpolation, with configurable left/right delimiters) via `strtr()`, and short-circuits to return the subject unchanged when it contains no opening delimiter. `Phalcon\Support\Helper\Str\AbstractStr` and the `Str\Interpolate` helper now `use` the trait, removing their inline copies, and `Phalcon\Logger\Formatter\AbstractFormatter` was switched from `extends Phalcon\Support\Helper\Str\AbstractStr` to `use InterpolateTrait`, so the logger no longer inherits the internal `Str` helper base just to reach this one method. `Phalcon\Translate\Interpolator\AssociativeArray` also now `use`s the trait directly instead of instantiating a `Str\Interpolate` helper per call. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\CamelizeTrait`, exposing the public `static toCamelize()` helper that converts a delimited string into `CamelCase` (or `camelCase` when `$lowerFirst` is `true`) by wrapping the native Zephir `camelize()` builtin and applying `lcfirst()`. The `Str\Camelize` helper now `use`s the trait in place of its inline implementation. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\UncamelizeTrait`, exposing the protected `static toUncamelize()` helper that converts a camel-cased string into a lower-cased, delimited one (e.g. `CamelCase` → `camel_case`) by wrapping the native Zephir `uncamelize()` builtin. The `Str\Uncamelize` helper now `use`s the trait in place of its inline `preg_replace()` implementation; output is unchanged for the usual identifier case, though (per the native builtin) the delimiter must be a single character and case-folding applies to ASCII letters only. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\LowerTrait`, exposing the protected `static toLower()` helper that lowercases a string in a multibyte-safe way (`mb_convert_case()` with `MB_CASE_LOWER`). `Phalcon\Support\Helper\Str\AbstractStr` now `use`s the trait, removing its inline `toLower()`; the `Str\Lower`, `Str\IsLower` and `Str\Decapitalize` helpers pick it up through the base. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Str\UpperTrait`, exposing the protected `static toUpper()` helper that uppercases a string in a multibyte-safe way (`mb_convert_case()` with `MB_CASE_UPPER`). `Phalcon\Support\Helper\Str\AbstractStr` now `use`s the trait, removing its inline `toUpper()`; the `Str\Upper`, `Str\IsUpper` and `Str\Decapitalize` helpers pick it up through the base, which is now composed entirely of `Str` traits. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\ApcuTrait`, exposing seven protected `static` wrappers around PHP's APCu functions - `phpApcuDec()`, `phpApcuDelete()`, `phpApcuExists()`, `phpApcuFetch()`, `phpApcuInc()`, `phpApcuIterator()` and `phpApcuStore()` - so APCu access can be substituted in tests. `Phalcon\Storage\Adapter\Apcu` now `use`s the trait (and `Phalcon\Cache\Adapter\Apcu` inherits it through the storage adapter), removing the seven inline wrapper copies it previously carried. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\OpensslTrait`, exposing two protected `static` wrappers around PHP's OpenSSL functions - `phpOpensslCipherIvLength()` (`openssl_cipher_iv_length()`) and `phpOpensslRandomPseudoBytes()` (`openssl_random_pseudo_bytes()`) - so those calls can be substituted in tests. `Phalcon\Encryption\Crypt` now `use`s the trait, removing its two inline wrapper copies. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\YamlTrait`, exposing the protected `static phpYamlParseFile()` wrapper around `yaml_parse_file()` so YAML file parsing can be substituted in tests. `Phalcon\Config\Adapter\Yaml` now `use`s the trait, removing its inline copy. The wrapper takes `($filename, $pos = 0, $callbacks = [])`; `yaml_parse_file()`'s by-reference `$ndocs` output is handled by an internal throwaway variable (Zephir cannot expose a by-reference parameter) rather than exposed as an argument. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Php\Base64Trait`, exposing protected `static` wrappers around PHP's base64 functions - `phpBase64Encode()` and `phpBase64Decode()` - plus `doEncodeUrl()`/`doDecodeUrl()` helpers for base64url (URL-safe) encoding and decoding. `Phalcon\Storage\Serializer\Base64` uses the `phpBase64*` wrappers, and `Phalcon\Encryption\Crypt`, `Phalcon\Encryption\Security\JWT\Builder` and `Phalcon\Encryption\Security\JWT\Token\Parser` now use `doEncodeUrl()`/`doDecodeUrl()` in place of their inline base64url logic. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
...
- Added `Phalcon\Traits\Support\Helper\Json\EncodeTrait`, exposing the protected `static toEncode()` helper that encodes data with `json_encode()` and throws the native `\JsonException` on failure, leaving any framework-flavored exception to the caller. `Phalcon\Support\Helper\Json\Encode` was rebuilt on top of the trait - it now `use`s it and catches the native `\JsonException` to rethrow its existing `Phalcon\Support\Helper\Json\Exceptions\JsonEncodeError`, so its behavior is unchanged - and `Phalcon\Logger\Formatter\Json` now `use`s the trait in place of its inline `json_encode()` call (it already raised `\JsonException` through `JSON_THROW_ON_ERROR`, so its behavior is unchanged too). [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
- Added `Phalcon\Traits\Support\Helper\Json\DecodeTrait`, exposing the protected `static toDecode()` helper that decodes a string with `json_decode()` and throws the native `\JsonException` on failure, leaving any framework-flavored exception to the caller. `Phalcon\Support\Helper\Json\Decode` was rebuilt on top of the trait - it now `use`s it and catches the native `\JsonException` to rethrow its existing `Phalcon\Support\Helper\Json\Exceptions\JsonDecodeError`, so its behavior is unchanged. [#17273](https://github.com/phalcon/cphalcon/issues/17273) [[doc]](https://docs.phalcon.io/5.17/traits)
