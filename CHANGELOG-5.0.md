...
- Fixed `Phalcon\Mvc\Model\Resultset` costing two statements for every resultset on SQLite. `Phalcon\Db\Result\PdoResult::numRows()` cannot get a row count out of PDO there - `PDOStatement::rowCount()` is only defined for `INSERT`, `UPDATE` and `DELETE`, and SQLite is a streaming cursor that does not know the count until the result has been stepped to the end - so it re-runs the query wrapped in a `COUNT(*)`. The constructor asked for that count unconditionally, so every `find()` issued the counting statement as well. The count is now resolved lazily by `count()`, and `valid()` is driven by the row the cursor is parked on rather than by a stored total, so an ordinary traversal never asks for it. Asking for the count explicitly - `count()`, `getLast()`, `offsetExists()`, `offsetGet()`, or turning `orm.resultset_prefetch_records` on - still costs the extra statement on SQLite. MySQL and PostgreSQL report the count from the original statement and are unaffected. [#17399](https://github.com/phalcon/cphalcon/issues/17399) [[doc]](https://docs.phalcon.io/5.18/db-models/)
- Fixed `Phalcon\Db\Result\PdoResult::numRows()` failing on multi-line statements, failing on statements that do not begin with `SELECT`, and mis-reporting grouped counts, on adapters that need the counting statement (SQLite). The statement was taken apart with `/^SELECT\s+(.*)/i` and rebuilt: without the `s` modifier a newline truncated it into `SQLSTATE[HY000]: General error: 1 no tables specified`, a `WITH ... SELECT` common table expression matched nothing and left the method returning `null` through an `int` signature, and anything starting with `SELECT COUNT(*) ` short-circuited to a hard-coded `1` - correct only without a `GROUP BY`. The statement is now wrapped verbatim as a sub-select instead of being parsed. [#17399](https://github.com/phalcon/cphalcon/issues/17399)
- Fixed `Phalcon\Mvc\Model\Resultset::seek()` leaving the previous row in place as the current one when seeking past the end of a resultset held in memory, so `valid()` kept reporting `true` and `current()` returned a row that is not at that position. [#17399](https://github.com/phalcon/cphalcon/issues/17399) [[doc]](https://docs.phalcon.io/5.18/db-models/)
