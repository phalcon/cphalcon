...
- Changed `Phalcon\Support\Settings::get()` and `Phalcon\Support\Settings::set()` to consult a single internal `key => cast` whitelist instead of two duplicated `switch` statements, removing the drift risk between the read and write paths (behavior-identical; unknown keys are still silently ignored). [#17152](https://github.com/phalcon/cphalcon/issues/17152) [[doc]](https://docs.phalcon.io/5.15/support-settings/)
